/**
 * assemble-workable-batch.ts
 *
 * Deterministic descriptor assembler for the **Workable** company-source
 * pipeline (Spec 1677). Joins:
 *   - a survivors file (output of `probe-workable-company-source.ts`), whose
 *     `slug` field holds the **live Workable account slug** (e.g.
 ...
 *
 * …into the `WorkableCompanyDescriptor[]` batch JSON consumed by both
 * `scaffold-workable-company-source.ts` and `wire-company-source.ts`.
 *
 ...
 * **`companySlug`** preserves the exact live account slug the public API is keyed
 ...
 *   tsx scripts/assemble-workable-batch.ts \
 *     .rc-survivors.json .rc-enrich.json .rc-batch.json <startSpecNo> <startPhaseNo>
 */
import * as fs from 'fs';
import * as path from 'path';
import type {
  WorkableCompanyDescriptor,
  ListingInput,
} from './scaffold-workable-company-source';

interface Survivor {
  slug: string; // the live Workable account slug (companySlug)
  boardName: string;
  jobCount: number;
...
function main(): void {
  ...
  const batch: WorkableCompanyDescriptor[] = [];
  ...
  console.log(
    `Assembled ${batch.length} Workable descriptors → ${outPath} (Specs ${startSpec}–${specNo - 1})`,
  );
  if (rejected.length) {
    // eslint-disable-next-line no-console
