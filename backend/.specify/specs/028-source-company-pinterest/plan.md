# Plan: 028 — Source Company Plugin: Pinterest
...
Pinterest's careers board is hosted on Greenhouse at the slug
`pinterest`, so the implementation is a thin wrapper around the same
public Greenhouse endpoint that `source-company-reddit` (Spec 027 /
run #237), `source-company-robinhood`, `source-company-airbnb`,
...
Reddit plugin's shape (single-file `service.ts`, three-line
`module.ts`, two-line `index.ts`, four-line `package.json`,
three-line `tsconfig.json`) and rename references from Reddit →
Pinterest, then wire the new package into the four registration
...
sixteen times before (Amazon, Apple, Cursor, Google, IBM, Meta,
OpenAI, Stripe, Anthropic, Databricks, Discord, Coinbase, DoorDash,
Airbnb, Robinhood, Reddit).

## 2. Phases

### Phase 1 — Scaffold + register + test (single PR)

- **Goal:** Land `source-company-pinterest` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-pinterest/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,pinterest.module.ts,pinterest.service.ts}`,
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 028 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-pinterest` → all green.
...
| `packages/models/src/enums/site.enum.ts`            | append `PINTEREST = 'pinterest'`.                            |
| `packages/plugins/index.ts`                         | import + append `PinterestModule` to `ALL_SOURCE_MODULES`.   |
| `tsconfig.base.json`                                | path-alias entry.                                            |
| `jest.config.js`                                    | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                   | add Pinterest shipped row.                                   |
| `docs/index.md`                                     | append Spec 028 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Pinterest moves off Greenhouse mid-maintenance.     | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-reddit`'s shape.         | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.PINTEREST` is unused outside this plugin, so the enum revert
  is safe.
...
(None — see Spec 028 § 10 Decisions for the five resolved points.)
