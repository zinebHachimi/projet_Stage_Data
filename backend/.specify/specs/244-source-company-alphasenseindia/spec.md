# Spec: 244 — Source Company Plugin: AlphaSense India

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 244                            |
| Slug           | source-company-alphasenseindia               |
| Status         | accepted                               |
...
AlphaSense India publishes its careers board through Greenhouse at
the bare slug `alphasenseindia`. The run-398 batch sweep confirmed
45 live role(s) via a direct probe of
...
- Ship a `source-company-alphasenseindia` plugin returning live
...
- Backfilling historical AlphaSense India postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALPHASENSEINDIA`** in the
> source registry, so that **a single `siteType: [Site.ALPHASENSEINDIA]`
> request returns AlphaSense India's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALPHASENSEINDIA = 'alphasenseindia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alphasenseindia`.                  | must     |
| FR-3  | `AlphasenseindiaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alphasenseindia-`, `site === Site.ALPHASENSEINDIA`. | must |
...
```ts
@SourcePlugin({ site: Site.ALPHASENSEINDIA, name: 'AlphaSense India', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alphasenseindia/jobs/<id>`), wire
  `company_name` pass-through (`'AlphaSense India'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alphasenseindia/jobs/<id>`).
