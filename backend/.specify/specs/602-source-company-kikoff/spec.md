# Spec: 602 — Source Company Plugin: Kikoff

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 602                            |
| Slug           | source-company-kikoff               |
| Status         | accepted                               |
...
Kikoff publishes its careers board through Greenhouse at
the bare slug `kikoff`. The run-398 batch sweep confirmed
...
- Ship a `source-company-kikoff` plugin returning live
...
- Backfilling historical Kikoff postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KIKOFF`** in the
> source registry, so that **a single `siteType: [Site.KIKOFF]`
> request returns Kikoff's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KIKOFF = 'kikoff'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-kikoff`.                  | must     |
| FR-3  | `KikoffService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `kikoff-`, `site === Site.KIKOFF`. | must |
...
```ts
@SourcePlugin({ site: Site.KIKOFF, name: 'Kikoff', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/kikoff/jobs/<id>`), wire
  `company_name` pass-through (`'Kikoff'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/kikoff/jobs/<id>`).
