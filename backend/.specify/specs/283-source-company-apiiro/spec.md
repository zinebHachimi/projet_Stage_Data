# Spec: 283 — Source Company Plugin: Apiiro

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 283                            |
| Slug           | source-company-apiiro               |
| Status         | accepted                               |
...
Apiiro publishes its careers board through Greenhouse at
the bare slug `apiiro`. The run-398 batch sweep confirmed
...
- Ship a `source-company-apiiro` plugin returning live
...
- Backfilling historical Apiiro postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APIIRO`** in the
> source registry, so that **a single `siteType: [Site.APIIRO]`
> request returns Apiiro's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APIIRO = 'apiiro'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apiiro`.                  | must     |
| FR-3  | `ApiiroService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apiiro-`, `site === Site.APIIRO`. | must |
...
```ts
@SourcePlugin({ site: Site.APIIRO, name: 'Apiiro', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apiiro/jobs/<id>`), wire
  `company_name` pass-through (`'Apiiro'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apiiro/jobs/<id>`).
