# Spec: 913 — Source Company Plugin: The Pokémon Company International

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 913                            |
| Slug           | source-company-pokemoncareers               |
| Status         | accepted                               |
...
The Pokémon Company International publishes its careers board through Greenhouse at
the bare slug `pokemoncareers`. The run-398 batch sweep confirmed
34 live role(s) via a direct probe of
...
- Ship a `source-company-pokemoncareers` plugin returning live
...
- Backfilling historical The Pokémon Company International postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THE_POK_MON_COMPANY_INTERNATIONAL`** in the
> source registry, so that **a single `siteType: [Site.THE_POK_MON_COMPANY_INTERNATIONAL]`
> request returns The Pokémon Company International's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THE_POK_MON_COMPANY_INTERNATIONAL = 'pokemoncareers'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pokemoncareers`.                  | must     |
| FR-3  | `ThePokMonCompanyInternationalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pokemoncareers-`, `site === Site.THE_POK_MON_COMPANY_INTERNATIONAL`. | must |
...
```ts
@SourcePlugin({ site: Site.THE_POK_MON_COMPANY_INTERNATIONAL, name: 'The Pokémon Company International', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pokemoncareers/jobs/<id>`), wire
  `company_name` pass-through (`'The Pokémon Company International'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pokemoncareers/jobs/<id>`).
