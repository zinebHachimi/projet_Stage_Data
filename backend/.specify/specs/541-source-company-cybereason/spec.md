# Spec: 541 — Source Company Plugin: Cybereason

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 541                            |
| Slug           | source-company-cybereason               |
| Status         | accepted                               |
...
Cybereason publishes its careers board through Greenhouse at
the bare slug `cybereason`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cybereason` plugin returning live
...
- Backfilling historical Cybereason postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CYBEREASON`** in the
> source registry, so that **a single `siteType: [Site.CYBEREASON]`
> request returns Cybereason's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CYBEREASON = 'cybereason'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cybereason`.                  | must     |
| FR-3  | `CybereasonService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cybereason-`, `site === Site.CYBEREASON`. | must |
...
```ts
@SourcePlugin({ site: Site.CYBEREASON, name: 'Cybereason', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cybereason/jobs/<id>`), wire
  `company_name` pass-through (`'Cybereason'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cybereason/jobs/<id>`).
