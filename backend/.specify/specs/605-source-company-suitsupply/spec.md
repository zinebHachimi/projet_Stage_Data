# Spec: 605 — Source Company Plugin: Suitsupply

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 605                            |
| Slug           | source-company-suitsupply               |
| Status         | accepted                               |
...
Suitsupply publishes its careers board through Greenhouse at
the bare slug `suitsupply`. The run-398 batch sweep confirmed
160 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/suitsupply/jobs?content=true`.
Sector: Menswear retail. HQ: Amsterdam, Netherlands.

## 2. Goals

- Ship a `source-company-suitsupply` plugin returning live
...
- Backfilling historical Suitsupply postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SUITSUPPLY`** in the
> source registry, so that **a single `siteType: [Site.SUITSUPPLY]`
> request returns Suitsupply's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SUITSUPPLY = 'suitsupply'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-suitsupply`.                  | must     |
| FR-3  | `SuitsupplyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `suitsupply-`, `site === Site.SUITSUPPLY`. | must |
...
```ts
@SourcePlugin({ site: Site.SUITSUPPLY, name: 'Suitsupply', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/suitsupply/jobs/<id>`), wire
  `company_name` pass-through (`'Suitsupply'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/suitsupply/jobs/<id>`).
