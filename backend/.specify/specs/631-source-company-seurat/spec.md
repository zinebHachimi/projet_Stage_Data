# Spec: 631 — Source Company Plugin: Seurat Technologies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 631                            |
| Slug           | source-company-seurat               |
| Status         | accepted                               |
...
Seurat Technologies publishes its careers board through Greenhouse at
the bare slug `seurat`. The run-398 batch sweep confirmed
...
- Ship a `source-company-seurat` plugin returning live
...
- Backfilling historical Seurat Technologies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SEURAT_TECHNOLOGIES`** in the
> source registry, so that **a single `siteType: [Site.SEURAT_TECHNOLOGIES]`
> request returns Seurat Technologies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SEURAT_TECHNOLOGIES = 'seurat'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-seurat`.                  | must     |
| FR-3  | `SeuratTechnologiesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `seurat-`, `site === Site.SEURAT_TECHNOLOGIES`. | must |
...
```ts
@SourcePlugin({ site: Site.SEURAT_TECHNOLOGIES, name: 'Seurat Technologies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/seurat/jobs/<id>`), wire
  `company_name` pass-through (`'Seurat Technologies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/seurat/jobs/<id>`).
