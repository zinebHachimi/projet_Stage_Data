# Spec: 956 — Source Company Plugin: Third Wave Automation

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 956                            |
| Slug           | source-company-thirdwaveautomation               |
| Status         | accepted                               |
...
Third Wave Automation publishes its careers board through Greenhouse at
the bare slug `thirdwaveautomation`. The run-398 batch sweep confirmed
...
- Ship a `source-company-thirdwaveautomation` plugin returning live
...
- Backfilling historical Third Wave Automation postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THIRD_WAVE_AUTOMATION`** in the
> source registry, so that **a single `siteType: [Site.THIRD_WAVE_AUTOMATION]`
> request returns Third Wave Automation's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THIRD_WAVE_AUTOMATION = 'thirdwaveautomation'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thirdwaveautomation`.                  | must     |
| FR-3  | `ThirdWaveAutomationService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thirdwaveautomation-`, `site === Site.THIRD_WAVE_AUTOMATION`. | must |
...
```ts
@SourcePlugin({ site: Site.THIRD_WAVE_AUTOMATION, name: 'Third Wave Automation', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thirdwaveautomation/jobs/<id>`), wire
  `company_name` pass-through (`'Third Wave Automation'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thirdwaveautomation/jobs/<id>`).
