# Spec: 847 — Source Company Plugin: Feedzai

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 847                            |
| Slug           | source-company-feedzai               |
| Status         | accepted                               |
...
Feedzai publishes its careers board through Greenhouse at
the bare slug `feedzai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-feedzai` plugin returning live
...
- Backfilling historical Feedzai postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FEEDZAI`** in the
> source registry, so that **a single `siteType: [Site.FEEDZAI]`
> request returns Feedzai's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FEEDZAI = 'feedzai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-feedzai`.                  | must     |
| FR-3  | `FeedzaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `feedzai-`, `site === Site.FEEDZAI`. | must |
...
```ts
@SourcePlugin({ site: Site.FEEDZAI, name: 'Feedzai', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/feedzai/jobs/<id>`), wire
  `company_name` pass-through (`'Feedzai'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/feedzai/jobs/<id>`).
