# Spec: 601 — Source Company Plugin: Cuyana

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 601                            |
| Slug           | source-company-cuyana               |
| Status         | accepted                               |
...
Cuyana publishes its careers board through Greenhouse at
the bare slug `cuyana`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cuyana` plugin returning live
...
- Backfilling historical Cuyana postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CUYANA`** in the
> source registry, so that **a single `siteType: [Site.CUYANA]`
> request returns Cuyana's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CUYANA = 'cuyana'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cuyana`.                  | must     |
| FR-3  | `CuyanaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cuyana-`, `site === Site.CUYANA`. | must |
...
```ts
@SourcePlugin({ site: Site.CUYANA, name: 'Cuyana', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cuyana/jobs/<id>`), wire
  `company_name` pass-through (`'Cuyana'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cuyana/jobs/<id>`).
