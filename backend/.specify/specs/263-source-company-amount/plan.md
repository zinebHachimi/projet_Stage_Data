# Plan: 263 — Source Company Plugin: FIS® Amount™
...
FIS® Amount™'s careers board is hosted on Greenhouse at the slug
`amount`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-amount` plugin.
- Deliverables: package files, `Site.AMOUNT` enum value, the
...
- `packages/plugins/source-company-amount` (new)
