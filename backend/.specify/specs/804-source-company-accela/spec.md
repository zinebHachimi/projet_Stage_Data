# Spec: 804 — Source Company Plugin: Accela

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 804                            |
| Slug           | source-company-accela               |
| Status         | accepted                               |
...
Accela publishes its careers board through Greenhouse at
the bare slug `accela`. The run-398 batch sweep confirmed
...
- Ship a `source-company-accela` plugin returning live
...
- Backfilling historical Accela postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ACCELA`** in the
> source registry, so that **a single `siteType: [Site.ACCELA]`
> request returns Accela's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ACCELA = 'accela'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-accela`.                  | must     |
| FR-3  | `AccelaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `accela-`, `site === Site.ACCELA`. | must |
...
```ts
@SourcePlugin({ site: Site.ACCELA, name: 'Accela', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/accela/jobs/<id>`), wire
  `company_name` pass-through (`'Accela'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/accela/jobs/<id>`).
