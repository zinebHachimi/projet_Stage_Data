# Spec: 897 — Source Company Plugin: Northspyre

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 897                            |
| Slug           | source-company-northspyre               |
| Status         | accepted                               |
...
Northspyre publishes its careers board through Greenhouse at
the bare slug `northspyre`. The run-398 batch sweep confirmed
...
- Ship a `source-company-northspyre` plugin returning live
...
- Backfilling historical Northspyre postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NORTHSPYRE`** in the
> source registry, so that **a single `siteType: [Site.NORTHSPYRE]`
> request returns Northspyre's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NORTHSPYRE = 'northspyre'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-northspyre`.                  | must     |
| FR-3  | `NorthspyreService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `northspyre-`, `site === Site.NORTHSPYRE`. | must |
...
```ts
@SourcePlugin({ site: Site.NORTHSPYRE, name: 'Northspyre', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/northspyre/jobs/<id>`), wire
  `company_name` pass-through (`'Northspyre'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/northspyre/jobs/<id>`).
