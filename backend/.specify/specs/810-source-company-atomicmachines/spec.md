# Spec: 810 — Source Company Plugin: Atomic Machines

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 810                            |
| Slug           | source-company-atomicmachines               |
| Status         | accepted                               |
...
Atomic Machines publishes its careers board through Greenhouse at
the bare slug `atomicmachines`. The run-398 batch sweep confirmed
37 live role(s) via a direct probe of
...
- Ship a `source-company-atomicmachines` plugin returning live
...
- Backfilling historical Atomic Machines postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ATOMIC_MACHINES`** in the
> source registry, so that **a single `siteType: [Site.ATOMIC_MACHINES]`
> request returns Atomic Machines's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ATOMIC_MACHINES = 'atomicmachines'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-atomicmachines`.                  | must     |
| FR-3  | `AtomicMachinesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `atomicmachines-`, `site === Site.ATOMIC_MACHINES`. | must |
...
```ts
@SourcePlugin({ site: Site.ATOMIC_MACHINES, name: 'Atomic Machines', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/atomicmachines/jobs/<id>`), wire
  `company_name` pass-through (`'Atomic Machines'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/atomicmachines/jobs/<id>`).
