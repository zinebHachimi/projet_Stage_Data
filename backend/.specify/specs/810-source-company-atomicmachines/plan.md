# Plan: 810 — Source Company Plugin: Atomic Machines
...
Atomic Machines's careers board is hosted on Greenhouse at the slug
`atomicmachines`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-atomicmachines` plugin.
- Deliverables: package files, `Site.ATOMIC_MACHINES` enum value, the
...
- `packages/plugins/source-company-atomicmachines` (new)
