# Spec: 229 — Source Company Plugin: Algolia

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 229                            |
| Slug           | source-company-algolia               |
| Status         | accepted                               |
...
Algolia publishes its careers board through Greenhouse at
the bare slug `algolia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-algolia` plugin returning live
...
- Backfilling historical Algolia postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALGOLIA`** in the
> source registry, so that **a single `siteType: [Site.ALGOLIA]`
> request returns Algolia's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALGOLIA = 'algolia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-algolia`.                  | must     |
| FR-3  | `AlgoliaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `algolia-`, `site === Site.ALGOLIA`. | must |
...
```ts
@SourcePlugin({ site: Site.ALGOLIA, name: 'Algolia', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/algolia/jobs/<id>`), wire
  `company_name` pass-through (`'Algolia'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/algolia/jobs/<id>`).
