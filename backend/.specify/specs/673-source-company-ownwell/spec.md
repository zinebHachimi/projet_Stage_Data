# Spec: 673 — Source Company Plugin: Ownwell

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 673                            |
| Slug           | source-company-ownwell               |
| Status         | accepted                               |
...
Ownwell publishes its careers board through Greenhouse at
the bare slug `ownwell`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ownwell` plugin returning live
...
- Backfilling historical Ownwell postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OWNWELL`** in the
> source registry, so that **a single `siteType: [Site.OWNWELL]`
> request returns Ownwell's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OWNWELL = 'ownwell'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ownwell`.                  | must     |
| FR-3  | `OwnwellService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ownwell-`, `site === Site.OWNWELL`. | must |
...
```ts
@SourcePlugin({ site: Site.OWNWELL, name: 'Ownwell', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ownwell/jobs/<id>`), wire
  `company_name` pass-through (`'Ownwell'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ownwell/jobs/<id>`).
