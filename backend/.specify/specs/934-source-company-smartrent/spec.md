# Spec: 934 — Source Company Plugin: SmartRent

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 934                            |
| Slug           | source-company-smartrent               |
| Status         | accepted                               |
...
SmartRent publishes its careers board through Greenhouse at
the bare slug `smartrent`. The run-398 batch sweep confirmed
...
- Ship a `source-company-smartrent` plugin returning live
...
- Backfilling historical SmartRent postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SMARTRENT`** in the
> source registry, so that **a single `siteType: [Site.SMARTRENT]`
> request returns SmartRent's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SMARTRENT = 'smartrent'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-smartrent`.                  | must     |
| FR-3  | `SmartRentService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `smartrent-`, `site === Site.SMARTRENT`. | must |
...
```ts
@SourcePlugin({ site: Site.SMARTRENT, name: 'SmartRent', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/smartrent/jobs/<id>`), wire
  `company_name` pass-through (`'SmartRent'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/smartrent/jobs/<id>`).
