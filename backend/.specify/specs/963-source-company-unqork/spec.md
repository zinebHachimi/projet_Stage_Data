# Spec: 963 — Source Company Plugin: Unqork

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 963                            |
| Slug           | source-company-unqork               |
| Status         | accepted                               |
...
Unqork publishes its careers board through Greenhouse at
the bare slug `unqork`. The run-398 batch sweep confirmed
...
- Ship a `source-company-unqork` plugin returning live
...
- Backfilling historical Unqork postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UNQORK`** in the
> source registry, so that **a single `siteType: [Site.UNQORK]`
> request returns Unqork's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UNQORK = 'unqork'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-unqork`.                  | must     |
| FR-3  | `UnqorkService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `unqork-`, `site === Site.UNQORK`. | must |
...
```ts
@SourcePlugin({ site: Site.UNQORK, name: 'Unqork', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/unqork/jobs/<id>`), wire
  `company_name` pass-through (`'Unqork'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/unqork/jobs/<id>`).
