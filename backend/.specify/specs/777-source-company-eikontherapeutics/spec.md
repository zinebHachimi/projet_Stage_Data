# Spec: 777 — Source Company Plugin: Eikon Therapeutics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 777                            |
| Slug           | source-company-eikontherapeutics               |
| Status         | accepted                               |
...
Eikon Therapeutics publishes its careers board through Greenhouse at
the bare slug `eikontherapeutics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-eikontherapeutics` plugin returning live
...
- Backfilling historical Eikon Therapeutics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EIKON_THERAPEUTICS`** in the
> source registry, so that **a single `siteType: [Site.EIKON_THERAPEUTICS]`
> request returns Eikon Therapeutics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EIKON_THERAPEUTICS = 'eikontherapeutics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-eikontherapeutics`.                  | must     |
| FR-3  | `EikonTherapeuticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `eikontherapeutics-`, `site === Site.EIKON_THERAPEUTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.EIKON_THERAPEUTICS, name: 'Eikon Therapeutics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/eikontherapeutics/jobs/<id>`), wire
  `company_name` pass-through (`'Eikon Therapeutics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/eikontherapeutics/jobs/<id>`).
