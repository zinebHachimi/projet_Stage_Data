# Spec: 477 — Source Company Plugin: FourKites

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 477                            |
| Slug           | source-company-fourkites               |
| Status         | accepted                               |
...
FourKites publishes its careers board through Greenhouse at
the bare slug `fourkites`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fourkites` plugin returning live
...
- Backfilling historical FourKites postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FOURKITES`** in the
> source registry, so that **a single `siteType: [Site.FOURKITES]`
> request returns FourKites's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FOURKITES = 'fourkites'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fourkites`.                  | must     |
| FR-3  | `FourkitesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fourkites-`, `site === Site.FOURKITES`. | must |
...
```ts
@SourcePlugin({ site: Site.FOURKITES, name: 'FourKites', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fourkites/jobs/<id>`), wire
  `company_name` pass-through (`'FourKites'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fourkites/jobs/<id>`).
