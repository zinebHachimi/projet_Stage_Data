# Spec: 592 — Source Company Plugin: Orchard

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 592                            |
| Slug           | source-company-orchard               |
| Status         | accepted                               |
...
Orchard publishes its careers board through Greenhouse at
the bare slug `orchard`. The run-398 batch sweep confirmed
73 live role(s) via a direct probe of
...
- Ship a `source-company-orchard` plugin returning live
...
- Backfilling historical Orchard postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ORCHARD`** in the
> source registry, so that **a single `siteType: [Site.ORCHARD]`
> request returns Orchard's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ORCHARD = 'orchard'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-orchard`.                  | must     |
| FR-3  | `OrchardService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `orchard-`, `site === Site.ORCHARD`. | must |
...
```ts
@SourcePlugin({ site: Site.ORCHARD, name: 'Orchard', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/orchard/jobs/<id>`), wire
  `company_name` pass-through (`'Orchard'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/orchard/jobs/<id>`).
