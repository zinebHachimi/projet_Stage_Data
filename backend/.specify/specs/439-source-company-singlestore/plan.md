# Plan: 439 — Source Company Plugin: SingleStore
...
SingleStore's careers board is hosted on Greenhouse at the slug
`singlestore`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-singlestore` plugin.
- Deliverables: package files, `Site.SINGLESTORE` enum value, the
...
- `packages/plugins/source-company-singlestore` (new)
