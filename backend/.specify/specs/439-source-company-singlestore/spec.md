# Spec: 439 — Source Company Plugin: SingleStore

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 439                            |
| Slug           | source-company-singlestore               |
| Status         | accepted                               |
...
SingleStore publishes its careers board through Greenhouse at
the bare slug `singlestore`. The run-398 batch sweep confirmed
...
- Ship a `source-company-singlestore` plugin returning live
...
- Backfilling historical SingleStore postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SINGLESTORE`** in the
> source registry, so that **a single `siteType: [Site.SINGLESTORE]`
> request returns SingleStore's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SINGLESTORE = 'singlestore'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-singlestore`.                  | must     |
| FR-3  | `SinglestoreService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `singlestore-`, `site === Site.SINGLESTORE`. | must |
...
```ts
@SourcePlugin({ site: Site.SINGLESTORE, name: 'SingleStore', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/singlestore/jobs/<id>`), wire
  `company_name` pass-through (`'SingleStore'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/singlestore/jobs/<id>`).
