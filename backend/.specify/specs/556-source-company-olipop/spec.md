# Spec: 556 — Source Company Plugin: OLIPOP

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 556                            |
| Slug           | source-company-olipop               |
| Status         | accepted                               |
...
OLIPOP publishes its careers board through Greenhouse at
the bare slug `olipop`. The run-398 batch sweep confirmed
...
- Ship a `source-company-olipop` plugin returning live
...
- Backfilling historical OLIPOP postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OLIPOP`** in the
> source registry, so that **a single `siteType: [Site.OLIPOP]`
> request returns OLIPOP's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OLIPOP = 'olipop'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-olipop`.                  | must     |
| FR-3  | `OlipopService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `olipop-`, `site === Site.OLIPOP`. | must |
...
```ts
@SourcePlugin({ site: Site.OLIPOP, name: 'OLIPOP', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/olipop/jobs/<id>`), wire
  `company_name` pass-through (`'OLIPOP'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/olipop/jobs/<id>`).
