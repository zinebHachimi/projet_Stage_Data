# Spec: 923 — Source Company Plugin: Roku

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 923                            |
| Slug           | source-company-roku               |
| Status         | accepted                               |
...
Roku publishes its careers board through Greenhouse at
the bare slug `roku`. The run-398 batch sweep confirmed
230 live role(s) via a direct probe of
...
- Ship a `source-company-roku` plugin returning live
...
- Backfilling historical Roku postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ROKU`** in the
> source registry, so that **a single `siteType: [Site.ROKU]`
> request returns Roku's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ROKU = 'roku'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-roku`.                  | must     |
| FR-3  | `RokuService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `roku-`, `site === Site.ROKU`. | must |
...
```ts
@SourcePlugin({ site: Site.ROKU, name: 'Roku', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/roku/jobs/<id>`), wire
  `company_name` pass-through (`'Roku'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/roku/jobs/<id>`).
