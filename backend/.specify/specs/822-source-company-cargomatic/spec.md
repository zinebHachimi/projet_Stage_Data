# Spec: 822 — Source Company Plugin: Cargomatic

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 822                            |
| Slug           | source-company-cargomatic               |
| Status         | accepted                               |
...
Cargomatic publishes its careers board through Greenhouse at
the bare slug `cargomatic`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cargomatic` plugin returning live
...
- Backfilling historical Cargomatic postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CARGOMATIC`** in the
> source registry, so that **a single `siteType: [Site.CARGOMATIC]`
> request returns Cargomatic's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CARGOMATIC = 'cargomatic'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cargomatic`.                  | must     |
| FR-3  | `CargomaticService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cargomatic-`, `site === Site.CARGOMATIC`. | must |
...
```ts
@SourcePlugin({ site: Site.CARGOMATIC, name: 'Cargomatic', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cargomatic/jobs/<id>`), wire
  `company_name` pass-through (`'Cargomatic'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cargomatic/jobs/<id>`).
