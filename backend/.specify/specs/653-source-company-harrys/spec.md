# Spec: 653 — Source Company Plugin: Harry's

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 653                            |
| Slug           | source-company-harrys               |
| Status         | accepted                               |
...
Harry's publishes its careers board through Greenhouse at
the bare slug `harrys`. The run-398 batch sweep confirmed
...
- Ship a `source-company-harrys` plugin returning live
...
- Backfilling historical Harry's postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HARRY_S`** in the
> source registry, so that **a single `siteType: [Site.HARRY_S]`
> request returns Harry's's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HARRY_S = 'harrys'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-harrys`.                  | must     |
| FR-3  | `HarrySService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `harrys-`, `site === Site.HARRY_S`. | must |
...
```ts
@SourcePlugin({ site: Site.HARRY_S, name: 'Harry\'s', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/harrys/jobs/<id>`), wire
  `company_name` pass-through (`'Harry\'s'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/harrys/jobs/<id>`).
