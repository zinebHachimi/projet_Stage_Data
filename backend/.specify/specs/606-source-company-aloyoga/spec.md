# Spec: 606 — Source Company Plugin: Alo Yoga

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 606                            |
| Slug           | source-company-aloyoga               |
| Status         | accepted                               |
...
Alo Yoga publishes its careers board through Greenhouse at
the bare slug `aloyoga`. The run-398 batch sweep confirmed
663 live role(s) via a direct probe of
...
- Ship a `source-company-aloyoga` plugin returning live
...
- Backfilling historical Alo Yoga postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALO_YOGA`** in the
> source registry, so that **a single `siteType: [Site.ALO_YOGA]`
> request returns Alo Yoga's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALO_YOGA = 'aloyoga'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aloyoga`.                  | must     |
| FR-3  | `AloYogaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aloyoga-`, `site === Site.ALO_YOGA`. | must |
...
```ts
@SourcePlugin({ site: Site.ALO_YOGA, name: 'Alo Yoga', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aloyoga/jobs/<id>`), wire
  `company_name` pass-through (`'Alo Yoga'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aloyoga/jobs/<id>`).
