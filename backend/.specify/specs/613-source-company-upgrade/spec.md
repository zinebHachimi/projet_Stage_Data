# Spec: 613 — Source Company Plugin: Upgrade

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 613                            |
| Slug           | source-company-upgrade               |
| Status         | accepted                               |
...
Upgrade publishes its careers board through Greenhouse at
the bare slug `upgrade`. The run-398 batch sweep confirmed
...
- Ship a `source-company-upgrade` plugin returning live
...
- Backfilling historical Upgrade postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UPGRADE`** in the
> source registry, so that **a single `siteType: [Site.UPGRADE]`
> request returns Upgrade's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UPGRADE = 'upgrade'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-upgrade`.                  | must     |
| FR-3  | `UpgradeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `upgrade-`, `site === Site.UPGRADE`. | must |
...
```ts
@SourcePlugin({ site: Site.UPGRADE, name: 'Upgrade', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/upgrade/jobs/<id>`), wire
  `company_name` pass-through (`'Upgrade'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/upgrade/jobs/<id>`).
