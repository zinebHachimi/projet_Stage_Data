# Spec: 693 — Source Company Plugin: SumUp

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 693                            |
| Slug           | source-company-sumup               |
| Status         | accepted                               |
...
SumUp publishes its careers board through Greenhouse at
the bare slug `sumup`. The run-398 batch sweep confirmed
437 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/sumup/jobs?content=true`.
Sector: Consumer/SMB fintech (payments). HQ: London, United Kingdom.

## 2. Goals

- Ship a `source-company-sumup` plugin returning live
...
- Backfilling historical SumUp postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SUMUP`** in the
> source registry, so that **a single `siteType: [Site.SUMUP]`
> request returns SumUp's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SUMUP = 'sumup'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sumup`.                  | must     |
| FR-3  | `SumUpService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sumup-`, `site === Site.SUMUP`. | must |
...
```ts
@SourcePlugin({ site: Site.SUMUP, name: 'SumUp', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sumup/jobs/<id>`), wire
  `company_name` pass-through (`'SumUp'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sumup/jobs/<id>`).
