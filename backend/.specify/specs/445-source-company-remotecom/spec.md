# Spec: 445 — Source Company Plugin: Remote

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 445                            |
| Slug           | source-company-remotecom               |
| Status         | accepted                               |
...
Remote publishes its careers board through Greenhouse at
the bare slug `remotecom`. The run-398 batch sweep confirmed
254 live role(s) via a direct probe of
...
- Ship a `source-company-remotecom` plugin returning live
...
- Backfilling historical Remote postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.REMOTECOM`** in the
> source registry, so that **a single `siteType: [Site.REMOTECOM]`
> request returns Remote's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.REMOTECOM = 'remotecom'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-remotecom`.                  | must     |
| FR-3  | `RemotecomService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `remotecom-`, `site === Site.REMOTECOM`. | must |
...
```ts
@SourcePlugin({ site: Site.REMOTECOM, name: 'Remote', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/remotecom/jobs/<id>`), wire
  `company_name` pass-through (`'Remote'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/remotecom/jobs/<id>`).
