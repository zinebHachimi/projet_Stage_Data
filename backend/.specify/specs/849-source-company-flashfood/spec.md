# Spec: 849 — Source Company Plugin: Flashfood

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 849                            |
| Slug           | source-company-flashfood               |
| Status         | accepted                               |
...
Flashfood publishes its careers board through Greenhouse at
the bare slug `flashfood`. The run-398 batch sweep confirmed
...
- Ship a `source-company-flashfood` plugin returning live
...
- Backfilling historical Flashfood postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FLASHFOOD`** in the
> source registry, so that **a single `siteType: [Site.FLASHFOOD]`
> request returns Flashfood's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FLASHFOOD = 'flashfood'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-flashfood`.                  | must     |
| FR-3  | `FlashfoodService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `flashfood-`, `site === Site.FLASHFOOD`. | must |
...
```ts
@SourcePlugin({ site: Site.FLASHFOOD, name: 'Flashfood', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/flashfood/jobs/<id>`), wire
  `company_name` pass-through (`'Flashfood'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/flashfood/jobs/<id>`).
