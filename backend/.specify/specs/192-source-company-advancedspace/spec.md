# Spec: 192 — Source Company Plugin: Advanced Space

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 192                            |
| Slug           | source-company-advancedspace               |
| Status         | accepted                               |
...
Advanced Space publishes its careers board through Greenhouse at
the bare slug `advancedspace`. The run-398 batch sweep confirmed
...
- Ship a `source-company-advancedspace` plugin returning live
...
- Backfilling historical Advanced Space postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ADVANCEDSPACE`** in the
> source registry, so that **a single `siteType: [Site.ADVANCEDSPACE]`
> request returns Advanced Space's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ADVANCEDSPACE = 'advancedspace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-advancedspace`.                  | must     |
| FR-3  | `AdvancedspaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `advancedspace-`, `site === Site.ADVANCEDSPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.ADVANCEDSPACE, name: 'Advanced Space', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/advancedspace/jobs/<id>`), wire
  `company_name` pass-through (`'Advanced Space'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/advancedspace/jobs/<id>`).
