# Spec: 288 — Source Company Plugin: Apollo.io

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 288                            |
| Slug           | source-company-apolloio               |
| Status         | accepted                               |
...
Apollo.io publishes its careers board through Greenhouse at
the bare slug `apolloio`. The run-398 batch sweep confirmed
...
- Ship a `source-company-apolloio` plugin returning live
...
- Backfilling historical Apollo.io postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APOLLOIO`** in the
> source registry, so that **a single `siteType: [Site.APOLLOIO]`
> request returns Apollo.io's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APOLLOIO = 'apolloio'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apolloio`.                  | must     |
| FR-3  | `ApolloioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apolloio-`, `site === Site.APOLLOIO`. | must |
...
```ts
@SourcePlugin({ site: Site.APOLLOIO, name: 'Apollo.io', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apolloio/jobs/<id>`), wire
  `company_name` pass-through (`'Apollo.io'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apolloio/jobs/<id>`).
