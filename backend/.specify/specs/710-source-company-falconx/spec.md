# Spec: 710 — Source Company Plugin: FalconX

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 710                            |
| Slug           | source-company-falconx               |
| Status         | accepted                               |
...
FalconX publishes its careers board through Greenhouse at
the bare slug `falconx`. The run-398 batch sweep confirmed
...
- Ship a `source-company-falconx` plugin returning live
...
- Backfilling historical FalconX postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FALCONX`** in the
> source registry, so that **a single `siteType: [Site.FALCONX]`
> request returns FalconX's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FALCONX = 'falconx'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-falconx`.                  | must     |
| FR-3  | `FalconXService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `falconx-`, `site === Site.FALCONX`. | must |
...
```ts
@SourcePlugin({ site: Site.FALCONX, name: 'FalconX', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/falconx/jobs/<id>`), wire
  `company_name` pass-through (`'FalconX'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/falconx/jobs/<id>`).
