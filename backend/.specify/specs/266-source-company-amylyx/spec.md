# Spec: 266 — Source Company Plugin: Amylyx Pharmaceuticals

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 266                            |
| Slug           | source-company-amylyx               |
| Status         | accepted                               |
...
Amylyx Pharmaceuticals publishes its careers board through Greenhouse at
the bare slug `amylyx`. The run-398 batch sweep confirmed
...
- Ship a `source-company-amylyx` plugin returning live
...
- Backfilling historical Amylyx Pharmaceuticals postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMYLYX`** in the
> source registry, so that **a single `siteType: [Site.AMYLYX]`
> request returns Amylyx Pharmaceuticals's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMYLYX = 'amylyx'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-amylyx`.                  | must     |
| FR-3  | `AmylyxService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `amylyx-`, `site === Site.AMYLYX`. | must |
...
```ts
@SourcePlugin({ site: Site.AMYLYX, name: 'Amylyx Pharmaceuticals', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/amylyx/jobs/<id>`), wire
  `company_name` pass-through (`'Amylyx Pharmaceuticals'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/amylyx/jobs/<id>`).
