# Spec: 498 — Source Company Plugin: Weee!

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 498                            |
| Slug           | source-company-weee               |
| Status         | accepted                               |
...
Weee! publishes its careers board through Greenhouse at
the bare slug `weee`. The run-398 batch sweep confirmed
...
- Ship a `source-company-weee` plugin returning live
...
- Backfilling historical Weee! postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WEEE`** in the
> source registry, so that **a single `siteType: [Site.WEEE]`
> request returns Weee!'s open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WEEE = 'weee'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-weee`.                  | must     |
| FR-3  | `WeeeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `weee-`, `site === Site.WEEE`. | must |
...
```ts
@SourcePlugin({ site: Site.WEEE, name: 'Weee!', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/weee/jobs/<id>`), wire
  `company_name` pass-through (`'Weee!'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/weee/jobs/<id>`).
