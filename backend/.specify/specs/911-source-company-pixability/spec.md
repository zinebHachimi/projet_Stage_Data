# Spec: 911 — Source Company Plugin: Pixability

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 911                            |
| Slug           | source-company-pixability               |
| Status         | accepted                               |
...
Pixability publishes its careers board through Greenhouse at
the bare slug `pixability`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pixability` plugin returning live
...
- Backfilling historical Pixability postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PIXABILITY`** in the
> source registry, so that **a single `siteType: [Site.PIXABILITY]`
> request returns Pixability's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PIXABILITY = 'pixability'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pixability`.                  | must     |
| FR-3  | `PixabilityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pixability-`, `site === Site.PIXABILITY`. | must |
...
```ts
@SourcePlugin({ site: Site.PIXABILITY, name: 'Pixability', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pixability/jobs/<id>`), wire
  `company_name` pass-through (`'Pixability'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pixability/jobs/<id>`).
