# Spec: 278 — Source Company Plugin: Apartment Life

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 278                            |
| Slug           | source-company-apartmentlife               |
| Status         | accepted                               |
...
Apartment Life publishes its careers board through Greenhouse at
the bare slug `apartmentlife`. The run-398 batch sweep confirmed
156 live role(s) via a direct probe of
...
- Ship a `source-company-apartmentlife` plugin returning live
...
- Backfilling historical Apartment Life postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APARTMENTLIFE`** in the
> source registry, so that **a single `siteType: [Site.APARTMENTLIFE]`
> request returns Apartment Life's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APARTMENTLIFE = 'apartmentlife'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apartmentlife`.                  | must     |
| FR-3  | `ApartmentlifeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apartmentlife-`, `site === Site.APARTMENTLIFE`. | must |
...
```ts
@SourcePlugin({ site: Site.APARTMENTLIFE, name: 'Apartment Life', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apartmentlife/jobs/<id>`), wire
  `company_name` pass-through (`'Apartment Life'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apartmentlife/jobs/<id>`).
