# Spec: 793 — Source Company Plugin: FastSpring

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 793                            |
| Slug           | source-company-fastspring               |
| Status         | accepted                               |
...
FastSpring publishes its careers board through Greenhouse at
the bare slug `fastspring`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fastspring` plugin returning live
...
- Backfilling historical FastSpring postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FASTSPRING`** in the
> source registry, so that **a single `siteType: [Site.FASTSPRING]`
> request returns FastSpring's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FASTSPRING = 'fastspring'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fastspring`.                  | must     |
| FR-3  | `FastSpringService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fastspring-`, `site === Site.FASTSPRING`. | must |
...
```ts
@SourcePlugin({ site: Site.FASTSPRING, name: 'FastSpring', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fastspring/jobs/<id>`), wire
  `company_name` pass-through (`'FastSpring'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fastspring/jobs/<id>`).
