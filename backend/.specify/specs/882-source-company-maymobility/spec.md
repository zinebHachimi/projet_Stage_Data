# Spec: 882 — Source Company Plugin: May Mobility

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 882                            |
| Slug           | source-company-maymobility               |
| Status         | accepted                               |
...
May Mobility publishes its careers board through Greenhouse at
the bare slug `maymobility`. The run-398 batch sweep confirmed
...
- Ship a `source-company-maymobility` plugin returning live
...
- Backfilling historical May Mobility postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MAY_MOBILITY`** in the
> source registry, so that **a single `siteType: [Site.MAY_MOBILITY]`
> request returns May Mobility's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MAY_MOBILITY = 'maymobility'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-maymobility`.                  | must     |
| FR-3  | `MayMobilityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `maymobility-`, `site === Site.MAY_MOBILITY`. | must |
...
```ts
@SourcePlugin({ site: Site.MAY_MOBILITY, name: 'May Mobility', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/maymobility/jobs/<id>`), wire
  `company_name` pass-through (`'May Mobility'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/maymobility/jobs/<id>`).
