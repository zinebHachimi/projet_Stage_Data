# Spec: 616 — Source Company Plugin: Stackline

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 616                            |
| Slug           | source-company-stackline               |
| Status         | accepted                               |
...
Stackline publishes its careers board through Greenhouse at
the bare slug `stackline`. The run-398 batch sweep confirmed
...
- Ship a `source-company-stackline` plugin returning live
...
- Backfilling historical Stackline postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STACKLINE`** in the
> source registry, so that **a single `siteType: [Site.STACKLINE]`
> request returns Stackline's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STACKLINE = 'stackline'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-stackline`.                  | must     |
| FR-3  | `StacklineService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `stackline-`, `site === Site.STACKLINE`. | must |
...
```ts
@SourcePlugin({ site: Site.STACKLINE, name: 'Stackline', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/stackline/jobs/<id>`), wire
  `company_name` pass-through (`'Stackline'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/stackline/jobs/<id>`).
