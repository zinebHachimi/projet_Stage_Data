# Spec: 534 — Source Company Plugin: Yotpo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 534                            |
| Slug           | source-company-yotpo               |
| Status         | accepted                               |
...
Yotpo publishes its careers board through Greenhouse at
the bare slug `yotpo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-yotpo` plugin returning live
...
- Backfilling historical Yotpo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.YOTPO`** in the
> source registry, so that **a single `siteType: [Site.YOTPO]`
> request returns Yotpo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.YOTPO = 'yotpo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-yotpo`.                  | must     |
| FR-3  | `YotpoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `yotpo-`, `site === Site.YOTPO`. | must |
...
```ts
@SourcePlugin({ site: Site.YOTPO, name: 'Yotpo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/yotpo/jobs/<id>`), wire
  `company_name` pass-through (`'Yotpo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/yotpo/jobs/<id>`).
