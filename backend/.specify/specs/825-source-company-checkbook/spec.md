# Spec: 825 — Source Company Plugin: Checkbook

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 825                            |
| Slug           | source-company-checkbook               |
| Status         | accepted                               |
...
Checkbook publishes its careers board through Greenhouse at
the bare slug `checkbook`. The run-398 batch sweep confirmed
...
- Ship a `source-company-checkbook` plugin returning live
...
- Backfilling historical Checkbook postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CHECKBOOK`** in the
> source registry, so that **a single `siteType: [Site.CHECKBOOK]`
> request returns Checkbook's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CHECKBOOK = 'checkbook'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-checkbook`.                  | must     |
| FR-3  | `CheckbookService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `checkbook-`, `site === Site.CHECKBOOK`. | must |
...
```ts
@SourcePlugin({ site: Site.CHECKBOOK, name: 'Checkbook', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/checkbook/jobs/<id>`), wire
  `company_name` pass-through (`'Checkbook'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/checkbook/jobs/<id>`).
