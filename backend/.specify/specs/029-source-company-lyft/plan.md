# Plan: 029 — Source Company Plugin: Lyft
...
Lyft's careers board is hosted on Greenhouse at the slug `lyft`, so
the implementation is a thin wrapper around the same public Greenhouse
endpoint that `source-company-pinterest` (Spec 028 / run #238),
...
call. The plan is to copy the Pinterest plugin's shape (single-file
`service.ts`, three-line `module.ts`, two-line `index.ts`, four-line
`package.json`, three-line `tsconfig.json`) and rename references
from Pinterest → Lyft, then wire the new package into the four
registration points: `Site` enum, plugins barrel, `tsconfig.base.json`
paths, `jest.config.js` `moduleNameMapper`. After the code lands,
...
used seventeen times before (Amazon, Apple, Cursor, Google, IBM,
Meta, OpenAI, Stripe, Anthropic, Databricks, Discord, Coinbase,
DoorDash, Airbnb, Robinhood, Reddit, Pinterest).

## 2. Phases

### Phase 1 — Scaffold + register + test (single PR)

- **Goal:** Land `source-company-lyft` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-lyft/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,lyft.module.ts,lyft.service.ts}`,
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 029 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-lyft` → all green.
...
| `packages/models/src/enums/site.enum.ts`            | append `LYFT = 'lyft'`.                                      |
| `packages/plugins/index.ts`                         | import + append `LyftModule` to `ALL_SOURCE_MODULES`.        |
| `tsconfig.base.json`                                | path-alias entry.                                            |
| `jest.config.js`                                    | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                   | add Lyft shipped row.                                        |
| `docs/index.md`                                     | append Spec 029 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Lyft moves off Greenhouse mid-maintenance.          | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-pinterest`'s shape.      | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
| Plugin-name collision with `source-ats-talentlyft`. | L          | L      | The new package is `source-company-lyft` (module `LyftModule`); the existing ATS adapter is `source-ats-talentlyft` (module `TalentLyftModule`). Different package id, different module class — no conflict. |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.LYFT` is unused outside this plugin, so the enum revert is
  safe.
...
(None — see Spec 029 § 10 Decisions for the five resolved points.)
