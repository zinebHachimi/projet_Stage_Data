# Spec: 639 — Source Company Plugin: Ezra

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 639                            |
| Slug           | source-company-ezra               |
| Status         | accepted                               |
...
Ezra publishes its careers board through Greenhouse at
the bare slug `ezra`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ezra` plugin returning live
...
- Backfilling historical Ezra postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EZRA`** in the
> source registry, so that **a single `siteType: [Site.EZRA]`
> request returns Ezra's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EZRA = 'ezra'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ezra`.                  | must     |
| FR-3  | `EzraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ezra-`, `site === Site.EZRA`. | must |
...
```ts
@SourcePlugin({ site: Site.EZRA, name: 'Ezra', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ezra/jobs/<id>`), wire
  `company_name` pass-through (`'Ezra'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ezra/jobs/<id>`).
