# Spec: 637 — Source Company Plugin: Axiom

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 637                            |
| Slug           | source-company-axiom               |
| Status         | accepted                               |
...
Axiom publishes its careers board through Greenhouse at
the bare slug `axiom`. The run-398 batch sweep confirmed
...
- Ship a `source-company-axiom` plugin returning live
...
- Backfilling historical Axiom postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AXIOM`** in the
> source registry, so that **a single `siteType: [Site.AXIOM]`
> request returns Axiom's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AXIOM = 'axiom'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-axiom`.                  | must     |
| FR-3  | `AxiomService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `axiom-`, `site === Site.AXIOM`. | must |
...
```ts
@SourcePlugin({ site: Site.AXIOM, name: 'Axiom', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/axiom/jobs/<id>`), wire
  `company_name` pass-through (`'Axiom'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/axiom/jobs/<id>`).
