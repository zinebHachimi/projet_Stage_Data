# Spec: 805 — Source Company Plugin: AEVEX Aerospace

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 805                            |
| Slug           | source-company-aevexaerospace               |
| Status         | accepted                               |
...
AEVEX Aerospace publishes its careers board through Greenhouse at
the bare slug `aevexaerospace`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aevexaerospace` plugin returning live
...
- Backfilling historical AEVEX Aerospace postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AEVEX_AEROSPACE`** in the
> source registry, so that **a single `siteType: [Site.AEVEX_AEROSPACE]`
> request returns AEVEX Aerospace's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AEVEX_AEROSPACE = 'aevexaerospace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aevexaerospace`.                  | must     |
| FR-3  | `AEVEXAerospaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aevexaerospace-`, `site === Site.AEVEX_AEROSPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.AEVEX_AEROSPACE, name: 'AEVEX Aerospace', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aevexaerospace/jobs/<id>`), wire
  `company_name` pass-through (`'AEVEX Aerospace'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aevexaerospace/jobs/<id>`).
