# Spec: 851 — Source Company Plugin: Forbes

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 851                            |
| Slug           | source-company-forbes               |
| Status         | accepted                               |
...
Forbes publishes its careers board through Greenhouse at
the bare slug `forbes`. The run-398 batch sweep confirmed
...
- Ship a `source-company-forbes` plugin returning live
...
- Backfilling historical Forbes postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORBES`** in the
> source registry, so that **a single `siteType: [Site.FORBES]`
> request returns Forbes's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORBES = 'forbes'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-forbes`.                  | must     |
| FR-3  | `ForbesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `forbes-`, `site === Site.FORBES`. | must |
...
```ts
@SourcePlugin({ site: Site.FORBES, name: 'Forbes', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/forbes/jobs/<id>`), wire
  `company_name` pass-through (`'Forbes'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/forbes/jobs/<id>`).
