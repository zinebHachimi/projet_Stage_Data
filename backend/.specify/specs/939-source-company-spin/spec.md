# Spec: 939 — Source Company Plugin: Spin

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 939                            |
| Slug           | source-company-spin               |
| Status         | accepted                               |
...
Spin publishes its careers board through Greenhouse at
the bare slug `spin`. The run-398 batch sweep confirmed
...
- Ship a `source-company-spin` plugin returning live
...
- Backfilling historical Spin postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPIN`** in the
> source registry, so that **a single `siteType: [Site.SPIN]`
> request returns Spin's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPIN = 'spin'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-spin`.                  | must     |
| FR-3  | `SpinService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `spin-`, `site === Site.SPIN`. | must |
...
```ts
@SourcePlugin({ site: Site.SPIN, name: 'Spin', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/spin/jobs/<id>`), wire
  `company_name` pass-through (`'Spin'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/spin/jobs/<id>`).
