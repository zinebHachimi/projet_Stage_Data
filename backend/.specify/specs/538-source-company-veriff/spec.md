# Spec: 538 — Source Company Plugin: Veriff

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 538                            |
| Slug           | source-company-veriff               |
| Status         | accepted                               |
...
Veriff publishes its careers board through Greenhouse at
the bare slug `veriff`. The run-398 batch sweep confirmed
...
- Ship a `source-company-veriff` plugin returning live
...
- Backfilling historical Veriff postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VERIFF`** in the
> source registry, so that **a single `siteType: [Site.VERIFF]`
> request returns Veriff's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VERIFF = 'veriff'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-veriff`.                  | must     |
| FR-3  | `VeriffService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `veriff-`, `site === Site.VERIFF`. | must |
...
```ts
@SourcePlugin({ site: Site.VERIFF, name: 'Veriff', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/veriff/jobs/<id>`), wire
  `company_name` pass-through (`'Veriff'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/veriff/jobs/<id>`).
