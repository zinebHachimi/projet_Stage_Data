# Plan: 538 — Source Company Plugin: Veriff
...
Veriff's careers board is hosted on Greenhouse at the slug
`veriff`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-veriff` plugin.
- Deliverables: package files, `Site.VERIFF` enum value, the
...
- `packages/plugins/source-company-veriff` (new)
