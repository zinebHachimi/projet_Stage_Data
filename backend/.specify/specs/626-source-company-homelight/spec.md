# Spec: 626 — Source Company Plugin: HomeLight

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 626                            |
| Slug           | source-company-homelight               |
| Status         | accepted                               |
...
HomeLight publishes its careers board through Greenhouse at
the bare slug `homelight`. The run-398 batch sweep confirmed
...
- Ship a `source-company-homelight` plugin returning live
...
- Backfilling historical HomeLight postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HOMELIGHT`** in the
> source registry, so that **a single `siteType: [Site.HOMELIGHT]`
> request returns HomeLight's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HOMELIGHT = 'homelight'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-homelight`.                  | must     |
| FR-3  | `HomeLightService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `homelight-`, `site === Site.HOMELIGHT`. | must |
...
```ts
@SourcePlugin({ site: Site.HOMELIGHT, name: 'HomeLight', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/homelight/jobs/<id>`), wire
  `company_name` pass-through (`'HomeLight'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/homelight/jobs/<id>`).
