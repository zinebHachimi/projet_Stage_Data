# Spec: 887 — Source Company Plugin: MrBeast (Beast Industries)

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 887                            |
| Slug           | source-company-mrbeastyoutube               |
| Status         | accepted                               |
...
MrBeast (Beast Industries) publishes its careers board through Greenhouse at
the bare slug `mrbeastyoutube`. The run-398 batch sweep confirmed
80 live role(s) via a direct probe of
...
- Ship a `source-company-mrbeastyoutube` plugin returning live
...
- Backfilling historical MrBeast (Beast Industries) postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MRBEAST_BEAST_INDUSTRIES`** in the
> source registry, so that **a single `siteType: [Site.MRBEAST_BEAST_INDUSTRIES]`
> request returns MrBeast (Beast Industries)'s open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MRBEAST_BEAST_INDUSTRIES = 'mrbeastyoutube'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mrbeastyoutube`.                  | must     |
| FR-3  | `MrBeastBeastIndustriesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mrbeastyoutube-`, `site === Site.MRBEAST_BEAST_INDUSTRIES`. | must |
...
```ts
@SourcePlugin({ site: Site.MRBEAST_BEAST_INDUSTRIES, name: 'MrBeast (Beast Industries)', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mrbeastyoutube/jobs/<id>`), wire
  `company_name` pass-through (`'MrBeast (Beast Industries)'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mrbeastyoutube/jobs/<id>`).
