# Spec: 681 — Source Company Plugin: Banyan Infrastructure

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 681                            |
| Slug           | source-company-banyaninfrastructure               |
| Status         | accepted                               |
...
Banyan Infrastructure publishes its careers board through Greenhouse at
the bare slug `banyaninfrastructure`. The run-398 batch sweep confirmed
...
- Ship a `source-company-banyaninfrastructure` plugin returning live
...
- Backfilling historical Banyan Infrastructure postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BANYAN_INFRASTRUCTURE`** in the
> source registry, so that **a single `siteType: [Site.BANYAN_INFRASTRUCTURE]`
> request returns Banyan Infrastructure's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BANYAN_INFRASTRUCTURE = 'banyaninfrastructure'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-banyaninfrastructure`.                  | must     |
| FR-3  | `BanyanInfrastructureService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `banyaninfrastructure-`, `site === Site.BANYAN_INFRASTRUCTURE`. | must |
...
```ts
@SourcePlugin({ site: Site.BANYAN_INFRASTRUCTURE, name: 'Banyan Infrastructure', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/banyaninfrastructure/jobs/<id>`), wire
  `company_name` pass-through (`'Banyan Infrastructure'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/banyaninfrastructure/jobs/<id>`).
