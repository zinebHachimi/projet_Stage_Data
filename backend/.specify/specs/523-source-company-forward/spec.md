# Spec: 523 — Source Company Plugin: Forward

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 523                            |
| Slug           | source-company-forward               |
| Status         | accepted                               |
...
Forward publishes its careers board through Greenhouse at
the bare slug `forward`. The run-398 batch sweep confirmed
...
- Ship a `source-company-forward` plugin returning live
...
- Backfilling historical Forward postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORWARD_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.FORWARD_HEALTH]`
> request returns Forward's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORWARD_HEALTH = 'forward'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-forward`.                  | must     |
| FR-3  | `ForwardService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `forward-`, `site === Site.FORWARD_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.FORWARD_HEALTH, name: 'Forward', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/forward/jobs/<id>`), wire
  `company_name` pass-through (`'Forward'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/forward/jobs/<id>`).
