# Plan: 523 — Source Company Plugin: Forward
...
Forward's careers board is hosted on Greenhouse at the slug
`forward`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-forward` plugin.
- Deliverables: package files, `Site.FORWARD_HEALTH` enum value, the
...
- `packages/plugins/source-company-forward` (new)
