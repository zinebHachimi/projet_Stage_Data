# Spec: 918 — Source Company Plugin: Recorded Future

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 918                            |
| Slug           | source-company-recordedfuture               |
| Status         | accepted                               |
...
Recorded Future publishes its careers board through Greenhouse at
the bare slug `recordedfuture`. The run-398 batch sweep confirmed
...
- Ship a `source-company-recordedfuture` plugin returning live
...
- Backfilling historical Recorded Future postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RECORDED_FUTURE`** in the
> source registry, so that **a single `siteType: [Site.RECORDED_FUTURE]`
> request returns Recorded Future's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RECORDED_FUTURE = 'recordedfuture'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-recordedfuture`.                  | must     |
| FR-3  | `RecordedFutureService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `recordedfuture-`, `site === Site.RECORDED_FUTURE`. | must |
...
```ts
@SourcePlugin({ site: Site.RECORDED_FUTURE, name: 'Recorded Future', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/recordedfuture/jobs/<id>`), wire
  `company_name` pass-through (`'Recorded Future'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/recordedfuture/jobs/<id>`).
