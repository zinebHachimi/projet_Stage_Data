# Spec: 539 — Source Company Plugin: Thoropass

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 539                            |
| Slug           | source-company-thoropass               |
| Status         | accepted                               |
...
Thoropass publishes its careers board through Greenhouse at
the bare slug `thoropass`. The run-398 batch sweep confirmed
...
- Ship a `source-company-thoropass` plugin returning live
...
- Backfilling historical Thoropass postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THOROPASS`** in the
> source registry, so that **a single `siteType: [Site.THOROPASS]`
> request returns Thoropass's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THOROPASS = 'thoropass'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thoropass`.                  | must     |
| FR-3  | `ThoropassService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thoropass-`, `site === Site.THOROPASS`. | must |
...
```ts
@SourcePlugin({ site: Site.THOROPASS, name: 'Thoropass', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thoropass/jobs/<id>`), wire
  `company_name` pass-through (`'Thoropass'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thoropass/jobs/<id>`).
