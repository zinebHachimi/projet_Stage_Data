# Spec: 465 — Source Company Plugin: SurveyMonkey

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 465                            |
| Slug           | source-company-surveymonkey               |
| Status         | accepted                               |
...
SurveyMonkey publishes its careers board through Greenhouse at
the bare slug `surveymonkey`. The run-398 batch sweep confirmed
...
- Ship a `source-company-surveymonkey` plugin returning live
...
- Backfilling historical SurveyMonkey postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SURVEYMONKEY`** in the
> source registry, so that **a single `siteType: [Site.SURVEYMONKEY]`
> request returns SurveyMonkey's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SURVEYMONKEY = 'surveymonkey'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-surveymonkey`.                  | must     |
| FR-3  | `SurveymonkeyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `surveymonkey-`, `site === Site.SURVEYMONKEY`. | must |
...
```ts
@SourcePlugin({ site: Site.SURVEYMONKEY, name: 'SurveyMonkey', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/surveymonkey/jobs/<id>`), wire
  `company_name` pass-through (`'SurveyMonkey'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/surveymonkey/jobs/<id>`).
