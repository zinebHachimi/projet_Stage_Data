# Plan: 465 — Source Company Plugin: SurveyMonkey
...
SurveyMonkey's careers board is hosted on Greenhouse at the slug
`surveymonkey`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-surveymonkey` plugin.
- Deliverables: package files, `Site.SURVEYMONKEY` enum value, the
...
- `packages/plugins/source-company-surveymonkey` (new)
