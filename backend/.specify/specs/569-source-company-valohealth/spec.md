# Spec: 569 — Source Company Plugin: Valo Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 569                            |
| Slug           | source-company-valohealth               |
| Status         | accepted                               |
...
Valo Health publishes its careers board through Greenhouse at
the bare slug `valohealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-valohealth` plugin returning live
...
- Backfilling historical Valo Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VALO_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.VALO_HEALTH]`
> request returns Valo Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VALO_HEALTH = 'valohealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-valohealth`.                  | must     |
| FR-3  | `ValohealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `valohealth-`, `site === Site.VALO_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.VALO_HEALTH, name: 'Valo Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/valohealth/jobs/<id>`), wire
  `company_name` pass-through (`'Valo Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/valohealth/jobs/<id>`).
