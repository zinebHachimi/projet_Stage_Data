# Spec: 834 — Source Company Plugin: Dealpath

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 834                            |
| Slug           | source-company-dealpath               |
| Status         | accepted                               |
...
Dealpath publishes its careers board through Greenhouse at
the bare slug `dealpath`. The run-398 batch sweep confirmed
...
- Ship a `source-company-dealpath` plugin returning live
...
- Backfilling historical Dealpath postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DEALPATH`** in the
> source registry, so that **a single `siteType: [Site.DEALPATH]`
> request returns Dealpath's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DEALPATH = 'dealpath'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-dealpath`.                  | must     |
| FR-3  | `DealpathService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `dealpath-`, `site === Site.DEALPATH`. | must |
...
```ts
@SourcePlugin({ site: Site.DEALPATH, name: 'Dealpath', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/dealpath/jobs/<id>`), wire
  `company_name` pass-through (`'Dealpath'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/dealpath/jobs/<id>`).
