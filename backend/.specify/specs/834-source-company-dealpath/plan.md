# Plan: 834 — Source Company Plugin: Dealpath
...
Dealpath's careers board is hosted on Greenhouse at the slug
`dealpath`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-dealpath` plugin.
- Deliverables: package files, `Site.DEALPATH` enum value, the
...
- `packages/plugins/source-company-dealpath` (new)
