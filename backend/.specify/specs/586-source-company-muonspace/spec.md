# Spec: 586 — Source Company Plugin: Muon Space

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 586                            |
| Slug           | source-company-muonspace               |
| Status         | accepted                               |
...
Muon Space publishes its careers board through Greenhouse at
the bare slug `muonspace`. The run-398 batch sweep confirmed
...
- Ship a `source-company-muonspace` plugin returning live
...
- Backfilling historical Muon Space postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MUON_SPACE`** in the
> source registry, so that **a single `siteType: [Site.MUON_SPACE]`
> request returns Muon Space's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MUON_SPACE = 'muonspace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-muonspace`.                  | must     |
| FR-3  | `MuonSpaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `muonspace-`, `site === Site.MUON_SPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.MUON_SPACE, name: 'Muon Space', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/muonspace/jobs/<id>`), wire
  `company_name` pass-through (`'Muon Space'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/muonspace/jobs/<id>`).
