# Spec: 216 — Source Company Plugin: AirSculpt

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 216                            |
| Slug           | source-company-airsculpt               |
| Status         | accepted                               |
...
AirSculpt publishes its careers board through Greenhouse at
the bare slug `airsculpt`. The run-398 batch sweep confirmed
...
- Ship a `source-company-airsculpt` plugin returning live
...
- Backfilling historical AirSculpt postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AIRSCULPT`** in the
> source registry, so that **a single `siteType: [Site.AIRSCULPT]`
> request returns AirSculpt's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AIRSCULPT = 'airsculpt'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-airsculpt`.                  | must     |
| FR-3  | `AirsculptService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `airsculpt-`, `site === Site.AIRSCULPT`. | must |
...
```ts
@SourcePlugin({ site: Site.AIRSCULPT, name: 'AirSculpt', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/airsculpt/jobs/<id>`), wire
  `company_name` pass-through (`'AirSculpt'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/airsculpt/jobs/<id>`).
