# Spec: 752 — Source Company Plugin: OfferUp

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 752                            |
| Slug           | source-company-offerup               |
| Status         | accepted                               |
...
OfferUp publishes its careers board through Greenhouse at
the bare slug `offerup`. The run-398 batch sweep confirmed
...
- Ship a `source-company-offerup` plugin returning live
...
- Backfilling historical OfferUp postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OFFERUP`** in the
> source registry, so that **a single `siteType: [Site.OFFERUP]`
> request returns OfferUp's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OFFERUP = 'offerup'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-offerup`.                  | must     |
| FR-3  | `OfferUpService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `offerup-`, `site === Site.OFFERUP`. | must |
...
```ts
@SourcePlugin({ site: Site.OFFERUP, name: 'OfferUp', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/offerup/jobs/<id>`), wire
  `company_name` pass-through (`'OfferUp'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/offerup/jobs/<id>`).
