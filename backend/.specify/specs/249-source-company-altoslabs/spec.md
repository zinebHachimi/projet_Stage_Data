# Spec: 249 — Source Company Plugin: Altos Labs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 249                            |
| Slug           | source-company-altoslabs               |
| Status         | accepted                               |
...
Altos Labs publishes its careers board through Greenhouse at
the bare slug `altoslabs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-altoslabs` plugin returning live
...
- Backfilling historical Altos Labs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALTOSLABS`** in the
> source registry, so that **a single `siteType: [Site.ALTOSLABS]`
> request returns Altos Labs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALTOSLABS = 'altoslabs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-altoslabs`.                  | must     |
| FR-3  | `AltoslabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `altoslabs-`, `site === Site.ALTOSLABS`. | must |
...
```ts
@SourcePlugin({ site: Site.ALTOSLABS, name: 'Altos Labs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/altoslabs/jobs/<id>`), wire
  `company_name` pass-through (`'Altos Labs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/altoslabs/jobs/<id>`).
