# Spec: 277 — Source Company Plugin: Apaleo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 277                            |
| Slug           | source-company-apaleo               |
| Status         | accepted                               |
...
Apaleo publishes its careers board through Greenhouse at
the bare slug `apaleo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-apaleo` plugin returning live
...
- Backfilling historical Apaleo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APALEO`** in the
> source registry, so that **a single `siteType: [Site.APALEO]`
> request returns Apaleo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APALEO = 'apaleo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apaleo`.                  | must     |
| FR-3  | `ApaleoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apaleo-`, `site === Site.APALEO`. | must |
...
```ts
@SourcePlugin({ site: Site.APALEO, name: 'Apaleo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apaleo/jobs/<id>`), wire
  `company_name` pass-through (`'Apaleo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apaleo/jobs/<id>`).
