# Plan: 443 — Source Company Plugin: Materialize
...
Materialize's careers board is hosted on Greenhouse at the slug
`materialize`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-materialize` plugin.
- Deliverables: package files, `Site.MATERIALIZE` enum value, the
...
- `packages/plugins/source-company-materialize` (new)
