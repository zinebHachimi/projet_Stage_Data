# Spec: 443 — Source Company Plugin: Materialize

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 443                            |
| Slug           | source-company-materialize               |
| Status         | accepted                               |
...
Materialize publishes its careers board through Greenhouse at
the bare slug `materialize`. The run-398 batch sweep confirmed
...
- Ship a `source-company-materialize` plugin returning live
...
- Backfilling historical Materialize postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MATERIALIZE`** in the
> source registry, so that **a single `siteType: [Site.MATERIALIZE]`
> request returns Materialize's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MATERIALIZE = 'materialize'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-materialize`.                  | must     |
| FR-3  | `MaterializeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `materialize-`, `site === Site.MATERIALIZE`. | must |
...
```ts
@SourcePlugin({ site: Site.MATERIALIZE, name: 'Materialize', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/materialize/jobs/<id>`), wire
  `company_name` pass-through (`'Materialize'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/materialize/jobs/<id>`).
