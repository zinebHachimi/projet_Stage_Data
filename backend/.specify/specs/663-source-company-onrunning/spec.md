# Spec: 663 — Source Company Plugin: On Running

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 663                            |
| Slug           | source-company-onrunning               |
| Status         | accepted                               |
...
On Running publishes its careers board through Greenhouse at
the bare slug `onrunning`. The run-398 batch sweep confirmed
313 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/onrunning/jobs?content=true`.
Sector: Footwear / Sportswear. HQ: Zurich, Switzerland.

## 2. Goals

- Ship a `source-company-onrunning` plugin returning live
...
- Backfilling historical On Running postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ON_RUNNING`** in the
> source registry, so that **a single `siteType: [Site.ON_RUNNING]`
> request returns On Running's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ON_RUNNING = 'onrunning'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-onrunning`.                  | must     |
| FR-3  | `OnRunningService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `onrunning-`, `site === Site.ON_RUNNING`. | must |
...
```ts
@SourcePlugin({ site: Site.ON_RUNNING, name: 'On Running', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/onrunning/jobs/<id>`), wire
  `company_name` pass-through (`'On Running'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/onrunning/jobs/<id>`).
