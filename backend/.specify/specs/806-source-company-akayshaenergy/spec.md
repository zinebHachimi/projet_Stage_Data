# Spec: 806 — Source Company Plugin: Akaysha Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 806                            |
| Slug           | source-company-akayshaenergy               |
| Status         | accepted                               |
...
Akaysha Energy publishes its careers board through Greenhouse at
the bare slug `akayshaenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-akayshaenergy` plugin returning live
...
- Backfilling historical Akaysha Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AKAYSHA_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.AKAYSHA_ENERGY]`
> request returns Akaysha Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AKAYSHA_ENERGY = 'akayshaenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-akayshaenergy`.                  | must     |
| FR-3  | `AkayshaEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `akayshaenergy-`, `site === Site.AKAYSHA_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.AKAYSHA_ENERGY, name: 'Akaysha Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/akayshaenergy/jobs/<id>`), wire
  `company_name` pass-through (`'Akaysha Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/akayshaenergy/jobs/<id>`).
