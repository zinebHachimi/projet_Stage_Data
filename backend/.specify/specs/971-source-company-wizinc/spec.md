# Spec: 971 — Source Company Plugin: Wiz

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 971                            |
| Slug           | source-company-wizinc               |
| Status         | accepted                               |
...
Wiz publishes its careers board through Greenhouse at
the bare slug `wizinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-wizinc` plugin returning live
...
- Backfilling historical Wiz postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WIZ`** in the
> source registry, so that **a single `siteType: [Site.WIZ]`
> request returns Wiz's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WIZ = 'wizinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-wizinc`.                  | must     |
| FR-3  | `WizService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `wizinc-`, `site === Site.WIZ`. | must |
...
```ts
@SourcePlugin({ site: Site.WIZ, name: 'Wiz', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/wizinc/jobs/<id>`), wire
  `company_name` pass-through (`'Wiz'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/wizinc/jobs/<id>`).
