# Spec: 482 — Source Company Plugin: Warp

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 482                            |
| Slug           | source-company-warp               |
| Status         | accepted                               |
...
Warp publishes its careers board through Greenhouse at
the bare slug `warp`. The run-398 batch sweep confirmed
...
- Ship a `source-company-warp` plugin returning live
...
- Backfilling historical Warp postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WARP`** in the
> source registry, so that **a single `siteType: [Site.WARP]`
> request returns Warp's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WARP = 'warp'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-warp`.                  | must     |
| FR-3  | `WarpService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `warp-`, `site === Site.WARP`. | must |
...
```ts
@SourcePlugin({ site: Site.WARP, name: 'Warp', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/warp/jobs/<id>`), wire
  `company_name` pass-through (`'Warp'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/warp/jobs/<id>`).
