# Spec: 474 — Source Company Plugin: Speechmatics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 474                            |
| Slug           | source-company-speechmatics               |
| Status         | accepted                               |
...
Speechmatics publishes its careers board through Greenhouse at
the bare slug `speechmatics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-speechmatics` plugin returning live
...
- Backfilling historical Speechmatics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPEECHMATICS`** in the
> source registry, so that **a single `siteType: [Site.SPEECHMATICS]`
> request returns Speechmatics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPEECHMATICS = 'speechmatics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-speechmatics`.                  | must     |
| FR-3  | `SpeechmaticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `speechmatics-`, `site === Site.SPEECHMATICS`. | must |
...
```ts
@SourcePlugin({ site: Site.SPEECHMATICS, name: 'Speechmatics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/speechmatics/jobs/<id>`), wire
  `company_name` pass-through (`'Speechmatics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/speechmatics/jobs/<id>`).
