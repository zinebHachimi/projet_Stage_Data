# Spec: 756 — Source Company Plugin: Absci

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 756                            |
| Slug           | source-company-absci               |
| Status         | accepted                               |
...
Absci publishes its careers board through Greenhouse at
the bare slug `absci`. The run-398 batch sweep confirmed
...
- Ship a `source-company-absci` plugin returning live
...
- Backfilling historical Absci postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ABSCI`** in the
> source registry, so that **a single `siteType: [Site.ABSCI]`
> request returns Absci's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ABSCI = 'absci'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-absci`.                  | must     |
| FR-3  | `AbsciService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `absci-`, `site === Site.ABSCI`. | must |
...
```ts
@SourcePlugin({ site: Site.ABSCI, name: 'Absci', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/absci/jobs/<id>`), wire
  `company_name` pass-through (`'Absci'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/absci/jobs/<id>`).
