# Spec: 543 — Source Company Plugin: Expel

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 543                            |
| Slug           | source-company-expel               |
| Status         | accepted                               |
...
Expel publishes its careers board through Greenhouse at
the bare slug `expel`. The run-398 batch sweep confirmed
...
- Ship a `source-company-expel` plugin returning live
...
- Backfilling historical Expel postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EXPEL`** in the
> source registry, so that **a single `siteType: [Site.EXPEL]`
> request returns Expel's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EXPEL = 'expel'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-expel`.                  | must     |
| FR-3  | `ExpelService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `expel-`, `site === Site.EXPEL`. | must |
...
```ts
@SourcePlugin({ site: Site.EXPEL, name: 'Expel', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/expel/jobs/<id>`), wire
  `company_name` pass-through (`'Expel'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/expel/jobs/<id>`).
