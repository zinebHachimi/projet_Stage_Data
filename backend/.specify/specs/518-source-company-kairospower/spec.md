# Spec: 518 — Source Company Plugin: Kairos Power

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 518                            |
| Slug           | source-company-kairospower               |
| Status         | accepted                               |
...
Kairos Power publishes its careers board through Greenhouse at
the bare slug `kairospower`. The run-398 batch sweep confirmed
...
- Ship a `source-company-kairospower` plugin returning live
...
- Backfilling historical Kairos Power postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KAIROS_POWER`** in the
> source registry, so that **a single `siteType: [Site.KAIROS_POWER]`
> request returns Kairos Power's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KAIROS_POWER = 'kairospower'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-kairospower`.                  | must     |
| FR-3  | `KairospowerService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `kairospower-`, `site === Site.KAIROS_POWER`. | must |
...
```ts
@SourcePlugin({ site: Site.KAIROS_POWER, name: 'Kairos Power', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/kairospower/jobs/<id>`), wire
  `company_name` pass-through (`'Kairos Power'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/kairospower/jobs/<id>`).
