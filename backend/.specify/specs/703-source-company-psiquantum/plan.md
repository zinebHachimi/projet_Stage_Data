# Plan: 703 — Source Company Plugin: PsiQuantum
...
PsiQuantum's careers board is hosted on Greenhouse at the slug
`psiquantum`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-psiquantum` plugin.
- Deliverables: package files, `Site.PSIQUANTUM` enum value, the
...
- `packages/plugins/source-company-psiquantum` (new)
