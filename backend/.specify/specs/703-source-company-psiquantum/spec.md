# Spec: 703 — Source Company Plugin: PsiQuantum

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 703                            |
| Slug           | source-company-psiquantum               |
| Status         | accepted                               |
...
PsiQuantum publishes its careers board through Greenhouse at
the bare slug `psiquantum`. The run-398 batch sweep confirmed
96 live role(s) via a direct probe of
...
- Ship a `source-company-psiquantum` plugin returning live
...
- Backfilling historical PsiQuantum postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PSIQUANTUM`** in the
> source registry, so that **a single `siteType: [Site.PSIQUANTUM]`
> request returns PsiQuantum's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PSIQUANTUM = 'psiquantum'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-psiquantum`.                  | must     |
| FR-3  | `PsiQuantumService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `psiquantum-`, `site === Site.PSIQUANTUM`. | must |
...
```ts
@SourcePlugin({ site: Site.PSIQUANTUM, name: 'PsiQuantum', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/psiquantum/jobs/<id>`), wire
  `company_name` pass-through (`'PsiQuantum'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/psiquantum/jobs/<id>`).
