# Spec: 246 — Source Company Plugin: Altana 

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 246                            |
| Slug           | source-company-altanaai               |
| Status         | accepted                               |
...
Altana  publishes its careers board through Greenhouse at
the bare slug `altanaai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-altanaai` plugin returning live
...
- Backfilling historical Altana  postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALTANAAI`** in the
> source registry, so that **a single `siteType: [Site.ALTANAAI]`
> request returns Altana 's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALTANAAI = 'altanaai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-altanaai`.                  | must     |
| FR-3  | `AltanaaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `altanaai-`, `site === Site.ALTANAAI`. | must |
...
```ts
@SourcePlugin({ site: Site.ALTANAAI, name: 'Altana ', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/altanaai/jobs/<id>`), wire
  `company_name` pass-through (`'Altana '`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/altanaai/jobs/<id>`).
