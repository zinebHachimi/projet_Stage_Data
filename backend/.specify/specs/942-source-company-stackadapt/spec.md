# Spec: 942 — Source Company Plugin: StackAdapt

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 942                            |
| Slug           | source-company-stackadapt               |
| Status         | accepted                               |
...
StackAdapt publishes its careers board through Greenhouse at
the bare slug `stackadapt`. The run-398 batch sweep confirmed
...
- Ship a `source-company-stackadapt` plugin returning live
...
- Backfilling historical StackAdapt postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STACKADAPT`** in the
> source registry, so that **a single `siteType: [Site.STACKADAPT]`
> request returns StackAdapt's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STACKADAPT = 'stackadapt'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-stackadapt`.                  | must     |
| FR-3  | `StackAdaptService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `stackadapt-`, `site === Site.STACKADAPT`. | must |
...
```ts
@SourcePlugin({ site: Site.STACKADAPT, name: 'StackAdapt', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/stackadapt/jobs/<id>`), wire
  `company_name` pass-through (`'StackAdapt'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/stackadapt/jobs/<id>`).
