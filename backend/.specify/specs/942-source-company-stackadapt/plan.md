# Plan: 942 — Source Company Plugin: StackAdapt
...
StackAdapt's careers board is hosted on Greenhouse at the slug
`stackadapt`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-stackadapt` plugin.
- Deliverables: package files, `Site.STACKADAPT` enum value, the
...
- `packages/plugins/source-company-stackadapt` (new)
