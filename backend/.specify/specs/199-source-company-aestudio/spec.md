# Spec: 199 — Source Company Plugin: AE Studio

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 199                            |
| Slug           | source-company-aestudio               |
| Status         | accepted                               |
...
AE Studio publishes its careers board through Greenhouse at
the bare slug `aestudio`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aestudio` plugin returning live
...
- Backfilling historical AE Studio postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AESTUDIO`** in the
> source registry, so that **a single `siteType: [Site.AESTUDIO]`
> request returns AE Studio's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AESTUDIO = 'aestudio'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aestudio`.                  | must     |
| FR-3  | `AestudioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aestudio-`, `site === Site.AESTUDIO`. | must |
...
```ts
@SourcePlugin({ site: Site.AESTUDIO, name: 'AE Studio', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aestudio/jobs/<id>`), wire
  `company_name` pass-through (`'AE Studio'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aestudio/jobs/<id>`).
