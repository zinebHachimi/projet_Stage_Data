# Spec: 771 — Source Company Plugin: Twist Bioscience

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 771                            |
| Slug           | source-company-twistbioscience               |
| Status         | accepted                               |
...
Twist Bioscience publishes its careers board through Greenhouse at
the bare slug `twistbioscience`. The run-398 batch sweep confirmed
...
- Ship a `source-company-twistbioscience` plugin returning live
...
- Backfilling historical Twist Bioscience postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TWIST_BIOSCIENCE`** in the
> source registry, so that **a single `siteType: [Site.TWIST_BIOSCIENCE]`
> request returns Twist Bioscience's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TWIST_BIOSCIENCE = 'twistbioscience'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-twistbioscience`.                  | must     |
| FR-3  | `TwistBioscienceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `twistbioscience-`, `site === Site.TWIST_BIOSCIENCE`. | must |
...
```ts
@SourcePlugin({ site: Site.TWIST_BIOSCIENCE, name: 'Twist Bioscience', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/twistbioscience/jobs/<id>`), wire
  `company_name` pass-through (`'Twist Bioscience'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/twistbioscience/jobs/<id>`).
