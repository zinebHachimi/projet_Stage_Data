# Spec: 222 — Source Company Plugin: AKKO

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 222                            |
| Slug           | source-company-akko               |
| Status         | accepted                               |
...
AKKO publishes its careers board through Greenhouse at
the bare slug `akko`. The run-398 batch sweep confirmed
...
- Ship a `source-company-akko` plugin returning live
...
- Backfilling historical AKKO postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AKKO`** in the
> source registry, so that **a single `siteType: [Site.AKKO]`
> request returns AKKO's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AKKO = 'akko'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-akko`.                  | must     |
| FR-3  | `AkkoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `akko-`, `site === Site.AKKO`. | must |
...
```ts
@SourcePlugin({ site: Site.AKKO, name: 'AKKO', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/akko/jobs/<id>`), wire
  `company_name` pass-through (`'AKKO'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/akko/jobs/<id>`).
