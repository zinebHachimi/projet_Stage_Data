# Spec: 828 — Source Company Plugin: Collibra

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 828                            |
| Slug           | source-company-collibra               |
| Status         | accepted                               |
...
Collibra publishes its careers board through Greenhouse at
the bare slug `collibra`. The run-398 batch sweep confirmed
...
- Ship a `source-company-collibra` plugin returning live
...
- Backfilling historical Collibra postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COLLIBRA`** in the
> source registry, so that **a single `siteType: [Site.COLLIBRA]`
> request returns Collibra's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COLLIBRA = 'collibra'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-collibra`.                  | must     |
| FR-3  | `CollibraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `collibra-`, `site === Site.COLLIBRA`. | must |
...
```ts
@SourcePlugin({ site: Site.COLLIBRA, name: 'Collibra', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/collibra/jobs/<id>`), wire
  `company_name` pass-through (`'Collibra'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/collibra/jobs/<id>`).
