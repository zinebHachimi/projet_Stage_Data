# Spec: 655 — Source Company Plugin: Ledgy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 655                            |
| Slug           | source-company-ledgy               |
| Status         | accepted                               |
...
Ledgy publishes its careers board through Greenhouse at
the bare slug `ledgy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ledgy` plugin returning live
...
- Backfilling historical Ledgy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LEDGY`** in the
> source registry, so that **a single `siteType: [Site.LEDGY]`
> request returns Ledgy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LEDGY = 'ledgy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ledgy`.                  | must     |
| FR-3  | `LedgyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ledgy-`, `site === Site.LEDGY`. | must |
...
```ts
@SourcePlugin({ site: Site.LEDGY, name: 'Ledgy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ledgy/jobs/<id>`), wire
  `company_name` pass-through (`'Ledgy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ledgy/jobs/<id>`).
