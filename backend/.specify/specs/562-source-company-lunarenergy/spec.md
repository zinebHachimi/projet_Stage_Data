# Spec: 562 — Source Company Plugin: Lunar Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 562                            |
| Slug           | source-company-lunarenergy               |
| Status         | accepted                               |
...
Lunar Energy publishes its careers board through Greenhouse at
the bare slug `lunarenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-lunarenergy` plugin returning live
...
- Backfilling historical Lunar Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LUNAR_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.LUNAR_ENERGY]`
> request returns Lunar Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LUNAR_ENERGY = 'lunarenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-lunarenergy`.                  | must     |
| FR-3  | `LunarenergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `lunarenergy-`, `site === Site.LUNAR_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.LUNAR_ENERGY, name: 'Lunar Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/lunarenergy/jobs/<id>`), wire
  `company_name` pass-through (`'Lunar Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/lunarenergy/jobs/<id>`).
