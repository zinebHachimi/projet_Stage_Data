# Spec: 954 — Source Company Plugin: The New York Times

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 954                            |
| Slug           | source-company-thenewyorktimes               |
| Status         | accepted                               |
...
The New York Times publishes its careers board through Greenhouse at
the bare slug `thenewyorktimes`. The run-398 batch sweep confirmed
157 live role(s) via a direct probe of
...
- Ship a `source-company-thenewyorktimes` plugin returning live
...
- Backfilling historical The New York Times postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THE_NEW_YORK_TIMES`** in the
> source registry, so that **a single `siteType: [Site.THE_NEW_YORK_TIMES]`
> request returns The New York Times's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THE_NEW_YORK_TIMES = 'thenewyorktimes'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thenewyorktimes`.                  | must     |
| FR-3  | `TheNewYorkTimesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thenewyorktimes-`, `site === Site.THE_NEW_YORK_TIMES`. | must |
...
```ts
@SourcePlugin({ site: Site.THE_NEW_YORK_TIMES, name: 'The New York Times', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thenewyorktimes/jobs/<id>`), wire
  `company_name` pass-through (`'The New York Times'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thenewyorktimes/jobs/<id>`).
