# Spec: 953 — Source Company Plugin: Dutchie

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 953                            |
| Slug           | source-company-thedutchie               |
| Status         | accepted                               |
...
Dutchie publishes its careers board through Greenhouse at
the bare slug `thedutchie`. The run-398 batch sweep confirmed
...
- Ship a `source-company-thedutchie` plugin returning live
...
- Backfilling historical Dutchie postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DUTCHIE`** in the
> source registry, so that **a single `siteType: [Site.DUTCHIE]`
> request returns Dutchie's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DUTCHIE = 'thedutchie'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thedutchie`.                  | must     |
| FR-3  | `DutchieService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thedutchie-`, `site === Site.DUTCHIE`. | must |
...
```ts
@SourcePlugin({ site: Site.DUTCHIE, name: 'Dutchie', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thedutchie/jobs/<id>`), wire
  `company_name` pass-through (`'Dutchie'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thedutchie/jobs/<id>`).
