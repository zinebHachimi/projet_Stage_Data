# Spec: 201 — Source Company Plugin: Affinity.co

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 201                            |
| Slug           | source-company-affinity               |
| Status         | accepted                               |
...
Affinity.co publishes its careers board through Greenhouse at
the bare slug `affinity`. The run-398 batch sweep confirmed
...
- Ship a `source-company-affinity` plugin returning live
...
- Backfilling historical Affinity.co postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AFFINITY`** in the
> source registry, so that **a single `siteType: [Site.AFFINITY]`
> request returns Affinity.co's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AFFINITY = 'affinity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-affinity`.                  | must     |
| FR-3  | `AffinityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `affinity-`, `site === Site.AFFINITY`. | must |
...
```ts
@SourcePlugin({ site: Site.AFFINITY, name: 'Affinity.co', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/affinity/jobs/<id>`), wire
  `company_name` pass-through (`'Affinity.co'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/affinity/jobs/<id>`).
