# Plan: 031 — Source Company Plugin: Asana
...
Asana's careers board is hosted on Greenhouse at the slug `asana`, so
the implementation is a thin wrapper around the same public Greenhouse
endpoint that `source-company-plaid` (Spec 030 / run #240),
`source-company-lyft` (Spec 029), `source-company-pinterest`
(Spec 028), `source-company-reddit` (Spec 027),
...
Plaid plugin's shape (single-file `service.ts`, three-line
`module.ts`, two-line `index.ts`, four-line `package.json`,
three-line `tsconfig.json`) and rename references from Plaid → Asana,
...
used nineteen times before (Amazon, Apple, Cursor, Google, IBM,
Meta, OpenAI, Stripe, Anthropic, Databricks, Discord, Coinbase,
DoorDash, Airbnb, Robinhood, Reddit, Pinterest, Lyft, Plaid).

## 2. Phases

### Phase 1 — Scaffold + register + test (single PR)

- **Goal:** Land `source-company-asana` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-asana/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,asana.module.ts,asana.service.ts}`,
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 031 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-asana` → all green.
...
| `packages/models/src/enums/site.enum.ts`            | append `ASANA = 'asana'`.                                    |
| `packages/plugins/index.ts`                         | import + append `AsanaModule` to `ALL_SOURCE_MODULES`.       |
| `tsconfig.base.json`                                | path-alias entry.                                            |
| `jest.config.js`                                    | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                   | add Asana shipped row.                                       |
| `docs/index.md`                                     | append Spec 031 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Asana moves off Greenhouse mid-maintenance.         | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-plaid`'s shape.          | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.ASANA` is unused outside this plugin, so the enum revert is
  safe.
...
(None — see Spec 031 § 10 Decisions for the five resolved points.)
