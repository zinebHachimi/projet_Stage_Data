# Spec: 633 — Source Company Plugin: Sparkfund

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 633                            |
| Slug           | source-company-sparkfund               |
| Status         | accepted                               |
...
Sparkfund publishes its careers board through Greenhouse at
the bare slug `sparkfund`. The run-398 batch sweep confirmed
...
- Ship a `source-company-sparkfund` plugin returning live
...
- Backfilling historical Sparkfund postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPARKFUND`** in the
> source registry, so that **a single `siteType: [Site.SPARKFUND]`
> request returns Sparkfund's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPARKFUND = 'sparkfund'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sparkfund`.                  | must     |
| FR-3  | `SparkfundService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sparkfund-`, `site === Site.SPARKFUND`. | must |
...
```ts
@SourcePlugin({ site: Site.SPARKFUND, name: 'Sparkfund', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sparkfund/jobs/<id>`), wire
  `company_name` pass-through (`'Sparkfund'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sparkfund/jobs/<id>`).
