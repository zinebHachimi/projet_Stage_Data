# Spec: 491 — Source Company Plugin: Thunes

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 491                            |
| Slug           | source-company-thunes               |
| Status         | accepted                               |
...
Thunes publishes its careers board through Greenhouse at
the bare slug `thunes`. The run-398 batch sweep confirmed
...
- Ship a `source-company-thunes` plugin returning live
...
- Backfilling historical Thunes postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THUNES`** in the
> source registry, so that **a single `siteType: [Site.THUNES]`
> request returns Thunes's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THUNES = 'thunes'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thunes`.                  | must     |
| FR-3  | `ThunesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thunes-`, `site === Site.THUNES`. | must |
...
```ts
@SourcePlugin({ site: Site.THUNES, name: 'Thunes', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thunes/jobs/<id>`), wire
  `company_name` pass-through (`'Thunes'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thunes/jobs/<id>`).
