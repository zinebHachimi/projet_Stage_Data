# Spec: 809 — Source Company Plugin: At-Bay

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 809                            |
| Slug           | source-company-atbayjobs               |
| Status         | accepted                               |
...
At-Bay publishes its careers board through Greenhouse at
the bare slug `atbayjobs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-atbayjobs` plugin returning live
...
- Backfilling historical At-Bay postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AT_BAY`** in the
> source registry, so that **a single `siteType: [Site.AT_BAY]`
> request returns At-Bay's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AT_BAY = 'atbayjobs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-atbayjobs`.                  | must     |
| FR-3  | `AtBayService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `atbayjobs-`, `site === Site.AT_BAY`. | must |
...
```ts
@SourcePlugin({ site: Site.AT_BAY, name: 'At-Bay', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/atbayjobs/jobs/<id>`), wire
  `company_name` pass-through (`'At-Bay'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/atbayjobs/jobs/<id>`).
