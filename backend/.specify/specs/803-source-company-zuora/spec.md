# Spec: 803 — Source Company Plugin: Zuora

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 803                            |
| Slug           | source-company-zuora               |
| Status         | accepted                               |
...
Zuora publishes its careers board through Greenhouse at
the bare slug `zuora`. The run-398 batch sweep confirmed
...
- Ship a `source-company-zuora` plugin returning live
...
- Backfilling historical Zuora postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ZUORA`** in the
> source registry, so that **a single `siteType: [Site.ZUORA]`
> request returns Zuora's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ZUORA = 'zuora'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-zuora`.                  | must     |
| FR-3  | `ZuoraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `zuora-`, `site === Site.ZUORA`. | must |
...
```ts
@SourcePlugin({ site: Site.ZUORA, name: 'Zuora', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/zuora/jobs/<id>`), wire
  `company_name` pass-through (`'Zuora'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/zuora/jobs/<id>`).
