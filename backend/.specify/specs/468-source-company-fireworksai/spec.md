# Spec: 468 — Source Company Plugin: Fireworks AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 468                            |
| Slug           | source-company-fireworksai               |
| Status         | accepted                               |
...
Fireworks AI publishes its careers board through Greenhouse at
the bare slug `fireworksai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fireworksai` plugin returning live
...
- Backfilling historical Fireworks AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FIREWORKSAI`** in the
> source registry, so that **a single `siteType: [Site.FIREWORKSAI]`
> request returns Fireworks AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FIREWORKSAI = 'fireworksai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fireworksai`.                  | must     |
| FR-3  | `FireworksaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fireworksai-`, `site === Site.FIREWORKSAI`. | must |
...
```ts
@SourcePlugin({ site: Site.FIREWORKSAI, name: 'Fireworks AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fireworksai/jobs/<id>`), wire
  `company_name` pass-through (`'Fireworks AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fireworksai/jobs/<id>`).
