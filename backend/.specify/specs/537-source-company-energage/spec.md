# Spec: 537 — Source Company Plugin: Energage

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 537                            |
| Slug           | source-company-energage               |
| Status         | accepted                               |
...
Energage publishes its careers board through Greenhouse at
the bare slug `energage`. The run-398 batch sweep confirmed
...
- Ship a `source-company-energage` plugin returning live
...
- Backfilling historical Energage postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ENERGAGE`** in the
> source registry, so that **a single `siteType: [Site.ENERGAGE]`
> request returns Energage's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ENERGAGE = 'energage'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-energage`.                  | must     |
| FR-3  | `EnergageService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `energage-`, `site === Site.ENERGAGE`. | must |
...
```ts
@SourcePlugin({ site: Site.ENERGAGE, name: 'Energage', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/energage/jobs/<id>`), wire
  `company_name` pass-through (`'Energage'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/energage/jobs/<id>`).
