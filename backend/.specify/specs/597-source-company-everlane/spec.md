# Spec: 597 — Source Company Plugin: Everlane

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 597                            |
| Slug           | source-company-everlane               |
| Status         | accepted                               |
...
Everlane publishes its careers board through Greenhouse at
the bare slug `everlane`. The run-398 batch sweep confirmed
...
- Ship a `source-company-everlane` plugin returning live
...
- Backfilling historical Everlane postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EVERLANE`** in the
> source registry, so that **a single `siteType: [Site.EVERLANE]`
> request returns Everlane's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EVERLANE = 'everlane'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-everlane`.                  | must     |
| FR-3  | `EverlaneService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `everlane-`, `site === Site.EVERLANE`. | must |
...
```ts
@SourcePlugin({ site: Site.EVERLANE, name: 'Everlane', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/everlane/jobs/<id>`), wire
  `company_name` pass-through (`'Everlane'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/everlane/jobs/<id>`).
