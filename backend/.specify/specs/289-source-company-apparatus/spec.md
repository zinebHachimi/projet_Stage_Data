# Spec: 289 — Source Company Plugin: APPARATUS

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 289                            |
| Slug           | source-company-apparatus               |
| Status         | accepted                               |
...
APPARATUS publishes its careers board through Greenhouse at
the bare slug `apparatus`. The run-398 batch sweep confirmed
...
- Ship a `source-company-apparatus` plugin returning live
...
- Backfilling historical APPARATUS postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APPARATUS`** in the
> source registry, so that **a single `siteType: [Site.APPARATUS]`
> request returns APPARATUS's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APPARATUS = 'apparatus'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apparatus`.                  | must     |
| FR-3  | `ApparatusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apparatus-`, `site === Site.APPARATUS`. | must |
...
```ts
@SourcePlugin({ site: Site.APPARATUS, name: 'APPARATUS', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apparatus/jobs/<id>`), wire
  `company_name` pass-through (`'APPARATUS'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apparatus/jobs/<id>`).
