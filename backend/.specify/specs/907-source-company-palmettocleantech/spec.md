# Spec: 907 — Source Company Plugin: Palmetto Clean Technology

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 907                            |
| Slug           | source-company-palmettocleantech               |
| Status         | accepted                               |
...
Palmetto Clean Technology publishes its careers board through Greenhouse at
the bare slug `palmettocleantech`. The run-398 batch sweep confirmed
...
- Ship a `source-company-palmettocleantech` plugin returning live
...
- Backfilling historical Palmetto Clean Technology postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PALMETTO_CLEAN_TECHNOLOGY`** in the
> source registry, so that **a single `siteType: [Site.PALMETTO_CLEAN_TECHNOLOGY]`
> request returns Palmetto Clean Technology's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PALMETTO_CLEAN_TECHNOLOGY = 'palmettocleantech'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-palmettocleantech`.                  | must     |
| FR-3  | `PalmettoCleanTechnologyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `palmettocleantech-`, `site === Site.PALMETTO_CLEAN_TECHNOLOGY`. | must |
...
```ts
@SourcePlugin({ site: Site.PALMETTO_CLEAN_TECHNOLOGY, name: 'Palmetto Clean Technology', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/palmettocleantech/jobs/<id>`), wire
  `company_name` pass-through (`'Palmetto Clean Technology'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/palmettocleantech/jobs/<id>`).
