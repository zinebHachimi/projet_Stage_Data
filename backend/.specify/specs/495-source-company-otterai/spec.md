# Spec: 495 — Source Company Plugin: Otter.ai

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 495                            |
| Slug           | source-company-otterai               |
| Status         | accepted                               |
...
Otter.ai publishes its careers board through Greenhouse at
the bare slug `otterai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-otterai` plugin returning live
...
- Backfilling historical Otter.ai postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OTTERAI`** in the
> source registry, so that **a single `siteType: [Site.OTTERAI]`
> request returns Otter.ai's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OTTERAI = 'otterai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-otterai`.                  | must     |
| FR-3  | `OtteraiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `otterai-`, `site === Site.OTTERAI`. | must |
...
```ts
@SourcePlugin({ site: Site.OTTERAI, name: 'Otter.ai', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/otterai/jobs/<id>`), wire
  `company_name` pass-through (`'Otter.ai'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/otterai/jobs/<id>`).
