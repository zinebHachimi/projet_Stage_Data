# Spec: 894 — Source Company Plugin: Next Insurance

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 894                            |
| Slug           | source-company-nextinsurance66               |
| Status         | accepted                               |
...
Next Insurance publishes its careers board through Greenhouse at
the bare slug `nextinsurance66`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nextinsurance66` plugin returning live
...
- Backfilling historical Next Insurance postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEXT_INSURANCE`** in the
> source registry, so that **a single `siteType: [Site.NEXT_INSURANCE]`
> request returns Next Insurance's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEXT_INSURANCE = 'nextinsurance66'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nextinsurance66`.                  | must     |
| FR-3  | `NextInsuranceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nextinsurance66-`, `site === Site.NEXT_INSURANCE`. | must |
...
```ts
@SourcePlugin({ site: Site.NEXT_INSURANCE, name: 'Next Insurance', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nextinsurance66/jobs/<id>`), wire
  `company_name` pass-through (`'Next Insurance'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nextinsurance66/jobs/<id>`).
