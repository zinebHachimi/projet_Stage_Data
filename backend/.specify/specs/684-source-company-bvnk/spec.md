# Spec: 684 — Source Company Plugin: BVNK

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 684                            |
| Slug           | source-company-bvnk               |
| Status         | accepted                               |
...
BVNK publishes its careers board through Greenhouse at
the bare slug `bvnk`. The run-398 batch sweep confirmed
...
- Ship a `source-company-bvnk` plugin returning live
...
- Backfilling historical BVNK postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BVNK`** in the
> source registry, so that **a single `siteType: [Site.BVNK]`
> request returns BVNK's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BVNK = 'bvnk'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-bvnk`.                  | must     |
| FR-3  | `BVNKService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `bvnk-`, `site === Site.BVNK`. | must |
...
```ts
@SourcePlugin({ site: Site.BVNK, name: 'BVNK', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/bvnk/jobs/<id>`), wire
  `company_name` pass-through (`'BVNK'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/bvnk/jobs/<id>`).
