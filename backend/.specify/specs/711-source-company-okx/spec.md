# Spec: 711 — Source Company Plugin: OKX

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 711                            |
| Slug           | source-company-okx               |
| Status         | accepted                               |
...
OKX publishes its careers board through Greenhouse at
the bare slug `okx`. The run-398 batch sweep confirmed
257 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/okx/jobs?content=true`.
Sector: Cryptocurrency Exchange. HQ: Dubai, United Arab Emirates.

## 2. Goals

- Ship a `source-company-okx` plugin returning live
...
- Backfilling historical OKX postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OKX`** in the
> source registry, so that **a single `siteType: [Site.OKX]`
> request returns OKX's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OKX = 'okx'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-okx`.                  | must     |
| FR-3  | `OKXService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `okx-`, `site === Site.OKX`. | must |
...
```ts
@SourcePlugin({ site: Site.OKX, name: 'OKX', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/okx/jobs/<id>`), wire
  `company_name` pass-through (`'OKX'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/okx/jobs/<id>`).
