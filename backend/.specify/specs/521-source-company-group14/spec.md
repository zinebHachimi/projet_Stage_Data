# Spec: 521 — Source Company Plugin: Group14 Technologies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 521                            |
| Slug           | source-company-group14               |
| Status         | accepted                               |
...
Group14 Technologies publishes its careers board through Greenhouse at
the bare slug `group14`. The run-398 batch sweep confirmed
...
- Ship a `source-company-group14` plugin returning live
...
- Backfilling historical Group14 Technologies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GROUP14`** in the
> source registry, so that **a single `siteType: [Site.GROUP14]`
> request returns Group14 Technologies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GROUP14 = 'group14'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-group14`.                  | must     |
| FR-3  | `Group14Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `group14-`, `site === Site.GROUP14`. | must |
...
```ts
@SourcePlugin({ site: Site.GROUP14, name: 'Group14 Technologies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/group14/jobs/<id>`), wire
  `company_name` pass-through (`'Group14 Technologies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/group14/jobs/<id>`).
