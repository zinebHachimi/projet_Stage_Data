# Plan: 938 — Source Company Plugin: Speechify
...
Speechify's careers board is hosted on Greenhouse at the slug
`speechify`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-speechify` plugin.
- Deliverables: package files, `Site.SPEECHIFY` enum value, the
...
- `packages/plugins/source-company-speechify` (new)
