# Spec: 938 — Source Company Plugin: Speechify

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 938                            |
| Slug           | source-company-speechify               |
| Status         | accepted                               |
...
Speechify publishes its careers board through Greenhouse at
the bare slug `speechify`. The run-398 batch sweep confirmed
1695 live role(s) via a direct probe of
...
- Ship a `source-company-speechify` plugin returning live
...
- Backfilling historical Speechify postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPEECHIFY`** in the
> source registry, so that **a single `siteType: [Site.SPEECHIFY]`
> request returns Speechify's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPEECHIFY = 'speechify'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-speechify`.                  | must     |
| FR-3  | `SpeechifyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `speechify-`, `site === Site.SPEECHIFY`. | must |
...
```ts
@SourcePlugin({ site: Site.SPEECHIFY, name: 'Speechify', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/speechify/jobs/<id>`), wire
  `company_name` pass-through (`'Speechify'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/speechify/jobs/<id>`).
