# Spec: 908 — Source Company Plugin: Pathward

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 908                            |
| Slug           | source-company-pathward               |
| Status         | accepted                               |
...
Pathward publishes its careers board through Greenhouse at
the bare slug `pathward`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pathward` plugin returning live
...
- Backfilling historical Pathward postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PATHWARD`** in the
> source registry, so that **a single `siteType: [Site.PATHWARD]`
> request returns Pathward's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PATHWARD = 'pathward'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pathward`.                  | must     |
| FR-3  | `PathwardService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pathward-`, `site === Site.PATHWARD`. | must |
...
```ts
@SourcePlugin({ site: Site.PATHWARD, name: 'Pathward', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pathward/jobs/<id>`), wire
  `company_name` pass-through (`'Pathward'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pathward/jobs/<id>`).
