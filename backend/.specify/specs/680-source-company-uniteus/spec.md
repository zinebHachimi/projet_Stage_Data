# Spec: 680 — Source Company Plugin: Unite Us

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 680                            |
| Slug           | source-company-uniteus               |
| Status         | accepted                               |
...
Unite Us publishes its careers board through Greenhouse at
the bare slug `uniteus`. The run-398 batch sweep confirmed
...
- Ship a `source-company-uniteus` plugin returning live
...
- Backfilling historical Unite Us postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UNITE_US`** in the
> source registry, so that **a single `siteType: [Site.UNITE_US]`
> request returns Unite Us's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UNITE_US = 'uniteus'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-uniteus`.                  | must     |
| FR-3  | `UniteUsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `uniteus-`, `site === Site.UNITE_US`. | must |
...
```ts
@SourcePlugin({ site: Site.UNITE_US, name: 'Unite Us', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/uniteus/jobs/<id>`), wire
  `company_name` pass-through (`'Unite Us'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/uniteus/jobs/<id>`).
