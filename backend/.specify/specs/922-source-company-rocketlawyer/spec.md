# Spec: 922 — Source Company Plugin: Rocket Lawyer

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 922                            |
| Slug           | source-company-rocketlawyer               |
| Status         | accepted                               |
...
Rocket Lawyer publishes its careers board through Greenhouse at
the bare slug `rocketlawyer`. The run-398 batch sweep confirmed
...
- Ship a `source-company-rocketlawyer` plugin returning live
...
- Backfilling historical Rocket Lawyer postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ROCKET_LAWYER`** in the
> source registry, so that **a single `siteType: [Site.ROCKET_LAWYER]`
> request returns Rocket Lawyer's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ROCKET_LAWYER = 'rocketlawyer'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-rocketlawyer`.                  | must     |
| FR-3  | `RocketLawyerService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `rocketlawyer-`, `site === Site.ROCKET_LAWYER`. | must |
...
```ts
@SourcePlugin({ site: Site.ROCKET_LAWYER, name: 'Rocket Lawyer', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/rocketlawyer/jobs/<id>`), wire
  `company_name` pass-through (`'Rocket Lawyer'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/rocketlawyer/jobs/<id>`).
