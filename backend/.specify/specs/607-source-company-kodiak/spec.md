# Spec: 607 — Source Company Plugin: Kodiak Robotics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 607                            |
| Slug           | source-company-kodiak               |
| Status         | accepted                               |
...
Kodiak Robotics publishes its careers board through Greenhouse at
the bare slug `kodiak`. The run-398 batch sweep confirmed
...
- Ship a `source-company-kodiak` plugin returning live
...
- Backfilling historical Kodiak Robotics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KODIAK_ROBOTICS`** in the
> source registry, so that **a single `siteType: [Site.KODIAK_ROBOTICS]`
> request returns Kodiak Robotics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KODIAK_ROBOTICS = 'kodiak'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-kodiak`.                  | must     |
| FR-3  | `KodiakService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `kodiak-`, `site === Site.KODIAK_ROBOTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.KODIAK_ROBOTICS, name: 'Kodiak Robotics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/kodiak/jobs/<id>`), wire
  `company_name` pass-through (`'Kodiak Robotics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/kodiak/jobs/<id>`).
