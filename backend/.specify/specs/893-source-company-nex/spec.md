# Spec: 893 — Source Company Plugin: Nex

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 893                            |
| Slug           | source-company-nex               |
| Status         | accepted                               |
...
Nex publishes its careers board through Greenhouse at
the bare slug `nex`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nex` plugin returning live
...
- Backfilling historical Nex postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEX`** in the
> source registry, so that **a single `siteType: [Site.NEX]`
> request returns Nex's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEX = 'nex'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nex`.                  | must     |
| FR-3  | `NexService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nex-`, `site === Site.NEX`. | must |
...
```ts
@SourcePlugin({ site: Site.NEX, name: 'Nex', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nex/jobs/<id>`), wire
  `company_name` pass-through (`'Nex'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nex/jobs/<id>`).
