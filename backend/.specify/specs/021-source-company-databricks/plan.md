# Plan: 021 — Source Company Plugin: Databricks
...
Databricks's careers board is hosted on Greenhouse at the slug
`databricks`, so the implementation is a thin wrapper around the same
public Greenhouse endpoint that `source-company-anthropic` (Spec 020 /
run #230) and `source-company-stripe` already call. The plan is to
copy the Anthropic plugin's shape (single-file `service.ts`,
three-line `module.ts`, two-line `index.ts`, four-line `package.json`,
three-line `tsconfig.json`) and rename references from Anthropic →
Databricks, then wire the new package into the four registration
...
nine times before (Amazon, Apple, Cursor, Google, IBM, Meta, OpenAI,
Stripe, Anthropic).
...
- **Deliverables:**
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 021 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-databricks` → all green.
...
| `packages/models/src/enums/site.enum.ts`         | append `DATABRICKS = 'databricks'`.                          |
| `packages/plugins/index.ts`                      | import + append `DatabricksModule` to `ALL_SOURCE_MODULES`.  |
| `tsconfig.base.json`                             | path-alias entry.                                            |
| `jest.config.js`                                 | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                | add Databricks shipped row.                                  |
| `docs/index.md`                                  | append Spec 021 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Databricks moves off Greenhouse mid-maintenance.    | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-anthropic`'s shape.      | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.DATABRICKS` is unused outside this plugin, so the enum revert
  is safe.
...
(None — see Spec 021 § 10 Decisions for the three resolved points.)
