# Spec: 967 — Source Company Plugin: Viral Nation

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 967                            |
| Slug           | source-company-viralnation               |
| Status         | accepted                               |
...
Viral Nation publishes its careers board through Greenhouse at
the bare slug `viralnation`. The run-398 batch sweep confirmed
...
- Ship a `source-company-viralnation` plugin returning live
...
- Backfilling historical Viral Nation postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VIRAL_NATION`** in the
> source registry, so that **a single `siteType: [Site.VIRAL_NATION]`
> request returns Viral Nation's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VIRAL_NATION = 'viralnation'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-viralnation`.                  | must     |
| FR-3  | `ViralNationService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `viralnation-`, `site === Site.VIRAL_NATION`. | must |
...
```ts
@SourcePlugin({ site: Site.VIRAL_NATION, name: 'Viral Nation', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/viralnation/jobs/<id>`), wire
  `company_name` pass-through (`'Viral Nation'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/viralnation/jobs/<id>`).
