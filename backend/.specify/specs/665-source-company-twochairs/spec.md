# Spec: 665 — Source Company Plugin: Two Chairs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 665                            |
| Slug           | source-company-twochairs               |
| Status         | accepted                               |
...
Two Chairs publishes its careers board through Greenhouse at
the bare slug `twochairs`. The run-398 batch sweep confirmed
90 live role(s) via a direct probe of
...
- Ship a `source-company-twochairs` plugin returning live
...
- Backfilling historical Two Chairs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TWO_CHAIRS`** in the
> source registry, so that **a single `siteType: [Site.TWO_CHAIRS]`
> request returns Two Chairs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TWO_CHAIRS = 'twochairs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-twochairs`.                  | must     |
| FR-3  | `TwoChairsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `twochairs-`, `site === Site.TWO_CHAIRS`. | must |
...
```ts
@SourcePlugin({ site: Site.TWO_CHAIRS, name: 'Two Chairs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/twochairs/jobs/<id>`), wire
  `company_name` pass-through (`'Two Chairs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/twochairs/jobs/<id>`).
