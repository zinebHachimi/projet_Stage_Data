# Spec: 285 — Source Company Plugin: MrBeast Contract Jobs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 285                            |
| Slug           | source-company-aplayers               |
| Status         | accepted                               |
...
MrBeast Contract Jobs publishes its careers board through Greenhouse at
the bare slug `aplayers`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aplayers` plugin returning live
...
- Backfilling historical MrBeast Contract Jobs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APLAYERS`** in the
> source registry, so that **a single `siteType: [Site.APLAYERS]`
> request returns MrBeast Contract Jobs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APLAYERS = 'aplayers'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aplayers`.                  | must     |
| FR-3  | `AplayersService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aplayers-`, `site === Site.APLAYERS`. | must |
...
```ts
@SourcePlugin({ site: Site.APLAYERS, name: 'MrBeast Contract Jobs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aplayers/jobs/<id>`), wire
  `company_name` pass-through (`'MrBeast Contract Jobs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aplayers/jobs/<id>`).
