# Spec: 572 — Source Company Plugin: Gymshark

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 572                            |
| Slug           | source-company-gymshark               |
| Status         | accepted                               |
...
Gymshark publishes its careers board through Greenhouse at
the bare slug `gymshark`. The run-398 batch sweep confirmed
...
- Ship a `source-company-gymshark` plugin returning live
...
- Backfilling historical Gymshark postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GYMSHARK`** in the
> source registry, so that **a single `siteType: [Site.GYMSHARK]`
> request returns Gymshark's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GYMSHARK = 'gymshark'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-gymshark`.                  | must     |
| FR-3  | `GymsharkService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `gymshark-`, `site === Site.GYMSHARK`. | must |
...
```ts
@SourcePlugin({ site: Site.GYMSHARK, name: 'Gymshark', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/gymshark/jobs/<id>`), wire
  `company_name` pass-through (`'Gymshark'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/gymshark/jobs/<id>`).
