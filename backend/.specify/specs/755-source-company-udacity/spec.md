# Spec: 755 — Source Company Plugin: Udacity

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 755                            |
| Slug           | source-company-udacity               |
| Status         | accepted                               |
...
Udacity publishes its careers board through Greenhouse at
the bare slug `udacity`. The run-398 batch sweep confirmed
...
- Ship a `source-company-udacity` plugin returning live
...
- Backfilling historical Udacity postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UDACITY`** in the
> source registry, so that **a single `siteType: [Site.UDACITY]`
> request returns Udacity's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UDACITY = 'udacity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-udacity`.                  | must     |
| FR-3  | `UdacityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `udacity-`, `site === Site.UDACITY`. | must |
...
```ts
@SourcePlugin({ site: Site.UDACITY, name: 'Udacity', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/udacity/jobs/<id>`), wire
  `company_name` pass-through (`'Udacity'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/udacity/jobs/<id>`).
