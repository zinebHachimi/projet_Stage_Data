# Spec: 661 — Source Company Plugin: Rondo Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 661                            |
| Slug           | source-company-rondoenergy               |
| Status         | accepted                               |
...
Rondo Energy publishes its careers board through Greenhouse at
the bare slug `rondoenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-rondoenergy` plugin returning live
...
- Backfilling historical Rondo Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RONDO_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.RONDO_ENERGY]`
> request returns Rondo Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RONDO_ENERGY = 'rondoenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-rondoenergy`.                  | must     |
| FR-3  | `RondoEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `rondoenergy-`, `site === Site.RONDO_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.RONDO_ENERGY, name: 'Rondo Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/rondoenergy/jobs/<id>`), wire
  `company_name` pass-through (`'Rondo Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/rondoenergy/jobs/<id>`).
