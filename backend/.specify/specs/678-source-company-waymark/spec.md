# Spec: 678 — Source Company Plugin: Waymark Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 678                            |
| Slug           | source-company-waymark               |
| Status         | accepted                               |
...
Waymark Health publishes its careers board through Greenhouse at
the bare slug `waymark`. The run-398 batch sweep confirmed
...
- Ship a `source-company-waymark` plugin returning live
...
- Backfilling historical Waymark Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WAYMARK_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.WAYMARK_HEALTH]`
> request returns Waymark Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WAYMARK_HEALTH = 'waymark'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-waymark`.                  | must     |
| FR-3  | `WaymarkHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `waymark-`, `site === Site.WAYMARK_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.WAYMARK_HEALTH, name: 'Waymark Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/waymark/jobs/<id>`), wire
  `company_name` pass-through (`'Waymark Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/waymark/jobs/<id>`).
