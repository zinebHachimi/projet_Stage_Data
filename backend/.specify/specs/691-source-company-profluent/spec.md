# Spec: 691 — Source Company Plugin: Profluent

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 691                            |
| Slug           | source-company-profluent               |
| Status         | accepted                               |
...
Profluent publishes its careers board through Greenhouse at
the bare slug `profluent`. The run-398 batch sweep confirmed
...
- Ship a `source-company-profluent` plugin returning live
...
- Backfilling historical Profluent postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PROFLUENT`** in the
> source registry, so that **a single `siteType: [Site.PROFLUENT]`
> request returns Profluent's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PROFLUENT = 'profluent'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-profluent`.                  | must     |
| FR-3  | `ProfluentService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `profluent-`, `site === Site.PROFLUENT`. | must |
...
```ts
@SourcePlugin({ site: Site.PROFLUENT, name: 'Profluent', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/profluent/jobs/<id>`), wire
  `company_name` pass-through (`'Profluent'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/profluent/jobs/<id>`).
