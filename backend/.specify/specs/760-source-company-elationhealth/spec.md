# Spec: 760 — Source Company Plugin: Elation Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 760                            |
| Slug           | source-company-elationhealth               |
| Status         | accepted                               |
...
Elation Health publishes its careers board through Greenhouse at
the bare slug `elationhealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-elationhealth` plugin returning live
...
- Backfilling historical Elation Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ELATION_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.ELATION_HEALTH]`
> request returns Elation Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ELATION_HEALTH = 'elationhealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-elationhealth`.                  | must     |
| FR-3  | `ElationHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `elationhealth-`, `site === Site.ELATION_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.ELATION_HEALTH, name: 'Elation Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/elationhealth/jobs/<id>`), wire
  `company_name` pass-through (`'Elation Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/elationhealth/jobs/<id>`).
