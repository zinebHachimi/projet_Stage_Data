# Plan: 819 — Source Company Plugin: BuildOps
...
BuildOps's careers board is hosted on Greenhouse at the slug
`buildops`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-buildops` plugin.
- Deliverables: package files, `Site.BUILDOPS` enum value, the
...
- `packages/plugins/source-company-buildops` (new)
