# Spec: 819 — Source Company Plugin: BuildOps

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 819                            |
| Slug           | source-company-buildops               |
| Status         | accepted                               |
...
BuildOps publishes its careers board through Greenhouse at
the bare slug `buildops`. The run-398 batch sweep confirmed
...
- Ship a `source-company-buildops` plugin returning live
...
- Backfilling historical BuildOps postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BUILDOPS`** in the
> source registry, so that **a single `siteType: [Site.BUILDOPS]`
> request returns BuildOps's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BUILDOPS = 'buildops'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-buildops`.                  | must     |
| FR-3  | `BuildOpsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `buildops-`, `site === Site.BUILDOPS`. | must |
...
```ts
@SourcePlugin({ site: Site.BUILDOPS, name: 'BuildOps', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/buildops/jobs/<id>`), wire
  `company_name` pass-through (`'BuildOps'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/buildops/jobs/<id>`).
