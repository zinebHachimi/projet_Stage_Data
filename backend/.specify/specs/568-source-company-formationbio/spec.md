# Spec: 568 — Source Company Plugin: Formation Bio

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 568                            |
| Slug           | source-company-formationbio               |
| Status         | accepted                               |
...
Formation Bio publishes its careers board through Greenhouse at
the bare slug `formationbio`. The run-398 batch sweep confirmed
...
- Ship a `source-company-formationbio` plugin returning live
...
- Backfilling historical Formation Bio postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORMATION_BIO`** in the
> source registry, so that **a single `siteType: [Site.FORMATION_BIO]`
> request returns Formation Bio's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORMATION_BIO = 'formationbio'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-formationbio`.                  | must     |
| FR-3  | `FormationbioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `formationbio-`, `site === Site.FORMATION_BIO`. | must |
...
```ts
@SourcePlugin({ site: Site.FORMATION_BIO, name: 'Formation Bio', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/formationbio/jobs/<id>`), wire
  `company_name` pass-through (`'Formation Bio'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/formationbio/jobs/<id>`).
