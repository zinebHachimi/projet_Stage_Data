# Spec: 936 — Source Company Plugin: Sol de Janeiro

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 936                            |
| Slug           | source-company-soldejaneiro               |
| Status         | accepted                               |
...
Sol de Janeiro publishes its careers board through Greenhouse at
the bare slug `soldejaneiro`. The run-398 batch sweep confirmed
...
- Ship a `source-company-soldejaneiro` plugin returning live
...
- Backfilling historical Sol de Janeiro postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SOL_DE_JANEIRO`** in the
> source registry, so that **a single `siteType: [Site.SOL_DE_JANEIRO]`
> request returns Sol de Janeiro's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SOL_DE_JANEIRO = 'soldejaneiro'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-soldejaneiro`.                  | must     |
| FR-3  | `SolDeJaneiroService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `soldejaneiro-`, `site === Site.SOL_DE_JANEIRO`. | must |
...
```ts
@SourcePlugin({ site: Site.SOL_DE_JANEIRO, name: 'Sol de Janeiro', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/soldejaneiro/jobs/<id>`), wire
  `company_name` pass-through (`'Sol de Janeiro'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/soldejaneiro/jobs/<id>`).
