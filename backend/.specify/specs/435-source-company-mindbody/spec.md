# Spec: 435 — Source Company Plugin: Mindbody

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 435                            |
| Slug           | source-company-mindbody               |
| Status         | accepted                               |
...
Mindbody publishes its careers board through Greenhouse at
the bare slug `mindbody`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mindbody` plugin returning live
...
- Backfilling historical Mindbody postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MINDBODY`** in the
> source registry, so that **a single `siteType: [Site.MINDBODY]`
> request returns Mindbody's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MINDBODY = 'mindbody'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mindbody`.                  | must     |
| FR-3  | `MindbodyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mindbody-`, `site === Site.MINDBODY`. | must |
...
```ts
@SourcePlugin({ site: Site.MINDBODY, name: 'Mindbody', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mindbody/jobs/<id>`), wire
  `company_name` pass-through (`'Mindbody'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mindbody/jobs/<id>`).
