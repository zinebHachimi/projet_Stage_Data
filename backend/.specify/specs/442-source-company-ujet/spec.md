# Spec: 442 — Source Company Plugin: UJET

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 442                            |
| Slug           | source-company-ujet               |
| Status         | accepted                               |
...
UJET publishes its careers board through Greenhouse at
the bare slug `ujet`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ujet` plugin returning live
...
- Backfilling historical UJET postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UJET`** in the
> source registry, so that **a single `siteType: [Site.UJET]`
> request returns UJET's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UJET = 'ujet'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ujet`.                  | must     |
| FR-3  | `UjetService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ujet-`, `site === Site.UJET`. | must |
...
```ts
@SourcePlugin({ site: Site.UJET, name: 'UJET', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ujet/jobs/<id>`), wire
  `company_name` pass-through (`'UJET'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ujet/jobs/<id>`).
