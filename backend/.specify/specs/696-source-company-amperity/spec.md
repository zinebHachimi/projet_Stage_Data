# Spec: 696 — Source Company Plugin: Amperity

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 696                            |
| Slug           | source-company-amperity               |
| Status         | accepted                               |
...
Amperity publishes its careers board through Greenhouse at
the bare slug `amperity`. The run-398 batch sweep confirmed
...
- Ship a `source-company-amperity` plugin returning live
...
- Backfilling historical Amperity postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMPERITY`** in the
> source registry, so that **a single `siteType: [Site.AMPERITY]`
> request returns Amperity's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMPERITY = 'amperity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-amperity`.                  | must     |
| FR-3  | `AmperityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `amperity-`, `site === Site.AMPERITY`. | must |
...
```ts
@SourcePlugin({ site: Site.AMPERITY, name: 'Amperity', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/amperity/jobs/<id>`), wire
  `company_name` pass-through (`'Amperity'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/amperity/jobs/<id>`).
