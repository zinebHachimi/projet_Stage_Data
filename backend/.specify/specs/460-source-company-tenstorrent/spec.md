# Spec: 460 — Source Company Plugin: Tenstorrent

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 460                            |
| Slug           | source-company-tenstorrent               |
| Status         | accepted                               |
...
Tenstorrent publishes its careers board through Greenhouse at
the bare slug `tenstorrent`. The run-398 batch sweep confirmed
121 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/tenstorrent/jobs?content=true`.
Sector: AI & RISC-V semiconductors. HQ: Toronto, ON, Canada.

## 2. Goals

- Ship a `source-company-tenstorrent` plugin returning live
...
- Backfilling historical Tenstorrent postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TENSTORRENT`** in the
> source registry, so that **a single `siteType: [Site.TENSTORRENT]`
> request returns Tenstorrent's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TENSTORRENT = 'tenstorrent'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tenstorrent`.                  | must     |
| FR-3  | `TenstorrentService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tenstorrent-`, `site === Site.TENSTORRENT`. | must |
...
```ts
@SourcePlugin({ site: Site.TENSTORRENT, name: 'Tenstorrent', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tenstorrent/jobs/<id>`), wire
  `company_name` pass-through (`'Tenstorrent'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tenstorrent/jobs/<id>`).
