# Plan: 731 — Source Company Plugin: Zencoder
...
Zencoder's careers board is hosted on Greenhouse at the slug
`zencoder`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-zencoder` plugin.
- Deliverables: package files, `Site.ZENCODER` enum value, the
...
- `packages/plugins/source-company-zencoder` (new)
