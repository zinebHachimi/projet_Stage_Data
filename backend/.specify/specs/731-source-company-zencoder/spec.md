# Spec: 731 — Source Company Plugin: Zencoder

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 731                            |
| Slug           | source-company-zencoder               |
| Status         | accepted                               |
...
Zencoder publishes its careers board through Greenhouse at
the bare slug `zencoder`. The run-398 batch sweep confirmed
...
- Ship a `source-company-zencoder` plugin returning live
...
- Backfilling historical Zencoder postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ZENCODER`** in the
> source registry, so that **a single `siteType: [Site.ZENCODER]`
> request returns Zencoder's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ZENCODER = 'zencoder'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-zencoder`.                  | must     |
| FR-3  | `ZencoderService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `zencoder-`, `site === Site.ZENCODER`. | must |
...
```ts
@SourcePlugin({ site: Site.ZENCODER, name: 'Zencoder', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/zencoder/jobs/<id>`), wire
  `company_name` pass-through (`'Zencoder'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/zencoder/jobs/<id>`).
