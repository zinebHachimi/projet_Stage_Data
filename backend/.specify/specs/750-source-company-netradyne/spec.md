# Spec: 750 — Source Company Plugin: Netradyne

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 750                            |
| Slug           | source-company-netradyne               |
| Status         | accepted                               |
...
Netradyne publishes its careers board through Greenhouse at
the bare slug `netradyne`. The run-398 batch sweep confirmed
70 live role(s) via a direct probe of
...
- Ship a `source-company-netradyne` plugin returning live
...
- Backfilling historical Netradyne postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NETRADYNE`** in the
> source registry, so that **a single `siteType: [Site.NETRADYNE]`
> request returns Netradyne's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NETRADYNE = 'netradyne'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-netradyne`.                  | must     |
| FR-3  | `NetradyneService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `netradyne-`, `site === Site.NETRADYNE`. | must |
...
```ts
@SourcePlugin({ site: Site.NETRADYNE, name: 'Netradyne', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/netradyne/jobs/<id>`), wire
  `company_name` pass-through (`'Netradyne'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/netradyne/jobs/<id>`).
