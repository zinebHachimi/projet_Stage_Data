# Plan: 603 — Source Company Plugin: DriveWealth
...
DriveWealth's careers board is hosted on Greenhouse at the slug
`drivewealth`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-drivewealth` plugin.
- Deliverables: package files, `Site.DRIVEWEALTH` enum value, the
...
- `packages/plugins/source-company-drivewealth` (new)
