# Spec: 603 — Source Company Plugin: DriveWealth

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 603                            |
| Slug           | source-company-drivewealth               |
| Status         | accepted                               |
...
DriveWealth publishes its careers board through Greenhouse at
the bare slug `drivewealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-drivewealth` plugin returning live
...
- Backfilling historical DriveWealth postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DRIVEWEALTH`** in the
> source registry, so that **a single `siteType: [Site.DRIVEWEALTH]`
> request returns DriveWealth's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DRIVEWEALTH = 'drivewealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-drivewealth`.                  | must     |
| FR-3  | `DriveWealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `drivewealth-`, `site === Site.DRIVEWEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.DRIVEWEALTH, name: 'DriveWealth', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/drivewealth/jobs/<id>`), wire
  `company_name` pass-through (`'DriveWealth'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/drivewealth/jobs/<id>`).
