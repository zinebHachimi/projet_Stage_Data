# Spec: 203 — Source Company Plugin: AfterShip

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 203                            |
| Slug           | source-company-aftership               |
| Status         | accepted                               |
...
AfterShip publishes its careers board through Greenhouse at
the bare slug `aftership`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aftership` plugin returning live
...
- Backfilling historical AfterShip postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AFTERSHIP`** in the
> source registry, so that **a single `siteType: [Site.AFTERSHIP]`
> request returns AfterShip's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AFTERSHIP = 'aftership'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aftership`.                  | must     |
| FR-3  | `AftershipService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aftership-`, `site === Site.AFTERSHIP`. | must |
...
```ts
@SourcePlugin({ site: Site.AFTERSHIP, name: 'AfterShip', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aftership/jobs/<id>`), wire
  `company_name` pass-through (`'AfterShip'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aftership/jobs/<id>`).
