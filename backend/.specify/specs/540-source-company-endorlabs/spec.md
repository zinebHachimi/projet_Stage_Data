# Spec: 540 — Source Company Plugin: Endor Labs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 540                            |
| Slug           | source-company-endorlabs               |
| Status         | accepted                               |
...
Endor Labs publishes its careers board through Greenhouse at
the bare slug `endorlabs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-endorlabs` plugin returning live
...
- Backfilling historical Endor Labs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ENDOR_LABS`** in the
> source registry, so that **a single `siteType: [Site.ENDOR_LABS]`
> request returns Endor Labs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ENDOR_LABS = 'endorlabs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-endorlabs`.                  | must     |
| FR-3  | `EndorlabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `endorlabs-`, `site === Site.ENDOR_LABS`. | must |
...
```ts
@SourcePlugin({ site: Site.ENDOR_LABS, name: 'Endor Labs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/endorlabs/jobs/<id>`), wire
  `company_name` pass-through (`'Endor Labs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/endorlabs/jobs/<id>`).
