# Spec: 431 — Source Company Plugin: Grafana Labs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 431                            |
| Slug           | source-company-grafanalabs               |
| Status         | accepted                               |
...
Grafana Labs publishes its careers board through Greenhouse at
the bare slug `grafanalabs`. The run-398 batch sweep confirmed
135 live role(s) via a direct probe of
...
- Ship a `source-company-grafanalabs` plugin returning live
...
- Backfilling historical Grafana Labs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GRAFANALABS`** in the
> source registry, so that **a single `siteType: [Site.GRAFANALABS]`
> request returns Grafana Labs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GRAFANALABS = 'grafanalabs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-grafanalabs`.                  | must     |
| FR-3  | `GrafanalabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `grafanalabs-`, `site === Site.GRAFANALABS`. | must |
...
```ts
@SourcePlugin({ site: Site.GRAFANALABS, name: 'Grafana Labs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/grafanalabs/jobs/<id>`), wire
  `company_name` pass-through (`'Grafana Labs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/grafanalabs/jobs/<id>`).
