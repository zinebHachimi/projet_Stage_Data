# Spec: 279 — Source Company Plugin: Apera AI Inc

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 279                            |
| Slug           | source-company-aperaaiinc               |
| Status         | accepted                               |
...
Apera AI Inc publishes its careers board through Greenhouse at
the bare slug `aperaaiinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aperaaiinc` plugin returning live
...
- Backfilling historical Apera AI Inc postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APERAAIINC`** in the
> source registry, so that **a single `siteType: [Site.APERAAIINC]`
> request returns Apera AI Inc's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APERAAIINC = 'aperaaiinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aperaaiinc`.                  | must     |
| FR-3  | `AperaaiincService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aperaaiinc-`, `site === Site.APERAAIINC`. | must |
...
```ts
@SourcePlugin({ site: Site.APERAAIINC, name: 'Apera AI Inc', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aperaaiinc/jobs/<id>`), wire
  `company_name` pass-through (`'Apera AI Inc'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aperaaiinc/jobs/<id>`).
