# Spec: 551 — Source Company Plugin: Neuralink

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 551                            |
| Slug           | source-company-neuralink               |
| Status         | accepted                               |
...
Neuralink publishes its careers board through Greenhouse at
the bare slug `neuralink`. The run-398 batch sweep confirmed
...
- Ship a `source-company-neuralink` plugin returning live
...
- Backfilling historical Neuralink postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEURALINK`** in the
> source registry, so that **a single `siteType: [Site.NEURALINK]`
> request returns Neuralink's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEURALINK = 'neuralink'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-neuralink`.                  | must     |
| FR-3  | `NeuralinkService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `neuralink-`, `site === Site.NEURALINK`. | must |
...
```ts
@SourcePlugin({ site: Site.NEURALINK, name: 'Neuralink', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/neuralink/jobs/<id>`), wire
  `company_name` pass-through (`'Neuralink'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/neuralink/jobs/<id>`).
