# Spec: 522 — Source Company Plugin: Carbon

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 522                            |
| Slug           | source-company-carbon               |
| Status         | accepted                               |
...
Carbon publishes its careers board through Greenhouse at
the bare slug `carbon`. The run-398 batch sweep confirmed
...
- Ship a `source-company-carbon` plugin returning live
...
- Backfilling historical Carbon postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CARBON`** in the
> source registry, so that **a single `siteType: [Site.CARBON]`
> request returns Carbon's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CARBON = 'carbon'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-carbon`.                  | must     |
| FR-3  | `CarbonService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `carbon-`, `site === Site.CARBON`. | must |
...
```ts
@SourcePlugin({ site: Site.CARBON, name: 'Carbon', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/carbon/jobs/<id>`), wire
  `company_name` pass-through (`'Carbon'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/carbon/jobs/<id>`).
