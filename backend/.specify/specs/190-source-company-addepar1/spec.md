# Spec: 190 — Source Company Plugin: Addepar

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 190                            |
| Slug           | source-company-addepar1               |
| Status         | accepted                               |
...
Addepar publishes its careers board through Greenhouse at
the bare slug `addepar1`. The run-398 batch sweep confirmed
119 live role(s) via a direct probe of
...
- Ship a `source-company-addepar1` plugin returning live
...
- Backfilling historical Addepar postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ADDEPAR1`** in the
> source registry, so that **a single `siteType: [Site.ADDEPAR1]`
> request returns Addepar's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ADDEPAR1 = 'addepar1'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-addepar1`.                  | must     |
| FR-3  | `Addepar1Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `addepar1-`, `site === Site.ADDEPAR1`. | must |
...
```ts
@SourcePlugin({ site: Site.ADDEPAR1, name: 'Addepar', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/addepar1/jobs/<id>`), wire
  `company_name` pass-through (`'Addepar'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/addepar1/jobs/<id>`).
