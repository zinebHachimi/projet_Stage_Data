# Spec: 194 — Source Company Plugin: Advocate Construction

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 194                            |
| Slug           | source-company-advocateconstruction               |
| Status         | accepted                               |
...
Advocate Construction publishes its careers board through Greenhouse at
the bare slug `advocateconstruction`. The run-398 batch sweep confirmed
26 live role(s) via a direct probe of
...
- Ship a `source-company-advocateconstruction` plugin returning live
...
- Backfilling historical Advocate Construction postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ADVOCATECONSTRUCTION`** in the
> source registry, so that **a single `siteType: [Site.ADVOCATECONSTRUCTION]`
> request returns Advocate Construction's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ADVOCATECONSTRUCTION = 'advocateconstruction'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-advocateconstruction`.                  | must     |
| FR-3  | `AdvocateconstructionService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `advocateconstruction-`, `site === Site.ADVOCATECONSTRUCTION`. | must |
...
```ts
@SourcePlugin({ site: Site.ADVOCATECONSTRUCTION, name: 'Advocate Construction', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/advocateconstruction/jobs/<id>`), wire
  `company_name` pass-through (`'Advocate Construction'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/advocateconstruction/jobs/<id>`).
