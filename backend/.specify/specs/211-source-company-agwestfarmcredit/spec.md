# Spec: 211 — Source Company Plugin: AgWest Farm Credit

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 211                            |
| Slug           | source-company-agwestfarmcredit               |
| Status         | accepted                               |
...
AgWest Farm Credit publishes its careers board through Greenhouse at
the bare slug `agwestfarmcredit`. The run-398 batch sweep confirmed
...
- Ship a `source-company-agwestfarmcredit` plugin returning live
...
- Backfilling historical AgWest Farm Credit postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AGWESTFARMCREDIT`** in the
> source registry, so that **a single `siteType: [Site.AGWESTFARMCREDIT]`
> request returns AgWest Farm Credit's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AGWESTFARMCREDIT = 'agwestfarmcredit'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-agwestfarmcredit`.                  | must     |
| FR-3  | `AgwestfarmcreditService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `agwestfarmcredit-`, `site === Site.AGWESTFARMCREDIT`. | must |
...
```ts
@SourcePlugin({ site: Site.AGWESTFARMCREDIT, name: 'AgWest Farm Credit', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/agwestfarmcredit/jobs/<id>`), wire
  `company_name` pass-through (`'AgWest Farm Credit'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/agwestfarmcredit/jobs/<id>`).
