# Spec: 774 — Source Company Plugin: Cohere Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 774                            |
| Slug           | source-company-coherehealth               |
| Status         | accepted                               |
...
Cohere Health publishes its careers board through Greenhouse at
the bare slug `coherehealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-coherehealth` plugin returning live
...
- Backfilling historical Cohere Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COHERE_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.COHERE_HEALTH]`
> request returns Cohere Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COHERE_HEALTH = 'coherehealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-coherehealth`.                  | must     |
| FR-3  | `CohereHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `coherehealth-`, `site === Site.COHERE_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.COHERE_HEALTH, name: 'Cohere Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/coherehealth/jobs/<id>`), wire
  `company_name` pass-through (`'Cohere Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/coherehealth/jobs/<id>`).
