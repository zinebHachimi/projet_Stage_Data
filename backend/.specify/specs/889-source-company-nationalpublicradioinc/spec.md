# Spec: 889 — Source Company Plugin: NPR (National Public Radio)

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 889                            |
| Slug           | source-company-nationalpublicradioinc               |
| Status         | accepted                               |
...
NPR (National Public Radio) publishes its careers board through Greenhouse at
the bare slug `nationalpublicradioinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nationalpublicradioinc` plugin returning live
...
- Backfilling historical NPR (National Public Radio) postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NPR_NATIONAL_PUBLIC_RADIO`** in the
> source registry, so that **a single `siteType: [Site.NPR_NATIONAL_PUBLIC_RADIO]`
> request returns NPR (National Public Radio)'s open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NPR_NATIONAL_PUBLIC_RADIO = 'nationalpublicradioinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nationalpublicradioinc`.                  | must     |
| FR-3  | `NPRNationalPublicRadioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nationalpublicradioinc-`, `site === Site.NPR_NATIONAL_PUBLIC_RADIO`. | must |
...
```ts
@SourcePlugin({ site: Site.NPR_NATIONAL_PUBLIC_RADIO, name: 'NPR (National Public Radio)', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nationalpublicradioinc/jobs/<id>`), wire
  `company_name` pass-through (`'NPR (National Public Radio)'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nationalpublicradioinc/jobs/<id>`).
