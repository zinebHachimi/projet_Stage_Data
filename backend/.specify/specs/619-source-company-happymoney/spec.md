# Spec: 619 — Source Company Plugin: Happy Money

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 619                            |
| Slug           | source-company-happymoney               |
| Status         | accepted                               |
...
Happy Money publishes its careers board through Greenhouse at
the bare slug `happymoney`. The run-398 batch sweep confirmed
...
- Ship a `source-company-happymoney` plugin returning live
...
- Backfilling historical Happy Money postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HAPPY_MONEY`** in the
> source registry, so that **a single `siteType: [Site.HAPPY_MONEY]`
> request returns Happy Money's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HAPPY_MONEY = 'happymoney'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-happymoney`.                  | must     |
| FR-3  | `HappyMoneyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `happymoney-`, `site === Site.HAPPY_MONEY`. | must |
...
```ts
@SourcePlugin({ site: Site.HAPPY_MONEY, name: 'Happy Money', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/happymoney/jobs/<id>`), wire
  `company_name` pass-through (`'Happy Money'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/happymoney/jobs/<id>`).
