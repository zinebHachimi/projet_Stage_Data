# Spec: 486 — Source Company Plugin: Nubank

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 486                            |
| Slug           | source-company-nubank               |
| Status         | accepted                               |
...
Nubank publishes its careers board through Greenhouse at
the bare slug `nubank`. The run-398 batch sweep confirmed
107 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/nubank/jobs?content=true`.
Sector: Fintech / Neobank. HQ: Sao Paulo, Brazil.

## 2. Goals

- Ship a `source-company-nubank` plugin returning live
...
- Backfilling historical Nubank postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NUBANK`** in the
> source registry, so that **a single `siteType: [Site.NUBANK]`
> request returns Nubank's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NUBANK = 'nubank'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nubank`.                  | must     |
| FR-3  | `NubankService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nubank-`, `site === Site.NUBANK`. | must |
...
```ts
@SourcePlugin({ site: Site.NUBANK, name: 'Nubank', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nubank/jobs/<id>`), wire
  `company_name` pass-through (`'Nubank'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nubank/jobs/<id>`).
