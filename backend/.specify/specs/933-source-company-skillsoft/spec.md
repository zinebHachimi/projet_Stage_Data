# Spec: 933 — Source Company Plugin: Skillsoft

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 933                            |
| Slug           | source-company-skillsoft               |
| Status         | accepted                               |
...
Skillsoft publishes its careers board through Greenhouse at
the bare slug `skillsoft`. The run-398 batch sweep confirmed
...
- Ship a `source-company-skillsoft` plugin returning live
...
- Backfilling historical Skillsoft postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SKILLSOFT`** in the
> source registry, so that **a single `siteType: [Site.SKILLSOFT]`
> request returns Skillsoft's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SKILLSOFT = 'skillsoft'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-skillsoft`.                  | must     |
| FR-3  | `SkillsoftService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `skillsoft-`, `site === Site.SKILLSOFT`. | must |
...
```ts
@SourcePlugin({ site: Site.SKILLSOFT, name: 'Skillsoft', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/skillsoft/jobs/<id>`), wire
  `company_name` pass-through (`'Skillsoft'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/skillsoft/jobs/<id>`).
