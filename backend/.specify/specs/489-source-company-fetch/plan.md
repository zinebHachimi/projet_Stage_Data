# Plan: 489 — Source Company Plugin: Fetch
...
Fetch's careers board is hosted on Greenhouse at the slug
`fetch`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-fetch` plugin.
- Deliverables: package files, `Site.FETCH` enum value, the
...
- `packages/plugins/source-company-fetch` (new)
