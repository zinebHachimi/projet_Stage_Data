# Spec: 489 — Source Company Plugin: Fetch

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 489                            |
| Slug           | source-company-fetch               |
| Status         | accepted                               |
...
Fetch publishes its careers board through Greenhouse at
the bare slug `fetch`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fetch` plugin returning live
...
- Backfilling historical Fetch postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FETCH`** in the
> source registry, so that **a single `siteType: [Site.FETCH]`
> request returns Fetch's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FETCH = 'fetch'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fetch`.                  | must     |
| FR-3  | `FetchService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fetch-`, `site === Site.FETCH`. | must |
...
```ts
@SourcePlugin({ site: Site.FETCH, name: 'Fetch', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fetch/jobs/<id>`), wire
  `company_name` pass-through (`'Fetch'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fetch/jobs/<id>`).
