# Spec: 531 — Source Company Plugin: Oura

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 531                            |
| Slug           | source-company-oura               |
| Status         | accepted                               |
...
Oura publishes its careers board through Greenhouse at
the bare slug `oura`. The run-398 batch sweep confirmed
...
- Ship a `source-company-oura` plugin returning live
...
- Backfilling historical Oura postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OURA`** in the
> source registry, so that **a single `siteType: [Site.OURA]`
> request returns Oura's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OURA = 'oura'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-oura`.                  | must     |
| FR-3  | `OuraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `oura-`, `site === Site.OURA`. | must |
...
```ts
@SourcePlugin({ site: Site.OURA, name: 'Oura', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/oura/jobs/<id>`), wire
  `company_name` pass-through (`'Oura'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/oura/jobs/<id>`).
