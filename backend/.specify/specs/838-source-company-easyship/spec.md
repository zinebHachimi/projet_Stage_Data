# Spec: 838 — Source Company Plugin: Easyship

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 838                            |
| Slug           | source-company-easyship               |
| Status         | accepted                               |
...
Easyship publishes its careers board through Greenhouse at
the bare slug `easyship`. The run-398 batch sweep confirmed
...
- Ship a `source-company-easyship` plugin returning live
...
- Backfilling historical Easyship postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EASYSHIP`** in the
> source registry, so that **a single `siteType: [Site.EASYSHIP]`
> request returns Easyship's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EASYSHIP = 'easyship'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-easyship`.                  | must     |
| FR-3  | `EasyshipService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `easyship-`, `site === Site.EASYSHIP`. | must |
...
```ts
@SourcePlugin({ site: Site.EASYSHIP, name: 'Easyship', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/easyship/jobs/<id>`), wire
  `company_name` pass-through (`'Easyship'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/easyship/jobs/<id>`).
