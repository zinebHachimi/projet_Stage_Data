# Spec: 204 — Source Company Plugin: AG1

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 204                            |
| Slug           | source-company-ag1               |
| Status         | accepted                               |
...
AG1 publishes its careers board through Greenhouse at
the bare slug `ag1`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ag1` plugin returning live
...
- Backfilling historical AG1 postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AG1`** in the
> source registry, so that **a single `siteType: [Site.AG1]`
> request returns AG1's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AG1 = 'ag1'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ag1`.                  | must     |
| FR-3  | `Ag1Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ag1-`, `site === Site.AG1`. | must |
...
```ts
@SourcePlugin({ site: Site.AG1, name: 'AG1', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ag1/jobs/<id>`), wire
  `company_name` pass-through (`'AG1'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ag1/jobs/<id>`).
