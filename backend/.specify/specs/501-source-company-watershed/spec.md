# Spec: 501 — Source Company Plugin: Watershed Informatics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 501                            |
| Slug           | source-company-watershed               |
| Status         | accepted                               |
...
Watershed Informatics publishes its careers board through Greenhouse at
the bare slug `watershed`. The run-398 batch sweep confirmed
...
- Ship a `source-company-watershed` plugin returning live
...
- Backfilling historical Watershed Informatics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WATERSHED`** in the
> source registry, so that **a single `siteType: [Site.WATERSHED]`
> request returns Watershed Informatics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WATERSHED = 'watershed'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-watershed`.                  | must     |
| FR-3  | `WatershedService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `watershed-`, `site === Site.WATERSHED`. | must |
...
```ts
@SourcePlugin({ site: Site.WATERSHED, name: 'Watershed Informatics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/watershed/jobs/<id>`), wire
  `company_name` pass-through (`'Watershed Informatics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/watershed/jobs/<id>`).
