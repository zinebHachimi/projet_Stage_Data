# Spec: 295 — Source Company Plugin: Aktos

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 295                            |
| Slug           | source-company-applytoaktos               |
| Status         | accepted                               |
...
Aktos publishes its careers board through Greenhouse at
the bare slug `applytoaktos`. The run-398 batch sweep confirmed
...
- Ship a `source-company-applytoaktos` plugin returning live
...
- Backfilling historical Aktos postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APPLYTOAKTOS`** in the
> source registry, so that **a single `siteType: [Site.APPLYTOAKTOS]`
> request returns Aktos's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APPLYTOAKTOS = 'applytoaktos'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-applytoaktos`.                  | must     |
| FR-3  | `ApplytoaktosService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `applytoaktos-`, `site === Site.APPLYTOAKTOS`. | must |
...
```ts
@SourcePlugin({ site: Site.APPLYTOAKTOS, name: 'Aktos', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/applytoaktos/jobs/<id>`), wire
  `company_name` pass-through (`'Aktos'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/applytoaktos/jobs/<id>`).
