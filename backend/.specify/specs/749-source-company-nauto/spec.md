# Spec: 749 — Source Company Plugin: Nauto

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 749                            |
| Slug           | source-company-nauto               |
| Status         | accepted                               |
...
Nauto publishes its careers board through Greenhouse at
the bare slug `nauto`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nauto` plugin returning live
...
- Backfilling historical Nauto postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NAUTO`** in the
> source registry, so that **a single `siteType: [Site.NAUTO]`
> request returns Nauto's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NAUTO = 'nauto'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nauto`.                  | must     |
| FR-3  | `NautoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nauto-`, `site === Site.NAUTO`. | must |
...
```ts
@SourcePlugin({ site: Site.NAUTO, name: 'Nauto', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nauto/jobs/<id>`), wire
  `company_name` pass-through (`'Nauto'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nauto/jobs/<id>`).
