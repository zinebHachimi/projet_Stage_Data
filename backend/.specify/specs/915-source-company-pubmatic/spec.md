# Spec: 915 — Source Company Plugin: PubMatic

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 915                            |
| Slug           | source-company-pubmatic               |
| Status         | accepted                               |
...
PubMatic publishes its careers board through Greenhouse at
the bare slug `pubmatic`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pubmatic` plugin returning live
...
- Backfilling historical PubMatic postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PUBMATIC`** in the
> source registry, so that **a single `siteType: [Site.PUBMATIC]`
> request returns PubMatic's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PUBMATIC = 'pubmatic'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pubmatic`.                  | must     |
| FR-3  | `PubMaticService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pubmatic-`, `site === Site.PUBMATIC`. | must |
...
```ts
@SourcePlugin({ site: Site.PUBMATIC, name: 'PubMatic', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pubmatic/jobs/<id>`), wire
  `company_name` pass-through (`'PubMatic'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pubmatic/jobs/<id>`).
