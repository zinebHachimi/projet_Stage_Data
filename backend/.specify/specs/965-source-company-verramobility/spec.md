# Spec: 965 — Source Company Plugin: Verra Mobility

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 965                            |
| Slug           | source-company-verramobility               |
| Status         | accepted                               |
...
Verra Mobility publishes its careers board through Greenhouse at
the bare slug `verramobility`. The run-398 batch sweep confirmed
...
- Ship a `source-company-verramobility` plugin returning live
...
- Backfilling historical Verra Mobility postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VERRA_MOBILITY`** in the
> source registry, so that **a single `siteType: [Site.VERRA_MOBILITY]`
> request returns Verra Mobility's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VERRA_MOBILITY = 'verramobility'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-verramobility`.                  | must     |
| FR-3  | `VerraMobilityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `verramobility-`, `site === Site.VERRA_MOBILITY`. | must |
...
```ts
@SourcePlugin({ site: Site.VERRA_MOBILITY, name: 'Verra Mobility', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/verramobility/jobs/<id>`), wire
  `company_name` pass-through (`'Verra Mobility'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/verramobility/jobs/<id>`).
