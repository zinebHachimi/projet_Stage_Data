# Spec: 241 — Source Company Plugin: Alpha Financial Markets Consulting

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 241                            |
| Slug           | source-company-alphafmcroles               |
| Status         | accepted                               |
...
Alpha Financial Markets Consulting publishes its careers board through Greenhouse at
the bare slug `alphafmcroles`. The run-398 batch sweep confirmed
92 live role(s) via a direct probe of
...
- Ship a `source-company-alphafmcroles` plugin returning live
...
- Backfilling historical Alpha Financial Markets Consulting postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALPHAFMCROLES`** in the
> source registry, so that **a single `siteType: [Site.ALPHAFMCROLES]`
> request returns Alpha Financial Markets Consulting's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALPHAFMCROLES = 'alphafmcroles'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alphafmcroles`.                  | must     |
| FR-3  | `AlphafmcrolesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alphafmcroles-`, `site === Site.ALPHAFMCROLES`. | must |
...
```ts
@SourcePlugin({ site: Site.ALPHAFMCROLES, name: 'Alpha Financial Markets Consulting', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alphafmcroles/jobs/<id>`), wire
  `company_name` pass-through (`'Alpha Financial Markets Consulting'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alphafmcroles/jobs/<id>`).
