# Spec: 951 — Source Company Plugin: Terran Orbital

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 951                            |
| Slug           | source-company-terranorbitalcorporation               |
| Status         | accepted                               |
...
Terran Orbital publishes its careers board through Greenhouse at
the bare slug `terranorbitalcorporation`. The run-398 batch sweep confirmed
...
- Ship a `source-company-terranorbitalcorporation` plugin returning live
...
- Backfilling historical Terran Orbital postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TERRAN_ORBITAL`** in the
> source registry, so that **a single `siteType: [Site.TERRAN_ORBITAL]`
> request returns Terran Orbital's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TERRAN_ORBITAL = 'terranorbitalcorporation'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-terranorbitalcorporation`.                  | must     |
| FR-3  | `TerranOrbitalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `terranorbitalcorporation-`, `site === Site.TERRAN_ORBITAL`. | must |
...
```ts
@SourcePlugin({ site: Site.TERRAN_ORBITAL, name: 'Terran Orbital', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/terranorbitalcorporation/jobs/<id>`), wire
  `company_name` pass-through (`'Terran Orbital'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/terranorbitalcorporation/jobs/<id>`).
