# Spec: 817 — Source Company Plugin: Blockchain.com

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 817                            |
| Slug           | source-company-blockchain               |
| Status         | accepted                               |
...
Blockchain.com publishes its careers board through Greenhouse at
the bare slug `blockchain`. The run-398 batch sweep confirmed
29 live role(s) via a direct probe of
...
- Ship a `source-company-blockchain` plugin returning live
...
- Backfilling historical Blockchain.com postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BLOCKCHAIN_COM`** in the
> source registry, so that **a single `siteType: [Site.BLOCKCHAIN_COM]`
> request returns Blockchain.com's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BLOCKCHAIN_COM = 'blockchain'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-blockchain`.                  | must     |
| FR-3  | `BlockchainComService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `blockchain-`, `site === Site.BLOCKCHAIN_COM`. | must |
...
```ts
@SourcePlugin({ site: Site.BLOCKCHAIN_COM, name: 'Blockchain.com', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/blockchain/jobs/<id>`), wire
  `company_name` pass-through (`'Blockchain.com'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/blockchain/jobs/<id>`).
