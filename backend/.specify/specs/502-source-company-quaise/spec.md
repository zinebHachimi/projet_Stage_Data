# Spec: 502 — Source Company Plugin: Quaise Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 502                            |
| Slug           | source-company-quaise               |
| Status         | accepted                               |
...
Quaise Energy publishes its careers board through Greenhouse at
the bare slug `quaise`. The run-398 batch sweep confirmed
...
- Ship a `source-company-quaise` plugin returning live
...
- Backfilling historical Quaise Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.QUAISE`** in the
> source registry, so that **a single `siteType: [Site.QUAISE]`
> request returns Quaise Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.QUAISE = 'quaise'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-quaise`.                  | must     |
| FR-3  | `QuaiseService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `quaise-`, `site === Site.QUAISE`. | must |
...
```ts
@SourcePlugin({ site: Site.QUAISE, name: 'Quaise Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/quaise/jobs/<id>`), wire
  `company_name` pass-through (`'Quaise Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/quaise/jobs/<id>`).
