# Spec: 470 — Source Company Plugin: Runpod

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 470                            |
| Slug           | source-company-runpod               |
| Status         | accepted                               |
...
Runpod publishes its careers board through Greenhouse at
the bare slug `runpod`. The run-398 batch sweep confirmed
...
- Ship a `source-company-runpod` plugin returning live
...
- Backfilling historical Runpod postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RUNPOD`** in the
> source registry, so that **a single `siteType: [Site.RUNPOD]`
> request returns Runpod's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RUNPOD = 'runpod'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-runpod`.                  | must     |
| FR-3  | `RunpodService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `runpod-`, `site === Site.RUNPOD`. | must |
...
```ts
@SourcePlugin({ site: Site.RUNPOD, name: 'Runpod', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/runpod/jobs/<id>`), wire
  `company_name` pass-through (`'Runpod'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/runpod/jobs/<id>`).
