# Spec: 293 — Source Company Plugin: Appier

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 293                            |
| Slug           | source-company-appier               |
| Status         | accepted                               |
...
Appier publishes its careers board through Greenhouse at
the bare slug `appier`. The run-398 batch sweep confirmed
...
- Ship a `source-company-appier` plugin returning live
...
- Backfilling historical Appier postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APPIER`** in the
> source registry, so that **a single `siteType: [Site.APPIER]`
> request returns Appier's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APPIER = 'appier'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-appier`.                  | must     |
| FR-3  | `AppierService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `appier-`, `site === Site.APPIER`. | must |
...
```ts
@SourcePlugin({ site: Site.APPIER, name: 'Appier', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/appier/jobs/<id>`), wire
  `company_name` pass-through (`'Appier'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/appier/jobs/<id>`).
