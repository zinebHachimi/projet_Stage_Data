# Spec: 220 — Source Company Plugin: AI Squared

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 220                            |
| Slug           | source-company-aisquared               |
| Status         | accepted                               |
...
AI Squared publishes its careers board through Greenhouse at
the bare slug `aisquared`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aisquared` plugin returning live
...
- Backfilling historical AI Squared postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AISQUARED`** in the
> source registry, so that **a single `siteType: [Site.AISQUARED]`
> request returns AI Squared's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AISQUARED = 'aisquared'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aisquared`.                  | must     |
| FR-3  | `AisquaredService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aisquared-`, `site === Site.AISQUARED`. | must |
...
```ts
@SourcePlugin({ site: Site.AISQUARED, name: 'AI Squared', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aisquared/jobs/<id>`), wire
  `company_name` pass-through (`'AI Squared'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aisquared/jobs/<id>`).
