# Spec: 559 — Source Company Plugin: Wayve

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 559                            |
| Slug           | source-company-wayve               |
| Status         | accepted                               |
...
Wayve publishes its careers board through Greenhouse at
the bare slug `wayve`. The run-398 batch sweep confirmed
...
- Ship a `source-company-wayve` plugin returning live
...
- Backfilling historical Wayve postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WAYVE`** in the
> source registry, so that **a single `siteType: [Site.WAYVE]`
> request returns Wayve's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WAYVE = 'wayve'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-wayve`.                  | must     |
| FR-3  | `WayveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `wayve-`, `site === Site.WAYVE`. | must |
...
```ts
@SourcePlugin({ site: Site.WAYVE, name: 'Wayve', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/wayve/jobs/<id>`), wire
  `company_name` pass-through (`'Wayve'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/wayve/jobs/<id>`).
