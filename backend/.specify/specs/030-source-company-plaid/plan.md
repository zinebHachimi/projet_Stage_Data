# Plan: 030 — Source Company Plugin: Plaid
...
Plaid's careers board is hosted on Greenhouse at the slug `plaid`, so
the implementation is a thin wrapper around the same public Greenhouse
endpoint that `source-company-lyft` (Spec 029 / run #239),
`source-company-pinterest` (Spec 028), `source-company-reddit`
...
Lyft plugin's shape (single-file `service.ts`, three-line `module.ts`,
two-line `index.ts`, four-line `package.json`, three-line
`tsconfig.json`) and rename references from Lyft → Plaid, then wire
the new package into the four registration points: `Site` enum,
plugins barrel, `tsconfig.base.json` paths, `jest.config.js`
`moduleNameMapper`. After the code lands, the unit-test fixture
and eight Jest cases run under the existing test config without
further changes.
...
used eighteen times before (Amazon, Apple, Cursor, Google, IBM,
Meta, OpenAI, Stripe, Anthropic, Databricks, Discord, Coinbase,
DoorDash, Airbnb, Robinhood, Reddit, Pinterest, Lyft).

## 2. Phases

### Phase 1 — Scaffold + register + test (single PR)

- **Goal:** Land `source-company-plaid` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-plaid/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,plaid.module.ts,plaid.service.ts}`,
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 030 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-plaid` → all green.
...
| `packages/models/src/enums/site.enum.ts`            | append `PLAID = 'plaid'`.                                    |
| `packages/plugins/index.ts`                         | import + append `PlaidModule` to `ALL_SOURCE_MODULES`.       |
| `tsconfig.base.json`                                | path-alias entry.                                            |
| `jest.config.js`                                    | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                   | add Plaid shipped row.                                       |
| `docs/index.md`                                     | append Spec 030 to the specs table.                          |
| `docs/log.md`                                       | run #240 entry.                                              |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Plaid moves off Greenhouse mid-maintenance.         | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-lyft`'s shape.           | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.PLAID` is unused outside this plugin, so the enum revert is
  safe.
...
(None — see Spec 030 § 10 Decisions for the five resolved points.)
