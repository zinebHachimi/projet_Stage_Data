# Spec: 768 — Source Company Plugin: Qventus

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 768                            |
| Slug           | source-company-qventus               |
| Status         | accepted                               |
...
Qventus publishes its careers board through Greenhouse at
the bare slug `qventus`. The run-398 batch sweep confirmed
...
- Ship a `source-company-qventus` plugin returning live
...
- Backfilling historical Qventus postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.QVENTUS`** in the
> source registry, so that **a single `siteType: [Site.QVENTUS]`
> request returns Qventus's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.QVENTUS = 'qventus'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-qventus`.                  | must     |
| FR-3  | `QventusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `qventus-`, `site === Site.QVENTUS`. | must |
...
```ts
@SourcePlugin({ site: Site.QVENTUS, name: 'Qventus', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/qventus/jobs/<id>`), wire
  `company_name` pass-through (`'Qventus'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/qventus/jobs/<id>`).
