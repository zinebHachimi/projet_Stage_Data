# Spec: 872 — Source Company Plugin: Integrated Specialty Coverages

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 872                            |
| Slug           | source-company-isccareers               |
| Status         | accepted                               |
...
Integrated Specialty Coverages publishes its careers board through Greenhouse at
the bare slug `isccareers`. The run-398 batch sweep confirmed
...
- Ship a `source-company-isccareers` plugin returning live
...
- Backfilling historical Integrated Specialty Coverages postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INTEGRATED_SPECIALTY_COVERAGES`** in the
> source registry, so that **a single `siteType: [Site.INTEGRATED_SPECIALTY_COVERAGES]`
> request returns Integrated Specialty Coverages's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INTEGRATED_SPECIALTY_COVERAGES = 'isccareers'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-isccareers`.                  | must     |
| FR-3  | `IntegratedSpecialtyCoveragesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `isccareers-`, `site === Site.INTEGRATED_SPECIALTY_COVERAGES`. | must |
...
```ts
@SourcePlugin({ site: Site.INTEGRATED_SPECIALTY_COVERAGES, name: 'Integrated Specialty Coverages', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/isccareers/jobs/<id>`), wire
  `company_name` pass-through (`'Integrated Specialty Coverages'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/isccareers/jobs/<id>`).
