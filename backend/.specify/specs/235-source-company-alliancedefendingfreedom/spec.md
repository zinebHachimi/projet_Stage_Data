# Spec: 235 — Source Company Plugin: Alliance Defending Freedom

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 235                            |
| Slug           | source-company-alliancedefendingfreedom               |
| Status         | accepted                               |
...
Alliance Defending Freedom publishes its careers board through Greenhouse at
the bare slug `alliancedefendingfreedom`. The run-398 batch sweep confirmed
31 live role(s) via a direct probe of
...
- Ship a `source-company-alliancedefendingfreedom` plugin returning live
...
- Backfilling historical Alliance Defending Freedom postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALLIANCEDEFENDINGFREEDOM`** in the
> source registry, so that **a single `siteType: [Site.ALLIANCEDEFENDINGFREEDOM]`
> request returns Alliance Defending Freedom's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALLIANCEDEFENDINGFREEDOM = 'alliancedefendingfreedom'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alliancedefendingfreedom`.                  | must     |
| FR-3  | `AlliancedefendingfreedomService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alliancedefendingfreedom-`, `site === Site.ALLIANCEDEFENDINGFREEDOM`. | must |
...
```ts
@SourcePlugin({ site: Site.ALLIANCEDEFENDINGFREEDOM, name: 'Alliance Defending Freedom', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alliancedefendingfreedom/jobs/<id>`), wire
  `company_name` pass-through (`'Alliance Defending Freedom'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alliancedefendingfreedom/jobs/<id>`).
