# Spec: 240 — Source Company Plugin: Alpha FMC - UK 

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 240                            |
| Slug           | source-company-alphafmc               |
| Status         | accepted                               |
...
Alpha FMC - UK  publishes its careers board through Greenhouse at
the bare slug `alphafmc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-alphafmc` plugin returning live
...
- Backfilling historical Alpha FMC - UK  postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALPHAFMC`** in the
> source registry, so that **a single `siteType: [Site.ALPHAFMC]`
> request returns Alpha FMC - UK 's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALPHAFMC = 'alphafmc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alphafmc`.                  | must     |
| FR-3  | `AlphafmcService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alphafmc-`, `site === Site.ALPHAFMC`. | must |
...
```ts
@SourcePlugin({ site: Site.ALPHAFMC, name: 'Alpha FMC - UK ', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alphafmc/jobs/<id>`), wire
  `company_name` pass-through (`'Alpha FMC - UK '`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alphafmc/jobs/<id>`).
