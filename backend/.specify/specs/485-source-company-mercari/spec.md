# Spec: 485 — Source Company Plugin: Mercari

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 485                            |
| Slug           | source-company-mercari               |
| Status         | accepted                               |
...
Mercari publishes its careers board through Greenhouse at
the bare slug `mercari`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mercari` plugin returning live
...
- Backfilling historical Mercari postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MERCARI`** in the
> source registry, so that **a single `siteType: [Site.MERCARI]`
> request returns Mercari's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MERCARI = 'mercari'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mercari`.                  | must     |
| FR-3  | `MercariService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mercari-`, `site === Site.MERCARI`. | must |
...
```ts
@SourcePlugin({ site: Site.MERCARI, name: 'Mercari', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mercari/jobs/<id>`), wire
  `company_name` pass-through (`'Mercari'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mercari/jobs/<id>`).
