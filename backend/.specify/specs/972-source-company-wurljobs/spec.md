# Spec: 972 — Source Company Plugin: Wurl

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 972                            |
| Slug           | source-company-wurljobs               |
| Status         | accepted                               |
...
Wurl publishes its careers board through Greenhouse at
the bare slug `wurljobs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-wurljobs` plugin returning live
...
- Backfilling historical Wurl postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WURL`** in the
> source registry, so that **a single `siteType: [Site.WURL]`
> request returns Wurl's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WURL = 'wurljobs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-wurljobs`.                  | must     |
| FR-3  | `WurlService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `wurljobs-`, `site === Site.WURL`. | must |
...
```ts
@SourcePlugin({ site: Site.WURL, name: 'Wurl', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/wurljobs/jobs/<id>`), wire
  `company_name` pass-through (`'Wurl'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/wurljobs/jobs/<id>`).
