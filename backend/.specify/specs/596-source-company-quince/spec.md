# Spec: 596 — Source Company Plugin: Quince

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 596                            |
| Slug           | source-company-quince               |
| Status         | accepted                               |
...
Quince publishes its careers board through Greenhouse at
the bare slug `quince`. The run-398 batch sweep confirmed
...
- Ship a `source-company-quince` plugin returning live
...
- Backfilling historical Quince postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.QUINCE`** in the
> source registry, so that **a single `siteType: [Site.QUINCE]`
> request returns Quince's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.QUINCE = 'quince'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-quince`.                  | must     |
| FR-3  | `QuinceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `quince-`, `site === Site.QUINCE`. | must |
...
```ts
@SourcePlugin({ site: Site.QUINCE, name: 'Quince', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/quince/jobs/<id>`), wire
  `company_name` pass-through (`'Quince'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/quince/jobs/<id>`).
