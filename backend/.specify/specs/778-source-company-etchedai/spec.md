# Spec: 778 — Source Company Plugin: Etched

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 778                            |
| Slug           | source-company-etchedai               |
| Status         | accepted                               |
...
Etched publishes its careers board through Greenhouse at
the bare slug `etchedai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-etchedai` plugin returning live
...
- Backfilling historical Etched postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ETCHED`** in the
> source registry, so that **a single `siteType: [Site.ETCHED]`
> request returns Etched's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ETCHED = 'etchedai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-etchedai`.                  | must     |
| FR-3  | `EtchedService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `etchedai-`, `site === Site.ETCHED`. | must |
...
```ts
@SourcePlugin({ site: Site.ETCHED, name: 'Etched', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/etchedai/jobs/<id>`), wire
  `company_name` pass-through (`'Etched'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/etchedai/jobs/<id>`).
