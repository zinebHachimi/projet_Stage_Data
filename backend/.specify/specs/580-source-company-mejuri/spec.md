# Spec: 580 — Source Company Plugin: Mejuri

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 580                            |
| Slug           | source-company-mejuri               |
| Status         | accepted                               |
...
Mejuri publishes its careers board through Greenhouse at
the bare slug `mejuri`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mejuri` plugin returning live
...
- Backfilling historical Mejuri postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MEJURI`** in the
> source registry, so that **a single `siteType: [Site.MEJURI]`
> request returns Mejuri's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MEJURI = 'mejuri'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mejuri`.                  | must     |
| FR-3  | `MejuriService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mejuri-`, `site === Site.MEJURI`. | must |
...
```ts
@SourcePlugin({ site: Site.MEJURI, name: 'Mejuri', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mejuri/jobs/<id>`), wire
  `company_name` pass-through (`'Mejuri'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mejuri/jobs/<id>`).
