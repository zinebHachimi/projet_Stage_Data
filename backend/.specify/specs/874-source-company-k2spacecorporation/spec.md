# Spec: 874 — Source Company Plugin: K2 Space

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 874                            |
| Slug           | source-company-k2spacecorporation               |
| Status         | accepted                               |
...
K2 Space publishes its careers board through Greenhouse at
the bare slug `k2spacecorporation`. The run-398 batch sweep confirmed
...
- Ship a `source-company-k2spacecorporation` plugin returning live
...
- Backfilling historical K2 Space postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.K2_SPACE`** in the
> source registry, so that **a single `siteType: [Site.K2_SPACE]`
> request returns K2 Space's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.K2_SPACE = 'k2spacecorporation'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-k2spacecorporation`.                  | must     |
| FR-3  | `K2SpaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `k2spacecorporation-`, `site === Site.K2_SPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.K2_SPACE, name: 'K2 Space', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/k2spacecorporation/jobs/<id>`), wire
  `company_name` pass-through (`'K2 Space'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/k2spacecorporation/jobs/<id>`).
