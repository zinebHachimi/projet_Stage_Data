# Spec: 779 — Source Company Plugin: Formic

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 779                            |
| Slug           | source-company-formic               |
| Status         | accepted                               |
...
Formic publishes its careers board through Greenhouse at
the bare slug `formic`. The run-398 batch sweep confirmed
...
- Ship a `source-company-formic` plugin returning live
...
- Backfilling historical Formic postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORMIC`** in the
> source registry, so that **a single `siteType: [Site.FORMIC]`
> request returns Formic's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORMIC = 'formic'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-formic`.                  | must     |
| FR-3  | `FormicService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `formic-`, `site === Site.FORMIC`. | must |
...
```ts
@SourcePlugin({ site: Site.FORMIC, name: 'Formic', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/formic/jobs/<id>`), wire
  `company_name` pass-through (`'Formic'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/formic/jobs/<id>`).
