# Spec: 618 — Source Company Plugin: LetsGetChecked

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 618                            |
| Slug           | source-company-letsgetchecked               |
| Status         | accepted                               |
...
LetsGetChecked publishes its careers board through Greenhouse at
the bare slug `letsgetchecked`. The run-398 batch sweep confirmed
...
- Ship a `source-company-letsgetchecked` plugin returning live
...
- Backfilling historical LetsGetChecked postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LETSGETCHECKED`** in the
> source registry, so that **a single `siteType: [Site.LETSGETCHECKED]`
> request returns LetsGetChecked's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LETSGETCHECKED = 'letsgetchecked'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-letsgetchecked`.                  | must     |
| FR-3  | `LetsGetCheckedService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `letsgetchecked-`, `site === Site.LETSGETCHECKED`. | must |
...
```ts
@SourcePlugin({ site: Site.LETSGETCHECKED, name: 'LetsGetChecked', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/letsgetchecked/jobs/<id>`), wire
  `company_name` pass-through (`'LetsGetChecked'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/letsgetchecked/jobs/<id>`).
