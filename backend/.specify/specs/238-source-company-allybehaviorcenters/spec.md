# Spec: 238 — Source Company Plugin: Ally Behavior Centers

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 238                            |
| Slug           | source-company-allybehaviorcenters               |
| Status         | accepted                               |
...
Ally Behavior Centers publishes its careers board through Greenhouse at
the bare slug `allybehaviorcenters`. The run-398 batch sweep confirmed
271 live role(s) via a direct probe of
...
- Ship a `source-company-allybehaviorcenters` plugin returning live
...
- Backfilling historical Ally Behavior Centers postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALLYBEHAVIORCENTERS`** in the
> source registry, so that **a single `siteType: [Site.ALLYBEHAVIORCENTERS]`
> request returns Ally Behavior Centers's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALLYBEHAVIORCENTERS = 'allybehaviorcenters'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-allybehaviorcenters`.                  | must     |
| FR-3  | `AllybehaviorcentersService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `allybehaviorcenters-`, `site === Site.ALLYBEHAVIORCENTERS`. | must |
...
```ts
@SourcePlugin({ site: Site.ALLYBEHAVIORCENTERS, name: 'Ally Behavior Centers', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/allybehaviorcenters/jobs/<id>`), wire
  `company_name` pass-through (`'Ally Behavior Centers'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/allybehaviorcenters/jobs/<id>`).
