# Spec: 667 — Source Company Plugin: firsthand Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 667                            |
| Slug           | source-company-firsthand               |
| Status         | accepted                               |
...
firsthand Health publishes its careers board through Greenhouse at
the bare slug `firsthand`. The run-398 batch sweep confirmed
30 live role(s) via a direct probe of
...
- Ship a `source-company-firsthand` plugin returning live
...
- Backfilling historical firsthand Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FIRSTHAND_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.FIRSTHAND_HEALTH]`
> request returns firsthand Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FIRSTHAND_HEALTH = 'firsthand'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-firsthand`.                  | must     |
| FR-3  | `FirsthandHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `firsthand-`, `site === Site.FIRSTHAND_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.FIRSTHAND_HEALTH, name: 'firsthand Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/firsthand/jobs/<id>`), wire
  `company_name` pass-through (`'firsthand Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/firsthand/jobs/<id>`).
