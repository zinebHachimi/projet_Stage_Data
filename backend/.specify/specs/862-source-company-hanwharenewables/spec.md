# Spec: 862 — Source Company Plugin: Hanwha Renewables

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 862                            |
| Slug           | source-company-hanwharenewables               |
| Status         | accepted                               |
...
Hanwha Renewables publishes its careers board through Greenhouse at
the bare slug `hanwharenewables`. The run-398 batch sweep confirmed
...
- Ship a `source-company-hanwharenewables` plugin returning live
...
- Backfilling historical Hanwha Renewables postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HANWHA_RENEWABLES`** in the
> source registry, so that **a single `siteType: [Site.HANWHA_RENEWABLES]`
> request returns Hanwha Renewables's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HANWHA_RENEWABLES = 'hanwharenewables'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-hanwharenewables`.                  | must     |
| FR-3  | `HanwhaRenewablesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `hanwharenewables-`, `site === Site.HANWHA_RENEWABLES`. | must |
...
```ts
@SourcePlugin({ site: Site.HANWHA_RENEWABLES, name: 'Hanwha Renewables', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/hanwharenewables/jobs/<id>`), wire
  `company_name` pass-through (`'Hanwha Renewables'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/hanwharenewables/jobs/<id>`).
