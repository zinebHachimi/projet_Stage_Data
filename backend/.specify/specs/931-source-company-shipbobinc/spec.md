# Spec: 931 — Source Company Plugin: ShipBob

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 931                            |
| Slug           | source-company-shipbobinc               |
| Status         | accepted                               |
...
ShipBob publishes its careers board through Greenhouse at
the bare slug `shipbobinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-shipbobinc` plugin returning live
...
- Backfilling historical ShipBob postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SHIPBOB`** in the
> source registry, so that **a single `siteType: [Site.SHIPBOB]`
> request returns ShipBob's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SHIPBOB = 'shipbobinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-shipbobinc`.                  | must     |
| FR-3  | `ShipBobService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `shipbobinc-`, `site === Site.SHIPBOB`. | must |
...
```ts
@SourcePlugin({ site: Site.SHIPBOB, name: 'ShipBob', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/shipbobinc/jobs/<id>`), wire
  `company_name` pass-through (`'ShipBob'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/shipbobinc/jobs/<id>`).
