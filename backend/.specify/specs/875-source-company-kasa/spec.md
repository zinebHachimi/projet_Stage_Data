# Spec: 875 — Source Company Plugin: Kasa

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 875                            |
| Slug           | source-company-kasa               |
| Status         | accepted                               |
...
Kasa publishes its careers board through Greenhouse at
the bare slug `kasa`. The run-398 batch sweep confirmed
...
- Ship a `source-company-kasa` plugin returning live
...
- Backfilling historical Kasa postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KASA`** in the
> source registry, so that **a single `siteType: [Site.KASA]`
> request returns Kasa's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KASA = 'kasa'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-kasa`.                  | must     |
| FR-3  | `KasaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `kasa-`, `site === Site.KASA`. | must |
...
```ts
@SourcePlugin({ site: Site.KASA, name: 'Kasa', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/kasa/jobs/<id>`), wire
  `company_name` pass-through (`'Kasa'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/kasa/jobs/<id>`).
