# Spec: 686 — Source Company Plugin: Cleo AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 686                            |
| Slug           | source-company-cleo               |
| Status         | accepted                               |
...
Cleo AI publishes its careers board through Greenhouse at
the bare slug `cleo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cleo` plugin returning live
...
- Backfilling historical Cleo AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CLEO_AI`** in the
> source registry, so that **a single `siteType: [Site.CLEO_AI]`
> request returns Cleo AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CLEO_AI = 'cleo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cleo`.                  | must     |
| FR-3  | `CleoAIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cleo-`, `site === Site.CLEO_AI`. | must |
...
```ts
@SourcePlugin({ site: Site.CLEO_AI, name: 'Cleo AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cleo/jobs/<id>`), wire
  `company_name` pass-through (`'Cleo AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cleo/jobs/<id>`).
