# Spec: 219 — Source Company Plugin: Aisera

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 219                            |
| Slug           | source-company-aiserajobs               |
| Status         | accepted                               |
...
Aisera publishes its careers board through Greenhouse at
the bare slug `aiserajobs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aiserajobs` plugin returning live
...
- Backfilling historical Aisera postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AISERAJOBS`** in the
> source registry, so that **a single `siteType: [Site.AISERAJOBS]`
> request returns Aisera's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AISERAJOBS = 'aiserajobs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aiserajobs`.                  | must     |
| FR-3  | `AiserajobsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aiserajobs-`, `site === Site.AISERAJOBS`. | must |
...
```ts
@SourcePlugin({ site: Site.AISERAJOBS, name: 'Aisera', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aiserajobs/jobs/<id>`), wire
  `company_name` pass-through (`'Aisera'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aiserajobs/jobs/<id>`).
