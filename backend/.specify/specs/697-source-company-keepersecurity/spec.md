# Spec: 697 — Source Company Plugin: Keeper Security

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 697                            |
| Slug           | source-company-keepersecurity               |
| Status         | accepted                               |
...
Keeper Security publishes its careers board through Greenhouse at
the bare slug `keepersecurity`. The run-398 batch sweep confirmed
...
- Ship a `source-company-keepersecurity` plugin returning live
...
- Backfilling historical Keeper Security postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KEEPER_SECURITY`** in the
> source registry, so that **a single `siteType: [Site.KEEPER_SECURITY]`
> request returns Keeper Security's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KEEPER_SECURITY = 'keepersecurity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-keepersecurity`.                  | must     |
| FR-3  | `KeeperSecurityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `keepersecurity-`, `site === Site.KEEPER_SECURITY`. | must |
...
```ts
@SourcePlugin({ site: Site.KEEPER_SECURITY, name: 'Keeper Security', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/keepersecurity/jobs/<id>`), wire
  `company_name` pass-through (`'Keeper Security'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/keepersecurity/jobs/<id>`).
