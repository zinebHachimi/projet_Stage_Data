# Spec: 789 — Source Company Plugin: Arbor Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 789                            |
| Slug           | source-company-arborenergy               |
| Status         | accepted                               |
...
Arbor Energy publishes its careers board through Greenhouse at
the bare slug `arborenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-arborenergy` plugin returning live
...
- Backfilling historical Arbor Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ARBOR_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.ARBOR_ENERGY]`
> request returns Arbor Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ARBOR_ENERGY = 'arborenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-arborenergy`.                  | must     |
| FR-3  | `ArborEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `arborenergy-`, `site === Site.ARBOR_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.ARBOR_ENERGY, name: 'Arbor Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/arborenergy/jobs/<id>`), wire
  `company_name` pass-through (`'Arbor Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/arborenergy/jobs/<id>`).
