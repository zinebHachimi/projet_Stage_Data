# Spec: 679 — Source Company Plugin: Seed Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 679                            |
| Slug           | source-company-seed               |
| Status         | accepted                               |
...
Seed Health publishes its careers board through Greenhouse at
the bare slug `seed`. The run-398 batch sweep confirmed
...
- Ship a `source-company-seed` plugin returning live
...
- Backfilling historical Seed Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SEED_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.SEED_HEALTH]`
> request returns Seed Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SEED_HEALTH = 'seed'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-seed`.                  | must     |
| FR-3  | `SeedHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `seed-`, `site === Site.SEED_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.SEED_HEALTH, name: 'Seed Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/seed/jobs/<id>`), wire
  `company_name` pass-through (`'Seed Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/seed/jobs/<id>`).
