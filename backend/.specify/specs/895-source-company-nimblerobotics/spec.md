# Spec: 895 — Source Company Plugin: Nimble Robotics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 895                            |
| Slug           | source-company-nimblerobotics               |
| Status         | accepted                               |
...
Nimble Robotics publishes its careers board through Greenhouse at
the bare slug `nimblerobotics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nimblerobotics` plugin returning live
...
- Backfilling historical Nimble Robotics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NIMBLE_ROBOTICS`** in the
> source registry, so that **a single `siteType: [Site.NIMBLE_ROBOTICS]`
> request returns Nimble Robotics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NIMBLE_ROBOTICS = 'nimblerobotics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nimblerobotics`.                  | must     |
| FR-3  | `NimbleRoboticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nimblerobotics-`, `site === Site.NIMBLE_ROBOTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.NIMBLE_ROBOTICS, name: 'Nimble Robotics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nimblerobotics/jobs/<id>`), wire
  `company_name` pass-through (`'Nimble Robotics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nimblerobotics/jobs/<id>`).
