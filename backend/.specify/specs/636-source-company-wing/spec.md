# Spec: 636 — Source Company Plugin: Wing

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 636                            |
| Slug           | source-company-wing               |
| Status         | accepted                               |
...
Wing publishes its careers board through Greenhouse at
the bare slug `wing`. The run-398 batch sweep confirmed
...
- Ship a `source-company-wing` plugin returning live
...
- Backfilling historical Wing postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WING`** in the
> source registry, so that **a single `siteType: [Site.WING]`
> request returns Wing's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WING = 'wing'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-wing`.                  | must     |
| FR-3  | `WingService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `wing-`, `site === Site.WING`. | must |
...
```ts
@SourcePlugin({ site: Site.WING, name: 'Wing', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/wing/jobs/<id>`), wire
  `company_name` pass-through (`'Wing'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/wing/jobs/<id>`).
