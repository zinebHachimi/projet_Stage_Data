# Spec: 622 — Source Company Plugin: EnergyHub

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 622                            |
| Slug           | source-company-energyhub               |
| Status         | accepted                               |
...
EnergyHub publishes its careers board through Greenhouse at
the bare slug `energyhub`. The run-398 batch sweep confirmed
...
- Ship a `source-company-energyhub` plugin returning live
...
- Backfilling historical EnergyHub postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ENERGYHUB`** in the
> source registry, so that **a single `siteType: [Site.ENERGYHUB]`
> request returns EnergyHub's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ENERGYHUB = 'energyhub'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-energyhub`.                  | must     |
| FR-3  | `EnergyHubService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `energyhub-`, `site === Site.ENERGYHUB`. | must |
...
```ts
@SourcePlugin({ site: Site.ENERGYHUB, name: 'EnergyHub', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/energyhub/jobs/<id>`), wire
  `company_name` pass-through (`'EnergyHub'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/energyhub/jobs/<id>`).
