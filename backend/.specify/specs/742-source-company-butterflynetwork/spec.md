# Spec: 742 — Source Company Plugin: Butterfly Network

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 742                            |
| Slug           | source-company-butterflynetwork               |
| Status         | accepted                               |
...
Butterfly Network publishes its careers board through Greenhouse at
the bare slug `butterflynetwork`. The run-398 batch sweep confirmed
...
- Ship a `source-company-butterflynetwork` plugin returning live
...
- Backfilling historical Butterfly Network postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BUTTERFLY_NETWORK`** in the
> source registry, so that **a single `siteType: [Site.BUTTERFLY_NETWORK]`
> request returns Butterfly Network's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BUTTERFLY_NETWORK = 'butterflynetwork'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-butterflynetwork`.                  | must     |
| FR-3  | `ButterflyNetworkService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `butterflynetwork-`, `site === Site.BUTTERFLY_NETWORK`. | must |
...
```ts
@SourcePlugin({ site: Site.BUTTERFLY_NETWORK, name: 'Butterfly Network', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/butterflynetwork/jobs/<id>`), wire
  `company_name` pass-through (`'Butterfly Network'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/butterflynetwork/jobs/<id>`).
