# Spec: 463 — Source Company Plugin: Epirus

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 463                            |
| Slug           | source-company-epirus               |
| Status         | accepted                               |
...
Epirus publishes its careers board through Greenhouse at
the bare slug `epirus`. The run-398 batch sweep confirmed
...
- Ship a `source-company-epirus` plugin returning live
...
- Backfilling historical Epirus postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EPIRUS`** in the
> source registry, so that **a single `siteType: [Site.EPIRUS]`
> request returns Epirus's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EPIRUS = 'epirus'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-epirus`.                  | must     |
| FR-3  | `EpirusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `epirus-`, `site === Site.EPIRUS`. | must |
...
```ts
@SourcePlugin({ site: Site.EPIRUS, name: 'Epirus', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/epirus/jobs/<id>`), wire
  `company_name` pass-through (`'Epirus'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/epirus/jobs/<id>`).
