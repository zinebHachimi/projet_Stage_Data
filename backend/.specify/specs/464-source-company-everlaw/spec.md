# Spec: 464 — Source Company Plugin: Everlaw

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 464                            |
| Slug           | source-company-everlaw               |
| Status         | accepted                               |
...
Everlaw publishes its careers board through Greenhouse at
the bare slug `everlaw`. The run-398 batch sweep confirmed
...
- Ship a `source-company-everlaw` plugin returning live
...
- Backfilling historical Everlaw postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EVERLAW`** in the
> source registry, so that **a single `siteType: [Site.EVERLAW]`
> request returns Everlaw's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EVERLAW = 'everlaw'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-everlaw`.                  | must     |
| FR-3  | `EverlawService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `everlaw-`, `site === Site.EVERLAW`. | must |
...
```ts
@SourcePlugin({ site: Site.EVERLAW, name: 'Everlaw', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/everlaw/jobs/<id>`), wire
  `company_name` pass-through (`'Everlaw'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/everlaw/jobs/<id>`).
