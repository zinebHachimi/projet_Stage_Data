# Spec: 855 — Source Company Plugin: Gatik AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 855                            |
| Slug           | source-company-gatikaiinc               |
| Status         | accepted                               |
...
Gatik AI publishes its careers board through Greenhouse at
the bare slug `gatikaiinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-gatikaiinc` plugin returning live
...
- Backfilling historical Gatik AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GATIK_AI`** in the
> source registry, so that **a single `siteType: [Site.GATIK_AI]`
> request returns Gatik AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GATIK_AI = 'gatikaiinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-gatikaiinc`.                  | must     |
| FR-3  | `GatikAIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `gatikaiinc-`, `site === Site.GATIK_AI`. | must |
...
```ts
@SourcePlugin({ site: Site.GATIK_AI, name: 'Gatik AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/gatikaiinc/jobs/<id>`), wire
  `company_name` pass-through (`'Gatik AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/gatikaiinc/jobs/<id>`).
