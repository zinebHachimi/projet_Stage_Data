# Spec: 441 — Source Company Plugin: Wrike

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 441                            |
| Slug           | source-company-wrike               |
| Status         | accepted                               |
...
Wrike publishes its careers board through Greenhouse at
the bare slug `wrike`. The run-398 batch sweep confirmed
...
- Ship a `source-company-wrike` plugin returning live
...
- Backfilling historical Wrike postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WRIKE`** in the
> source registry, so that **a single `siteType: [Site.WRIKE]`
> request returns Wrike's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WRIKE = 'wrike'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-wrike`.                  | must     |
| FR-3  | `WrikeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `wrike-`, `site === Site.WRIKE`. | must |
...
```ts
@SourcePlugin({ site: Site.WRIKE, name: 'Wrike', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/wrike/jobs/<id>`), wire
  `company_name` pass-through (`'Wrike'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/wrike/jobs/<id>`).
