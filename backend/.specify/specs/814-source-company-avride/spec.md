# Spec: 814 — Source Company Plugin: Avride

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 814                            |
| Slug           | source-company-avride               |
| Status         | accepted                               |
...
Avride publishes its careers board through Greenhouse at
the bare slug `avride`. The run-398 batch sweep confirmed
...
- Ship a `source-company-avride` plugin returning live
...
- Backfilling historical Avride postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AVRIDE`** in the
> source registry, so that **a single `siteType: [Site.AVRIDE]`
> request returns Avride's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AVRIDE = 'avride'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-avride`.                  | must     |
| FR-3  | `AvrideService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `avride-`, `site === Site.AVRIDE`. | must |
...
```ts
@SourcePlugin({ site: Site.AVRIDE, name: 'Avride', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/avride/jobs/<id>`), wire
  `company_name` pass-through (`'Avride'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/avride/jobs/<id>`).
