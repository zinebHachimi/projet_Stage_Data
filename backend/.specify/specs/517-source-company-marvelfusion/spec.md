# Spec: 517 — Source Company Plugin: Marvel Fusion

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 517                            |
| Slug           | source-company-marvelfusion               |
| Status         | accepted                               |
...
Marvel Fusion publishes its careers board through Greenhouse at
the bare slug `marvelfusion`. The run-398 batch sweep confirmed
...
- Ship a `source-company-marvelfusion` plugin returning live
...
- Backfilling historical Marvel Fusion postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MARVEL_FUSION`** in the
> source registry, so that **a single `siteType: [Site.MARVEL_FUSION]`
> request returns Marvel Fusion's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MARVEL_FUSION = 'marvelfusion'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-marvelfusion`.                  | must     |
| FR-3  | `MarvelfusionService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `marvelfusion-`, `site === Site.MARVEL_FUSION`. | must |
...
```ts
@SourcePlugin({ site: Site.MARVEL_FUSION, name: 'Marvel Fusion', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/marvelfusion/jobs/<id>`), wire
  `company_name` pass-through (`'Marvel Fusion'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/marvelfusion/jobs/<id>`).
