# Spec: 668 — Source Company Plugin: Atoms Tech

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 668                            |
| Slug           | source-company-atoms               |
| Status         | accepted                               |
...
Atoms Tech publishes its careers board through Greenhouse at
the bare slug `atoms`. The run-398 batch sweep confirmed
...
- Ship a `source-company-atoms` plugin returning live
...
- Backfilling historical Atoms Tech postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ATOMS_TECH`** in the
> source registry, so that **a single `siteType: [Site.ATOMS_TECH]`
> request returns Atoms Tech's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ATOMS_TECH = 'atoms'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-atoms`.                  | must     |
| FR-3  | `AtomsTechService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `atoms-`, `site === Site.ATOMS_TECH`. | must |
...
```ts
@SourcePlugin({ site: Site.ATOMS_TECH, name: 'Atoms Tech', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/atoms/jobs/<id>`), wire
  `company_name` pass-through (`'Atoms Tech'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/atoms/jobs/<id>`).
