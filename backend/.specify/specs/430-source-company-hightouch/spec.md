# Spec: 430 — Source Company Plugin: Hightouch

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 430                            |
| Slug           | source-company-hightouch               |
| Status         | accepted                               |
...
Hightouch publishes its careers board through Greenhouse at
the bare slug `hightouch`. The run-398 batch sweep confirmed
66 live role(s) via a direct probe of
...
- Ship a `source-company-hightouch` plugin returning live
...
- Backfilling historical Hightouch postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HIGHTOUCH`** in the
> source registry, so that **a single `siteType: [Site.HIGHTOUCH]`
> request returns Hightouch's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HIGHTOUCH = 'hightouch'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-hightouch`.                  | must     |
| FR-3  | `HightouchService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `hightouch-`, `site === Site.HIGHTOUCH`. | must |
...
```ts
@SourcePlugin({ site: Site.HIGHTOUCH, name: 'Hightouch', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/hightouch/jobs/<id>`), wire
  `company_name` pass-through (`'Hightouch'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/hightouch/jobs/<id>`).
