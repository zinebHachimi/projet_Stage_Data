# Plan: 022 — Source Company Plugin: Discord
...
Discord's careers board is hosted on Greenhouse at the slug `discord`,
so the implementation is a thin wrapper around the same public
Greenhouse endpoint that `source-company-databricks` (Spec 021 / run
#231) and `source-company-anthropic` already call. The plan is to
copy the Databricks plugin's shape (single-file `service.ts`,
three-line `module.ts`, two-line `index.ts`, four-line `package.json`,
three-line `tsconfig.json`) and rename references from Databricks →
Discord, then wire the new package into the four registration
...
ten times before (Amazon, Apple, Cursor, Google, IBM, Meta, OpenAI,
Stripe, Anthropic, Databricks).

## 2. Phases

### Phase 1 — Scaffold + register + test (single PR)

- **Goal:** Land `source-company-discord` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-discord/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,discord.module.ts,discord.service.ts}`,
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 022 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-discord` → all green.
...
| `packages/models/src/enums/site.enum.ts`         | append `DISCORD = 'discord'`.                                |
| `packages/plugins/index.ts`                      | import + append `DiscordModule` to `ALL_SOURCE_MODULES`.     |
| `tsconfig.base.json`                             | path-alias entry.                                            |
| `jest.config.js`                                 | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                | add Discord shipped row.                                     |
| `docs/index.md`                                  | append Spec 022 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Discord moves off Greenhouse mid-maintenance.       | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-databricks`'s shape.     | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.DISCORD` is unused outside this plugin, so the enum revert
  is safe.
...
(None — see Spec 022 § 10 Decisions for the three resolved points.)
