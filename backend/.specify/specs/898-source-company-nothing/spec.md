# Spec: 898 — Source Company Plugin: Nothing

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 898                            |
| Slug           | source-company-nothing               |
| Status         | accepted                               |
...
Nothing publishes its careers board through Greenhouse at
the bare slug `nothing`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nothing` plugin returning live
...
- Backfilling historical Nothing postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NOTHING`** in the
> source registry, so that **a single `siteType: [Site.NOTHING]`
> request returns Nothing's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NOTHING = 'nothing'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nothing`.                  | must     |
| FR-3  | `NothingService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nothing-`, `site === Site.NOTHING`. | must |
...
```ts
@SourcePlugin({ site: Site.NOTHING, name: 'Nothing', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nothing/jobs/<id>`), wire
  `company_name` pass-through (`'Nothing'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nothing/jobs/<id>`).
