# Spec: 818 — Source Company Plugin: Bot Auto

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 818                            |
| Slug           | source-company-botauto               |
| Status         | accepted                               |
...
Bot Auto publishes its careers board through Greenhouse at
the bare slug `botauto`. The run-398 batch sweep confirmed
...
- Ship a `source-company-botauto` plugin returning live
...
- Backfilling historical Bot Auto postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BOT_AUTO`** in the
> source registry, so that **a single `siteType: [Site.BOT_AUTO]`
> request returns Bot Auto's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BOT_AUTO = 'botauto'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-botauto`.                  | must     |
| FR-3  | `BotAutoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `botauto-`, `site === Site.BOT_AUTO`. | must |
...
```ts
@SourcePlugin({ site: Site.BOT_AUTO, name: 'Bot Auto', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/botauto/jobs/<id>`), wire
  `company_name` pass-through (`'Bot Auto'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/botauto/jobs/<id>`).
