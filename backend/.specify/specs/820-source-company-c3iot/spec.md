# Spec: 820 — Source Company Plugin: C3 AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 820                            |
| Slug           | source-company-c3iot               |
| Status         | accepted                               |
...
C3 AI publishes its careers board through Greenhouse at
the bare slug `c3iot`. The run-398 batch sweep confirmed
57 live role(s) via a direct probe of
...
- Ship a `source-company-c3iot` plugin returning live
...
- Backfilling historical C3 AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.C3_AI`** in the
> source registry, so that **a single `siteType: [Site.C3_AI]`
> request returns C3 AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.C3_AI = 'c3iot'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-c3iot`.                  | must     |
| FR-3  | `C3AIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `c3iot-`, `site === Site.C3_AI`. | must |
...
```ts
@SourcePlugin({ site: Site.C3_AI, name: 'C3 AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/c3iot/jobs/<id>`), wire
  `company_name` pass-through (`'C3 AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/c3iot/jobs/<id>`).
