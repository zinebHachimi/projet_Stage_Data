# Plan: 026 — Source Company Plugin: Robinhood
...
Robinhood's careers board is hosted on Greenhouse at the slug
`robinhoodjobs` (note: not the bare `robinhood`, see Spec 026 § 10
D-05), so the implementation is a thin wrapper around the same public
Greenhouse endpoint that `source-company-airbnb` (Spec 025 /
run #235), `source-company-doordash`, `source-company-coinbase`,
`source-company-discord`, `source-company-databricks`, and
`source-company-anthropic` already call. The plan is to copy the
Airbnb plugin's shape (single-file `service.ts`, three-line
`module.ts`, two-line `index.ts`, four-line `package.json`,
three-line `tsconfig.json`) and rename references from Airbnb →
Robinhood, then wire the new package into the four registration
...
fourteen times before (Amazon, Apple, Cursor, Google, IBM, Meta,
...
- **Goal:** Land `source-company-robinhood` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-robinhood/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,robinhood.module.ts,robinhood.service.ts}`,
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 026 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-robinhood` → all green.
...
| `packages/models/src/enums/site.enum.ts`         | append `ROBINHOOD = 'robinhood'`.                            |
| `packages/plugins/index.ts`                      | import + append `RobinhoodModule` to `ALL_SOURCE_MODULES`.   |
| `tsconfig.base.json`                             | path-alias entry.                                            |
| `jest.config.js`                                 | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                | add Robinhood shipped row.                                   |
| `docs/index.md`                                  | append Spec 026 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Robinhood moves off Greenhouse mid-maintenance.     | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-airbnb`'s shape.         | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
| Wrong slug (`robinhood` vs `robinhoodjobs`).        | M          | M      | D-05 pins the slug as a constant; covered by happy-path unit test asserting the called URL string. |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.ROBINHOOD` is unused outside this plugin, so the enum revert
  is safe.
...
(None — see Spec 026 § 10 Decisions for the five resolved points.)
