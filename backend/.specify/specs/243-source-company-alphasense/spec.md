# Spec: 243 — Source Company Plugin: AlphaSense

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 243                            |
| Slug           | source-company-alphasense               |
| Status         | accepted                               |
...
AlphaSense publishes its careers board through Greenhouse at
the bare slug `alphasense`. The run-398 batch sweep confirmed
214 live role(s) via a direct probe of
...
- Ship a `source-company-alphasense` plugin returning live
...
- Backfilling historical AlphaSense postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALPHASENSE`** in the
> source registry, so that **a single `siteType: [Site.ALPHASENSE]`
> request returns AlphaSense's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALPHASENSE = 'alphasense'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alphasense`.                  | must     |
| FR-3  | `AlphasenseService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alphasense-`, `site === Site.ALPHASENSE`. | must |
...
```ts
@SourcePlugin({ site: Site.ALPHASENSE, name: 'AlphaSense', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alphasense/jobs/<id>`), wire
  `company_name` pass-through (`'AlphaSense'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alphasense/jobs/<id>`).
