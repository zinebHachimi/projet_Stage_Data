# Spec: 700 — Source Company Plugin: StackBlitz

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 700                            |
| Slug           | source-company-stackblitz               |
| Status         | accepted                               |
...
StackBlitz publishes its careers board through Greenhouse at
the bare slug `stackblitz`. The run-398 batch sweep confirmed
...
- Ship a `source-company-stackblitz` plugin returning live
...
- Backfilling historical StackBlitz postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STACKBLITZ`** in the
> source registry, so that **a single `siteType: [Site.STACKBLITZ]`
> request returns StackBlitz's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STACKBLITZ = 'stackblitz'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-stackblitz`.                  | must     |
| FR-3  | `StackBlitzService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `stackblitz-`, `site === Site.STACKBLITZ`. | must |
...
```ts
@SourcePlugin({ site: Site.STACKBLITZ, name: 'StackBlitz', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/stackblitz/jobs/<id>`), wire
  `company_name` pass-through (`'StackBlitz'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/stackblitz/jobs/<id>`).
