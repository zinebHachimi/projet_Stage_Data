# Spec: 645 — Source Company Plugin: Prenuvo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 645                            |
| Slug           | source-company-prenuvo               |
| Status         | accepted                               |
...
Prenuvo publishes its careers board through Greenhouse at
the bare slug `prenuvo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-prenuvo` plugin returning live
...
- Backfilling historical Prenuvo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PRENUVO`** in the
> source registry, so that **a single `siteType: [Site.PRENUVO]`
> request returns Prenuvo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PRENUVO = 'prenuvo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-prenuvo`.                  | must     |
| FR-3  | `PrenuvoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `prenuvo-`, `site === Site.PRENUVO`. | must |
...
```ts
@SourcePlugin({ site: Site.PRENUVO, name: 'Prenuvo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/prenuvo/jobs/<id>`), wire
  `company_name` pass-through (`'Prenuvo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/prenuvo/jobs/<id>`).
