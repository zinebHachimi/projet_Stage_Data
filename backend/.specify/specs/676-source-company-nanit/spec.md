# Spec: 676 — Source Company Plugin: Nanit

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 676                            |
| Slug           | source-company-nanit               |
| Status         | accepted                               |
...
Nanit publishes its careers board through Greenhouse at
the bare slug `nanit`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nanit` plugin returning live
...
- Backfilling historical Nanit postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NANIT`** in the
> source registry, so that **a single `siteType: [Site.NANIT]`
> request returns Nanit's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NANIT = 'nanit'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nanit`.                  | must     |
| FR-3  | `NanitService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nanit-`, `site === Site.NANIT`. | must |
...
```ts
@SourcePlugin({ site: Site.NANIT, name: 'Nanit', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nanit/jobs/<id>`), wire
  `company_name` pass-through (`'Nanit'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nanit/jobs/<id>`).
