# Plan: 676 — Source Company Plugin: Nanit
...
Nanit's careers board is hosted on Greenhouse at the slug
`nanit`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-nanit` plugin.
- Deliverables: package files, `Site.NANIT` enum value, the
...
- `packages/plugins/source-company-nanit` (new)
