# Spec: 282 — Source Company Plugin: Apex Companies - CSW

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 282                            |
| Slug           | source-company-apexcompaniescsw               |
| Status         | accepted                               |
...
Apex Companies - CSW publishes its careers board through Greenhouse at
the bare slug `apexcompaniescsw`. The run-398 batch sweep confirmed
...
- Ship a `source-company-apexcompaniescsw` plugin returning live
...
- Backfilling historical Apex Companies - CSW postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APEXCOMPANIESCSW`** in the
> source registry, so that **a single `siteType: [Site.APEXCOMPANIESCSW]`
> request returns Apex Companies - CSW's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APEXCOMPANIESCSW = 'apexcompaniescsw'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apexcompaniescsw`.                  | must     |
| FR-3  | `ApexcompaniescswService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apexcompaniescsw-`, `site === Site.APEXCOMPANIESCSW`. | must |
...
```ts
@SourcePlugin({ site: Site.APEXCOMPANIESCSW, name: 'Apex Companies - CSW', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apexcompaniescsw/jobs/<id>`), wire
  `company_name` pass-through (`'Apex Companies - CSW'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apexcompaniescsw/jobs/<id>`).
