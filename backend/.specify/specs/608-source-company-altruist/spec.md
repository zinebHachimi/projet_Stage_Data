# Spec: 608 — Source Company Plugin: Altruist

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 608                            |
| Slug           | source-company-altruist               |
| Status         | accepted                               |
...
Altruist publishes its careers board through Greenhouse at
the bare slug `altruist`. The run-398 batch sweep confirmed
...
- Ship a `source-company-altruist` plugin returning live
...
- Backfilling historical Altruist postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALTRUIST`** in the
> source registry, so that **a single `siteType: [Site.ALTRUIST]`
> request returns Altruist's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALTRUIST = 'altruist'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-altruist`.                  | must     |
| FR-3  | `AltruistService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `altruist-`, `site === Site.ALTRUIST`. | must |
...
```ts
@SourcePlugin({ site: Site.ALTRUIST, name: 'Altruist', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/altruist/jobs/<id>`), wire
  `company_name` pass-through (`'Altruist'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/altruist/jobs/<id>`).
