# Spec: 223 — Source Company Plugin: Akoya

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 223                            |
| Slug           | source-company-akoya               |
| Status         | accepted                               |
...
Akoya publishes its careers board through Greenhouse at
the bare slug `akoya`. The run-398 batch sweep confirmed
...
- Ship a `source-company-akoya` plugin returning live
...
- Backfilling historical Akoya postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AKOYA`** in the
> source registry, so that **a single `siteType: [Site.AKOYA]`
> request returns Akoya's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AKOYA = 'akoya'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-akoya`.                  | must     |
| FR-3  | `AkoyaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `akoya-`, `site === Site.AKOYA`. | must |
...
```ts
@SourcePlugin({ site: Site.AKOYA, name: 'Akoya', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/akoya/jobs/<id>`), wire
  `company_name` pass-through (`'Akoya'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/akoya/jobs/<id>`).
