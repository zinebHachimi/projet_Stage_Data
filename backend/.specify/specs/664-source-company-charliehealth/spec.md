# Spec: 664 — Source Company Plugin: Charlie Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 664                            |
| Slug           | source-company-charliehealth               |
| Status         | accepted                               |
...
Charlie Health publishes its careers board through Greenhouse at
the bare slug `charliehealth`. The run-398 batch sweep confirmed
327 live role(s) via a direct probe of
...
- Ship a `source-company-charliehealth` plugin returning live
...
- Backfilling historical Charlie Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CHARLIE_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.CHARLIE_HEALTH]`
> request returns Charlie Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CHARLIE_HEALTH = 'charliehealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-charliehealth`.                  | must     |
| FR-3  | `CharlieHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `charliehealth-`, `site === Site.CHARLIE_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.CHARLIE_HEALTH, name: 'Charlie Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/charliehealth/jobs/<id>`), wire
  `company_name` pass-through (`'Charlie Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/charliehealth/jobs/<id>`).
