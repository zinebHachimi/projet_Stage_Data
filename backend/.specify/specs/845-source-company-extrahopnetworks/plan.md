# Plan: 845 — Source Company Plugin: ExtraHop
...
ExtraHop's careers board is hosted on Greenhouse at the slug
`extrahopnetworks`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-extrahopnetworks` plugin.
- Deliverables: package files, `Site.EXTRAHOP` enum value, the
...
- `packages/plugins/source-company-extrahopnetworks` (new)
