# Spec: 845 — Source Company Plugin: ExtraHop

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 845                            |
| Slug           | source-company-extrahopnetworks               |
| Status         | accepted                               |
...
ExtraHop publishes its careers board through Greenhouse at
the bare slug `extrahopnetworks`. The run-398 batch sweep confirmed
...
- Ship a `source-company-extrahopnetworks` plugin returning live
...
- Backfilling historical ExtraHop postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EXTRAHOP`** in the
> source registry, so that **a single `siteType: [Site.EXTRAHOP]`
> request returns ExtraHop's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EXTRAHOP = 'extrahopnetworks'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-extrahopnetworks`.                  | must     |
| FR-3  | `ExtraHopService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `extrahopnetworks-`, `site === Site.EXTRAHOP`. | must |
...
```ts
@SourcePlugin({ site: Site.EXTRAHOP, name: 'ExtraHop', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/extrahopnetworks/jobs/<id>`), wire
  `company_name` pass-through (`'ExtraHop'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/extrahopnetworks/jobs/<id>`).
