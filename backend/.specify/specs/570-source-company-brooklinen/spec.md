# Spec: 570 — Source Company Plugin: Brooklinen

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 570                            |
| Slug           | source-company-brooklinen               |
| Status         | accepted                               |
...
Brooklinen publishes its careers board through Greenhouse at
the bare slug `brooklinen`. The run-398 batch sweep confirmed
...
- Ship a `source-company-brooklinen` plugin returning live
...
- Backfilling historical Brooklinen postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BROOKLINEN`** in the
> source registry, so that **a single `siteType: [Site.BROOKLINEN]`
> request returns Brooklinen's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BROOKLINEN = 'brooklinen'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-brooklinen`.                  | must     |
| FR-3  | `BrooklinenService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `brooklinen-`, `site === Site.BROOKLINEN`. | must |
...
```ts
@SourcePlugin({ site: Site.BROOKLINEN, name: 'Brooklinen', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/brooklinen/jobs/<id>`), wire
  `company_name` pass-through (`'Brooklinen'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/brooklinen/jobs/<id>`).
