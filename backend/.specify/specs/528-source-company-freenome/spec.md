# Spec: 528 — Source Company Plugin: Freenome

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 528                            |
| Slug           | source-company-freenome               |
| Status         | accepted                               |
...
Freenome publishes its careers board through Greenhouse at
the bare slug `freenome`. The run-398 batch sweep confirmed
...
- Ship a `source-company-freenome` plugin returning live
...
- Backfilling historical Freenome postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FREENOME`** in the
> source registry, so that **a single `siteType: [Site.FREENOME]`
> request returns Freenome's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FREENOME = 'freenome'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-freenome`.                  | must     |
| FR-3  | `FreenomeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `freenome-`, `site === Site.FREENOME`. | must |
...
```ts
@SourcePlugin({ site: Site.FREENOME, name: 'Freenome', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/freenome/jobs/<id>`), wire
  `company_name` pass-through (`'Freenome'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/freenome/jobs/<id>`).
