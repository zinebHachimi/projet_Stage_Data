# Spec: 687 — Source Company Plugin: Inceptive

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 687                            |
| Slug           | source-company-inceptive               |
| Status         | accepted                               |
...
Inceptive publishes its careers board through Greenhouse at
the bare slug `inceptive`. The run-398 batch sweep confirmed
...
- Ship a `source-company-inceptive` plugin returning live
...
- Backfilling historical Inceptive postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INCEPTIVE`** in the
> source registry, so that **a single `siteType: [Site.INCEPTIVE]`
> request returns Inceptive's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INCEPTIVE = 'inceptive'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-inceptive`.                  | must     |
| FR-3  | `InceptiveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `inceptive-`, `site === Site.INCEPTIVE`. | must |
...
```ts
@SourcePlugin({ site: Site.INCEPTIVE, name: 'Inceptive', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/inceptive/jobs/<id>`), wire
  `company_name` pass-through (`'Inceptive'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/inceptive/jobs/<id>`).
