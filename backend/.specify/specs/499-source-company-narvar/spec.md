# Spec: 499 — Source Company Plugin: Narvar

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 499                            |
| Slug           | source-company-narvar               |
| Status         | accepted                               |
...
Narvar publishes its careers board through Greenhouse at
the bare slug `narvar`. The run-398 batch sweep confirmed
...
- Ship a `source-company-narvar` plugin returning live
...
- Backfilling historical Narvar postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NARVAR`** in the
> source registry, so that **a single `siteType: [Site.NARVAR]`
> request returns Narvar's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NARVAR = 'narvar'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-narvar`.                  | must     |
| FR-3  | `NarvarService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `narvar-`, `site === Site.NARVAR`. | must |
...
```ts
@SourcePlugin({ site: Site.NARVAR, name: 'Narvar', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/narvar/jobs/<id>`), wire
  `company_name` pass-through (`'Narvar'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/narvar/jobs/<id>`).
