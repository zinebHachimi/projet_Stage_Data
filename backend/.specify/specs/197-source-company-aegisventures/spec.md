# Spec: 197 — Source Company Plugin: Aegis Ventures

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 197                            |
| Slug           | source-company-aegisventures               |
| Status         | accepted                               |
...
Aegis Ventures publishes its careers board through Greenhouse at
the bare slug `aegisventures`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aegisventures` plugin returning live
...
- Backfilling historical Aegis Ventures postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AEGISVENTURES`** in the
> source registry, so that **a single `siteType: [Site.AEGISVENTURES]`
> request returns Aegis Ventures's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AEGISVENTURES = 'aegisventures'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aegisventures`.                  | must     |
| FR-3  | `AegisventuresService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aegisventures-`, `site === Site.AEGISVENTURES`. | must |
...
```ts
@SourcePlugin({ site: Site.AEGISVENTURES, name: 'Aegis Ventures', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aegisventures/jobs/<id>`), wire
  `company_name` pass-through (`'Aegis Ventures'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aegisventures/jobs/<id>`).
