# Spec: 213 — Source Company Plugin: AIFT

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 213                            |
| Slug           | source-company-aift               |
| Status         | accepted                               |
...
AIFT publishes its careers board through Greenhouse at
the bare slug `aift`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aift` plugin returning live
...
- Backfilling historical AIFT postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AIFT`** in the
> source registry, so that **a single `siteType: [Site.AIFT]`
> request returns AIFT's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AIFT = 'aift'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aift`.                  | must     |
| FR-3  | `AiftService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aift-`, `site === Site.AIFT`. | must |
...
```ts
@SourcePlugin({ site: Site.AIFT, name: 'AIFT', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aift/jobs/<id>`), wire
  `company_name` pass-through (`'AIFT'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aift/jobs/<id>`).
