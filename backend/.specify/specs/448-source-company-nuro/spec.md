# Spec: 448 — Source Company Plugin: Nuro

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 448                            |
| Slug           | source-company-nuro               |
| Status         | accepted                               |
...
Nuro publishes its careers board through Greenhouse at
the bare slug `nuro`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nuro` plugin returning live
...
- Backfilling historical Nuro postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NURO`** in the
> source registry, so that **a single `siteType: [Site.NURO]`
> request returns Nuro's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NURO = 'nuro'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nuro`.                  | must     |
| FR-3  | `NuroService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nuro-`, `site === Site.NURO`. | must |
...
```ts
@SourcePlugin({ site: Site.NURO, name: 'Nuro', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nuro/jobs/<id>`), wire
  `company_name` pass-through (`'Nuro'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nuro/jobs/<id>`).
