# Spec: 841 — Source Company Plugin: emnify

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 841                            |
| Slug           | source-company-emnify               |
| Status         | accepted                               |
...
emnify publishes its careers board through Greenhouse at
the bare slug `emnify`. The run-398 batch sweep confirmed
...
- Ship a `source-company-emnify` plugin returning live
...
- Backfilling historical emnify postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EMNIFY`** in the
> source registry, so that **a single `siteType: [Site.EMNIFY]`
> request returns emnify's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EMNIFY = 'emnify'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-emnify`.                  | must     |
| FR-3  | `EmnifyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `emnify-`, `site === Site.EMNIFY`. | must |
...
```ts
@SourcePlugin({ site: Site.EMNIFY, name: 'emnify', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/emnify/jobs/<id>`), wire
  `company_name` pass-through (`'emnify'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/emnify/jobs/<id>`).
