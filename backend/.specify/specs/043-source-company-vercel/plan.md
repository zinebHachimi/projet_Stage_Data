# Plan: 043 — Source Company Plugin: Vercel
...
Vercel's careers board is hosted on Greenhouse at the slug `vercel`, so the
implementation is a thin wrapper around the same public Greenhouse endpoint
that `source-company-block` (Spec 042 / run #252), `source-company-roblox`
...
`source-company-gitlab` (Spec 033 / run #243), `source-company-figma` (Spec
032 / run #242), `source-company-asana` (Spec 031), `source-company-plaid`
(Spec 030), `source-company-lyft` (Spec 029), `source-company-pinterest`
(Spec 028), `source-company-reddit` (Spec 027), `source-company-robinhood`,
...
The plan is to copy the Block plugin's shape (single-file `service.ts`,
three-line `module.ts`, two-line `index.ts`, four-line `package.json`,
three-line `tsconfig.json`) and rename references from Block → Vercel, then
...
comes entirely from leaning on a pattern the codebase has used thirty-one
...
- **Goal:** Land `source-company-vercel` under `ALL_SOURCE_MODULES` with
  passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-vercel/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,vercel.module.ts,vercel.service.ts}`,
    `__tests__/vercel.service.spec.ts`,
    `__tests__/fixtures/vercel-jobs.json`).
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md` ("shipped" status),
    index-table addition for Spec 043 in `docs/index.md`, log entry in
    `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-vercel` → all green.
...
| `packages/models/src/enums/site.enum.ts`             | append `VERCEL = 'vercel'`.                                  |
| `packages/plugins/index.ts`                          | import + append `VercelModule` to `ALL_SOURCE_MODULES`.      |
| `tsconfig.base.json`                                 | path-alias entry.                                            |
| `jest.config.js`                                     | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                    | add Vercel shipped row.                                      |
| `docs/index.md`                                      | append Spec 043 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Vercel moves off Greenhouse mid-maintenance.        | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-block`'s shape.          | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
| Greenhouse `job-boards` permalink-host migration.   | L          | L      | Fallback uses the new `job-boards.greenhouse.io` host (D-04); legacy `boards.greenhouse.io/vercel/jobs/<id>` issues a 301 to the new host, so any caller following either form lands at the same canonical URL. |
...
- `Site.VERCEL` is unused outside this plugin, so the enum revert is safe.
...
(None — see Spec 043 § 10 Decisions for the six resolved points.)
