# Spec: 866 — Source Company Plugin: Hyperproof

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 866                            |
| Slug           | source-company-hyperproof               |
| Status         | accepted                               |
...
Hyperproof publishes its careers board through Greenhouse at
the bare slug `hyperproof`. The run-398 batch sweep confirmed
...
- Ship a `source-company-hyperproof` plugin returning live
...
- Backfilling historical Hyperproof postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HYPERPROOF`** in the
> source registry, so that **a single `siteType: [Site.HYPERPROOF]`
> request returns Hyperproof's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HYPERPROOF = 'hyperproof'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-hyperproof`.                  | must     |
| FR-3  | `HyperproofService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `hyperproof-`, `site === Site.HYPERPROOF`. | must |
...
```ts
@SourcePlugin({ site: Site.HYPERPROOF, name: 'Hyperproof', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/hyperproof/jobs/<id>`), wire
  `company_name` pass-through (`'Hyperproof'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/hyperproof/jobs/<id>`).
