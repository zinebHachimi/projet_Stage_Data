# Spec: 867 — Source Company Plugin: ID.me

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 867                            |
| Slug           | source-company-idme               |
| Status         | accepted                               |
...
ID.me publishes its careers board through Greenhouse at
the bare slug `idme`. The run-398 batch sweep confirmed
...
- Ship a `source-company-idme` plugin returning live
...
- Backfilling historical ID.me postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ID_ME`** in the
> source registry, so that **a single `siteType: [Site.ID_ME]`
> request returns ID.me's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ID_ME = 'idme'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-idme`.                  | must     |
| FR-3  | `IDMeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `idme-`, `site === Site.ID_ME`. | must |
...
```ts
@SourcePlugin({ site: Site.ID_ME, name: 'ID.me', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/idme/jobs/<id>`), wire
  `company_name` pass-through (`'ID.me'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/idme/jobs/<id>`).
