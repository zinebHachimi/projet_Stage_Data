# Spec: 212 — Source Company Plugin: Ahrefs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 212                            |
| Slug           | source-company-ahrefsjobs               |
| Status         | accepted                               |
...
Ahrefs publishes its careers board through Greenhouse at
the bare slug `ahrefsjobs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ahrefsjobs` plugin returning live
...
- Backfilling historical Ahrefs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AHREFSJOBS`** in the
> source registry, so that **a single `siteType: [Site.AHREFSJOBS]`
> request returns Ahrefs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AHREFSJOBS = 'ahrefsjobs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ahrefsjobs`.                  | must     |
| FR-3  | `AhrefsjobsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ahrefsjobs-`, `site === Site.AHREFSJOBS`. | must |
...
```ts
@SourcePlugin({ site: Site.AHREFSJOBS, name: 'Ahrefs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ahrefsjobs/jobs/<id>`), wire
  `company_name` pass-through (`'Ahrefs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ahrefsjobs/jobs/<id>`).
