# Spec: 900 — Source Company Plugin: OpenSpace

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 900                            |
| Slug           | source-company-openspace               |
| Status         | accepted                               |
...
OpenSpace publishes its careers board through Greenhouse at
the bare slug `openspace`. The run-398 batch sweep confirmed
...
- Ship a `source-company-openspace` plugin returning live
...
- Backfilling historical OpenSpace postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OPENSPACE`** in the
> source registry, so that **a single `siteType: [Site.OPENSPACE]`
> request returns OpenSpace's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OPENSPACE = 'openspace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-openspace`.                  | must     |
| FR-3  | `OpenSpaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `openspace-`, `site === Site.OPENSPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.OPENSPACE, name: 'OpenSpace', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/openspace/jobs/<id>`), wire
  `company_name` pass-through (`'OpenSpace'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/openspace/jobs/<id>`).
