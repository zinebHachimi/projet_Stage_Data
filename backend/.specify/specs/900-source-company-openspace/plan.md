# Plan: 900 — Source Company Plugin: OpenSpace
...
OpenSpace's careers board is hosted on Greenhouse at the slug
`openspace`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-openspace` plugin.
- Deliverables: package files, `Site.OPENSPACE` enum value, the
...
- `packages/plugins/source-company-openspace` (new)
