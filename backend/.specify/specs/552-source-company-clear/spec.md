# Spec: 552 — Source Company Plugin: CLEAR

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 552                            |
| Slug           | source-company-clear               |
| Status         | accepted                               |
...
CLEAR publishes its careers board through Greenhouse at
the bare slug `clear`. The run-398 batch sweep confirmed
...
- Ship a `source-company-clear` plugin returning live
...
- Backfilling historical CLEAR postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CLEAR`** in the
> source registry, so that **a single `siteType: [Site.CLEAR]`
> request returns CLEAR's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CLEAR = 'clear'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-clear`.                  | must     |
| FR-3  | `ClearService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `clear-`, `site === Site.CLEAR`. | must |
...
```ts
@SourcePlugin({ site: Site.CLEAR, name: 'CLEAR', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/clear/jobs/<id>`), wire
  `company_name` pass-through (`'CLEAR'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/clear/jobs/<id>`).
