# Spec: 533 — Source Company Plugin: unybrands

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 533                            |
| Slug           | source-company-unybrands               |
| Status         | accepted                               |
...
unybrands publishes its careers board through Greenhouse at
the bare slug `unybrands`. The run-398 batch sweep confirmed
...
- Ship a `source-company-unybrands` plugin returning live
...
- Backfilling historical unybrands postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UNYBRANDS`** in the
> source registry, so that **a single `siteType: [Site.UNYBRANDS]`
> request returns unybrands's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UNYBRANDS = 'unybrands'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-unybrands`.                  | must     |
| FR-3  | `UnybrandsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `unybrands-`, `site === Site.UNYBRANDS`. | must |
...
```ts
@SourcePlugin({ site: Site.UNYBRANDS, name: 'unybrands', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/unybrands/jobs/<id>`), wire
  `company_name` pass-through (`'unybrands'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/unybrands/jobs/<id>`).
