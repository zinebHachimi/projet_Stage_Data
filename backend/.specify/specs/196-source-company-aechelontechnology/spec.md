# Spec: 196 — Source Company Plugin: Aechelon Technology

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 196                            |
| Slug           | source-company-aechelontechnology               |
| Status         | accepted                               |
...
Aechelon Technology publishes its careers board through Greenhouse at
the bare slug `aechelontechnology`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aechelontechnology` plugin returning live
...
- Backfilling historical Aechelon Technology postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AECHELONTECHNOLOGY`** in the
> source registry, so that **a single `siteType: [Site.AECHELONTECHNOLOGY]`
> request returns Aechelon Technology's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AECHELONTECHNOLOGY = 'aechelontechnology'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aechelontechnology`.                  | must     |
| FR-3  | `AechelontechnologyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aechelontechnology-`, `site === Site.AECHELONTECHNOLOGY`. | must |
...
```ts
@SourcePlugin({ site: Site.AECHELONTECHNOLOGY, name: 'Aechelon Technology', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aechelontechnology/jobs/<id>`), wire
  `company_name` pass-through (`'Aechelon Technology'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aechelontechnology/jobs/<id>`).
