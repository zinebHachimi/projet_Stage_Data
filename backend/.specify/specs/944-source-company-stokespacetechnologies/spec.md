# Spec: 944 — Source Company Plugin: Stoke Space

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 944                            |
| Slug           | source-company-stokespacetechnologies               |
| Status         | accepted                               |
...
Stoke Space publishes its careers board through Greenhouse at
the bare slug `stokespacetechnologies`. The run-398 batch sweep confirmed
53 live role(s) via a direct probe of
...
- Ship a `source-company-stokespacetechnologies` plugin returning live
...
- Backfilling historical Stoke Space postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STOKE_SPACE`** in the
> source registry, so that **a single `siteType: [Site.STOKE_SPACE]`
> request returns Stoke Space's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STOKE_SPACE = 'stokespacetechnologies'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-stokespacetechnologies`.                  | must     |
| FR-3  | `StokeSpaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `stokespacetechnologies-`, `site === Site.STOKE_SPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.STOKE_SPACE, name: 'Stoke Space', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/stokespacetechnologies/jobs/<id>`), wire
  `company_name` pass-through (`'Stoke Space'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/stokespacetechnologies/jobs/<id>`).
