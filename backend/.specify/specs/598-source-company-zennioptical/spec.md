# Spec: 598 — Source Company Plugin: Zenni Optical

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 598                            |
| Slug           | source-company-zennioptical               |
| Status         | accepted                               |
...
Zenni Optical publishes its careers board through Greenhouse at
the bare slug `zennioptical`. The run-398 batch sweep confirmed
...
- Ship a `source-company-zennioptical` plugin returning live
...
- Backfilling historical Zenni Optical postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ZENNI_OPTICAL`** in the
> source registry, so that **a single `siteType: [Site.ZENNI_OPTICAL]`
> request returns Zenni Optical's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ZENNI_OPTICAL = 'zennioptical'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-zennioptical`.                  | must     |
| FR-3  | `ZenniOpticalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `zennioptical-`, `site === Site.ZENNI_OPTICAL`. | must |
...
```ts
@SourcePlugin({ site: Site.ZENNI_OPTICAL, name: 'Zenni Optical', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/zennioptical/jobs/<id>`), wire
  `company_name` pass-through (`'Zenni Optical'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/zennioptical/jobs/<id>`).
