# Spec: 826 — Source Company Plugin: CodePath

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 826                            |
| Slug           | source-company-codepath               |
| Status         | accepted                               |
...
CodePath publishes its careers board through Greenhouse at
the bare slug `codepath`. The run-398 batch sweep confirmed
...
- Ship a `source-company-codepath` plugin returning live
...
- Backfilling historical CodePath postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CODEPATH`** in the
> source registry, so that **a single `siteType: [Site.CODEPATH]`
> request returns CodePath's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CODEPATH = 'codepath'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-codepath`.                  | must     |
| FR-3  | `CodePathService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `codepath-`, `site === Site.CODEPATH`. | must |
...
```ts
@SourcePlugin({ site: Site.CODEPATH, name: 'CodePath', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/codepath/jobs/<id>`), wire
  `company_name` pass-through (`'CodePath'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/codepath/jobs/<id>`).
