# Plan: 826 — Source Company Plugin: CodePath
...
CodePath's careers board is hosted on Greenhouse at the slug
`codepath`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-codepath` plugin.
- Deliverables: package files, `Site.CODEPATH` enum value, the
...
- `packages/plugins/source-company-codepath` (new)
