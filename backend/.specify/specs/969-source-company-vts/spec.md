# Spec: 969 — Source Company Plugin: VTS

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 969                            |
| Slug           | source-company-vts               |
| Status         | accepted                               |
...
VTS publishes its careers board through Greenhouse at
the bare slug `vts`. The run-398 batch sweep confirmed
...
- Ship a `source-company-vts` plugin returning live
...
- Backfilling historical VTS postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VTS`** in the
> source registry, so that **a single `siteType: [Site.VTS]`
> request returns VTS's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VTS = 'vts'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-vts`.                  | must     |
| FR-3  | `VTSService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `vts-`, `site === Site.VTS`. | must |
...
```ts
@SourcePlugin({ site: Site.VTS, name: 'VTS', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/vts/jobs/<id>`), wire
  `company_name` pass-through (`'VTS'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/vts/jobs/<id>`).
