# Spec: 945 — Source Company Plugin: Strand Therapeutics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 945                            |
| Slug           | source-company-strandtherapeutics               |
| Status         | accepted                               |
...
Strand Therapeutics publishes its careers board through Greenhouse at
the bare slug `strandtherapeutics`. The run-398 batch sweep confirmed
5 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/strandtherapeutics/jobs?content=true`.
Sector: mRNA / synthetic biology. HQ: Boston, MA, USA.

## 2. Goals

- Ship a `source-company-strandtherapeutics` plugin returning live
...
- Backfilling historical Strand Therapeutics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STRAND_THERAPEUTICS`** in the
> source registry, so that **a single `siteType: [Site.STRAND_THERAPEUTICS]`
> request returns Strand Therapeutics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STRAND_THERAPEUTICS = 'strandtherapeutics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-strandtherapeutics`.                  | must     |
| FR-3  | `StrandTherapeuticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `strandtherapeutics-`, `site === Site.STRAND_THERAPEUTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.STRAND_THERAPEUTICS, name: 'Strand Therapeutics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/strandtherapeutics/jobs/<id>`), wire
  `company_name` pass-through (`'Strand Therapeutics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/strandtherapeutics/jobs/<id>`).
