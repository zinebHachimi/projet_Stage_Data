# Spec: 520 — Source Company Plugin: Redwood Materials

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 520                            |
| Slug           | source-company-redwoodmaterials               |
| Status         | accepted                               |
...
Redwood Materials publishes its careers board through Greenhouse at
the bare slug `redwoodmaterials`. The run-398 batch sweep confirmed
82 live role(s) via a direct probe of
...
- Ship a `source-company-redwoodmaterials` plugin returning live
...
- Backfilling historical Redwood Materials postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.REDWOOD_MATERIALS`** in the
> source registry, so that **a single `siteType: [Site.REDWOOD_MATERIALS]`
> request returns Redwood Materials's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.REDWOOD_MATERIALS = 'redwoodmaterials'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-redwoodmaterials`.                  | must     |
| FR-3  | `RedwoodmaterialsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `redwoodmaterials-`, `site === Site.REDWOOD_MATERIALS`. | must |
...
```ts
@SourcePlugin({ site: Site.REDWOOD_MATERIALS, name: 'Redwood Materials', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/redwoodmaterials/jobs/<id>`), wire
  `company_name` pass-through (`'Redwood Materials'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/redwoodmaterials/jobs/<id>`).
