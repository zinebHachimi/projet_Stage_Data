# Spec: 674 — Source Company Plugin: Grove Collaborative

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 674                            |
| Slug           | source-company-grovecollaborative               |
| Status         | accepted                               |
...
Grove Collaborative publishes its careers board through Greenhouse at
the bare slug `grovecollaborative`. The run-398 batch sweep confirmed
...
- Ship a `source-company-grovecollaborative` plugin returning live
...
- Backfilling historical Grove Collaborative postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GROVE_COLLABORATIVE`** in the
> source registry, so that **a single `siteType: [Site.GROVE_COLLABORATIVE]`
> request returns Grove Collaborative's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GROVE_COLLABORATIVE = 'grovecollaborative'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-grovecollaborative`.                  | must     |
| FR-3  | `GroveCollaborativeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `grovecollaborative-`, `site === Site.GROVE_COLLABORATIVE`. | must |
...
```ts
@SourcePlugin({ site: Site.GROVE_COLLABORATIVE, name: 'Grove Collaborative', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/grovecollaborative/jobs/<id>`), wire
  `company_name` pass-through (`'Grove Collaborative'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/grovecollaborative/jobs/<id>`).
