# Spec: 902 — Source Company Plugin: Veo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 902                            |
| Slug           | source-company-operationscareers               |
| Status         | accepted                               |
...
Veo publishes its careers board through Greenhouse at
the bare slug `operationscareers`. The run-398 batch sweep confirmed
...
- Ship a `source-company-operationscareers` plugin returning live
...
- Backfilling historical Veo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VEO`** in the
> source registry, so that **a single `siteType: [Site.VEO]`
> request returns Veo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VEO = 'operationscareers'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-operationscareers`.                  | must     |
| FR-3  | `VeoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `operationscareers-`, `site === Site.VEO`. | must |
...
```ts
@SourcePlugin({ site: Site.VEO, name: 'Veo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/operationscareers/jobs/<id>`), wire
  `company_name` pass-through (`'Veo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/operationscareers/jobs/<id>`).
