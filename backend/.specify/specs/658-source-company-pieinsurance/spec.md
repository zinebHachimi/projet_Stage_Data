# Spec: 658 — Source Company Plugin: Pie Insurance

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 658                            |
| Slug           | source-company-pieinsurance               |
| Status         | accepted                               |
...
Pie Insurance publishes its careers board through Greenhouse at
the bare slug `pieinsurance`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pieinsurance` plugin returning live
...
- Backfilling historical Pie Insurance postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PIE_INSURANCE`** in the
> source registry, so that **a single `siteType: [Site.PIE_INSURANCE]`
> request returns Pie Insurance's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PIE_INSURANCE = 'pieinsurance'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pieinsurance`.                  | must     |
| FR-3  | `PieInsuranceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pieinsurance-`, `site === Site.PIE_INSURANCE`. | must |
...
```ts
@SourcePlugin({ site: Site.PIE_INSURANCE, name: 'Pie Insurance', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pieinsurance/jobs/<id>`), wire
  `company_name` pass-through (`'Pie Insurance'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pieinsurance/jobs/<id>`).
