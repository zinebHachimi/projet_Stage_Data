# Spec: 783 — Source Company Plugin: Pivot Bio

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 783                            |
| Slug           | source-company-pivotbio               |
| Status         | accepted                               |
...
Pivot Bio publishes its careers board through Greenhouse at
the bare slug `pivotbio`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pivotbio` plugin returning live
...
- Backfilling historical Pivot Bio postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PIVOT_BIO`** in the
> source registry, so that **a single `siteType: [Site.PIVOT_BIO]`
> request returns Pivot Bio's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PIVOT_BIO = 'pivotbio'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pivotbio`.                  | must     |
| FR-3  | `PivotBioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pivotbio-`, `site === Site.PIVOT_BIO`. | must |
...
```ts
@SourcePlugin({ site: Site.PIVOT_BIO, name: 'Pivot Bio', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pivotbio/jobs/<id>`), wire
  `company_name` pass-through (`'Pivot Bio'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pivotbio/jobs/<id>`).
