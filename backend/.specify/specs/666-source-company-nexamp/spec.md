# Spec: 666 — Source Company Plugin: Nexamp

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 666                            |
| Slug           | source-company-nexamp               |
| Status         | accepted                               |
...
Nexamp publishes its careers board through Greenhouse at
the bare slug `nexamp`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nexamp` plugin returning live
...
- Backfilling historical Nexamp postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEXAMP`** in the
> source registry, so that **a single `siteType: [Site.NEXAMP]`
> request returns Nexamp's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEXAMP = 'nexamp'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nexamp`.                  | must     |
| FR-3  | `NexampService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nexamp-`, `site === Site.NEXAMP`. | must |
...
```ts
@SourcePlugin({ site: Site.NEXAMP, name: 'Nexamp', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nexamp/jobs/<id>`), wire
  `company_name` pass-through (`'Nexamp'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nexamp/jobs/<id>`).
