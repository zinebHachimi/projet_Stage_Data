# Spec: 920 — Source Company Plugin: Riskified

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 920                            |
| Slug           | source-company-riskified               |
| Status         | accepted                               |
...
Riskified publishes its careers board through Greenhouse at
the bare slug `riskified`. The run-398 batch sweep confirmed
...
- Ship a `source-company-riskified` plugin returning live
...
- Backfilling historical Riskified postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RISKIFIED`** in the
> source registry, so that **a single `siteType: [Site.RISKIFIED]`
> request returns Riskified's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RISKIFIED = 'riskified'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-riskified`.                  | must     |
| FR-3  | `RiskifiedService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `riskified-`, `site === Site.RISKIFIED`. | must |
...
```ts
@SourcePlugin({ site: Site.RISKIFIED, name: 'Riskified', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/riskified/jobs/<id>`), wire
  `company_name` pass-through (`'Riskified'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/riskified/jobs/<id>`).
