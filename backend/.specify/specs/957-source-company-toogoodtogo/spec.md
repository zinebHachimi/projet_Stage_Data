# Spec: 957 — Source Company Plugin: Too Good To Go

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 957                            |
| Slug           | source-company-toogoodtogo               |
| Status         | accepted                               |
...
Too Good To Go publishes its careers board through Greenhouse at
the bare slug `toogoodtogo`. The run-398 batch sweep confirmed
84 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/toogoodtogo/jobs?content=true`.
Sector: Foodtech / food waste. HQ: Copenhagen, Denmark.

## 2. Goals

- Ship a `source-company-toogoodtogo` plugin returning live
...
- Backfilling historical Too Good To Go postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TOO_GOOD_TO_GO`** in the
> source registry, so that **a single `siteType: [Site.TOO_GOOD_TO_GO]`
> request returns Too Good To Go's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TOO_GOOD_TO_GO = 'toogoodtogo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-toogoodtogo`.                  | must     |
| FR-3  | `TooGoodToGoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `toogoodtogo-`, `site === Site.TOO_GOOD_TO_GO`. | must |
...
```ts
@SourcePlugin({ site: Site.TOO_GOOD_TO_GO, name: 'Too Good To Go', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/toogoodtogo/jobs/<id>`), wire
  `company_name` pass-through (`'Too Good To Go'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/toogoodtogo/jobs/<id>`).
