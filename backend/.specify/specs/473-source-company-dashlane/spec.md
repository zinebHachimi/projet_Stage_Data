# Spec: 473 — Source Company Plugin: Dashlane

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 473                            |
| Slug           | source-company-dashlane               |
| Status         | accepted                               |
...
Dashlane publishes its careers board through Greenhouse at
the bare slug `dashlane`. The run-398 batch sweep confirmed
...
- Ship a `source-company-dashlane` plugin returning live
...
- Backfilling historical Dashlane postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DASHLANE`** in the
> source registry, so that **a single `siteType: [Site.DASHLANE]`
> request returns Dashlane's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DASHLANE = 'dashlane'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-dashlane`.                  | must     |
| FR-3  | `DashlaneService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `dashlane-`, `site === Site.DASHLANE`. | must |
...
```ts
@SourcePlugin({ site: Site.DASHLANE, name: 'Dashlane', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/dashlane/jobs/<id>`), wire
  `company_name` pass-through (`'Dashlane'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/dashlane/jobs/<id>`).
