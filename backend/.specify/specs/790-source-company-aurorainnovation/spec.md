# Spec: 790 — Source Company Plugin: Aurora Innovation

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 790                            |
| Slug           | source-company-aurorainnovation               |
| Status         | accepted                               |
...
Aurora Innovation publishes its careers board through Greenhouse at
the bare slug `aurorainnovation`. The run-398 batch sweep confirmed
147 live role(s) via a direct probe of
...
- Ship a `source-company-aurorainnovation` plugin returning live
...
- Backfilling historical Aurora Innovation postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AURORA_INNOVATION`** in the
> source registry, so that **a single `siteType: [Site.AURORA_INNOVATION]`
> request returns Aurora Innovation's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AURORA_INNOVATION = 'aurorainnovation'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aurorainnovation`.                  | must     |
| FR-3  | `AuroraInnovationService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aurorainnovation-`, `site === Site.AURORA_INNOVATION`. | must |
...
```ts
@SourcePlugin({ site: Site.AURORA_INNOVATION, name: 'Aurora Innovation', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aurorainnovation/jobs/<id>`), wire
  `company_name` pass-through (`'Aurora Innovation'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aurorainnovation/jobs/<id>`).
