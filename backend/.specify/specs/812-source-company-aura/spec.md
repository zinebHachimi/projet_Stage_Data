# Spec: 812 — Source Company Plugin: Aura

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 812                            |
| Slug           | source-company-aura               |
| Status         | accepted                               |
...
Aura publishes its careers board through Greenhouse at
the bare slug `aura`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aura` plugin returning live
...
- Backfilling historical Aura postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AURA`** in the
> source registry, so that **a single `siteType: [Site.AURA]`
> request returns Aura's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AURA = 'aura'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aura`.                  | must     |
| FR-3  | `AuraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aura-`, `site === Site.AURA`. | must |
...
```ts
@SourcePlugin({ site: Site.AURA, name: 'Aura', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aura/jobs/<id>`), wire
  `company_name` pass-through (`'Aura'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aura/jobs/<id>`).
