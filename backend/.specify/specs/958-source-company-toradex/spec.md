# Spec: 958 — Source Company Plugin: Toradex

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 958                            |
| Slug           | source-company-toradex               |
| Status         | accepted                               |
...
Toradex publishes its careers board through Greenhouse at
the bare slug `toradex`. The run-398 batch sweep confirmed
...
- Ship a `source-company-toradex` plugin returning live
...
- Backfilling historical Toradex postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TORADEX`** in the
> source registry, so that **a single `siteType: [Site.TORADEX]`
> request returns Toradex's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TORADEX = 'toradex'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-toradex`.                  | must     |
| FR-3  | `ToradexService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `toradex-`, `site === Site.TORADEX`. | must |
...
```ts
@SourcePlugin({ site: Site.TORADEX, name: 'Toradex', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/toradex/jobs/<id>`), wire
  `company_name` pass-through (`'Toradex'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/toradex/jobs/<id>`).
