# Spec: 725 — Source Company Plugin: Mesh Payments

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 725                            |
| Slug           | source-company-mesh               |
| Status         | accepted                               |
...
Mesh Payments publishes its careers board through Greenhouse at
the bare slug `mesh`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mesh` plugin returning live
...
- Backfilling historical Mesh Payments postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MESH_PAYMENTS`** in the
> source registry, so that **a single `siteType: [Site.MESH_PAYMENTS]`
> request returns Mesh Payments's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MESH_PAYMENTS = 'mesh'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mesh`.                  | must     |
| FR-3  | `MeshPaymentsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mesh-`, `site === Site.MESH_PAYMENTS`. | must |
...
```ts
@SourcePlugin({ site: Site.MESH_PAYMENTS, name: 'Mesh Payments', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mesh/jobs/<id>`), wire
  `company_name` pass-through (`'Mesh Payments'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mesh/jobs/<id>`).
