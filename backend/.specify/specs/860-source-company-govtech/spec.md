# Spec: 860 — Source Company Plugin: GovTech Singapore (Government Technology Agency)

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 860                            |
| Slug           | source-company-govtech               |
| Status         | accepted                               |
...
GovTech Singapore (Government Technology Agency) publishes its careers board through Greenhouse at
the bare slug `govtech`. The run-398 batch sweep confirmed
265 live role(s) via a direct probe of
...
- Ship a `source-company-govtech` plugin returning live
...
- Backfilling historical GovTech Singapore (Government Technology Agency) postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY`** in the
> source registry, so that **a single `siteType: [Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY]`
> request returns GovTech Singapore (Government Technology Agency)'s open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY = 'govtech'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-govtech`.                  | must     |
| FR-3  | `GovTechSingaporeGovernmentTechnologyAgencyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `govtech-`, `site === Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY`. | must |
...
```ts
@SourcePlugin({ site: Site.GOVTECH_SINGAPORE_GOVERNMENT_TECHNOLOGY_AGENCY, name: 'GovTech Singapore (Government Technology Agency)', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/govtech/jobs/<id>`), wire
  `company_name` pass-through (`'GovTech Singapore (Government Technology Agency)'`),
