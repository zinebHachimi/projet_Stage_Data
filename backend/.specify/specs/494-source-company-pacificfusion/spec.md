# Spec: 494 — Source Company Plugin: Pacific Fusion

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 494                            |
| Slug           | source-company-pacificfusion               |
| Status         | accepted                               |
...
Pacific Fusion publishes its careers board through Greenhouse at
the bare slug `pacificfusion`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pacificfusion` plugin returning live
...
- Backfilling historical Pacific Fusion postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PACIFICFUSION`** in the
> source registry, so that **a single `siteType: [Site.PACIFICFUSION]`
> request returns Pacific Fusion's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PACIFICFUSION = 'pacificfusion'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pacificfusion`.                  | must     |
| FR-3  | `PacificfusionService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pacificfusion-`, `site === Site.PACIFICFUSION`. | must |
...
```ts
@SourcePlugin({ site: Site.PACIFICFUSION, name: 'Pacific Fusion', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pacificfusion/jobs/<id>`), wire
  `company_name` pass-through (`'Pacific Fusion'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pacificfusion/jobs/<id>`).
