# Spec: 581 — Source Company Plugin: Parachute Home

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 581                            |
| Slug           | source-company-parachutehome               |
| Status         | accepted                               |
...
Parachute Home publishes its careers board through Greenhouse at
the bare slug `parachutehome`. The run-398 batch sweep confirmed
...
- Ship a `source-company-parachutehome` plugin returning live
...
- Backfilling historical Parachute Home postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PARACHUTE_HOME`** in the
> source registry, so that **a single `siteType: [Site.PARACHUTE_HOME]`
> request returns Parachute Home's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PARACHUTE_HOME = 'parachutehome'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-parachutehome`.                  | must     |
| FR-3  | `ParachuteHomeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `parachutehome-`, `site === Site.PARACHUTE_HOME`. | must |
...
```ts
@SourcePlugin({ site: Site.PARACHUTE_HOME, name: 'Parachute Home', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/parachutehome/jobs/<id>`), wire
  `company_name` pass-through (`'Parachute Home'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/parachutehome/jobs/<id>`).
