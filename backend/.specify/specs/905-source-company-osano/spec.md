# Spec: 905 — Source Company Plugin: Osano

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 905                            |
| Slug           | source-company-osano               |
| Status         | accepted                               |
...
Osano publishes its careers board through Greenhouse at
the bare slug `osano`. The run-398 batch sweep confirmed
...
- Ship a `source-company-osano` plugin returning live
...
- Backfilling historical Osano postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OSANO`** in the
> source registry, so that **a single `siteType: [Site.OSANO]`
> request returns Osano's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OSANO = 'osano'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-osano`.                  | must     |
| FR-3  | `OsanoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `osano-`, `site === Site.OSANO`. | must |
...
```ts
@SourcePlugin({ site: Site.OSANO, name: 'Osano', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/osano/jobs/<id>`), wire
  `company_name` pass-through (`'Osano'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/osano/jobs/<id>`).
