# Spec: 614 — Source Company Plugin: Bombas

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 614                            |
| Slug           | source-company-bombas               |
| Status         | accepted                               |
...
Bombas publishes its careers board through Greenhouse at
the bare slug `bombas`. The run-398 batch sweep confirmed
...
- Ship a `source-company-bombas` plugin returning live
...
- Backfilling historical Bombas postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BOMBAS`** in the
> source registry, so that **a single `siteType: [Site.BOMBAS]`
> request returns Bombas's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BOMBAS = 'bombas'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-bombas`.                  | must     |
| FR-3  | `BombasService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `bombas-`, `site === Site.BOMBAS`. | must |
...
```ts
@SourcePlugin({ site: Site.BOMBAS, name: 'Bombas', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/bombas/jobs/<id>`), wire
  `company_name` pass-through (`'Bombas'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/bombas/jobs/<id>`).
