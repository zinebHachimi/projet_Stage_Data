# Spec: 545 — Source Company Plugin: Slice

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 545                            |
| Slug           | source-company-slice               |
| Status         | accepted                               |
...
Slice publishes its careers board through Greenhouse at
the bare slug `slice`. The run-398 batch sweep confirmed
77 live role(s) via a direct probe of
...
- Ship a `source-company-slice` plugin returning live
...
- Backfilling historical Slice postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SLICE`** in the
> source registry, so that **a single `siteType: [Site.SLICE]`
> request returns Slice's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SLICE = 'slice'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-slice`.                  | must     |
| FR-3  | `SliceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `slice-`, `site === Site.SLICE`. | must |
...
```ts
@SourcePlugin({ site: Site.SLICE, name: 'Slice', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/slice/jobs/<id>`), wire
  `company_name` pass-through (`'Slice'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/slice/jobs/<id>`).
