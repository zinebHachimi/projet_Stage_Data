# Plan: 545 — Source Company Plugin: Slice
...
Slice's careers board is hosted on Greenhouse at the slug
`slice`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-slice` plugin.
- Deliverables: package files, `Site.SLICE` enum value, the
...
- `packages/plugins/source-company-slice` (new)
