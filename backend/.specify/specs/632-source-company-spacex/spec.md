# Spec: 632 — Source Company Plugin: SpaceX

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 632                            |
| Slug           | source-company-spacex               |
| Status         | accepted                               |
...
SpaceX publishes its careers board through Greenhouse at
the bare slug `spacex`. The run-398 batch sweep confirmed
1725 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/spacex/jobs?content=true`.
Sector: Aerospace & launch. HQ: Hawthorne, CA.

## 2. Goals

- Ship a `source-company-spacex` plugin returning live
...
- Backfilling historical SpaceX postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPACEX`** in the
> source registry, so that **a single `siteType: [Site.SPACEX]`
> request returns SpaceX's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPACEX = 'spacex'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-spacex`.                  | must     |
| FR-3  | `SpaceXService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `spacex-`, `site === Site.SPACEX`. | must |
...
```ts
@SourcePlugin({ site: Site.SPACEX, name: 'SpaceX', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/spacex/jobs/<id>`), wire
  `company_name` pass-through (`'SpaceX'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/spacex/jobs/<id>`).
