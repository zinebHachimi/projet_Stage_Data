# Spec: 916 — Source Company Plugin: Qualia

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 916                            |
| Slug           | source-company-qualia               |
| Status         | accepted                               |
...
Qualia publishes its careers board through Greenhouse at
the bare slug `qualia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-qualia` plugin returning live
...
- Backfilling historical Qualia postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.QUALIA`** in the
> source registry, so that **a single `siteType: [Site.QUALIA]`
> request returns Qualia's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.QUALIA = 'qualia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-qualia`.                  | must     |
| FR-3  | `QualiaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `qualia-`, `site === Site.QUALIA`. | must |
...
```ts
@SourcePlugin({ site: Site.QUALIA, name: 'Qualia', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/qualia/jobs/<id>`), wire
  `company_name` pass-through (`'Qualia'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/qualia/jobs/<id>`).
