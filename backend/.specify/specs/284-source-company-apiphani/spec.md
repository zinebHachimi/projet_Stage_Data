# Spec: 284 — Source Company Plugin: apiphani

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 284                            |
| Slug           | source-company-apiphani               |
| Status         | accepted                               |
...
apiphani publishes its careers board through Greenhouse at
the bare slug `apiphani`. The run-398 batch sweep confirmed
...
- Ship a `source-company-apiphani` plugin returning live
...
- Backfilling historical apiphani postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APIPHANI`** in the
> source registry, so that **a single `siteType: [Site.APIPHANI]`
> request returns apiphani's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APIPHANI = 'apiphani'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apiphani`.                  | must     |
| FR-3  | `ApiphaniService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apiphani-`, `site === Site.APIPHANI`. | must |
...
```ts
@SourcePlugin({ site: Site.APIPHANI, name: 'apiphani', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apiphani/jobs/<id>`), wire
  `company_name` pass-through (`'apiphani'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apiphani/jobs/<id>`).
