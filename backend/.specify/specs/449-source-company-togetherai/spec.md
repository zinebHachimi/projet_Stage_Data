# Spec: 449 — Source Company Plugin: Together AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 449                            |
| Slug           | source-company-togetherai               |
| Status         | accepted                               |
...
Together AI publishes its careers board through Greenhouse at
the bare slug `togetherai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-togetherai` plugin returning live
...
- Backfilling historical Together AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TOGETHERAI`** in the
> source registry, so that **a single `siteType: [Site.TOGETHERAI]`
> request returns Together AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TOGETHERAI = 'togetherai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-togetherai`.                  | must     |
| FR-3  | `TogetheraiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `togetherai-`, `site === Site.TOGETHERAI`. | must |
...
```ts
@SourcePlugin({ site: Site.TOGETHERAI, name: 'Together AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/togetherai/jobs/<id>`), wire
  `company_name` pass-through (`'Together AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/togetherai/jobs/<id>`).
