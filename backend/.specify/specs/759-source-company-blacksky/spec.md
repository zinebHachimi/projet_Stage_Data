# Spec: 759 — Source Company Plugin: BlackSky Technology

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 759                            |
| Slug           | source-company-blacksky               |
| Status         | accepted                               |
...
BlackSky Technology publishes its careers board through Greenhouse at
the bare slug `blacksky`. The run-398 batch sweep confirmed
...
- Ship a `source-company-blacksky` plugin returning live
...
- Backfilling historical BlackSky Technology postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BLACKSKY_TECHNOLOGY`** in the
> source registry, so that **a single `siteType: [Site.BLACKSKY_TECHNOLOGY]`
> request returns BlackSky Technology's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BLACKSKY_TECHNOLOGY = 'blacksky'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-blacksky`.                  | must     |
| FR-3  | `BlackSkyTechnologyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `blacksky-`, `site === Site.BLACKSKY_TECHNOLOGY`. | must |
...
```ts
@SourcePlugin({ site: Site.BLACKSKY_TECHNOLOGY, name: 'BlackSky Technology', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/blacksky/jobs/<id>`), wire
  `company_name` pass-through (`'BlackSky Technology'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/blacksky/jobs/<id>`).
