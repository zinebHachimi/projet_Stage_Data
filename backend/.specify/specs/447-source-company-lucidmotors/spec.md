# Spec: 447 — Source Company Plugin: Lucid Motors

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 447                            |
| Slug           | source-company-lucidmotors               |
| Status         | accepted                               |
...
Lucid Motors publishes its careers board through Greenhouse at
the bare slug `lucidmotors`. The run-398 batch sweep confirmed
165 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/lucidmotors/jobs?content=true`.
Sector: Luxury electric vehicles. HQ: Newark, CA, USA.

## 2. Goals

- Ship a `source-company-lucidmotors` plugin returning live
...
- Backfilling historical Lucid Motors postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LUCIDMOTORS`** in the
> source registry, so that **a single `siteType: [Site.LUCIDMOTORS]`
> request returns Lucid Motors's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LUCIDMOTORS = 'lucidmotors'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-lucidmotors`.                  | must     |
| FR-3  | `LucidmotorsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `lucidmotors-`, `site === Site.LUCIDMOTORS`. | must |
...
```ts
@SourcePlugin({ site: Site.LUCIDMOTORS, name: 'Lucid Motors', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/lucidmotors/jobs/<id>`), wire
  `company_name` pass-through (`'Lucid Motors'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/lucidmotors/jobs/<id>`).
