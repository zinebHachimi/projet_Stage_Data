# Spec: 560 — Source Company Plugin: Modern Animal

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 560                            |
| Slug           | source-company-modernanimal               |
| Status         | accepted                               |
...
Modern Animal publishes its careers board through Greenhouse at
the bare slug `modernanimal`. The run-398 batch sweep confirmed
324 live role(s) via a direct probe of
...
- Ship a `source-company-modernanimal` plugin returning live
...
- Backfilling historical Modern Animal postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MODERN_ANIMAL`** in the
> source registry, so that **a single `siteType: [Site.MODERN_ANIMAL]`
> request returns Modern Animal's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MODERN_ANIMAL = 'modernanimal'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-modernanimal`.                  | must     |
| FR-3  | `ModernanimalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `modernanimal-`, `site === Site.MODERN_ANIMAL`. | must |
...
```ts
@SourcePlugin({ site: Site.MODERN_ANIMAL, name: 'Modern Animal', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/modernanimal/jobs/<id>`), wire
  `company_name` pass-through (`'Modern Animal'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/modernanimal/jobs/<id>`).
