# Spec: 208 — Source Company Plugin: Agilisys

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 208                            |
| Slug           | source-company-agilisys               |
| Status         | accepted                               |
...
Agilisys publishes its careers board through Greenhouse at
the bare slug `agilisys`. The run-398 batch sweep confirmed
...
- Ship a `source-company-agilisys` plugin returning live
...
- Backfilling historical Agilisys postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AGILISYS`** in the
> source registry, so that **a single `siteType: [Site.AGILISYS]`
> request returns Agilisys's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AGILISYS = 'agilisys'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-agilisys`.                  | must     |
| FR-3  | `AgilisysService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `agilisys-`, `site === Site.AGILISYS`. | must |
...
```ts
@SourcePlugin({ site: Site.AGILISYS, name: 'Agilisys', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/agilisys/jobs/<id>`), wire
  `company_name` pass-through (`'Agilisys'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/agilisys/jobs/<id>`).
