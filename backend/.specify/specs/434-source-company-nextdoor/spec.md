# Spec: 434 — Source Company Plugin: Nextdoor

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 434                            |
| Slug           | source-company-nextdoor               |
| Status         | accepted                               |
...
Nextdoor publishes its careers board through Greenhouse at
the bare slug `nextdoor`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nextdoor` plugin returning live
...
- Backfilling historical Nextdoor postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEXTDOOR`** in the
> source registry, so that **a single `siteType: [Site.NEXTDOOR]`
> request returns Nextdoor's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEXTDOOR = 'nextdoor'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nextdoor`.                  | must     |
| FR-3  | `NextdoorService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nextdoor-`, `site === Site.NEXTDOOR`. | must |
...
```ts
@SourcePlugin({ site: Site.NEXTDOOR, name: 'Nextdoor', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nextdoor/jobs/<id>`), wire
  `company_name` pass-through (`'Nextdoor'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nextdoor/jobs/<id>`).
