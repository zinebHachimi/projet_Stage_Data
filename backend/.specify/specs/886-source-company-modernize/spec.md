# Spec: 886 — Source Company Plugin: Modernize Home Services

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 886                            |
| Slug           | source-company-modernize               |
| Status         | accepted                               |
...
Modernize Home Services publishes its careers board through Greenhouse at
the bare slug `modernize`. The run-398 batch sweep confirmed
...
- Ship a `source-company-modernize` plugin returning live
...
- Backfilling historical Modernize Home Services postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MODERNIZE_HOME_SERVICES`** in the
> source registry, so that **a single `siteType: [Site.MODERNIZE_HOME_SERVICES]`
> request returns Modernize Home Services's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MODERNIZE_HOME_SERVICES = 'modernize'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-modernize`.                  | must     |
| FR-3  | `ModernizeHomeServicesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `modernize-`, `site === Site.MODERNIZE_HOME_SERVICES`. | must |
...
```ts
@SourcePlugin({ site: Site.MODERNIZE_HOME_SERVICES, name: 'Modernize Home Services', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/modernize/jobs/<id>`), wire
  `company_name` pass-through (`'Modernize Home Services'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/modernize/jobs/<id>`).
