# Spec: 280 — Source Company Plugin: Aperia

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 280                            |
| Slug           | source-company-aperiasolutions               |
| Status         | accepted                               |
...
Aperia publishes its careers board through Greenhouse at
the bare slug `aperiasolutions`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aperiasolutions` plugin returning live
...
- Backfilling historical Aperia postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APERIASOLUTIONS`** in the
> source registry, so that **a single `siteType: [Site.APERIASOLUTIONS]`
> request returns Aperia's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APERIASOLUTIONS = 'aperiasolutions'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aperiasolutions`.                  | must     |
| FR-3  | `AperiasolutionsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aperiasolutions-`, `site === Site.APERIASOLUTIONS`. | must |
...
```ts
@SourcePlugin({ site: Site.APERIASOLUTIONS, name: 'Aperia', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aperiasolutions/jobs/<id>`), wire
  `company_name` pass-through (`'Aperia'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aperiasolutions/jobs/<id>`).
