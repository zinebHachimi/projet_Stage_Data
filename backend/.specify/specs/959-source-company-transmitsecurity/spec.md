# Spec: 959 — Source Company Plugin: Transmit Security

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 959                            |
| Slug           | source-company-transmitsecurity               |
| Status         | accepted                               |
...
Transmit Security publishes its careers board through Greenhouse at
the bare slug `transmitsecurity`. The run-398 batch sweep confirmed
...
- Ship a `source-company-transmitsecurity` plugin returning live
...
- Backfilling historical Transmit Security postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TRANSMIT_SECURITY`** in the
> source registry, so that **a single `siteType: [Site.TRANSMIT_SECURITY]`
> request returns Transmit Security's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TRANSMIT_SECURITY = 'transmitsecurity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-transmitsecurity`.                  | must     |
| FR-3  | `TransmitSecurityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `transmitsecurity-`, `site === Site.TRANSMIT_SECURITY`. | must |
...
```ts
@SourcePlugin({ site: Site.TRANSMIT_SECURITY, name: 'Transmit Security', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/transmitsecurity/jobs/<id>`), wire
  `company_name` pass-through (`'Transmit Security'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/transmitsecurity/jobs/<id>`).
