# Spec: 842 — Source Company Plugin: Energy Solutions

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 842                            |
| Slug           | source-company-energysolutions               |
| Status         | accepted                               |
...
Energy Solutions publishes its careers board through Greenhouse at
the bare slug `energysolutions`. The run-398 batch sweep confirmed
33 live role(s) via a direct probe of
...
- Ship a `source-company-energysolutions` plugin returning live
...
- Backfilling historical Energy Solutions postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ENERGY_SOLUTIONS`** in the
> source registry, so that **a single `siteType: [Site.ENERGY_SOLUTIONS]`
> request returns Energy Solutions's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ENERGY_SOLUTIONS = 'energysolutions'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-energysolutions`.                  | must     |
| FR-3  | `EnergySolutionsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `energysolutions-`, `site === Site.ENERGY_SOLUTIONS`. | must |
...
```ts
@SourcePlugin({ site: Site.ENERGY_SOLUTIONS, name: 'Energy Solutions', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/energysolutions/jobs/<id>`), wire
  `company_name` pass-through (`'Energy Solutions'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/energysolutions/jobs/<id>`).
