# Spec: 585 — Source Company Plugin: Spire Global

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 585                            |
| Slug           | source-company-spire               |
| Status         | accepted                               |
...
Spire Global publishes its careers board through Greenhouse at
the bare slug `spire`. The run-398 batch sweep confirmed
...
- Ship a `source-company-spire` plugin returning live
...
- Backfilling historical Spire Global postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPIRE_GLOBAL`** in the
> source registry, so that **a single `siteType: [Site.SPIRE_GLOBAL]`
> request returns Spire Global's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPIRE_GLOBAL = 'spire'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-spire`.                  | must     |
| FR-3  | `SpireGlobalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `spire-`, `site === Site.SPIRE_GLOBAL`. | must |
...
```ts
@SourcePlugin({ site: Site.SPIRE_GLOBAL, name: 'Spire Global', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/spire/jobs/<id>`), wire
  `company_name` pass-through (`'Spire Global'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/spire/jobs/<id>`).
