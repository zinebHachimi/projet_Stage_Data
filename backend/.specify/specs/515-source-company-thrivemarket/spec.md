# Spec: 515 — Source Company Plugin: Thrive Market

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 515                            |
| Slug           | source-company-thrivemarket               |
| Status         | accepted                               |
...
Thrive Market publishes its careers board through Greenhouse at
the bare slug `thrivemarket`. The run-398 batch sweep confirmed
...
- Ship a `source-company-thrivemarket` plugin returning live
...
- Backfilling historical Thrive Market postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THRIVE_MARKET`** in the
> source registry, so that **a single `siteType: [Site.THRIVE_MARKET]`
> request returns Thrive Market's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THRIVE_MARKET = 'thrivemarket'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thrivemarket`.                  | must     |
| FR-3  | `ThrivemarketService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thrivemarket-`, `site === Site.THRIVE_MARKET`. | must |
...
```ts
@SourcePlugin({ site: Site.THRIVE_MARKET, name: 'Thrive Market', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thrivemarket/jobs/<id>`), wire
  `company_name` pass-through (`'Thrive Market'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thrivemarket/jobs/<id>`).
