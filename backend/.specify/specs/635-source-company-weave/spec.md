# Spec: 635 — Source Company Plugin: Weave

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 635                            |
| Slug           | source-company-weave               |
| Status         | accepted                               |
...
Weave publishes its careers board through Greenhouse at
the bare slug `weave`. The run-398 batch sweep confirmed
...
- Ship a `source-company-weave` plugin returning live
...
- Backfilling historical Weave postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WEAVE`** in the
> source registry, so that **a single `siteType: [Site.WEAVE]`
> request returns Weave's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WEAVE = 'weave'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-weave`.                  | must     |
| FR-3  | `WeaveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `weave-`, `site === Site.WEAVE`. | must |
...
```ts
@SourcePlugin({ site: Site.WEAVE, name: 'Weave', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/weave/jobs/<id>`), wire
  `company_name` pass-through (`'Weave'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/weave/jobs/<id>`).
