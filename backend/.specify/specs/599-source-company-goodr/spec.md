# Spec: 599 — Source Company Plugin: goodr

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 599                            |
| Slug           | source-company-goodr               |
| Status         | accepted                               |
...
goodr publishes its careers board through Greenhouse at
the bare slug `goodr`. The run-398 batch sweep confirmed
...
- Ship a `source-company-goodr` plugin returning live
...
- Backfilling historical goodr postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GOODR`** in the
> source registry, so that **a single `siteType: [Site.GOODR]`
> request returns goodr's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GOODR = 'goodr'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-goodr`.                  | must     |
| FR-3  | `GoodrService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `goodr-`, `site === Site.GOODR`. | must |
...
```ts
@SourcePlugin({ site: Site.GOODR, name: 'goodr', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/goodr/jobs/<id>`), wire
  `company_name` pass-through (`'goodr'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/goodr/jobs/<id>`).
