# Spec: 919 — Source Company Plugin: Renaissance Learning

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 919                            |
| Slug           | source-company-renaissancelearning-nam               |
| Status         | accepted                               |
...
Renaissance Learning publishes its careers board through Greenhouse at
the bare slug `renaissancelearning-nam`. The run-398 batch sweep confirmed
14 live role(s) via a direct probe of
...
- Ship a `source-company-renaissancelearning-nam` plugin returning live
...
- Backfilling historical Renaissance Learning postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RENAISSANCE_LEARNING`** in the
> source registry, so that **a single `siteType: [Site.RENAISSANCE_LEARNING]`
> request returns Renaissance Learning's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RENAISSANCE_LEARNING = 'renaissancelearning-nam'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-renaissancelearning-nam`.                  | must     |
| FR-3  | `RenaissanceLearningService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `renaissancelearning-nam-`, `site === Site.RENAISSANCE_LEARNING`. | must |
...
```ts
@SourcePlugin({ site: Site.RENAISSANCE_LEARNING, name: 'Renaissance Learning', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/renaissancelearning-nam/jobs/<id>`), wire
  `company_name` pass-through (`'Renaissance Learning'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/renaissancelearning-nam/jobs/<id>`).
