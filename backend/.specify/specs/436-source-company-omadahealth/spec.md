# Spec: 436 — Source Company Plugin: Omada Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 436                            |
| Slug           | source-company-omadahealth               |
| Status         | accepted                               |
...
Omada Health publishes its careers board through Greenhouse at
the bare slug `omadahealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-omadahealth` plugin returning live
...
- Backfilling historical Omada Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OMADAHEALTH`** in the
> source registry, so that **a single `siteType: [Site.OMADAHEALTH]`
> request returns Omada Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OMADAHEALTH = 'omadahealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-omadahealth`.                  | must     |
| FR-3  | `OmadahealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `omadahealth-`, `site === Site.OMADAHEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.OMADAHEALTH, name: 'Omada Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/omadahealth/jobs/<id>`), wire
  `company_name` pass-through (`'Omada Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/omadahealth/jobs/<id>`).
