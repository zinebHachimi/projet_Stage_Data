# Spec: 763 — Source Company Plugin: HawkEye 360

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 763                            |
| Slug           | source-company-hawkeye360               |
| Status         | accepted                               |
...
HawkEye 360 publishes its careers board through Greenhouse at
the bare slug `hawkeye360`. The run-398 batch sweep confirmed
...
- Ship a `source-company-hawkeye360` plugin returning live
...
- Backfilling historical HawkEye 360 postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HAWKEYE_360`** in the
> source registry, so that **a single `siteType: [Site.HAWKEYE_360]`
> request returns HawkEye 360's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HAWKEYE_360 = 'hawkeye360'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-hawkeye360`.                  | must     |
| FR-3  | `HawkEye360Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `hawkeye360-`, `site === Site.HAWKEYE_360`. | must |
...
```ts
@SourcePlugin({ site: Site.HAWKEYE_360, name: 'HawkEye 360', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/hawkeye360/jobs/<id>`), wire
  `company_name` pass-through (`'HawkEye 360'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/hawkeye360/jobs/<id>`).
