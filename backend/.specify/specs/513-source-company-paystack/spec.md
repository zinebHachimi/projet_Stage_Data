# Spec: 513 — Source Company Plugin: Paystack

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 513                            |
| Slug           | source-company-paystack               |
| Status         | accepted                               |
...
Paystack publishes its careers board through Greenhouse at
the bare slug `paystack`. The run-398 batch sweep confirmed
...
- Ship a `source-company-paystack` plugin returning live
...
- Backfilling historical Paystack postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PAYSTACK`** in the
> source registry, so that **a single `siteType: [Site.PAYSTACK]`
> request returns Paystack's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PAYSTACK = 'paystack'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-paystack`.                  | must     |
| FR-3  | `PaystackService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `paystack-`, `site === Site.PAYSTACK`. | must |
...
```ts
@SourcePlugin({ site: Site.PAYSTACK, name: 'Paystack', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/paystack/jobs/<id>`), wire
  `company_name` pass-through (`'Paystack'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/paystack/jobs/<id>`).
