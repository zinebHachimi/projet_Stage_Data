# Spec: 925 — Source Company Plugin: Sayari

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 925                            |
| Slug           | source-company-sayari               |
| Status         | accepted                               |
...
Sayari publishes its careers board through Greenhouse at
the bare slug `sayari`. The run-398 batch sweep confirmed
...
- Ship a `source-company-sayari` plugin returning live
...
- Backfilling historical Sayari postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SAYARI`** in the
> source registry, so that **a single `siteType: [Site.SAYARI]`
> request returns Sayari's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SAYARI = 'sayari'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sayari`.                  | must     |
| FR-3  | `SayariService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sayari-`, `site === Site.SAYARI`. | must |
...
```ts
@SourcePlugin({ site: Site.SAYARI, name: 'Sayari', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sayari/jobs/<id>`), wire
  `company_name` pass-through (`'Sayari'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sayari/jobs/<id>`).
