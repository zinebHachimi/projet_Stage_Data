# Spec: 672 — Source Company Plugin: Sidecar Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 672                            |
| Slug           | source-company-sidecarhealth               |
| Status         | accepted                               |
...
Sidecar Health publishes its careers board through Greenhouse at
the bare slug `sidecarhealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-sidecarhealth` plugin returning live
...
- Backfilling historical Sidecar Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SIDECAR_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.SIDECAR_HEALTH]`
> request returns Sidecar Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SIDECAR_HEALTH = 'sidecarhealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sidecarhealth`.                  | must     |
| FR-3  | `SidecarHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sidecarhealth-`, `site === Site.SIDECAR_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.SIDECAR_HEALTH, name: 'Sidecar Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sidecarhealth/jobs/<id>`), wire
  `company_name` pass-through (`'Sidecar Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sidecarhealth/jobs/<id>`).
