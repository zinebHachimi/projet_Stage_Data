# Spec: 500 — Source Company Plugin: Transcarent

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 500                            |
| Slug           | source-company-transcarent               |
| Status         | accepted                               |
...
Transcarent publishes its careers board through Greenhouse at
the bare slug `transcarent`. The run-398 batch sweep confirmed
...
- Ship a `source-company-transcarent` plugin returning live
...
- Backfilling historical Transcarent postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TRANSCARENT`** in the
> source registry, so that **a single `siteType: [Site.TRANSCARENT]`
> request returns Transcarent's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TRANSCARENT = 'transcarent'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-transcarent`.                  | must     |
| FR-3  | `TranscarentService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `transcarent-`, `site === Site.TRANSCARENT`. | must |
...
```ts
@SourcePlugin({ site: Site.TRANSCARENT, name: 'Transcarent', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/transcarent/jobs/<id>`), wire
  `company_name` pass-through (`'Transcarent'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/transcarent/jobs/<id>`).
