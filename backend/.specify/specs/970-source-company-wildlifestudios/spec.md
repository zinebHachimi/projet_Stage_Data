# Spec: 970 — Source Company Plugin: Wildlife Studios

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 970                            |
| Slug           | source-company-wildlifestudios               |
| Status         | accepted                               |
...
Wildlife Studios publishes its careers board through Greenhouse at
the bare slug `wildlifestudios`. The run-398 batch sweep confirmed
...
- Ship a `source-company-wildlifestudios` plugin returning live
...
- Backfilling historical Wildlife Studios postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WILDLIFE_STUDIOS`** in the
> source registry, so that **a single `siteType: [Site.WILDLIFE_STUDIOS]`
> request returns Wildlife Studios's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WILDLIFE_STUDIOS = 'wildlifestudios'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-wildlifestudios`.                  | must     |
| FR-3  | `WildlifeStudiosService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `wildlifestudios-`, `site === Site.WILDLIFE_STUDIOS`. | must |
...
```ts
@SourcePlugin({ site: Site.WILDLIFE_STUDIOS, name: 'Wildlife Studios', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/wildlifestudios/jobs/<id>`), wire
  `company_name` pass-through (`'Wildlife Studios'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/wildlifestudios/jobs/<id>`).
