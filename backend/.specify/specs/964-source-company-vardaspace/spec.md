# Spec: 964 — Source Company Plugin: Varda Space Industries

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 964                            |
| Slug           | source-company-vardaspace               |
| Status         | accepted                               |
...
Varda Space Industries publishes its careers board through Greenhouse at
the bare slug `vardaspace`. The run-398 batch sweep confirmed
89 live role(s) via a direct probe of
...
- Ship a `source-company-vardaspace` plugin returning live
...
- Backfilling historical Varda Space Industries postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VARDA_SPACE_INDUSTRIES`** in the
> source registry, so that **a single `siteType: [Site.VARDA_SPACE_INDUSTRIES]`
> request returns Varda Space Industries's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VARDA_SPACE_INDUSTRIES = 'vardaspace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-vardaspace`.                  | must     |
| FR-3  | `VardaSpaceIndustriesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `vardaspace-`, `site === Site.VARDA_SPACE_INDUSTRIES`. | must |
...
```ts
@SourcePlugin({ site: Site.VARDA_SPACE_INDUSTRIES, name: 'Varda Space Industries', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/vardaspace/jobs/<id>`), wire
  `company_name` pass-through (`'Varda Space Industries'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/vardaspace/jobs/<id>`).
