# Spec: 262 — Source Company Plugin: American Institutes for Research

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 262                            |
| Slug           | source-company-americaninstitutesforresearch               |
| Status         | accepted                               |
...
American Institutes for Research publishes its careers board through Greenhouse at
the bare slug `americaninstitutesforresearch`. The run-398 batch sweep confirmed
...
- Ship a `source-company-americaninstitutesforresearch` plugin returning live
...
- Backfilling historical American Institutes for Research postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMERICANINSTITUTESFORRESEARCH`** in the
> source registry, so that **a single `siteType: [Site.AMERICANINSTITUTESFORRESEARCH]`
> request returns American Institutes for Research's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMERICANINSTITUTESFORRESEARCH = 'americaninstitutesforresearch'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-americaninstitutesforresearch`.                  | must     |
| FR-3  | `AmericaninstitutesforresearchService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `americaninstitutesforresearch-`, `site === Site.AMERICANINSTITUTESFORRESEARCH`. | must |
...
```ts
@SourcePlugin({ site: Site.AMERICANINSTITUTESFORRESEARCH, name: 'American Institutes for Research', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/americaninstitutesforresearch/jobs/<id>`), wire
  `company_name` pass-through (`'American Institutes for Research'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/americaninstitutesforresearch/jobs/<id>`).
