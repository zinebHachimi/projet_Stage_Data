# Spec: 526 — Source Company Plugin: Talkspace

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 526                            |
| Slug           | source-company-talkspace               |
| Status         | accepted                               |
...
Talkspace publishes its careers board through Greenhouse at
the bare slug `talkspace`. The run-398 batch sweep confirmed
...
- Ship a `source-company-talkspace` plugin returning live
...
- Backfilling historical Talkspace postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TALKSPACE`** in the
> source registry, so that **a single `siteType: [Site.TALKSPACE]`
> request returns Talkspace's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TALKSPACE = 'talkspace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-talkspace`.                  | must     |
| FR-3  | `TalkspaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `talkspace-`, `site === Site.TALKSPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.TALKSPACE, name: 'Talkspace', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/talkspace/jobs/<id>`), wire
  `company_name` pass-through (`'Talkspace'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/talkspace/jobs/<id>`).
