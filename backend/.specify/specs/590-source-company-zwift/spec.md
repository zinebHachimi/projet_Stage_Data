# Spec: 590 — Source Company Plugin: Zwift

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 590                            |
| Slug           | source-company-zwift               |
| Status         | accepted                               |
...
Zwift publishes its careers board through Greenhouse at
the bare slug `zwift`. The run-398 batch sweep confirmed
...
- Ship a `source-company-zwift` plugin returning live
...
- Backfilling historical Zwift postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ZWIFT`** in the
> source registry, so that **a single `siteType: [Site.ZWIFT]`
> request returns Zwift's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ZWIFT = 'zwift'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-zwift`.                  | must     |
| FR-3  | `ZwiftService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `zwift-`, `site === Site.ZWIFT`. | must |
...
```ts
@SourcePlugin({ site: Site.ZWIFT, name: 'Zwift', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/zwift/jobs/<id>`), wire
  `company_name` pass-through (`'Zwift'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/zwift/jobs/<id>`).
