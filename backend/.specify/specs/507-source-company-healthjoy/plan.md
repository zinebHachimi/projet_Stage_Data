# Plan: 507 — Source Company Plugin: HealthJoy
...
HealthJoy's careers board is hosted on Greenhouse at the slug
`healthjoy`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-healthjoy` plugin.
- Deliverables: package files, `Site.HEALTHJOY` enum value, the
...
- `packages/plugins/source-company-healthjoy` (new)
