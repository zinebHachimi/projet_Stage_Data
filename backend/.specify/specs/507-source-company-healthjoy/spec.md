# Spec: 507 — Source Company Plugin: HealthJoy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 507                            |
| Slug           | source-company-healthjoy               |
| Status         | accepted                               |
...
HealthJoy publishes its careers board through Greenhouse at
the bare slug `healthjoy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-healthjoy` plugin returning live
...
- Backfilling historical HealthJoy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HEALTHJOY`** in the
> source registry, so that **a single `siteType: [Site.HEALTHJOY]`
> request returns HealthJoy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HEALTHJOY = 'healthjoy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-healthjoy`.                  | must     |
| FR-3  | `HealthjoyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `healthjoy-`, `site === Site.HEALTHJOY`. | must |
...
```ts
@SourcePlugin({ site: Site.HEALTHJOY, name: 'HealthJoy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/healthjoy/jobs/<id>`), wire
  `company_name` pass-through (`'HealthJoy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/healthjoy/jobs/<id>`).
