# Spec: 480 — Source Company Plugin: Inflection AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 480                            |
| Slug           | source-company-inflectionai               |
| Status         | accepted                               |
...
Inflection AI publishes its careers board through Greenhouse at
the bare slug `inflectionai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-inflectionai` plugin returning live
...
- Backfilling historical Inflection AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INFLECTIONAI`** in the
> source registry, so that **a single `siteType: [Site.INFLECTIONAI]`
> request returns Inflection AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INFLECTIONAI = 'inflectionai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-inflectionai`.                  | must     |
| FR-3  | `InflectionaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `inflectionai-`, `site === Site.INFLECTIONAI`. | must |
...
```ts
@SourcePlugin({ site: Site.INFLECTIONAI, name: 'Inflection AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/inflectionai/jobs/<id>`), wire
  `company_name` pass-through (`'Inflection AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/inflectionai/jobs/<id>`).
