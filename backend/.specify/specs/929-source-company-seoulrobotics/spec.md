# Spec: 929 — Source Company Plugin: Seoul Robotics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 929                            |
| Slug           | source-company-seoulrobotics               |
| Status         | accepted                               |
...
Seoul Robotics publishes its careers board through Greenhouse at
the bare slug `seoulrobotics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-seoulrobotics` plugin returning live
...
- Backfilling historical Seoul Robotics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SEOUL_ROBOTICS`** in the
> source registry, so that **a single `siteType: [Site.SEOUL_ROBOTICS]`
> request returns Seoul Robotics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SEOUL_ROBOTICS = 'seoulrobotics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-seoulrobotics`.                  | must     |
| FR-3  | `SeoulRoboticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `seoulrobotics-`, `site === Site.SEOUL_ROBOTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.SEOUL_ROBOTICS, name: 'Seoul Robotics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/seoulrobotics/jobs/<id>`), wire
  `company_name` pass-through (`'Seoul Robotics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/seoulrobotics/jobs/<id>`).
