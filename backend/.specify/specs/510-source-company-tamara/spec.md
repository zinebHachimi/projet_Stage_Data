# Spec: 510 — Source Company Plugin: Tamara

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 510                            |
| Slug           | source-company-tamara               |
| Status         | accepted                               |
...
Tamara publishes its careers board through Greenhouse at
the bare slug `tamara`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tamara` plugin returning live
...
- Backfilling historical Tamara postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TAMARA`** in the
> source registry, so that **a single `siteType: [Site.TAMARA]`
> request returns Tamara's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TAMARA = 'tamara'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tamara`.                  | must     |
| FR-3  | `TamaraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tamara-`, `site === Site.TAMARA`. | must |
...
```ts
@SourcePlugin({ site: Site.TAMARA, name: 'Tamara', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tamara/jobs/<id>`), wire
  `company_name` pass-through (`'Tamara'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tamara/jobs/<id>`).
