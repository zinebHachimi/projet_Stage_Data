# Spec: 438 — Source Company Plugin: ClickHouse

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 438                            |
| Slug           | source-company-clickhouse               |
| Status         | accepted                               |
...
ClickHouse publishes its careers board through Greenhouse at
the bare slug `clickhouse`. The run-398 batch sweep confirmed
167 live role(s) via a direct probe of
...
- Ship a `source-company-clickhouse` plugin returning live
...
- Backfilling historical ClickHouse postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CLICKHOUSE`** in the
> source registry, so that **a single `siteType: [Site.CLICKHOUSE]`
> request returns ClickHouse's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CLICKHOUSE = 'clickhouse'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-clickhouse`.                  | must     |
| FR-3  | `ClickhouseService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `clickhouse-`, `site === Site.CLICKHOUSE`. | must |
...
```ts
@SourcePlugin({ site: Site.CLICKHOUSE, name: 'ClickHouse', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/clickhouse/jobs/<id>`), wire
  `company_name` pass-through (`'ClickHouse'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/clickhouse/jobs/<id>`).
