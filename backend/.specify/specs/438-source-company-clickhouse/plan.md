# Plan: 438 — Source Company Plugin: ClickHouse
...
ClickHouse's careers board is hosted on Greenhouse at the slug
`clickhouse`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-clickhouse` plugin.
- Deliverables: package files, `Site.CLICKHOUSE` enum value, the
...
- `packages/plugins/source-company-clickhouse` (new)
