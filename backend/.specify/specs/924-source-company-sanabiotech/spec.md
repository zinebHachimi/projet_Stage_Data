# Spec: 924 — Source Company Plugin: Sana Biotechnology

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 924                            |
| Slug           | source-company-sanabiotech               |
| Status         | accepted                               |
...
Sana Biotechnology publishes its careers board through Greenhouse at
the bare slug `sanabiotech`. The run-398 batch sweep confirmed
...
- Ship a `source-company-sanabiotech` plugin returning live
...
- Backfilling historical Sana Biotechnology postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SANA_BIOTECHNOLOGY`** in the
> source registry, so that **a single `siteType: [Site.SANA_BIOTECHNOLOGY]`
> request returns Sana Biotechnology's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SANA_BIOTECHNOLOGY = 'sanabiotech'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sanabiotech`.                  | must     |
| FR-3  | `SanaBiotechnologyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sanabiotech-`, `site === Site.SANA_BIOTECHNOLOGY`. | must |
...
```ts
@SourcePlugin({ site: Site.SANA_BIOTECHNOLOGY, name: 'Sana Biotechnology', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sanabiotech/jobs/<id>`), wire
  `company_name` pass-through (`'Sana Biotechnology'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sanabiotech/jobs/<id>`).
