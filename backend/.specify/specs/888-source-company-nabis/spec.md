# Spec: 888 — Source Company Plugin: Nabis

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 888                            |
| Slug           | source-company-nabis               |
| Status         | accepted                               |
...
Nabis publishes its careers board through Greenhouse at
the bare slug `nabis`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nabis` plugin returning live
...
- Backfilling historical Nabis postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NABIS`** in the
> source registry, so that **a single `siteType: [Site.NABIS]`
> request returns Nabis's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NABIS = 'nabis'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nabis`.                  | must     |
| FR-3  | `NabisService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nabis-`, `site === Site.NABIS`. | must |
...
```ts
@SourcePlugin({ site: Site.NABIS, name: 'Nabis', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nabis/jobs/<id>`), wire
  `company_name` pass-through (`'Nabis'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nabis/jobs/<id>`).
