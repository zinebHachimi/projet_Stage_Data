# Spec: 252 — Source Company Plugin: Alumni Ventures

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 252                            |
| Slug           | source-company-alumniventures               |
| Status         | accepted                               |
...
Alumni Ventures publishes its careers board through Greenhouse at
the bare slug `alumniventures`. The run-398 batch sweep confirmed
...
- Ship a `source-company-alumniventures` plugin returning live
...
- Backfilling historical Alumni Ventures postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALUMNIVENTURES`** in the
> source registry, so that **a single `siteType: [Site.ALUMNIVENTURES]`
> request returns Alumni Ventures's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALUMNIVENTURES = 'alumniventures'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alumniventures`.                  | must     |
| FR-3  | `AlumniventuresService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alumniventures-`, `site === Site.ALUMNIVENTURES`. | must |
...
```ts
@SourcePlugin({ site: Site.ALUMNIVENTURES, name: 'Alumni Ventures', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alumniventures/jobs/<id>`), wire
  `company_name` pass-through (`'Alumni Ventures'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alumniventures/jobs/<id>`).
