# Spec: 641 — Source Company Plugin: Fingerprint

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 641                            |
| Slug           | source-company-fingerprint               |
| Status         | accepted                               |
...
Fingerprint publishes its careers board through Greenhouse at
the bare slug `fingerprint`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fingerprint` plugin returning live
...
- Backfilling historical Fingerprint postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FINGERPRINT`** in the
> source registry, so that **a single `siteType: [Site.FINGERPRINT]`
> request returns Fingerprint's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FINGERPRINT = 'fingerprint'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fingerprint`.                  | must     |
| FR-3  | `FingerprintService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fingerprint-`, `site === Site.FINGERPRINT`. | must |
...
```ts
@SourcePlugin({ site: Site.FINGERPRINT, name: 'Fingerprint', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fingerprint/jobs/<id>`), wire
  `company_name` pass-through (`'Fingerprint'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fingerprint/jobs/<id>`).
