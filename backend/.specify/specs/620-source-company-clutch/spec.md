# Spec: 620 — Source Company Plugin: Clutch

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 620                            |
| Slug           | source-company-clutch               |
| Status         | accepted                               |
...
Clutch publishes its careers board through Greenhouse at
the bare slug `clutch`. The run-398 batch sweep confirmed
...
- Ship a `source-company-clutch` plugin returning live
...
- Backfilling historical Clutch postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CLUTCH`** in the
> source registry, so that **a single `siteType: [Site.CLUTCH]`
> request returns Clutch's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CLUTCH = 'clutch'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-clutch`.                  | must     |
| FR-3  | `ClutchService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `clutch-`, `site === Site.CLUTCH`. | must |
...
```ts
@SourcePlugin({ site: Site.CLUTCH, name: 'Clutch', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/clutch/jobs/<id>`), wire
  `company_name` pass-through (`'Clutch'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/clutch/jobs/<id>`).
