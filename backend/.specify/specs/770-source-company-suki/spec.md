# Spec: 770 — Source Company Plugin: Suki AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 770                            |
| Slug           | source-company-suki               |
| Status         | accepted                               |
...
Suki AI publishes its careers board through Greenhouse at
the bare slug `suki`. The run-398 batch sweep confirmed
...
- Ship a `source-company-suki` plugin returning live
...
- Backfilling historical Suki AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SUKI_AI`** in the
> source registry, so that **a single `siteType: [Site.SUKI_AI]`
> request returns Suki AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SUKI_AI = 'suki'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-suki`.                  | must     |
| FR-3  | `SukiAIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `suki-`, `site === Site.SUKI_AI`. | must |
...
```ts
@SourcePlugin({ site: Site.SUKI_AI, name: 'Suki AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/suki/jobs/<id>`), wire
  `company_name` pass-through (`'Suki AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/suki/jobs/<id>`).
