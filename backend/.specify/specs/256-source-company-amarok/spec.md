# Spec: 256 — Source Company Plugin: AMAROK

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 256                            |
| Slug           | source-company-amarok               |
| Status         | accepted                               |
...
AMAROK publishes its careers board through Greenhouse at
the bare slug `amarok`. The run-398 batch sweep confirmed
...
- Ship a `source-company-amarok` plugin returning live
...
- Backfilling historical AMAROK postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMAROK`** in the
> source registry, so that **a single `siteType: [Site.AMAROK]`
> request returns AMAROK's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMAROK = 'amarok'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-amarok`.                  | must     |
| FR-3  | `AmarokService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `amarok-`, `site === Site.AMAROK`. | must |
...
```ts
@SourcePlugin({ site: Site.AMAROK, name: 'AMAROK', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/amarok/jobs/<id>`), wire
  `company_name` pass-through (`'AMAROK'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/amarok/jobs/<id>`).
