# Spec: 776 — Source Company Plugin: Dyno Therapeutics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 776                            |
| Slug           | source-company-dynotherapeutics               |
| Status         | accepted                               |
...
Dyno Therapeutics publishes its careers board through Greenhouse at
the bare slug `dynotherapeutics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-dynotherapeutics` plugin returning live
...
- Backfilling historical Dyno Therapeutics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DYNO_THERAPEUTICS`** in the
> source registry, so that **a single `siteType: [Site.DYNO_THERAPEUTICS]`
> request returns Dyno Therapeutics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DYNO_THERAPEUTICS = 'dynotherapeutics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-dynotherapeutics`.                  | must     |
| FR-3  | `DynoTherapeuticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `dynotherapeutics-`, `site === Site.DYNO_THERAPEUTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.DYNO_THERAPEUTICS, name: 'Dyno Therapeutics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/dynotherapeutics/jobs/<id>`), wire
  `company_name` pass-through (`'Dyno Therapeutics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/dynotherapeutics/jobs/<id>`).
