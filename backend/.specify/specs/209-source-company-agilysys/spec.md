# Spec: 209 — Source Company Plugin: Agilysys

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 209                            |
| Slug           | source-company-agilysys               |
| Status         | accepted                               |
...
Agilysys publishes its careers board through Greenhouse at
the bare slug `agilysys`. The run-398 batch sweep confirmed
...
- Ship a `source-company-agilysys` plugin returning live
...
- Backfilling historical Agilysys postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AGILYSYS`** in the
> source registry, so that **a single `siteType: [Site.AGILYSYS]`
> request returns Agilysys's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AGILYSYS = 'agilysys'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-agilysys`.                  | must     |
| FR-3  | `AgilysysService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `agilysys-`, `site === Site.AGILYSYS`. | must |
...
```ts
@SourcePlugin({ site: Site.AGILYSYS, name: 'Agilysys', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/agilysys/jobs/<id>`), wire
  `company_name` pass-through (`'Agilysys'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/agilysys/jobs/<id>`).
