# Spec: 712 — Source Company Plugin: Prove

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 712                            |
| Slug           | source-company-prove               |
| Status         | accepted                               |
...
Prove publishes its careers board through Greenhouse at
the bare slug `prove`. The run-398 batch sweep confirmed
...
- Ship a `source-company-prove` plugin returning live
...
- Backfilling historical Prove postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PROVE`** in the
> source registry, so that **a single `siteType: [Site.PROVE]`
> request returns Prove's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PROVE = 'prove'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-prove`.                  | must     |
| FR-3  | `ProveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `prove-`, `site === Site.PROVE`. | must |
...
```ts
@SourcePlugin({ site: Site.PROVE, name: 'Prove', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/prove/jobs/<id>`), wire
  `company_name` pass-through (`'Prove'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/prove/jobs/<id>`).
