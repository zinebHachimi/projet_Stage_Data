# Spec: 914 — Source Company Plugin: Prime Medicine

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 914                            |
| Slug           | source-company-primemedicine               |
| Status         | accepted                               |
...
Prime Medicine publishes its careers board through Greenhouse at
the bare slug `primemedicine`. The run-398 batch sweep confirmed
...
- Ship a `source-company-primemedicine` plugin returning live
...
- Backfilling historical Prime Medicine postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PRIME_MEDICINE`** in the
> source registry, so that **a single `siteType: [Site.PRIME_MEDICINE]`
> request returns Prime Medicine's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PRIME_MEDICINE = 'primemedicine'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-primemedicine`.                  | must     |
| FR-3  | `PrimeMedicineService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `primemedicine-`, `site === Site.PRIME_MEDICINE`. | must |
...
```ts
@SourcePlugin({ site: Site.PRIME_MEDICINE, name: 'Prime Medicine', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/primemedicine/jobs/<id>`), wire
  `company_name` pass-through (`'Prime Medicine'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/primemedicine/jobs/<id>`).
