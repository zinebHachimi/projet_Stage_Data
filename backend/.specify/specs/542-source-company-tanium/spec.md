# Spec: 542 — Source Company Plugin: Tanium

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 542                            |
| Slug           | source-company-tanium               |
| Status         | accepted                               |
...
Tanium publishes its careers board through Greenhouse at
the bare slug `tanium`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tanium` plugin returning live
...
- Backfilling historical Tanium postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TANIUM`** in the
> source registry, so that **a single `siteType: [Site.TANIUM]`
> request returns Tanium's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TANIUM = 'tanium'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tanium`.                  | must     |
| FR-3  | `TaniumService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tanium-`, `site === Site.TANIUM`. | must |
...
```ts
@SourcePlugin({ site: Site.TANIUM, name: 'Tanium', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tanium/jobs/<id>`), wire
  `company_name` pass-through (`'Tanium'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tanium/jobs/<id>`).
