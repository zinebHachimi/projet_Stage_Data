# Spec: 554 — Source Company Plugin: Mill

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 554                            |
| Slug           | source-company-mill               |
| Status         | accepted                               |
...
Mill publishes its careers board through Greenhouse at
the bare slug `mill`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mill` plugin returning live
...
- Backfilling historical Mill postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MILL`** in the
> source registry, so that **a single `siteType: [Site.MILL]`
> request returns Mill's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MILL = 'mill'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mill`.                  | must     |
| FR-3  | `MillService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mill-`, `site === Site.MILL`. | must |
...
```ts
@SourcePlugin({ site: Site.MILL, name: 'Mill', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mill/jobs/<id>`), wire
  `company_name` pass-through (`'Mill'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mill/jobs/<id>`).
