# Spec: 968 — Source Company Plugin: Vox Media

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 968                            |
| Slug           | source-company-voxmedia               |
| Status         | accepted                               |
...
Vox Media publishes its careers board through Greenhouse at
the bare slug `voxmedia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-voxmedia` plugin returning live
...
- Backfilling historical Vox Media postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VOX_MEDIA`** in the
> source registry, so that **a single `siteType: [Site.VOX_MEDIA]`
> request returns Vox Media's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VOX_MEDIA = 'voxmedia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-voxmedia`.                  | must     |
| FR-3  | `VoxMediaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `voxmedia-`, `site === Site.VOX_MEDIA`. | must |
...
```ts
@SourcePlugin({ site: Site.VOX_MEDIA, name: 'Vox Media', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/voxmedia/jobs/<id>`), wire
  `company_name` pass-through (`'Vox Media'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/voxmedia/jobs/<id>`).
