# Plan: 695 — Source Company Plugin: Verve Group
...
Verve Group's careers board is hosted on Greenhouse at the slug
`verve`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-verve` plugin.
- Deliverables: package files, `Site.VERVE_GROUP` enum value, the
...
- `packages/plugins/source-company-verve` (new)
