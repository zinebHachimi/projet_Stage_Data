# Spec: 695 — Source Company Plugin: Verve Group

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 695                            |
| Slug           | source-company-verve               |
| Status         | accepted                               |
...
Verve Group publishes its careers board through Greenhouse at
the bare slug `verve`. The run-398 batch sweep confirmed
...
- Ship a `source-company-verve` plugin returning live
...
- Backfilling historical Verve Group postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VERVE_GROUP`** in the
> source registry, so that **a single `siteType: [Site.VERVE_GROUP]`
> request returns Verve Group's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VERVE_GROUP = 'verve'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-verve`.                  | must     |
| FR-3  | `VerveGroupService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `verve-`, `site === Site.VERVE_GROUP`. | must |
...
```ts
@SourcePlugin({ site: Site.VERVE_GROUP, name: 'Verve Group', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/verve/jobs/<id>`), wire
  `company_name` pass-through (`'Verve Group'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/verve/jobs/<id>`).
