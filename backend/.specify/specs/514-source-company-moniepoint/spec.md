# Spec: 514 — Source Company Plugin: Moniepoint

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 514                            |
| Slug           | source-company-moniepoint               |
| Status         | accepted                               |
...
Moniepoint publishes its careers board through Greenhouse at
the bare slug `moniepoint`. The run-398 batch sweep confirmed
...
- Ship a `source-company-moniepoint` plugin returning live
...
- Backfilling historical Moniepoint postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MONIEPOINT`** in the
> source registry, so that **a single `siteType: [Site.MONIEPOINT]`
> request returns Moniepoint's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MONIEPOINT = 'moniepoint'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-moniepoint`.                  | must     |
| FR-3  | `MoniepointService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `moniepoint-`, `site === Site.MONIEPOINT`. | must |
...
```ts
@SourcePlugin({ site: Site.MONIEPOINT, name: 'Moniepoint', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/moniepoint/jobs/<id>`), wire
  `company_name` pass-through (`'Moniepoint'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/moniepoint/jobs/<id>`).
