# Spec: 476 — Source Company Plugin: Lithic

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 476                            |
| Slug           | source-company-lithic               |
| Status         | accepted                               |
...
Lithic publishes its careers board through Greenhouse at
the bare slug `lithic`. The run-398 batch sweep confirmed
...
- Ship a `source-company-lithic` plugin returning live
...
- Backfilling historical Lithic postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LITHIC`** in the
> source registry, so that **a single `siteType: [Site.LITHIC]`
> request returns Lithic's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LITHIC = 'lithic'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-lithic`.                  | must     |
| FR-3  | `LithicService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `lithic-`, `site === Site.LITHIC`. | must |
...
```ts
@SourcePlugin({ site: Site.LITHIC, name: 'Lithic', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/lithic/jobs/<id>`), wire
  `company_name` pass-through (`'Lithic'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/lithic/jobs/<id>`).
