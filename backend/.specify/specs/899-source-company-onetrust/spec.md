# Spec: 899 — Source Company Plugin: OneTrust

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 899                            |
| Slug           | source-company-onetrust               |
| Status         | accepted                               |
...
OneTrust publishes its careers board through Greenhouse at
the bare slug `onetrust`. The run-398 batch sweep confirmed
91 live role(s) via a direct probe of
...
- Ship a `source-company-onetrust` plugin returning live
...
- Backfilling historical OneTrust postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ONETRUST`** in the
> source registry, so that **a single `siteType: [Site.ONETRUST]`
> request returns OneTrust's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ONETRUST = 'onetrust'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-onetrust`.                  | must     |
| FR-3  | `OneTrustService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `onetrust-`, `site === Site.ONETRUST`. | must |
...
```ts
@SourcePlugin({ site: Site.ONETRUST, name: 'OneTrust', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/onetrust/jobs/<id>`), wire
  `company_name` pass-through (`'OneTrust'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/onetrust/jobs/<id>`).
