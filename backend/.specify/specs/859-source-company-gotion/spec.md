# Spec: 859 — Source Company Plugin: Gotion

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 859                            |
| Slug           | source-company-gotion               |
| Status         | accepted                               |
...
Gotion publishes its careers board through Greenhouse at
the bare slug `gotion`. The run-398 batch sweep confirmed
148 live role(s) via a direct probe of
...
- Ship a `source-company-gotion` plugin returning live
...
- Backfilling historical Gotion postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GOTION`** in the
> source registry, so that **a single `siteType: [Site.GOTION]`
> request returns Gotion's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GOTION = 'gotion'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-gotion`.                  | must     |
| FR-3  | `GotionService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `gotion-`, `site === Site.GOTION`. | must |
...
```ts
@SourcePlugin({ site: Site.GOTION, name: 'Gotion', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/gotion/jobs/<id>`), wire
  `company_name` pass-through (`'Gotion'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/gotion/jobs/<id>`).
