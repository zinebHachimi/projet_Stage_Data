# Spec: 950 — Source Company Plugin: Tenable

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 950                            |
| Slug           | source-company-tenableinc               |
| Status         | accepted                               |
...
Tenable publishes its careers board through Greenhouse at
the bare slug `tenableinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tenableinc` plugin returning live
...
- Backfilling historical Tenable postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TENABLE`** in the
> source registry, so that **a single `siteType: [Site.TENABLE]`
> request returns Tenable's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TENABLE = 'tenableinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tenableinc`.                  | must     |
| FR-3  | `TenableService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tenableinc-`, `site === Site.TENABLE`. | must |
...
```ts
@SourcePlugin({ site: Site.TENABLE, name: 'Tenable', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tenableinc/jobs/<id>`), wire
  `company_name` pass-through (`'Tenable'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tenableinc/jobs/<id>`).
