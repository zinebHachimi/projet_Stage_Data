# Spec: 646 — Source Company Plugin: Vast

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 646                            |
| Slug           | source-company-vast               |
| Status         | accepted                               |
...
Vast publishes its careers board through Greenhouse at
the bare slug `vast`. The run-398 batch sweep confirmed
143 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/vast/jobs?content=true`.
Sector: Aerospace. HQ: Long Beach, California, USA.

## 2. Goals

- Ship a `source-company-vast` plugin returning live
...
- Backfilling historical Vast postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VAST`** in the
> source registry, so that **a single `siteType: [Site.VAST]`
> request returns Vast's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VAST = 'vast'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-vast`.                  | must     |
| FR-3  | `VastService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `vast-`, `site === Site.VAST`. | must |
...
```ts
@SourcePlugin({ site: Site.VAST, name: 'Vast', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/vast/jobs/<id>`), wire
  `company_name` pass-through (`'Vast'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/vast/jobs/<id>`).
