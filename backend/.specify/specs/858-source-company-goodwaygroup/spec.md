# Spec: 858 — Source Company Plugin: Goodway Group

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 858                            |
| Slug           | source-company-goodwaygroup               |
| Status         | accepted                               |
...
Goodway Group publishes its careers board through Greenhouse at
the bare slug `goodwaygroup`. The run-398 batch sweep confirmed
...
- Ship a `source-company-goodwaygroup` plugin returning live
...
- Backfilling historical Goodway Group postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GOODWAY_GROUP`** in the
> source registry, so that **a single `siteType: [Site.GOODWAY_GROUP]`
> request returns Goodway Group's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GOODWAY_GROUP = 'goodwaygroup'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-goodwaygroup`.                  | must     |
| FR-3  | `GoodwayGroupService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `goodwaygroup-`, `site === Site.GOODWAY_GROUP`. | must |
...
```ts
@SourcePlugin({ site: Site.GOODWAY_GROUP, name: 'Goodway Group', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/goodwaygroup/jobs/<id>`), wire
  `company_name` pass-through (`'Goodway Group'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/goodwaygroup/jobs/<id>`).
