# Spec: 475 — Source Company Plugin: Highnote

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 475                            |
| Slug           | source-company-highnote               |
| Status         | accepted                               |
...
Highnote publishes its careers board through Greenhouse at
the bare slug `highnote`. The run-398 batch sweep confirmed
...
- Ship a `source-company-highnote` plugin returning live
...
- Backfilling historical Highnote postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HIGHNOTE`** in the
> source registry, so that **a single `siteType: [Site.HIGHNOTE]`
> request returns Highnote's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HIGHNOTE = 'highnote'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-highnote`.                  | must     |
| FR-3  | `HighnoteService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `highnote-`, `site === Site.HIGHNOTE`. | must |
...
```ts
@SourcePlugin({ site: Site.HIGHNOTE, name: 'Highnote', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/highnote/jobs/<id>`), wire
  `company_name` pass-through (`'Highnote'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/highnote/jobs/<id>`).
