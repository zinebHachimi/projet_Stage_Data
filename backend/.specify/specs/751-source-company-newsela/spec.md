# Spec: 751 — Source Company Plugin: Newsela

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 751                            |
| Slug           | source-company-newsela               |
| Status         | accepted                               |
...
Newsela publishes its careers board through Greenhouse at
the bare slug `newsela`. The run-398 batch sweep confirmed
...
- Ship a `source-company-newsela` plugin returning live
...
- Backfilling historical Newsela postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEWSELA`** in the
> source registry, so that **a single `siteType: [Site.NEWSELA]`
> request returns Newsela's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEWSELA = 'newsela'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-newsela`.                  | must     |
| FR-3  | `NewselaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `newsela-`, `site === Site.NEWSELA`. | must |
...
```ts
@SourcePlugin({ site: Site.NEWSELA, name: 'Newsela', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/newsela/jobs/<id>`), wire
  `company_name` pass-through (`'Newsela'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/newsela/jobs/<id>`).
