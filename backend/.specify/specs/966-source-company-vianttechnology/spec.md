# Spec: 966 — Source Company Plugin: Viant Technology

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 966                            |
| Slug           | source-company-vianttechnology               |
| Status         | accepted                               |
...
Viant Technology publishes its careers board through Greenhouse at
the bare slug `vianttechnology`. The run-398 batch sweep confirmed
36 live role(s) via a direct probe of
...
- Ship a `source-company-vianttechnology` plugin returning live
...
- Backfilling historical Viant Technology postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VIANT_TECHNOLOGY`** in the
> source registry, so that **a single `siteType: [Site.VIANT_TECHNOLOGY]`
> request returns Viant Technology's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VIANT_TECHNOLOGY = 'vianttechnology'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-vianttechnology`.                  | must     |
| FR-3  | `ViantTechnologyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `vianttechnology-`, `site === Site.VIANT_TECHNOLOGY`. | must |
...
```ts
@SourcePlugin({ site: Site.VIANT_TECHNOLOGY, name: 'Viant Technology', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/vianttechnology/jobs/<id>`), wire
  `company_name` pass-through (`'Viant Technology'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/vianttechnology/jobs/<id>`).
