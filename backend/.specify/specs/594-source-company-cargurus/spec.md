# Spec: 594 — Source Company Plugin: CarGurus

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 594                            |
| Slug           | source-company-cargurus               |
| Status         | accepted                               |
...
CarGurus publishes its careers board through Greenhouse at
the bare slug `cargurus`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cargurus` plugin returning live
...
- Backfilling historical CarGurus postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CARGURUS`** in the
> source registry, so that **a single `siteType: [Site.CARGURUS]`
> request returns CarGurus's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CARGURUS = 'cargurus'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cargurus`.                  | must     |
| FR-3  | `CarGurusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cargurus-`, `site === Site.CARGURUS`. | must |
...
```ts
@SourcePlugin({ site: Site.CARGURUS, name: 'CarGurus', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cargurus/jobs/<id>`), wire
  `company_name` pass-through (`'CarGurus'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cargurus/jobs/<id>`).
