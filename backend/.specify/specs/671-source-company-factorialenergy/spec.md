# Spec: 671 — Source Company Plugin: Factorial Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 671                            |
| Slug           | source-company-factorialenergy               |
| Status         | accepted                               |
...
Factorial Energy publishes its careers board through Greenhouse at
the bare slug `factorialenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-factorialenergy` plugin returning live
...
- Backfilling historical Factorial Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FACTORIAL_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.FACTORIAL_ENERGY]`
> request returns Factorial Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FACTORIAL_ENERGY = 'factorialenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-factorialenergy`.                  | must     |
| FR-3  | `FactorialEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `factorialenergy-`, `site === Site.FACTORIAL_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.FACTORIAL_ENERGY, name: 'Factorial Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/factorialenergy/jobs/<id>`), wire
  `company_name` pass-through (`'Factorial Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/factorialenergy/jobs/<id>`).
