# Spec: 864 — Source Company Plugin: Homeward

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 864                            |
| Slug           | source-company-homeward               |
| Status         | accepted                               |
...
Homeward publishes its careers board through Greenhouse at
the bare slug `homeward`. The run-398 batch sweep confirmed
...
- Ship a `source-company-homeward` plugin returning live
...
- Backfilling historical Homeward postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HOMEWARD`** in the
> source registry, so that **a single `siteType: [Site.HOMEWARD]`
> request returns Homeward's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HOMEWARD = 'homeward'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-homeward`.                  | must     |
| FR-3  | `HomewardService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `homeward-`, `site === Site.HOMEWARD`. | must |
...
```ts
@SourcePlugin({ site: Site.HOMEWARD, name: 'Homeward', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/homeward/jobs/<id>`), wire
  `company_name` pass-through (`'Homeward'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/homeward/jobs/<id>`).
