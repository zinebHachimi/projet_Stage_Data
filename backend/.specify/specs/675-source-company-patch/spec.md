# Spec: 675 — Source Company Plugin: Patch Caregiving

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 675                            |
| Slug           | source-company-patch               |
| Status         | accepted                               |
...
Patch Caregiving publishes its careers board through Greenhouse at
the bare slug `patch`. The run-398 batch sweep confirmed
...
- Ship a `source-company-patch` plugin returning live
...
- Backfilling historical Patch Caregiving postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PATCH_CAREGIVING`** in the
> source registry, so that **a single `siteType: [Site.PATCH_CAREGIVING]`
> request returns Patch Caregiving's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PATCH_CAREGIVING = 'patch'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-patch`.                  | must     |
| FR-3  | `PatchCaregivingService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `patch-`, `site === Site.PATCH_CAREGIVING`. | must |
...
```ts
@SourcePlugin({ site: Site.PATCH_CAREGIVING, name: 'Patch Caregiving', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/patch/jobs/<id>`), wire
  `company_name` pass-through (`'Patch Caregiving'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/patch/jobs/<id>`).
