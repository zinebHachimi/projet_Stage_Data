# Plan: 675 — Source Company Plugin: Patch Caregiving
...
Patch Caregiving's careers board is hosted on Greenhouse at the slug
`patch`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-patch` plugin.
- Deliverables: package files, `Site.PATCH_CAREGIVING` enum value, the
...
- `packages/plugins/source-company-patch` (new)
