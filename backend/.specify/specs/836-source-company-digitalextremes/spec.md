# Spec: 836 — Source Company Plugin: Digital Extremes

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 836                            |
| Slug           | source-company-digitalextremes               |
| Status         | accepted                               |
...
Digital Extremes publishes its careers board through Greenhouse at
the bare slug `digitalextremes`. The run-398 batch sweep confirmed
...
- Ship a `source-company-digitalextremes` plugin returning live
...
- Backfilling historical Digital Extremes postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DIGITAL_EXTREMES`** in the
> source registry, so that **a single `siteType: [Site.DIGITAL_EXTREMES]`
> request returns Digital Extremes's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DIGITAL_EXTREMES = 'digitalextremes'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-digitalextremes`.                  | must     |
| FR-3  | `DigitalExtremesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `digitalextremes-`, `site === Site.DIGITAL_EXTREMES`. | must |
...
```ts
@SourcePlugin({ site: Site.DIGITAL_EXTREMES, name: 'Digital Extremes', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/digitalextremes/jobs/<id>`), wire
  `company_name` pass-through (`'Digital Extremes'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/digitalextremes/jobs/<id>`).
