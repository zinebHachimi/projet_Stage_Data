# Spec: 490 — Source Company Plugin: Zocdoc

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 490                            |
| Slug           | source-company-zocdoc               |
| Status         | accepted                               |
...
Zocdoc publishes its careers board through Greenhouse at
the bare slug `zocdoc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-zocdoc` plugin returning live
...
- Backfilling historical Zocdoc postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ZOCDOC`** in the
> source registry, so that **a single `siteType: [Site.ZOCDOC]`
> request returns Zocdoc's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ZOCDOC = 'zocdoc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-zocdoc`.                  | must     |
| FR-3  | `ZocdocService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `zocdoc-`, `site === Site.ZOCDOC`. | must |
...
```ts
@SourcePlugin({ site: Site.ZOCDOC, name: 'Zocdoc', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/zocdoc/jobs/<id>`), wire
  `company_name` pass-through (`'Zocdoc'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/zocdoc/jobs/<id>`).
