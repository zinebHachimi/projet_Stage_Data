# Spec: 879 — Source Company Plugin: LogicGate

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 879                            |
| Slug           | source-company-logicgate               |
| Status         | accepted                               |
...
LogicGate publishes its careers board through Greenhouse at
the bare slug `logicgate`. The run-398 batch sweep confirmed
...
- Ship a `source-company-logicgate` plugin returning live
...
- Backfilling historical LogicGate postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LOGICGATE`** in the
> source registry, so that **a single `siteType: [Site.LOGICGATE]`
> request returns LogicGate's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LOGICGATE = 'logicgate'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-logicgate`.                  | must     |
| FR-3  | `LogicGateService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `logicgate-`, `site === Site.LOGICGATE`. | must |
...
```ts
@SourcePlugin({ site: Site.LOGICGATE, name: 'LogicGate', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/logicgate/jobs/<id>`), wire
  `company_name` pass-through (`'LogicGate'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/logicgate/jobs/<id>`).
