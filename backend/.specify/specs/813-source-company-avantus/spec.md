# Spec: 813 — Source Company Plugin: Avantus

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 813                            |
| Slug           | source-company-avantus               |
| Status         | accepted                               |
...
Avantus publishes its careers board through Greenhouse at
the bare slug `avantus`. The run-398 batch sweep confirmed
...
- Ship a `source-company-avantus` plugin returning live
...
- Backfilling historical Avantus postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AVANTUS`** in the
> source registry, so that **a single `siteType: [Site.AVANTUS]`
> request returns Avantus's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AVANTUS = 'avantus'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-avantus`.                  | must     |
| FR-3  | `AvantusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `avantus-`, `site === Site.AVANTUS`. | must |
...
```ts
@SourcePlugin({ site: Site.AVANTUS, name: 'Avantus', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/avantus/jobs/<id>`), wire
  `company_name` pass-through (`'Avantus'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/avantus/jobs/<id>`).
