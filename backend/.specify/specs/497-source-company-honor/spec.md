# Spec: 497 — Source Company Plugin: Honor

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 497                            |
| Slug           | source-company-honor               |
| Status         | accepted                               |
...
Honor publishes its careers board through Greenhouse at
the bare slug `honor`. The run-398 batch sweep confirmed
...
- Ship a `source-company-honor` plugin returning live
...
- Backfilling historical Honor postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HONOR`** in the
> source registry, so that **a single `siteType: [Site.HONOR]`
> request returns Honor's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HONOR = 'honor'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-honor`.                  | must     |
| FR-3  | `HonorService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `honor-`, `site === Site.HONOR`. | must |
...
```ts
@SourcePlugin({ site: Site.HONOR, name: 'Honor', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/honor/jobs/<id>`), wire
  `company_name` pass-through (`'Honor'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/honor/jobs/<id>`).
