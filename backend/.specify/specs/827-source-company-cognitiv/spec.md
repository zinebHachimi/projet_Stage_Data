# Spec: 827 — Source Company Plugin: Cognitiv

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 827                            |
| Slug           | source-company-cognitiv               |
| Status         | accepted                               |
...
Cognitiv publishes its careers board through Greenhouse at
the bare slug `cognitiv`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cognitiv` plugin returning live
...
- Backfilling historical Cognitiv postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COGNITIV`** in the
> source registry, so that **a single `siteType: [Site.COGNITIV]`
> request returns Cognitiv's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COGNITIV = 'cognitiv'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cognitiv`.                  | must     |
| FR-3  | `CognitivService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cognitiv-`, `site === Site.COGNITIV`. | must |
...
```ts
@SourcePlugin({ site: Site.COGNITIV, name: 'Cognitiv', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cognitiv/jobs/<id>`), wire
  `company_name` pass-through (`'Cognitiv'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cognitiv/jobs/<id>`).
