# Spec: 610 — Source Company Plugin: Divergent

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 610                            |
| Slug           | source-company-divergent               |
| Status         | accepted                               |
...
Divergent publishes its careers board through Greenhouse at
the bare slug `divergent`. The run-398 batch sweep confirmed
...
- Ship a `source-company-divergent` plugin returning live
...
- Backfilling historical Divergent postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DIVERGENT`** in the
> source registry, so that **a single `siteType: [Site.DIVERGENT]`
> request returns Divergent's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DIVERGENT = 'divergent'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-divergent`.                  | must     |
| FR-3  | `DivergentService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `divergent-`, `site === Site.DIVERGENT`. | must |
...
```ts
@SourcePlugin({ site: Site.DIVERGENT, name: 'Divergent', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/divergent/jobs/<id>`), wire
  `company_name` pass-through (`'Divergent'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/divergent/jobs/<id>`).
