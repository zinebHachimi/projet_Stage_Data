# Spec: 565 — Source Company Plugin: Imbue

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 565                            |
| Slug           | source-company-imbue               |
| Status         | accepted                               |
...
Imbue publishes its careers board through Greenhouse at
the bare slug `imbue`. The run-398 batch sweep confirmed
...
- Ship a `source-company-imbue` plugin returning live
...
- Backfilling historical Imbue postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.IMBUE`** in the
> source registry, so that **a single `siteType: [Site.IMBUE]`
> request returns Imbue's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.IMBUE = 'imbue'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-imbue`.                  | must     |
| FR-3  | `ImbueService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `imbue-`, `site === Site.IMBUE`. | must |
...
```ts
@SourcePlugin({ site: Site.IMBUE, name: 'Imbue', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/imbue/jobs/<id>`), wire
  `company_name` pass-through (`'Imbue'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/imbue/jobs/<id>`).
