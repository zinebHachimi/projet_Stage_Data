# Spec: 271 — Source Company Plugin: Ansa

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 271                            |
| Slug           | source-company-ansa               |
| Status         | accepted                               |
...
Ansa publishes its careers board through Greenhouse at
the bare slug `ansa`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ansa` plugin returning live
...
- Backfilling historical Ansa postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ANSA`** in the
> source registry, so that **a single `siteType: [Site.ANSA]`
> request returns Ansa's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ANSA = 'ansa'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ansa`.                  | must     |
| FR-3  | `AnsaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ansa-`, `site === Site.ANSA`. | must |
...
```ts
@SourcePlugin({ site: Site.ANSA, name: 'Ansa', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ansa/jobs/<id>`), wire
  `company_name` pass-through (`'Ansa'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ansa/jobs/<id>`).
