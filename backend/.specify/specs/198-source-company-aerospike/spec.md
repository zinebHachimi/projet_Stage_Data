# Spec: 198 — Source Company Plugin: Aerospike

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 198                            |
| Slug           | source-company-aerospike               |
| Status         | accepted                               |
...
Aerospike publishes its careers board through Greenhouse at
the bare slug `aerospike`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aerospike` plugin returning live
...
- Backfilling historical Aerospike postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AEROSPIKE`** in the
> source registry, so that **a single `siteType: [Site.AEROSPIKE]`
> request returns Aerospike's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AEROSPIKE = 'aerospike'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aerospike`.                  | must     |
| FR-3  | `AerospikeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aerospike-`, `site === Site.AEROSPIKE`. | must |
...
```ts
@SourcePlugin({ site: Site.AEROSPIKE, name: 'Aerospike', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aerospike/jobs/<id>`), wire
  `company_name` pass-through (`'Aerospike'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aerospike/jobs/<id>`).
