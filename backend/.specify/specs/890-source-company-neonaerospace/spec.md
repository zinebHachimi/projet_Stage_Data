# Spec: 890 — Source Company Plugin: Neon Aerospace

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 890                            |
| Slug           | source-company-neonaerospace               |
| Status         | accepted                               |
...
Neon Aerospace publishes its careers board through Greenhouse at
the bare slug `neonaerospace`. The run-398 batch sweep confirmed
...
- Ship a `source-company-neonaerospace` plugin returning live
...
- Backfilling historical Neon Aerospace postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEON_AEROSPACE`** in the
> source registry, so that **a single `siteType: [Site.NEON_AEROSPACE]`
> request returns Neon Aerospace's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEON_AEROSPACE = 'neonaerospace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-neonaerospace`.                  | must     |
| FR-3  | `NeonAerospaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `neonaerospace-`, `site === Site.NEON_AEROSPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.NEON_AEROSPACE, name: 'Neon Aerospace', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/neonaerospace/jobs/<id>`), wire
  `company_name` pass-through (`'Neon Aerospace'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/neonaerospace/jobs/<id>`).
