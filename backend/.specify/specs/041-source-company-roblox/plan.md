# Plan: 041 — Source Company Plugin: Roblox
...
Roblox's careers board is hosted on Greenhouse at the slug `roblox`,
so the implementation is a thin wrapper around the same public
Greenhouse endpoint that `source-company-dropbox` (Spec 040 / run
#250), `source-company-instacart` (Spec 039 / run #249),
...
already call. The plan is to copy the Dropbox plugin's shape
(single-file `service.ts`, three-line `module.ts`, two-line
`index.ts`, four-line `package.json`, three-line `tsconfig.json`)
and rename references from Dropbox → Roblox, then wire the new
...
used twenty-nine times before (Amazon, Apple, Cursor, Google, IBM,
...
- **Goal:** Land `source-company-roblox` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-roblox/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,roblox.module.ts,roblox.service.ts}`,
    `__tests__/roblox.service.spec.ts`,
    `__tests__/fixtures/roblox-jobs.json`).
  - One-line edits to: `packages/models/src/enums/site.enum.ts`,
    `packages/plugins/index.ts`, `tsconfig.base.json`,
    `jest.config.js`.
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 041 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-roblox` → all green.
...
| `packages/models/src/enums/site.enum.ts`             | append `ROBLOX = 'roblox'`.                                  |
| `packages/plugins/index.ts`                          | import + append `RobloxModule` to `ALL_SOURCE_MODULES`.      |
| `tsconfig.base.json`                                 | path-alias entry.                                            |
| `jest.config.js`                                     | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                    | add Roblox shipped row.                                      |
| `docs/index.md`                                      | append Spec 041 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Roblox moves off Greenhouse mid-maintenance.        | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-dropbox`'s shape.        | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
| Roblox surfaces first-party roles through Talent Hub. | L        | L      | Plugin still resolves through `Site.ROBLOX` for the corporate Greenhouse tenant; aggregator-side analytics is preserved. A follow-up spec can add a separate `source-roblox-talenthub` niche plugin (see Spec 041 § 10 D-06). |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.ROBLOX` is unused outside this plugin, so the enum revert is
  safe.
...
(None — see Spec 041 § 10 Decisions for the seven resolved points.)
