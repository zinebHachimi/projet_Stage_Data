# Spec: 683 — Source Company Plugin: Bitpanda

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 683                            |
| Slug           | source-company-bitpanda               |
| Status         | accepted                               |
...
Bitpanda publishes its careers board through Greenhouse at
the bare slug `bitpanda`. The run-398 batch sweep confirmed
...
- Ship a `source-company-bitpanda` plugin returning live
...
- Backfilling historical Bitpanda postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BITPANDA`** in the
> source registry, so that **a single `siteType: [Site.BITPANDA]`
> request returns Bitpanda's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BITPANDA = 'bitpanda'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-bitpanda`.                  | must     |
| FR-3  | `BitpandaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `bitpanda-`, `site === Site.BITPANDA`. | must |
...
```ts
@SourcePlugin({ site: Site.BITPANDA, name: 'Bitpanda', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/bitpanda/jobs/<id>`), wire
  `company_name` pass-through (`'Bitpanda'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/bitpanda/jobs/<id>`).
