# Spec: 429 — Source Company Plugin: Abnormal Security

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 429                            |
| Slug           | source-company-abnormalsecurity               |
| Status         | accepted                               |
...
Abnormal Security publishes its careers board through Greenhouse at
the bare slug `abnormalsecurity`. The run-398 batch sweep confirmed
87 live role(s) via a direct probe of
...
- Ship a `source-company-abnormalsecurity` plugin returning live
...
- Backfilling historical Abnormal Security postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ABNORMALSECURITY`** in the
> source registry, so that **a single `siteType: [Site.ABNORMALSECURITY]`
> request returns Abnormal Security's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ABNORMALSECURITY = 'abnormalsecurity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-abnormalsecurity`.                  | must     |
| FR-3  | `AbnormalsecurityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `abnormalsecurity-`, `site === Site.ABNORMALSECURITY`. | must |
...
```ts
@SourcePlugin({ site: Site.ABNORMALSECURITY, name: 'Abnormal Security', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/abnormalsecurity/jobs/<id>`), wire
  `company_name` pass-through (`'Abnormal Security'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/abnormalsecurity/jobs/<id>`).
