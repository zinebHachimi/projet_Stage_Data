# Spec: 885 — Source Company Plugin: MobilityWare

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 885                            |
| Slug           | source-company-mobilityware               |
| Status         | accepted                               |
...
MobilityWare publishes its careers board through Greenhouse at
the bare slug `mobilityware`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mobilityware` plugin returning live
...
- Backfilling historical MobilityWare postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MOBILITYWARE`** in the
> source registry, so that **a single `siteType: [Site.MOBILITYWARE]`
> request returns MobilityWare's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MOBILITYWARE = 'mobilityware'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mobilityware`.                  | must     |
| FR-3  | `MobilityWareService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mobilityware-`, `site === Site.MOBILITYWARE`. | must |
...
```ts
@SourcePlugin({ site: Site.MOBILITYWARE, name: 'MobilityWare', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mobilityware/jobs/<id>`), wire
  `company_name` pass-through (`'MobilityWare'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mobilityware/jobs/<id>`).
