# Spec: 831 — Source Company Plugin: Cypress Creek Renewables

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 831                            |
| Slug           | source-company-cypresscreekrenewables               |
| Status         | accepted                               |
...
Cypress Creek Renewables publishes its careers board through Greenhouse at
the bare slug `cypresscreekrenewables`. The run-398 batch sweep confirmed
28 live role(s) via a direct probe of
...
- Ship a `source-company-cypresscreekrenewables` plugin returning live
...
- Backfilling historical Cypress Creek Renewables postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CYPRESS_CREEK_RENEWABLES`** in the
> source registry, so that **a single `siteType: [Site.CYPRESS_CREEK_RENEWABLES]`
> request returns Cypress Creek Renewables's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CYPRESS_CREEK_RENEWABLES = 'cypresscreekrenewables'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cypresscreekrenewables`.                  | must     |
| FR-3  | `CypressCreekRenewablesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cypresscreekrenewables-`, `site === Site.CYPRESS_CREEK_RENEWABLES`. | must |
...
```ts
@SourcePlugin({ site: Site.CYPRESS_CREEK_RENEWABLES, name: 'Cypress Creek Renewables', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cypresscreekrenewables/jobs/<id>`), wire
  `company_name` pass-through (`'Cypress Creek Renewables'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cypresscreekrenewables/jobs/<id>`).
