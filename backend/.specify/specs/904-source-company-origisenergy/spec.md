# Spec: 904 — Source Company Plugin: Origis Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 904                            |
| Slug           | source-company-origisenergy               |
| Status         | accepted                               |
...
Origis Energy publishes its careers board through Greenhouse at
the bare slug `origisenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-origisenergy` plugin returning live
...
- Backfilling historical Origis Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ORIGIS_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.ORIGIS_ENERGY]`
> request returns Origis Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ORIGIS_ENERGY = 'origisenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-origisenergy`.                  | must     |
| FR-3  | `OrigisEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `origisenergy-`, `site === Site.ORIGIS_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.ORIGIS_ENERGY, name: 'Origis Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/origisenergy/jobs/<id>`), wire
  `company_name` pass-through (`'Origis Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/origisenergy/jobs/<id>`).
