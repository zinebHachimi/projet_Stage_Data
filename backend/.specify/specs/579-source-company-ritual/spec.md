# Spec: 579 — Source Company Plugin: Ritual

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 579                            |
| Slug           | source-company-ritual               |
| Status         | accepted                               |
...
Ritual publishes its careers board through Greenhouse at
the bare slug `ritual`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ritual` plugin returning live
...
- Backfilling historical Ritual postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RITUAL`** in the
> source registry, so that **a single `siteType: [Site.RITUAL]`
> request returns Ritual's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RITUAL = 'ritual'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ritual`.                  | must     |
| FR-3  | `RitualService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ritual-`, `site === Site.RITUAL`. | must |
...
```ts
@SourcePlugin({ site: Site.RITUAL, name: 'Ritual', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ritual/jobs/<id>`), wire
  `company_name` pass-through (`'Ritual'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ritual/jobs/<id>`).
