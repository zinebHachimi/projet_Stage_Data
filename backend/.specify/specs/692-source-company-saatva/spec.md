# Spec: 692 — Source Company Plugin: Saatva

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 692                            |
| Slug           | source-company-saatva               |
| Status         | accepted                               |
...
Saatva publishes its careers board through Greenhouse at
the bare slug `saatva`. The run-398 batch sweep confirmed
...
- Ship a `source-company-saatva` plugin returning live
...
- Backfilling historical Saatva postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SAATVA`** in the
> source registry, so that **a single `siteType: [Site.SAATVA]`
> request returns Saatva's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SAATVA = 'saatva'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-saatva`.                  | must     |
| FR-3  | `SaatvaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `saatva-`, `site === Site.SAATVA`. | must |
...
```ts
@SourcePlugin({ site: Site.SAATVA, name: 'Saatva', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/saatva/jobs/<id>`), wire
  `company_name` pass-through (`'Saatva'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/saatva/jobs/<id>`).
