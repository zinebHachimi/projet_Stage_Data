# Spec: 634 — Source Company Plugin: Rocket Money

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 634                            |
| Slug           | source-company-truebill               |
| Status         | accepted                               |
...
Rocket Money publishes its careers board through Greenhouse at
the bare slug `truebill`. The run-398 batch sweep confirmed
...
- Ship a `source-company-truebill` plugin returning live
...
- Backfilling historical Rocket Money postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ROCKET_MONEY`** in the
> source registry, so that **a single `siteType: [Site.ROCKET_MONEY]`
> request returns Rocket Money's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ROCKET_MONEY = 'truebill'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-truebill`.                  | must     |
| FR-3  | `RocketMoneyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `truebill-`, `site === Site.ROCKET_MONEY`. | must |
...
```ts
@SourcePlugin({ site: Site.ROCKET_MONEY, name: 'Rocket Money', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/truebill/jobs/<id>`), wire
  `company_name` pass-through (`'Rocket Money'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/truebill/jobs/<id>`).
