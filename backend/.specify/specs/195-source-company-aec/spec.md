# Spec: 195 — Source Company Plugin: AEC

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 195                            |
| Slug           | source-company-aec               |
| Status         | accepted                               |
...
AEC publishes its careers board through Greenhouse at
the bare slug `aec`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aec` plugin returning live
...
- Backfilling historical AEC postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AEC`** in the
> source registry, so that **a single `siteType: [Site.AEC]`
> request returns AEC's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AEC = 'aec'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aec`.                  | must     |
| FR-3  | `AecService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aec-`, `site === Site.AEC`. | must |
...
```ts
@SourcePlugin({ site: Site.AEC, name: 'AEC', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aec/jobs/<id>`), wire
  `company_name` pass-through (`'AEC'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aec/jobs/<id>`).
