# Spec: 660 — Source Company Plugin: quip

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 660                            |
| Slug           | source-company-quip               |
| Status         | accepted                               |
...
quip publishes its careers board through Greenhouse at
the bare slug `quip`. The run-398 batch sweep confirmed
...
- Ship a `source-company-quip` plugin returning live
...
- Backfilling historical quip postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.QUIP`** in the
> source registry, so that **a single `siteType: [Site.QUIP]`
> request returns quip's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.QUIP = 'quip'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-quip`.                  | must     |
| FR-3  | `QuipService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `quip-`, `site === Site.QUIP`. | must |
...
```ts
@SourcePlugin({ site: Site.QUIP, name: 'quip', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/quip/jobs/<id>`), wire
  `company_name` pass-through (`'quip'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/quip/jobs/<id>`).
