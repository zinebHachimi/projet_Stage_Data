# Spec: 865 — Source Company Plugin: Hyliion

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 865                            |
| Slug           | source-company-hyliion               |
| Status         | accepted                               |
...
Hyliion publishes its careers board through Greenhouse at
the bare slug `hyliion`. The run-398 batch sweep confirmed
...
- Ship a `source-company-hyliion` plugin returning live
...
- Backfilling historical Hyliion postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HYLIION`** in the
> source registry, so that **a single `siteType: [Site.HYLIION]`
> request returns Hyliion's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HYLIION = 'hyliion'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-hyliion`.                  | must     |
| FR-3  | `HyliionService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `hyliion-`, `site === Site.HYLIION`. | must |
...
```ts
@SourcePlugin({ site: Site.HYLIION, name: 'Hyliion', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/hyliion/jobs/<id>`), wire
  `company_name` pass-through (`'Hyliion'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/hyliion/jobs/<id>`).
