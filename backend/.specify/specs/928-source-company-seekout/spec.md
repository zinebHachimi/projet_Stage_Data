# Spec: 928 — Source Company Plugin: SeekOut

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 928                            |
| Slug           | source-company-seekout               |
| Status         | accepted                               |
...
SeekOut publishes its careers board through Greenhouse at
the bare slug `seekout`. The run-398 batch sweep confirmed
...
- Ship a `source-company-seekout` plugin returning live
...
- Backfilling historical SeekOut postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SEEKOUT`** in the
> source registry, so that **a single `siteType: [Site.SEEKOUT]`
> request returns SeekOut's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SEEKOUT = 'seekout'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-seekout`.                  | must     |
| FR-3  | `SeekOutService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `seekout-`, `site === Site.SEEKOUT`. | must |
...
```ts
@SourcePlugin({ site: Site.SEEKOUT, name: 'SeekOut', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/seekout/jobs/<id>`), wire
  `company_name` pass-through (`'SeekOut'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/seekout/jobs/<id>`).
