# Spec: 906 — Source Company Plugin: Pacvue

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 906                            |
| Slug           | source-company-pacvue               |
| Status         | accepted                               |
...
Pacvue publishes its careers board through Greenhouse at
the bare slug `pacvue`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pacvue` plugin returning live
...
- Backfilling historical Pacvue postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PACVUE`** in the
> source registry, so that **a single `siteType: [Site.PACVUE]`
> request returns Pacvue's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PACVUE = 'pacvue'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pacvue`.                  | must     |
| FR-3  | `PacvueService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pacvue-`, `site === Site.PACVUE`. | must |
...
```ts
@SourcePlugin({ site: Site.PACVUE, name: 'Pacvue', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pacvue/jobs/<id>`), wire
  `company_name` pass-through (`'Pacvue'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pacvue/jobs/<id>`).
