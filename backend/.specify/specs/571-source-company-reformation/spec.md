# Spec: 571 — Source Company Plugin: Reformation

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 571                            |
| Slug           | source-company-reformation               |
| Status         | accepted                               |
...
Reformation publishes its careers board through Greenhouse at
the bare slug `reformation`. The run-398 batch sweep confirmed
...
- Ship a `source-company-reformation` plugin returning live
...
- Backfilling historical Reformation postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.REFORMATION`** in the
> source registry, so that **a single `siteType: [Site.REFORMATION]`
> request returns Reformation's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.REFORMATION = 'reformation'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-reformation`.                  | must     |
| FR-3  | `ReformationService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `reformation-`, `site === Site.REFORMATION`. | must |
...
```ts
@SourcePlugin({ site: Site.REFORMATION, name: 'Reformation', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/reformation/jobs/<id>`), wire
  `company_name` pass-through (`'Reformation'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/reformation/jobs/<id>`).
