# Spec: 611 — Source Company Plugin: Typeface

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 611                            |
| Slug           | source-company-typeface               |
| Status         | accepted                               |
...
Typeface publishes its careers board through Greenhouse at
the bare slug `typeface`. The run-398 batch sweep confirmed
...
- Ship a `source-company-typeface` plugin returning live
...
- Backfilling historical Typeface postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TYPEFACE`** in the
> source registry, so that **a single `siteType: [Site.TYPEFACE]`
> request returns Typeface's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TYPEFACE = 'typeface'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-typeface`.                  | must     |
| FR-3  | `TypefaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `typeface-`, `site === Site.TYPEFACE`. | must |
...
```ts
@SourcePlugin({ site: Site.TYPEFACE, name: 'Typeface', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/typeface/jobs/<id>`), wire
  `company_name` pass-through (`'Typeface'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/typeface/jobs/<id>`).
