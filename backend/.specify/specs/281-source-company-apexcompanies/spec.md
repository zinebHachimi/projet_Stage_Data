# Spec: 281 — Source Company Plugin: Apex Companies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 281                            |
| Slug           | source-company-apexcompanies               |
| Status         | accepted                               |
...
Apex Companies publishes its careers board through Greenhouse at
the bare slug `apexcompanies`. The run-398 batch sweep confirmed
226 live role(s) via a direct probe of
...
- Ship a `source-company-apexcompanies` plugin returning live
...
- Backfilling historical Apex Companies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APEXCOMPANIES`** in the
> source registry, so that **a single `siteType: [Site.APEXCOMPANIES]`
> request returns Apex Companies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APEXCOMPANIES = 'apexcompanies'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apexcompanies`.                  | must     |
| FR-3  | `ApexcompaniesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apexcompanies-`, `site === Site.APEXCOMPANIES`. | must |
...
```ts
@SourcePlugin({ site: Site.APEXCOMPANIES, name: 'Apex Companies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apexcompanies/jobs/<id>`), wire
  `company_name` pass-through (`'Apex Companies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apexcompanies/jobs/<id>`).
