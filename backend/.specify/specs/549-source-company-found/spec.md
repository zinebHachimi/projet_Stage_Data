# Spec: 549 — Source Company Plugin: Found

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 549                            |
| Slug           | source-company-found               |
| Status         | accepted                               |
...
Found publishes its careers board through Greenhouse at
the bare slug `found`. The run-398 batch sweep confirmed
...
- Ship a `source-company-found` plugin returning live
...
- Backfilling historical Found postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FOUND`** in the
> source registry, so that **a single `siteType: [Site.FOUND]`
> request returns Found's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FOUND = 'found'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-found`.                  | must     |
| FR-3  | `FoundService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `found-`, `site === Site.FOUND`. | must |
...
```ts
@SourcePlugin({ site: Site.FOUND, name: 'Found', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/found/jobs/<id>`), wire
  `company_name` pass-through (`'Found'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/found/jobs/<id>`).
