# Spec: 689 — Source Company Plugin: NewLimit

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 689                            |
| Slug           | source-company-newlimit               |
| Status         | accepted                               |
...
NewLimit publishes its careers board through Greenhouse at
the bare slug `newlimit`. The run-398 batch sweep confirmed
...
- Ship a `source-company-newlimit` plugin returning live
...
- Backfilling historical NewLimit postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEWLIMIT`** in the
> source registry, so that **a single `siteType: [Site.NEWLIMIT]`
> request returns NewLimit's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEWLIMIT = 'newlimit'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-newlimit`.                  | must     |
| FR-3  | `NewLimitService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `newlimit-`, `site === Site.NEWLIMIT`. | must |
...
```ts
@SourcePlugin({ site: Site.NEWLIMIT, name: 'NewLimit', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/newlimit/jobs/<id>`), wire
  `company_name` pass-through (`'NewLimit'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/newlimit/jobs/<id>`).
