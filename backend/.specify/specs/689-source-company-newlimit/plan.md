# Plan: 689 — Source Company Plugin: NewLimit
...
NewLimit's careers board is hosted on Greenhouse at the slug
`newlimit`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-newlimit` plugin.
- Deliverables: package files, `Site.NEWLIMIT` enum value, the
...
- `packages/plugins/source-company-newlimit` (new)
