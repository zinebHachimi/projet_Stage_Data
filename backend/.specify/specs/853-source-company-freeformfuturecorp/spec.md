# Spec: 853 — Source Company Plugin: Freeform

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 853                            |
| Slug           | source-company-freeformfuturecorp               |
| Status         | accepted                               |
...
Freeform publishes its careers board through Greenhouse at
the bare slug `freeformfuturecorp`. The run-398 batch sweep confirmed
55 live role(s) via a direct probe of
...
- Ship a `source-company-freeformfuturecorp` plugin returning live
...
- Backfilling historical Freeform postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FREEFORM`** in the
> source registry, so that **a single `siteType: [Site.FREEFORM]`
> request returns Freeform's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FREEFORM = 'freeformfuturecorp'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-freeformfuturecorp`.                  | must     |
| FR-3  | `FreeformService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `freeformfuturecorp-`, `site === Site.FREEFORM`. | must |
...
```ts
@SourcePlugin({ site: Site.FREEFORM, name: 'Freeform', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/freeformfuturecorp/jobs/<id>`), wire
  `company_name` pass-through (`'Freeform'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/freeformfuturecorp/jobs/<id>`).
