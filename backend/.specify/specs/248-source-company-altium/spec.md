# Spec: 248 — Source Company Plugin: Altium

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 248                            |
| Slug           | source-company-altium               |
| Status         | accepted                               |
...
Altium publishes its careers board through Greenhouse at
the bare slug `altium`. The run-398 batch sweep confirmed
...
- Ship a `source-company-altium` plugin returning live
...
- Backfilling historical Altium postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALTIUM`** in the
> source registry, so that **a single `siteType: [Site.ALTIUM]`
> request returns Altium's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALTIUM = 'altium'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-altium`.                  | must     |
| FR-3  | `AltiumService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `altium-`, `site === Site.ALTIUM`. | must |
...
```ts
@SourcePlugin({ site: Site.ALTIUM, name: 'Altium', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/altium/jobs/<id>`), wire
  `company_name` pass-through (`'Altium'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/altium/jobs/<id>`).
