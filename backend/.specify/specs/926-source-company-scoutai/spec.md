# Spec: 926 — Source Company Plugin: Scout AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 926                            |
| Slug           | source-company-scoutai               |
| Status         | accepted                               |
...
Scout AI publishes its careers board through Greenhouse at
the bare slug `scoutai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-scoutai` plugin returning live
...
- Backfilling historical Scout AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SCOUT_AI`** in the
> source registry, so that **a single `siteType: [Site.SCOUT_AI]`
> request returns Scout AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SCOUT_AI = 'scoutai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-scoutai`.                  | must     |
| FR-3  | `ScoutAIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `scoutai-`, `site === Site.SCOUT_AI`. | must |
...
```ts
@SourcePlugin({ site: Site.SCOUT_AI, name: 'Scout AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/scoutai/jobs/<id>`), wire
  `company_name` pass-through (`'Scout AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/scoutai/jobs/<id>`).
