# Spec: 758 — Source Company Plugin: AST SpaceMobile

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 758                            |
| Slug           | source-company-astspacemobile               |
| Status         | accepted                               |
...
AST SpaceMobile publishes its careers board through Greenhouse at
the bare slug `astspacemobile`. The run-398 batch sweep confirmed
223 live role(s) via a direct probe of
...
- Ship a `source-company-astspacemobile` plugin returning live
...
- Backfilling historical AST SpaceMobile postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AST_SPACEMOBILE`** in the
> source registry, so that **a single `siteType: [Site.AST_SPACEMOBILE]`
> request returns AST SpaceMobile's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AST_SPACEMOBILE = 'astspacemobile'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-astspacemobile`.                  | must     |
| FR-3  | `ASTSpaceMobileService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `astspacemobile-`, `site === Site.AST_SPACEMOBILE`. | must |
...
```ts
@SourcePlugin({ site: Site.AST_SPACEMOBILE, name: 'AST SpaceMobile', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/astspacemobile/jobs/<id>`), wire
  `company_name` pass-through (`'AST SpaceMobile'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/astspacemobile/jobs/<id>`).
