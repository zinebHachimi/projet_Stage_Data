# Plan: 758 — Source Company Plugin: AST SpaceMobile
...
AST SpaceMobile's careers board is hosted on Greenhouse at the slug
`astspacemobile`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-astspacemobile` plugin.
- Deliverables: package files, `Site.AST_SPACEMOBILE` enum value, the
...
- `packages/plugins/source-company-astspacemobile` (new)
