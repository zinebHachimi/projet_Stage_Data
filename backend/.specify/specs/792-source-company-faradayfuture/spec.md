# Spec: 792 — Source Company Plugin: Faraday Future

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 792                            |
| Slug           | source-company-faradayfuture               |
| Status         | accepted                               |
...
Faraday Future publishes its careers board through Greenhouse at
the bare slug `faradayfuture`. The run-398 batch sweep confirmed
...
- Ship a `source-company-faradayfuture` plugin returning live
...
- Backfilling historical Faraday Future postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FARADAY_FUTURE`** in the
> source registry, so that **a single `siteType: [Site.FARADAY_FUTURE]`
> request returns Faraday Future's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FARADAY_FUTURE = 'faradayfuture'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-faradayfuture`.                  | must     |
| FR-3  | `FaradayFutureService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `faradayfuture-`, `site === Site.FARADAY_FUTURE`. | must |
...
```ts
@SourcePlugin({ site: Site.FARADAY_FUTURE, name: 'Faraday Future', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/faradayfuture/jobs/<id>`), wire
  `company_name` pass-through (`'Faraday Future'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/faradayfuture/jobs/<id>`).
