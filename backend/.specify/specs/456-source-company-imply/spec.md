# Spec: 456 — Source Company Plugin: Imply

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 456                            |
| Slug           | source-company-imply               |
| Status         | accepted                               |
...
Imply publishes its careers board through Greenhouse at
the bare slug `imply`. The run-398 batch sweep confirmed
...
- Ship a `source-company-imply` plugin returning live
...
- Backfilling historical Imply postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.IMPLY`** in the
> source registry, so that **a single `siteType: [Site.IMPLY]`
> request returns Imply's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.IMPLY = 'imply'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-imply`.                  | must     |
| FR-3  | `ImplyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `imply-`, `site === Site.IMPLY`. | must |
...
```ts
@SourcePlugin({ site: Site.IMPLY, name: 'Imply', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/imply/jobs/<id>`), wire
  `company_name` pass-through (`'Imply'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/imply/jobs/<id>`).
