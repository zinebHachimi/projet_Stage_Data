# Spec: 566 — Source Company Plugin: One Medical

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 566                            |
| Slug           | source-company-onemedical               |
| Status         | accepted                               |
...
One Medical publishes its careers board through Greenhouse at
the bare slug `onemedical`. The run-398 batch sweep confirmed
293 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/onemedical/jobs?content=true`.
Sector: Healthcare / Primary Care. HQ: New York, New York, USA.

## 2. Goals

- Ship a `source-company-onemedical` plugin returning live
...
- Backfilling historical One Medical postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ONE_MEDICAL`** in the
> source registry, so that **a single `siteType: [Site.ONE_MEDICAL]`
> request returns One Medical's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ONE_MEDICAL = 'onemedical'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-onemedical`.                  | must     |
| FR-3  | `OnemedicalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `onemedical-`, `site === Site.ONE_MEDICAL`. | must |
...
```ts
@SourcePlugin({ site: Site.ONE_MEDICAL, name: 'One Medical', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/onemedical/jobs/<id>`), wire
  `company_name` pass-through (`'One Medical'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/onemedical/jobs/<id>`).
