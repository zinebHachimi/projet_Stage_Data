# Spec: 615 — Source Company Plugin: Overstory

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 615                            |
| Slug           | source-company-overstory               |
| Status         | accepted                               |
...
Overstory publishes its careers board through Greenhouse at
the bare slug `overstory`. The run-398 batch sweep confirmed
...
- Ship a `source-company-overstory` plugin returning live
...
- Backfilling historical Overstory postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OVERSTORY`** in the
> source registry, so that **a single `siteType: [Site.OVERSTORY]`
> request returns Overstory's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OVERSTORY = 'overstory'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-overstory`.                  | must     |
| FR-3  | `OverstoryService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `overstory-`, `site === Site.OVERSTORY`. | must |
...
```ts
@SourcePlugin({ site: Site.OVERSTORY, name: 'Overstory', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/overstory/jobs/<id>`), wire
  `company_name` pass-through (`'Overstory'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/overstory/jobs/<id>`).
