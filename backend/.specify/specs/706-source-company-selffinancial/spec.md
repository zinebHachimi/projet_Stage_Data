# Spec: 706 — Source Company Plugin: Self Financial

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 706                            |
| Slug           | source-company-selffinancial               |
| Status         | accepted                               |
...
Self Financial publishes its careers board through Greenhouse at
the bare slug `selffinancial`. The run-398 batch sweep confirmed
...
- Ship a `source-company-selffinancial` plugin returning live
...
- Backfilling historical Self Financial postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SELF_FINANCIAL`** in the
> source registry, so that **a single `siteType: [Site.SELF_FINANCIAL]`
> request returns Self Financial's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SELF_FINANCIAL = 'selffinancial'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-selffinancial`.                  | must     |
| FR-3  | `SelfFinancialService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `selffinancial-`, `site === Site.SELF_FINANCIAL`. | must |
...
```ts
@SourcePlugin({ site: Site.SELF_FINANCIAL, name: 'Self Financial', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/selffinancial/jobs/<id>`), wire
  `company_name` pass-through (`'Self Financial'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/selffinancial/jobs/<id>`).
