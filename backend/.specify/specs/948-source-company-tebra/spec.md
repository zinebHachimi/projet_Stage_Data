# Spec: 948 — Source Company Plugin: Tebra

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 948                            |
| Slug           | source-company-tebra               |
| Status         | accepted                               |
...
Tebra publishes its careers board through Greenhouse at
the bare slug `tebra`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tebra` plugin returning live
...
- Backfilling historical Tebra postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TEBRA`** in the
> source registry, so that **a single `siteType: [Site.TEBRA]`
> request returns Tebra's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TEBRA = 'tebra'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tebra`.                  | must     |
| FR-3  | `TebraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tebra-`, `site === Site.TEBRA`. | must |
...
```ts
@SourcePlugin({ site: Site.TEBRA, name: 'Tebra', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tebra/jobs/<id>`), wire
  `company_name` pass-through (`'Tebra'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tebra/jobs/<id>`).
