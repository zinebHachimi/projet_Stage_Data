# Spec: 857 — Source Company Plugin: GlossGenius

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 857                            |
| Slug           | source-company-glossgenius               |
| Status         | accepted                               |
...
GlossGenius publishes its careers board through Greenhouse at
the bare slug `glossgenius`. The run-398 batch sweep confirmed
...
- Ship a `source-company-glossgenius` plugin returning live
...
- Backfilling historical GlossGenius postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GLOSSGENIUS`** in the
> source registry, so that **a single `siteType: [Site.GLOSSGENIUS]`
> request returns GlossGenius's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GLOSSGENIUS = 'glossgenius'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-glossgenius`.                  | must     |
| FR-3  | `GlossGeniusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `glossgenius-`, `site === Site.GLOSSGENIUS`. | must |
...
```ts
@SourcePlugin({ site: Site.GLOSSGENIUS, name: 'GlossGenius', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/glossgenius/jobs/<id>`), wire
  `company_name` pass-through (`'GlossGenius'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/glossgenius/jobs/<id>`).
