# Spec: 815 — Source Company Plugin: Axonius

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 815                            |
| Slug           | source-company-axonius               |
| Status         | accepted                               |
...
Axonius publishes its careers board through Greenhouse at
the bare slug `axonius`. The run-398 batch sweep confirmed
...
- Ship a `source-company-axonius` plugin returning live
...
- Backfilling historical Axonius postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AXONIUS`** in the
> source registry, so that **a single `siteType: [Site.AXONIUS]`
> request returns Axonius's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AXONIUS = 'axonius'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-axonius`.                  | must     |
| FR-3  | `AxoniusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `axonius-`, `site === Site.AXONIUS`. | must |
...
```ts
@SourcePlugin({ site: Site.AXONIUS, name: 'Axonius', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/axonius/jobs/<id>`), wire
  `company_name` pass-through (`'Axonius'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/axonius/jobs/<id>`).
