# Spec: 205 — Source Company Plugin: Age Bold

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 205                            |
| Slug           | source-company-agebold               |
| Status         | accepted                               |
...
Age Bold publishes its careers board through Greenhouse at
the bare slug `agebold`. The run-398 batch sweep confirmed
...
- Ship a `source-company-agebold` plugin returning live
...
- Backfilling historical Age Bold postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AGEBOLD`** in the
> source registry, so that **a single `siteType: [Site.AGEBOLD]`
> request returns Age Bold's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AGEBOLD = 'agebold'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-agebold`.                  | must     |
| FR-3  | `AgeboldService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `agebold-`, `site === Site.AGEBOLD`. | must |
...
```ts
@SourcePlugin({ site: Site.AGEBOLD, name: 'Age Bold', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/agebold/jobs/<id>`), wire
  `company_name` pass-through (`'Age Bold'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/agebold/jobs/<id>`).
