# Spec: 511 — Source Company Plugin: TrueLayer

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 511                            |
| Slug           | source-company-truelayer               |
| Status         | accepted                               |
...
TrueLayer publishes its careers board through Greenhouse at
the bare slug `truelayer`. The run-398 batch sweep confirmed
...
- Ship a `source-company-truelayer` plugin returning live
...
- Backfilling historical TrueLayer postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TRUELAYER`** in the
> source registry, so that **a single `siteType: [Site.TRUELAYER]`
> request returns TrueLayer's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TRUELAYER = 'truelayer'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-truelayer`.                  | must     |
| FR-3  | `TruelayerService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `truelayer-`, `site === Site.TRUELAYER`. | must |
...
```ts
@SourcePlugin({ site: Site.TRUELAYER, name: 'TrueLayer', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/truelayer/jobs/<id>`), wire
  `company_name` pass-through (`'TrueLayer'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/truelayer/jobs/<id>`).
