# Spec: 782 — Source Company Plugin: Path Robotics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 782                            |
| Slug           | source-company-pathrobotics               |
| Status         | accepted                               |
...
Path Robotics publishes its careers board through Greenhouse at
the bare slug `pathrobotics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pathrobotics` plugin returning live
...
- Backfilling historical Path Robotics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PATH_ROBOTICS`** in the
> source registry, so that **a single `siteType: [Site.PATH_ROBOTICS]`
> request returns Path Robotics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PATH_ROBOTICS = 'pathrobotics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pathrobotics`.                  | must     |
| FR-3  | `PathRoboticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pathrobotics-`, `site === Site.PATH_ROBOTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.PATH_ROBOTICS, name: 'Path Robotics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pathrobotics/jobs/<id>`), wire
  `company_name` pass-through (`'Path Robotics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pathrobotics/jobs/<id>`).
