# Spec: 583 — Source Company Plugin: World Labs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 583                            |
| Slug           | source-company-worldlabs               |
| Status         | accepted                               |
...
World Labs publishes its careers board through Greenhouse at
the bare slug `worldlabs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-worldlabs` plugin returning live
...
- Backfilling historical World Labs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WORLD_LABS`** in the
> source registry, so that **a single `siteType: [Site.WORLD_LABS]`
> request returns World Labs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WORLD_LABS = 'worldlabs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-worldlabs`.                  | must     |
| FR-3  | `WorldLabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `worldlabs-`, `site === Site.WORLD_LABS`. | must |
...
```ts
@SourcePlugin({ site: Site.WORLD_LABS, name: 'World Labs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/worldlabs/jobs/<id>`), wire
  `company_name` pass-through (`'World Labs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/worldlabs/jobs/<id>`).
