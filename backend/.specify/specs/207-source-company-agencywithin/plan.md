# Plan: 207 — Source Company Plugin: WITHIN
...
WITHIN's careers board is hosted on Greenhouse at the slug
`agencywithin`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-agencywithin` plugin.
- Deliverables: package files, `Site.AGENCYWITHIN` enum value, the
...
- `packages/plugins/source-company-agencywithin` (new)
