# Spec: 214 — Source Company Plugin: Airia

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 214                            |
| Slug           | source-company-airia               |
| Status         | accepted                               |
...
Airia publishes its careers board through Greenhouse at
the bare slug `airia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-airia` plugin returning live
...
- Backfilling historical Airia postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AIRIA`** in the
> source registry, so that **a single `siteType: [Site.AIRIA]`
> request returns Airia's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AIRIA = 'airia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-airia`.                  | must     |
| FR-3  | `AiriaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `airia-`, `site === Site.AIRIA`. | must |
...
```ts
@SourcePlugin({ site: Site.AIRIA, name: 'Airia', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/airia/jobs/<id>`), wire
  `company_name` pass-through (`'Airia'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/airia/jobs/<id>`).
