# Spec: 940 — Source Company Plugin: Splice

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 940                            |
| Slug           | source-company-splice               |
| Status         | accepted                               |
...
Splice publishes its careers board through Greenhouse at
the bare slug `splice`. The run-398 batch sweep confirmed
...
- Ship a `source-company-splice` plugin returning live
...
- Backfilling historical Splice postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPLICE`** in the
> source registry, so that **a single `siteType: [Site.SPLICE]`
> request returns Splice's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPLICE = 'splice'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-splice`.                  | must     |
| FR-3  | `SpliceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `splice-`, `site === Site.SPLICE`. | must |
...
```ts
@SourcePlugin({ site: Site.SPLICE, name: 'Splice', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/splice/jobs/<id>`), wire
  `company_name` pass-through (`'Splice'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/splice/jobs/<id>`).
