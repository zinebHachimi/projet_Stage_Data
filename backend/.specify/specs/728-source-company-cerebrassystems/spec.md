# Spec: 728 — Source Company Plugin: Cerebras Systems

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 728                            |
| Slug           | source-company-cerebrassystems               |
| Status         | accepted                               |
...
Cerebras Systems publishes its careers board through Greenhouse at
the bare slug `cerebrassystems`. The run-398 batch sweep confirmed
99 live role(s) via a direct probe of
...
- Ship a `source-company-cerebrassystems` plugin returning live
...
- Backfilling historical Cerebras Systems postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CEREBRAS_SYSTEMS`** in the
> source registry, so that **a single `siteType: [Site.CEREBRAS_SYSTEMS]`
> request returns Cerebras Systems's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CEREBRAS_SYSTEMS = 'cerebrassystems'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cerebrassystems`.                  | must     |
| FR-3  | `CerebrasSystemsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cerebrassystems-`, `site === Site.CEREBRAS_SYSTEMS`. | must |
...
```ts
@SourcePlugin({ site: Site.CEREBRAS_SYSTEMS, name: 'Cerebras Systems', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cerebrassystems/jobs/<id>`), wire
  `company_name` pass-through (`'Cerebras Systems'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cerebrassystems/jobs/<id>`).
