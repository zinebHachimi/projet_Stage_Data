# Spec: 821 — Source Company Plugin: Cabify

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 821                            |
| Slug           | source-company-cabify               |
| Status         | accepted                               |
...
Cabify publishes its careers board through Greenhouse at
the bare slug `cabify`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cabify` plugin returning live
...
- Backfilling historical Cabify postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CABIFY`** in the
> source registry, so that **a single `siteType: [Site.CABIFY]`
> request returns Cabify's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CABIFY = 'cabify'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cabify`.                  | must     |
| FR-3  | `CabifyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cabify-`, `site === Site.CABIFY`. | must |
...
```ts
@SourcePlugin({ site: Site.CABIFY, name: 'Cabify', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cabify/jobs/<id>`), wire
  `company_name` pass-through (`'Cabify'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cabify/jobs/<id>`).
