# Spec: 250 — Source Company Plugin: AltScore

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 250                            |
| Slug           | source-company-altscore               |
| Status         | accepted                               |
...
AltScore publishes its careers board through Greenhouse at
the bare slug `altscore`. The run-398 batch sweep confirmed
...
- Ship a `source-company-altscore` plugin returning live
...
- Backfilling historical AltScore postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALTSCORE`** in the
> source registry, so that **a single `siteType: [Site.ALTSCORE]`
> request returns AltScore's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALTSCORE = 'altscore'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-altscore`.                  | must     |
| FR-3  | `AltscoreService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `altscore-`, `site === Site.ALTSCORE`. | must |
...
```ts
@SourcePlugin({ site: Site.ALTSCORE, name: 'AltScore', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/altscore/jobs/<id>`), wire
  `company_name` pass-through (`'AltScore'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/altscore/jobs/<id>`).
