# Plan: 941 — Source Company Plugin: SpotHopper
...
SpotHopper's careers board is hosted on Greenhouse at the slug
`spothopper`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-spothopper` plugin.
- Deliverables: package files, `Site.SPOTHOPPER` enum value, the
...
- `packages/plugins/source-company-spothopper` (new)
