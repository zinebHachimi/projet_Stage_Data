# Spec: 941 — Source Company Plugin: SpotHopper

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 941                            |
| Slug           | source-company-spothopper               |
| Status         | accepted                               |
...
SpotHopper publishes its careers board through Greenhouse at
the bare slug `spothopper`. The run-398 batch sweep confirmed
...
- Ship a `source-company-spothopper` plugin returning live
...
- Backfilling historical SpotHopper postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SPOTHOPPER`** in the
> source registry, so that **a single `siteType: [Site.SPOTHOPPER]`
> request returns SpotHopper's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SPOTHOPPER = 'spothopper'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-spothopper`.                  | must     |
| FR-3  | `SpotHopperService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `spothopper-`, `site === Site.SPOTHOPPER`. | must |
...
```ts
@SourcePlugin({ site: Site.SPOTHOPPER, name: 'SpotHopper', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/spothopper/jobs/<id>`), wire
  `company_name` pass-through (`'SpotHopper'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/spothopper/jobs/<id>`).
