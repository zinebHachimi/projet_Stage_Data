# Spec: 547 — Source Company Plugin: TabaPay

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 547                            |
| Slug           | source-company-tabapay               |
| Status         | accepted                               |
...
TabaPay publishes its careers board through Greenhouse at
the bare slug `tabapay`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tabapay` plugin returning live
...
- Backfilling historical TabaPay postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TABAPAY`** in the
> source registry, so that **a single `siteType: [Site.TABAPAY]`
> request returns TabaPay's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TABAPAY = 'tabapay'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tabapay`.                  | must     |
| FR-3  | `TabapayService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tabapay-`, `site === Site.TABAPAY`. | must |
...
```ts
@SourcePlugin({ site: Site.TABAPAY, name: 'TabaPay', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tabapay/jobs/<id>`), wire
  `company_name` pass-through (`'TabaPay'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tabapay/jobs/<id>`).
