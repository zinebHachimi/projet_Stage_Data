# Spec: 265 — Source Company Plugin: Amwell

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 265                            |
| Slug           | source-company-amwell               |
| Status         | accepted                               |
...
Amwell publishes its careers board through Greenhouse at
the bare slug `amwell`. The run-398 batch sweep confirmed
...
- Ship a `source-company-amwell` plugin returning live
...
- Backfilling historical Amwell postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMWELL`** in the
> source registry, so that **a single `siteType: [Site.AMWELL]`
> request returns Amwell's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMWELL = 'amwell'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-amwell`.                  | must     |
| FR-3  | `AmwellService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `amwell-`, `site === Site.AMWELL`. | must |
...
```ts
@SourcePlugin({ site: Site.AMWELL, name: 'Amwell', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/amwell/jobs/<id>`), wire
  `company_name` pass-through (`'Amwell'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/amwell/jobs/<id>`).
