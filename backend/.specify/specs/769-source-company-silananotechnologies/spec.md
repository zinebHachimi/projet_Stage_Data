# Spec: 769 — Source Company Plugin: Sila Nanotechnologies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 769                            |
| Slug           | source-company-silananotechnologies               |
| Status         | accepted                               |
...
Sila Nanotechnologies publishes its careers board through Greenhouse at
the bare slug `silananotechnologies`. The run-398 batch sweep confirmed
27 live role(s) via a direct probe of
...
- Ship a `source-company-silananotechnologies` plugin returning live
...
- Backfilling historical Sila Nanotechnologies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SILA_NANOTECHNOLOGIES`** in the
> source registry, so that **a single `siteType: [Site.SILA_NANOTECHNOLOGIES]`
> request returns Sila Nanotechnologies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SILA_NANOTECHNOLOGIES = 'silananotechnologies'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-silananotechnologies`.                  | must     |
| FR-3  | `SilaNanotechnologiesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `silananotechnologies-`, `site === Site.SILA_NANOTECHNOLOGIES`. | must |
...
```ts
@SourcePlugin({ site: Site.SILA_NANOTECHNOLOGIES, name: 'Sila Nanotechnologies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/silananotechnologies/jobs/<id>`), wire
  `company_name` pass-through (`'Sila Nanotechnologies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/silananotechnologies/jobs/<id>`).
