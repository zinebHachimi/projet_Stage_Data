# Spec: 743 — Source Company Plugin: Figure Lending

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 743                            |
| Slug           | source-company-figure               |
| Status         | accepted                               |
...
Figure Lending publishes its careers board through Greenhouse at
the bare slug `figure`. The run-398 batch sweep confirmed
...
- Ship a `source-company-figure` plugin returning live
...
- Backfilling historical Figure Lending postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FIGURE_LENDING`** in the
> source registry, so that **a single `siteType: [Site.FIGURE_LENDING]`
> request returns Figure Lending's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FIGURE_LENDING = 'figure'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-figure`.                  | must     |
| FR-3  | `FigureLendingService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `figure-`, `site === Site.FIGURE_LENDING`. | must |
...
```ts
@SourcePlugin({ site: Site.FIGURE_LENDING, name: 'Figure Lending', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/figure/jobs/<id>`), wire
  `company_name` pass-through (`'Figure Lending'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/figure/jobs/<id>`).
