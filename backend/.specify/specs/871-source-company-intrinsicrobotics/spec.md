# Spec: 871 — Source Company Plugin: Intrinsic

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 871                            |
| Slug           | source-company-intrinsicrobotics               |
| Status         | accepted                               |
...
Intrinsic publishes its careers board through Greenhouse at
the bare slug `intrinsicrobotics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-intrinsicrobotics` plugin returning live
...
- Backfilling historical Intrinsic postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INTRINSIC`** in the
> source registry, so that **a single `siteType: [Site.INTRINSIC]`
> request returns Intrinsic's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INTRINSIC = 'intrinsicrobotics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-intrinsicrobotics`.                  | must     |
| FR-3  | `IntrinsicService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `intrinsicrobotics-`, `site === Site.INTRINSIC`. | must |
...
```ts
@SourcePlugin({ site: Site.INTRINSIC, name: 'Intrinsic', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/intrinsicrobotics/jobs/<id>`), wire
  `company_name` pass-through (`'Intrinsic'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/intrinsicrobotics/jobs/<id>`).
