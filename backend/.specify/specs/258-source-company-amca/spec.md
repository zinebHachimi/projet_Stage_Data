# Spec: 258 — Source Company Plugin: Amca

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 258                            |
| Slug           | source-company-amca               |
| Status         | accepted                               |
...
Amca publishes its careers board through Greenhouse at
the bare slug `amca`. The run-398 batch sweep confirmed
...
- Ship a `source-company-amca` plugin returning live
...
- Backfilling historical Amca postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMCA`** in the
> source registry, so that **a single `siteType: [Site.AMCA]`
> request returns Amca's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMCA = 'amca'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-amca`.                  | must     |
| FR-3  | `AmcaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `amca-`, `site === Site.AMCA`. | must |
...
```ts
@SourcePlugin({ site: Site.AMCA, name: 'Amca', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/amca/jobs/<id>`), wire
  `company_name` pass-through (`'Amca'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/amca/jobs/<id>`).
