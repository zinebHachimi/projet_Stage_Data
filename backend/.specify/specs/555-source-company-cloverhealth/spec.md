# Spec: 555 — Source Company Plugin: Clover Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 555                            |
| Slug           | source-company-cloverhealth               |
| Status         | accepted                               |
...
Clover Health publishes its careers board through Greenhouse at
the bare slug `cloverhealth`. The run-398 batch sweep confirmed
114 live role(s) via a direct probe of
...
- Ship a `source-company-cloverhealth` plugin returning live
...
- Backfilling historical Clover Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CLOVER_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.CLOVER_HEALTH]`
> request returns Clover Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CLOVER_HEALTH = 'cloverhealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cloverhealth`.                  | must     |
| FR-3  | `CloverhealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cloverhealth-`, `site === Site.CLOVER_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.CLOVER_HEALTH, name: 'Clover Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cloverhealth/jobs/<id>`), wire
  `company_name` pass-through (`'Clover Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cloverhealth/jobs/<id>`).
