# Spec: 766 — Source Company Plugin: Ophelia Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 766                            |
| Slug           | source-company-ophelia               |
| Status         | accepted                               |
...
Ophelia Health publishes its careers board through Greenhouse at
the bare slug `ophelia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ophelia` plugin returning live
...
- Backfilling historical Ophelia Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OPHELIA_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.OPHELIA_HEALTH]`
> request returns Ophelia Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OPHELIA_HEALTH = 'ophelia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ophelia`.                  | must     |
| FR-3  | `OpheliaHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ophelia-`, `site === Site.OPHELIA_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.OPHELIA_HEALTH, name: 'Ophelia Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ophelia/jobs/<id>`), wire
  `company_name` pass-through (`'Ophelia Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ophelia/jobs/<id>`).
