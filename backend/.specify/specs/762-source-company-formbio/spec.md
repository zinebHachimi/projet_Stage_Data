# Spec: 762 — Source Company Plugin: Form Bio

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 762                            |
| Slug           | source-company-formbio               |
| Status         | accepted                               |
...
Form Bio publishes its careers board through Greenhouse at
the bare slug `formbio`. The run-398 batch sweep confirmed
...
- Ship a `source-company-formbio` plugin returning live
...
- Backfilling historical Form Bio postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORM_BIO`** in the
> source registry, so that **a single `siteType: [Site.FORM_BIO]`
> request returns Form Bio's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORM_BIO = 'formbio'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-formbio`.                  | must     |
| FR-3  | `FormBioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `formbio-`, `site === Site.FORM_BIO`. | must |
...
```ts
@SourcePlugin({ site: Site.FORM_BIO, name: 'Form Bio', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/formbio/jobs/<id>`), wire
  `company_name` pass-through (`'Form Bio'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/formbio/jobs/<id>`).
