# Spec: 427 — Source Company Plugin: Gemini

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 427                            |
| Slug           | source-company-gemini               |
| Status         | accepted                               |
...
Gemini publishes its careers board through Greenhouse at
the bare slug `gemini`. The run-398 batch sweep confirmed
...
- Ship a `source-company-gemini` plugin returning live
...
- Backfilling historical Gemini postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GEMINI`** in the
> source registry, so that **a single `siteType: [Site.GEMINI]`
> request returns Gemini's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GEMINI = 'gemini'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-gemini`.                  | must     |
| FR-3  | `GeminiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `gemini-`, `site === Site.GEMINI`. | must |
...
```ts
@SourcePlugin({ site: Site.GEMINI, name: 'Gemini', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/gemini/jobs/<id>`), wire
  `company_name` pass-through (`'Gemini'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/gemini/jobs/<id>`).
