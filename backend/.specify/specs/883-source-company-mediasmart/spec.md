# Spec: 883 — Source Company Plugin: mediasmart

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 883                            |
| Slug           | source-company-mediasmart               |
| Status         | accepted                               |
...
mediasmart publishes its careers board through Greenhouse at
the bare slug `mediasmart`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mediasmart` plugin returning live
...
- Backfilling historical mediasmart postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MEDIASMART`** in the
> source registry, so that **a single `siteType: [Site.MEDIASMART]`
> request returns mediasmart's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MEDIASMART = 'mediasmart'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mediasmart`.                  | must     |
| FR-3  | `MediasmartService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mediasmart-`, `site === Site.MEDIASMART`. | must |
...
```ts
@SourcePlugin({ site: Site.MEDIASMART, name: 'mediasmart', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mediasmart/jobs/<id>`), wire
  `company_name` pass-through (`'mediasmart'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mediasmart/jobs/<id>`).
