# Spec: 291 — Source Company Plugin: Appfire

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 291                            |
| Slug           | source-company-appfire               |
| Status         | accepted                               |
...
Appfire publishes its careers board through Greenhouse at
the bare slug `appfire`. The run-398 batch sweep confirmed
...
- Ship a `source-company-appfire` plugin returning live
...
- Backfilling historical Appfire postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APPFIRE`** in the
> source registry, so that **a single `siteType: [Site.APPFIRE]`
> request returns Appfire's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APPFIRE = 'appfire'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-appfire`.                  | must     |
| FR-3  | `AppfireService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `appfire-`, `site === Site.APPFIRE`. | must |
...
```ts
@SourcePlugin({ site: Site.APPFIRE, name: 'Appfire', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/appfire/jobs/<id>`), wire
  `company_name` pass-through (`'Appfire'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/appfire/jobs/<id>`).
