# Spec: 753 — Source Company Plugin: Ophelos

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 753                            |
| Slug           | source-company-ophelos               |
| Status         | accepted                               |
...
Ophelos publishes its careers board through Greenhouse at
the bare slug `ophelos`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ophelos` plugin returning live
...
- Backfilling historical Ophelos postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OPHELOS`** in the
> source registry, so that **a single `siteType: [Site.OPHELOS]`
> request returns Ophelos's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OPHELOS = 'ophelos'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ophelos`.                  | must     |
| FR-3  | `OphelosService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ophelos-`, `site === Site.OPHELOS`. | must |
...
```ts
@SourcePlugin({ site: Site.OPHELOS, name: 'Ophelos', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ophelos/jobs/<id>`), wire
  `company_name` pass-through (`'Ophelos'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ophelos/jobs/<id>`).
