# Spec: 479 — Source Company Plugin: Galileo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 479                            |
| Slug           | source-company-galileo               |
| Status         | accepted                               |
...
Galileo publishes its careers board through Greenhouse at
the bare slug `galileo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-galileo` plugin returning live
...
- Backfilling historical Galileo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GALILEO`** in the
> source registry, so that **a single `siteType: [Site.GALILEO]`
> request returns Galileo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GALILEO = 'galileo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-galileo`.                  | must     |
| FR-3  | `GalileoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `galileo-`, `site === Site.GALILEO`. | must |
...
```ts
@SourcePlugin({ site: Site.GALILEO, name: 'Galileo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/galileo/jobs/<id>`), wire
  `company_name` pass-through (`'Galileo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/galileo/jobs/<id>`).
