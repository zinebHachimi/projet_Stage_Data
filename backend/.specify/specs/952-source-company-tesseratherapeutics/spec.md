# Spec: 952 — Source Company Plugin: Tessera Therapeutics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 952                            |
| Slug           | source-company-tesseratherapeutics               |
| Status         | accepted                               |
...
Tessera Therapeutics publishes its careers board through Greenhouse at
the bare slug `tesseratherapeutics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tesseratherapeutics` plugin returning live
...
- Backfilling historical Tessera Therapeutics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TESSERA_THERAPEUTICS`** in the
> source registry, so that **a single `siteType: [Site.TESSERA_THERAPEUTICS]`
> request returns Tessera Therapeutics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TESSERA_THERAPEUTICS = 'tesseratherapeutics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tesseratherapeutics`.                  | must     |
| FR-3  | `TesseraTherapeuticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tesseratherapeutics-`, `site === Site.TESSERA_THERAPEUTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.TESSERA_THERAPEUTICS, name: 'Tessera Therapeutics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tesseratherapeutics/jobs/<id>`), wire
  `company_name` pass-through (`'Tessera Therapeutics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tesseratherapeutics/jobs/<id>`).
