# Spec: 754 — Source Company Plugin: Oportun

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 754                            |
| Slug           | source-company-oportun               |
| Status         | accepted                               |
...
Oportun publishes its careers board through Greenhouse at
the bare slug `oportun`. The run-398 batch sweep confirmed
...
- Ship a `source-company-oportun` plugin returning live
...
- Backfilling historical Oportun postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OPORTUN`** in the
> source registry, so that **a single `siteType: [Site.OPORTUN]`
> request returns Oportun's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OPORTUN = 'oportun'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-oportun`.                  | must     |
| FR-3  | `OportunService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `oportun-`, `site === Site.OPORTUN`. | must |
...
```ts
@SourcePlugin({ site: Site.OPORTUN, name: 'Oportun', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/oportun/jobs/<id>`), wire
  `company_name` pass-through (`'Oportun'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/oportun/jobs/<id>`).
