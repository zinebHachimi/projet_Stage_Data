# Plan: 440 — Source Company Plugin: YugabyteDB
...
YugabyteDB's careers board is hosted on Greenhouse at the slug
`yugabyte`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-yugabyte` plugin.
- Deliverables: package files, `Site.YUGABYTE` enum value, the
...
- `packages/plugins/source-company-yugabyte` (new)
