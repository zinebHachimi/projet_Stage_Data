# Spec: 440 — Source Company Plugin: YugabyteDB

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 440                            |
| Slug           | source-company-yugabyte               |
| Status         | accepted                               |
...
YugabyteDB publishes its careers board through Greenhouse at
the bare slug `yugabyte`. The run-398 batch sweep confirmed
...
- Ship a `source-company-yugabyte` plugin returning live
...
- Backfilling historical YugabyteDB postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.YUGABYTE`** in the
> source registry, so that **a single `siteType: [Site.YUGABYTE]`
> request returns YugabyteDB's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.YUGABYTE = 'yugabyte'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-yugabyte`.                  | must     |
| FR-3  | `YugabyteService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `yugabyte-`, `site === Site.YUGABYTE`. | must |
...
```ts
@SourcePlugin({ site: Site.YUGABYTE, name: 'YugabyteDB', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/yugabyte/jobs/<id>`), wire
  `company_name` pass-through (`'YugabyteDB'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/yugabyte/jobs/<id>`).
