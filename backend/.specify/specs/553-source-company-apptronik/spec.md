# Spec: 553 — Source Company Plugin: Apptronik

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 553                            |
| Slug           | source-company-apptronik               |
| Status         | accepted                               |
...
Apptronik publishes its careers board through Greenhouse at
the bare slug `apptronik`. The run-398 batch sweep confirmed
...
- Ship a `source-company-apptronik` plugin returning live
...
- Backfilling historical Apptronik postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APPTRONIK`** in the
> source registry, so that **a single `siteType: [Site.APPTRONIK]`
> request returns Apptronik's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APPTRONIK = 'apptronik'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-apptronik`.                  | must     |
| FR-3  | `ApptronikService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `apptronik-`, `site === Site.APPTRONIK`. | must |
...
```ts
@SourcePlugin({ site: Site.APPTRONIK, name: 'Apptronik', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/apptronik/jobs/<id>`), wire
  `company_name` pass-through (`'Apptronik'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/apptronik/jobs/<id>`).
