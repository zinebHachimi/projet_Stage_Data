# Spec: 466 — Source Company Plugin: Turing

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 466                            |
| Slug           | source-company-turing               |
| Status         | accepted                               |
...
Turing publishes its careers board through Greenhouse at
the bare slug `turing`. The run-398 batch sweep confirmed
...
- Ship a `source-company-turing` plugin returning live
...
- Backfilling historical Turing postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TURING`** in the
> source registry, so that **a single `siteType: [Site.TURING]`
> request returns Turing's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TURING = 'turing'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-turing`.                  | must     |
| FR-3  | `TuringService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `turing-`, `site === Site.TURING`. | must |
...
```ts
@SourcePlugin({ site: Site.TURING, name: 'Turing', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/turing/jobs/<id>`), wire
  `company_name` pass-through (`'Turing'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/turing/jobs/<id>`).
