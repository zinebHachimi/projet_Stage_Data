# Spec: 578 — Source Company Plugin: Groww

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 578                            |
| Slug           | source-company-groww               |
| Status         | accepted                               |
...
Groww publishes its careers board through Greenhouse at
the bare slug `groww`. The run-398 batch sweep confirmed
...
- Ship a `source-company-groww` plugin returning live
...
- Backfilling historical Groww postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GROWW`** in the
> source registry, so that **a single `siteType: [Site.GROWW]`
> request returns Groww's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GROWW = 'groww'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-groww`.                  | must     |
| FR-3  | `GrowwService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `groww-`, `site === Site.GROWW`. | must |
...
```ts
@SourcePlugin({ site: Site.GROWW, name: 'Groww', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/groww/jobs/<id>`), wire
  `company_name` pass-through (`'Groww'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/groww/jobs/<id>`).
