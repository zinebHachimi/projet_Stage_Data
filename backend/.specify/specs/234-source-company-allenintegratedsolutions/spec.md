# Spec: 234 — Source Company Plugin: Allen Integrated Solutions

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 234                            |
| Slug           | source-company-allenintegratedsolutions               |
| Status         | accepted                               |
...
Allen Integrated Solutions publishes its careers board through Greenhouse at
the bare slug `allenintegratedsolutions`. The run-398 batch sweep confirmed
54 live role(s) via a direct probe of
...
- Ship a `source-company-allenintegratedsolutions` plugin returning live
...
- Backfilling historical Allen Integrated Solutions postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALLENINTEGRATEDSOLUTIONS`** in the
> source registry, so that **a single `siteType: [Site.ALLENINTEGRATEDSOLUTIONS]`
> request returns Allen Integrated Solutions's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALLENINTEGRATEDSOLUTIONS = 'allenintegratedsolutions'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-allenintegratedsolutions`.                  | must     |
| FR-3  | `AllenintegratedsolutionsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `allenintegratedsolutions-`, `site === Site.ALLENINTEGRATEDSOLUTIONS`. | must |
...
```ts
@SourcePlugin({ site: Site.ALLENINTEGRATEDSOLUTIONS, name: 'Allen Integrated Solutions', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/allenintegratedsolutions/jobs/<id>`), wire
  `company_name` pass-through (`'Allen Integrated Solutions'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/allenintegratedsolutions/jobs/<id>`).
