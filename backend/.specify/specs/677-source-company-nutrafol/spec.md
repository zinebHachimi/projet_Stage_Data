# Spec: 677 — Source Company Plugin: Nutrafol

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 677                            |
| Slug           | source-company-nutrafol               |
| Status         | accepted                               |
...
Nutrafol publishes its careers board through Greenhouse at
the bare slug `nutrafol`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nutrafol` plugin returning live
...
- Backfilling historical Nutrafol postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NUTRAFOL`** in the
> source registry, so that **a single `siteType: [Site.NUTRAFOL]`
> request returns Nutrafol's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NUTRAFOL = 'nutrafol'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nutrafol`.                  | must     |
| FR-3  | `NutrafolService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nutrafol-`, `site === Site.NUTRAFOL`. | must |
...
```ts
@SourcePlugin({ site: Site.NUTRAFOL, name: 'Nutrafol', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nutrafol/jobs/<id>`), wire
  `company_name` pass-through (`'Nutrafol'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nutrafol/jobs/<id>`).
