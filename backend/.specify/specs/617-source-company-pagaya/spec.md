# Spec: 617 — Source Company Plugin: Pagaya

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 617                            |
| Slug           | source-company-pagaya               |
| Status         | accepted                               |
...
Pagaya publishes its careers board through Greenhouse at
the bare slug `pagaya`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pagaya` plugin returning live
...
- Backfilling historical Pagaya postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PAGAYA`** in the
> source registry, so that **a single `siteType: [Site.PAGAYA]`
> request returns Pagaya's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PAGAYA = 'pagaya'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pagaya`.                  | must     |
| FR-3  | `PagayaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pagaya-`, `site === Site.PAGAYA`. | must |
...
```ts
@SourcePlugin({ site: Site.PAGAYA, name: 'Pagaya', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pagaya/jobs/<id>`), wire
  `company_name` pass-through (`'Pagaya'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pagaya/jobs/<id>`).
