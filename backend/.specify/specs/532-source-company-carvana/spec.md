# Spec: 532 — Source Company Plugin: Carvana

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 532                            |
| Slug           | source-company-carvana               |
| Status         | accepted                               |
...
Carvana publishes its careers board through Greenhouse at
the bare slug `carvana`. The run-398 batch sweep confirmed
2220 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/carvana/jobs?content=true`.
Sector: E-commerce / Auto Retail. HQ: Tempe, Arizona, USA.

## 2. Goals

- Ship a `source-company-carvana` plugin returning live
...
- Backfilling historical Carvana postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CARVANA`** in the
> source registry, so that **a single `siteType: [Site.CARVANA]`
> request returns Carvana's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CARVANA = 'carvana'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-carvana`.                  | must     |
| FR-3  | `CarvanaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `carvana-`, `site === Site.CARVANA`. | must |
...
```ts
@SourcePlugin({ site: Site.CARVANA, name: 'Carvana', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/carvana/jobs/<id>`), wire
  `company_name` pass-through (`'Carvana'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/carvana/jobs/<id>`).
