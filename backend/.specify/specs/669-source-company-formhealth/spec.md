# Spec: 669 — Source Company Plugin: Form Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 669                            |
| Slug           | source-company-formhealth               |
| Status         | accepted                               |
...
Form Health publishes its careers board through Greenhouse at
the bare slug `formhealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-formhealth` plugin returning live
...
- Backfilling historical Form Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORM_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.FORM_HEALTH]`
> request returns Form Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORM_HEALTH = 'formhealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-formhealth`.                  | must     |
| FR-3  | `FormHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `formhealth-`, `site === Site.FORM_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.FORM_HEALTH, name: 'Form Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/formhealth/jobs/<id>`), wire
  `company_name` pass-through (`'Form Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/formhealth/jobs/<id>`).
