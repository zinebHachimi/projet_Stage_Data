# Spec: 955 — Source Company Plugin: The Trade Desk

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 955                            |
| Slug           | source-company-thetradedesk               |
| Status         | accepted                               |
...
The Trade Desk publishes its careers board through Greenhouse at
the bare slug `thetradedesk`. The run-398 batch sweep confirmed
199 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/thetradedesk/jobs?content=true`.
Sector: Adtech (DSP). HQ: Ventura, California, United States.

## 2. Goals

- Ship a `source-company-thetradedesk` plugin returning live
...
- Backfilling historical The Trade Desk postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THE_TRADE_DESK`** in the
> source registry, so that **a single `siteType: [Site.THE_TRADE_DESK]`
> request returns The Trade Desk's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THE_TRADE_DESK = 'thetradedesk'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thetradedesk`.                  | must     |
| FR-3  | `TheTradeDeskService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thetradedesk-`, `site === Site.THE_TRADE_DESK`. | must |
...
```ts
@SourcePlugin({ site: Site.THE_TRADE_DESK, name: 'The Trade Desk', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thetradedesk/jobs/<id>`), wire
  `company_name` pass-through (`'The Trade Desk'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thetradedesk/jobs/<id>`).
