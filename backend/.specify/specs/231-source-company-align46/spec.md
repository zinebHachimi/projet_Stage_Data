# Spec: 231 — Source Company Plugin: Align Communications

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 231                            |
| Slug           | source-company-align46               |
| Status         | accepted                               |
...
Align Communications publishes its careers board through Greenhouse at
the bare slug `align46`. The run-398 batch sweep confirmed
...
- Ship a `source-company-align46` plugin returning live
...
- Backfilling historical Align Communications postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALIGN46`** in the
> source registry, so that **a single `siteType: [Site.ALIGN46]`
> request returns Align Communications's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALIGN46 = 'align46'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-align46`.                  | must     |
| FR-3  | `Align46Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `align46-`, `site === Site.ALIGN46`. | must |
...
```ts
@SourcePlugin({ site: Site.ALIGN46, name: 'Align Communications', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/align46/jobs/<id>`), wire
  `company_name` pass-through (`'Align Communications'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/align46/jobs/<id>`).
