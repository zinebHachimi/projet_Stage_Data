# Spec: 947 — Source Company Plugin: Take-Two Interactive

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 947                            |
| Slug           | source-company-taketwo               |
| Status         | accepted                               |
...
Take-Two Interactive publishes its careers board through Greenhouse at
the bare slug `taketwo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-taketwo` plugin returning live
...
- Backfilling historical Take-Two Interactive postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TAKE_TWO_INTERACTIVE`** in the
> source registry, so that **a single `siteType: [Site.TAKE_TWO_INTERACTIVE]`
> request returns Take-Two Interactive's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TAKE_TWO_INTERACTIVE = 'taketwo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-taketwo`.                  | must     |
| FR-3  | `TakeTwoInteractiveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `taketwo-`, `site === Site.TAKE_TWO_INTERACTIVE`. | must |
...
```ts
@SourcePlugin({ site: Site.TAKE_TWO_INTERACTIVE, name: 'Take-Two Interactive', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/taketwo/jobs/<id>`), wire
  `company_name` pass-through (`'Take-Two Interactive'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/taketwo/jobs/<id>`).
