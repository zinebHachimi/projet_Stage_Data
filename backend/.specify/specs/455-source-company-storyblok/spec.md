# Spec: 455 — Source Company Plugin: Storyblok

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 455                            |
| Slug           | source-company-storyblok               |
| Status         | accepted                               |
...
Storyblok publishes its careers board through Greenhouse at
the bare slug `storyblok`. The run-398 batch sweep confirmed
...
- Ship a `source-company-storyblok` plugin returning live
...
- Backfilling historical Storyblok postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STORYBLOK`** in the
> source registry, so that **a single `siteType: [Site.STORYBLOK]`
> request returns Storyblok's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STORYBLOK = 'storyblok'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-storyblok`.                  | must     |
| FR-3  | `StoryblokService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `storyblok-`, `site === Site.STORYBLOK`. | must |
...
```ts
@SourcePlugin({ site: Site.STORYBLOK, name: 'Storyblok', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/storyblok/jobs/<id>`), wire
  `company_name` pass-through (`'Storyblok'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/storyblok/jobs/<id>`).
