# Spec: 930 — Source Company Plugin: Shift Technology

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 930                            |
| Slug           | source-company-shifttechnology               |
| Status         | accepted                               |
...
Shift Technology publishes its careers board through Greenhouse at
the bare slug `shifttechnology`. The run-398 batch sweep confirmed
...
- Ship a `source-company-shifttechnology` plugin returning live
...
- Backfilling historical Shift Technology postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SHIFT_TECHNOLOGY`** in the
> source registry, so that **a single `siteType: [Site.SHIFT_TECHNOLOGY]`
> request returns Shift Technology's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SHIFT_TECHNOLOGY = 'shifttechnology'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-shifttechnology`.                  | must     |
| FR-3  | `ShiftTechnologyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `shifttechnology-`, `site === Site.SHIFT_TECHNOLOGY`. | must |
...
```ts
@SourcePlugin({ site: Site.SHIFT_TECHNOLOGY, name: 'Shift Technology', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/shifttechnology/jobs/<id>`), wire
  `company_name` pass-through (`'Shift Technology'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/shifttechnology/jobs/<id>`).
