# Spec: 935 — Source Company Plugin: Snorkel AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 935                            |
| Slug           | source-company-snorkelai               |
| Status         | accepted                               |
...
Snorkel AI publishes its careers board through Greenhouse at
the bare slug `snorkelai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-snorkelai` plugin returning live
...
- Backfilling historical Snorkel AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SNORKEL_AI`** in the
> source registry, so that **a single `siteType: [Site.SNORKEL_AI]`
> request returns Snorkel AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SNORKEL_AI = 'snorkelai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-snorkelai`.                  | must     |
| FR-3  | `SnorkelAIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `snorkelai-`, `site === Site.SNORKEL_AI`. | must |
...
```ts
@SourcePlugin({ site: Site.SNORKEL_AI, name: 'Snorkel AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/snorkelai/jobs/<id>`), wire
  `company_name` pass-through (`'Snorkel AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/snorkelai/jobs/<id>`).
