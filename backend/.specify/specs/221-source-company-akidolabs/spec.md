# Spec: 221 — Source Company Plugin: Akido

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 221                            |
| Slug           | source-company-akidolabs               |
| Status         | accepted                               |
...
Akido publishes its careers board through Greenhouse at
the bare slug `akidolabs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-akidolabs` plugin returning live
...
- Backfilling historical Akido postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AKIDOLABS`** in the
> source registry, so that **a single `siteType: [Site.AKIDOLABS]`
> request returns Akido's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AKIDOLABS = 'akidolabs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-akidolabs`.                  | must     |
| FR-3  | `AkidolabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `akidolabs-`, `site === Site.AKIDOLABS`. | must |
...
```ts
@SourcePlugin({ site: Site.AKIDOLABS, name: 'Akido', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/akidolabs/jobs/<id>`), wire
  `company_name` pass-through (`'Akido'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/akidolabs/jobs/<id>`).
