# Spec: 609 — Source Company Plugin: ON.energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 609                            |
| Slug           | source-company-onenergy               |
| Status         | accepted                               |
...
ON.energy publishes its careers board through Greenhouse at
the bare slug `onenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-onenergy` plugin returning live
...
- Backfilling historical ON.energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ON_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.ON_ENERGY]`
> request returns ON.energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ON_ENERGY = 'onenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-onenergy`.                  | must     |
| FR-3  | `OnEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `onenergy-`, `site === Site.ON_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.ON_ENERGY, name: 'ON.energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/onenergy/jobs/<id>`), wire
  `company_name` pass-through (`'ON.energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/onenergy/jobs/<id>`).
