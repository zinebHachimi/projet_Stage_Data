# Plan: 609 — Source Company Plugin: ON.energy
...
ON.energy's careers board is hosted on Greenhouse at the slug
`onenergy`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-onenergy` plugin.
- Deliverables: package files, `Site.ON_ENERGY` enum value, the
...
- `packages/plugins/source-company-onenergy` (new)
