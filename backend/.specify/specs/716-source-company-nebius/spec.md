# Spec: 716 — Source Company Plugin: Nebius

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 716                            |
| Slug           | source-company-nebius               |
| Status         | accepted                               |
...
Nebius publishes its careers board through Greenhouse at
the bare slug `nebius`. The run-398 batch sweep confirmed
321 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/nebius/jobs?content=true`.
Sector: AI Cloud Infrastructure. HQ: Amsterdam, Netherlands.

## 2. Goals

- Ship a `source-company-nebius` plugin returning live
...
- Backfilling historical Nebius postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEBIUS`** in the
> source registry, so that **a single `siteType: [Site.NEBIUS]`
> request returns Nebius's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEBIUS = 'nebius'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nebius`.                  | must     |
| FR-3  | `NebiusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nebius-`, `site === Site.NEBIUS`. | must |
...
```ts
@SourcePlugin({ site: Site.NEBIUS, name: 'Nebius', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nebius/jobs/<id>`), wire
  `company_name` pass-through (`'Nebius'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nebius/jobs/<id>`).
