# Spec: 833 — Source Company Plugin: dbt Labs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 833                            |
| Slug           | source-company-dbtlabsinc               |
| Status         | accepted                               |
...
dbt Labs publishes its careers board through Greenhouse at
the bare slug `dbtlabsinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-dbtlabsinc` plugin returning live
...
- Backfilling historical dbt Labs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DBT_LABS`** in the
> source registry, so that **a single `siteType: [Site.DBT_LABS]`
> request returns dbt Labs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DBT_LABS = 'dbtlabsinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-dbtlabsinc`.                  | must     |
| FR-3  | `DbtLabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `dbtlabsinc-`, `site === Site.DBT_LABS`. | must |
...
```ts
@SourcePlugin({ site: Site.DBT_LABS, name: 'dbt Labs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/dbtlabsinc/jobs/<id>`), wire
  `company_name` pass-through (`'dbt Labs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/dbtlabsinc/jobs/<id>`).
