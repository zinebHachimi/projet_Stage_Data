# Spec: 685 — Source Company Plugin: ChargePoint

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 685                            |
| Slug           | source-company-chargepoint               |
| Status         | accepted                               |
...
ChargePoint publishes its careers board through Greenhouse at
the bare slug `chargepoint`. The run-398 batch sweep confirmed
...
- Ship a `source-company-chargepoint` plugin returning live
...
- Backfilling historical ChargePoint postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CHARGEPOINT`** in the
> source registry, so that **a single `siteType: [Site.CHARGEPOINT]`
> request returns ChargePoint's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CHARGEPOINT = 'chargepoint'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-chargepoint`.                  | must     |
| FR-3  | `ChargePointService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `chargepoint-`, `site === Site.CHARGEPOINT`. | must |
...
```ts
@SourcePlugin({ site: Site.CHARGEPOINT, name: 'ChargePoint', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/chargepoint/jobs/<id>`), wire
  `company_name` pass-through (`'ChargePoint'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/chargepoint/jobs/<id>`).
