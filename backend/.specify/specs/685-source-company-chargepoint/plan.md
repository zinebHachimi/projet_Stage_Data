# Plan: 685 — Source Company Plugin: ChargePoint
...
ChargePoint's careers board is hosted on Greenhouse at the slug
`chargepoint`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-chargepoint` plugin.
- Deliverables: package files, `Site.CHARGEPOINT` enum value, the
...
- `packages/plugins/source-company-chargepoint` (new)
