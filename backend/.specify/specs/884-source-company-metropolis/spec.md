# Spec: 884 — Source Company Plugin: Metropolis Technologies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 884                            |
| Slug           | source-company-metropolis               |
| Status         | accepted                               |
...
Metropolis Technologies publishes its careers board through Greenhouse at
the bare slug `metropolis`. The run-398 batch sweep confirmed
129 live role(s) via a direct probe of
...
- Ship a `source-company-metropolis` plugin returning live
...
- Backfilling historical Metropolis Technologies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.METROPOLIS_TECHNOLOGIES`** in the
> source registry, so that **a single `siteType: [Site.METROPOLIS_TECHNOLOGIES]`
> request returns Metropolis Technologies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.METROPOLIS_TECHNOLOGIES = 'metropolis'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-metropolis`.                  | must     |
| FR-3  | `MetropolisTechnologiesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `metropolis-`, `site === Site.METROPOLIS_TECHNOLOGIES`. | must |
...
```ts
@SourcePlugin({ site: Site.METROPOLIS_TECHNOLOGIES, name: 'Metropolis Technologies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/metropolis/jobs/<id>`), wire
  `company_name` pass-through (`'Metropolis Technologies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/metropolis/jobs/<id>`).
