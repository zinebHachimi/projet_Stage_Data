# Spec: 629 — Source Company Plugin: Openly

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 629                            |
| Slug           | source-company-openly               |
| Status         | accepted                               |
...
Openly publishes its careers board through Greenhouse at
the bare slug `openly`. The run-398 batch sweep confirmed
...
- Ship a `source-company-openly` plugin returning live
...
- Backfilling historical Openly postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OPENLY`** in the
> source registry, so that **a single `siteType: [Site.OPENLY]`
> request returns Openly's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OPENLY = 'openly'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-openly`.                  | must     |
| FR-3  | `OpenlyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `openly-`, `site === Site.OPENLY`. | must |
...
```ts
@SourcePlugin({ site: Site.OPENLY, name: 'Openly', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/openly/jobs/<id>`), wire
  `company_name` pass-through (`'Openly'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/openly/jobs/<id>`).
