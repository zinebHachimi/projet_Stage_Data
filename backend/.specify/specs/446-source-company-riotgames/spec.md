# Spec: 446 — Source Company Plugin: Riot Games

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 446                            |
| Slug           | source-company-riotgames               |
| Status         | accepted                               |
...
Riot Games publishes its careers board through Greenhouse at
the bare slug `riotgames`. The run-398 batch sweep confirmed
187 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/riotgames/jobs?content=true`.
Sector: Video games & esports. HQ: Los Angeles, CA, USA.

## 2. Goals

- Ship a `source-company-riotgames` plugin returning live
...
- Backfilling historical Riot Games postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RIOTGAMES`** in the
> source registry, so that **a single `siteType: [Site.RIOTGAMES]`
> request returns Riot Games's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RIOTGAMES = 'riotgames'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-riotgames`.                  | must     |
| FR-3  | `RiotgamesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `riotgames-`, `site === Site.RIOTGAMES`. | must |
...
```ts
@SourcePlugin({ site: Site.RIOTGAMES, name: 'Riot Games', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/riotgames/jobs/<id>`), wire
  `company_name` pass-through (`'Riot Games'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/riotgames/jobs/<id>`).
