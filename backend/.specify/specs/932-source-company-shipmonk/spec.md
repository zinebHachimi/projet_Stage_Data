# Spec: 932 — Source Company Plugin: ShipMonk

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 932                            |
| Slug           | source-company-shipmonk               |
| Status         | accepted                               |
...
ShipMonk publishes its careers board through Greenhouse at
the bare slug `shipmonk`. The run-398 batch sweep confirmed
...
- Ship a `source-company-shipmonk` plugin returning live
...
- Backfilling historical ShipMonk postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SHIPMONK`** in the
> source registry, so that **a single `siteType: [Site.SHIPMONK]`
> request returns ShipMonk's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SHIPMONK = 'shipmonk'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-shipmonk`.                  | must     |
| FR-3  | `ShipMonkService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `shipmonk-`, `site === Site.SHIPMONK`. | must |
...
```ts
@SourcePlugin({ site: Site.SHIPMONK, name: 'ShipMonk', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/shipmonk/jobs/<id>`), wire
  `company_name` pass-through (`'ShipMonk'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/shipmonk/jobs/<id>`).
