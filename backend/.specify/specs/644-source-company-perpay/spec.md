# Spec: 644 — Source Company Plugin: Perpay

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 644                            |
| Slug           | source-company-perpay               |
| Status         | accepted                               |
...
Perpay publishes its careers board through Greenhouse at
the bare slug `perpay`. The run-398 batch sweep confirmed
...
- Ship a `source-company-perpay` plugin returning live
...
- Backfilling historical Perpay postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PERPAY`** in the
> source registry, so that **a single `siteType: [Site.PERPAY]`
> request returns Perpay's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PERPAY = 'perpay'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-perpay`.                  | must     |
| FR-3  | `PerpayService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `perpay-`, `site === Site.PERPAY`. | must |
...
```ts
@SourcePlugin({ site: Site.PERPAY, name: 'Perpay', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/perpay/jobs/<id>`), wire
  `company_name` pass-through (`'Perpay'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/perpay/jobs/<id>`).
