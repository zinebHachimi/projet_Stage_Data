# Spec: 432 — Source Company Plugin: Cockroach Labs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 432                            |
| Slug           | source-company-cockroachlabs               |
| Status         | accepted                               |
...
Cockroach Labs publishes its careers board through Greenhouse at
the bare slug `cockroachlabs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cockroachlabs` plugin returning live
...
- Backfilling historical Cockroach Labs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COCKROACHLABS`** in the
> source registry, so that **a single `siteType: [Site.COCKROACHLABS]`
> request returns Cockroach Labs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COCKROACHLABS = 'cockroachlabs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cockroachlabs`.                  | must     |
| FR-3  | `CockroachlabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cockroachlabs-`, `site === Site.COCKROACHLABS`. | must |
...
```ts
@SourcePlugin({ site: Site.COCKROACHLABS, name: 'Cockroach Labs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cockroachlabs/jobs/<id>`), wire
  `company_name` pass-through (`'Cockroach Labs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cockroachlabs/jobs/<id>`).
