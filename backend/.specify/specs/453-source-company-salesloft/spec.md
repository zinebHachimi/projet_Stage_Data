# Spec: 453 — Source Company Plugin: Salesloft

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 453                            |
| Slug           | source-company-salesloft               |
| Status         | accepted                               |
...
Salesloft publishes its careers board through Greenhouse at
the bare slug `salesloft`. The run-398 batch sweep confirmed
...
- Ship a `source-company-salesloft` plugin returning live
...
- Backfilling historical Salesloft postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SALESLOFT`** in the
> source registry, so that **a single `siteType: [Site.SALESLOFT]`
> request returns Salesloft's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SALESLOFT = 'salesloft'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-salesloft`.                  | must     |
| FR-3  | `SalesloftService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `salesloft-`, `site === Site.SALESLOFT`. | must |
...
```ts
@SourcePlugin({ site: Site.SALESLOFT, name: 'Salesloft', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/salesloft/jobs/<id>`), wire
  `company_name` pass-through (`'Salesloft'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/salesloft/jobs/<id>`).
