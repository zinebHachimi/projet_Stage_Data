# Spec: 839 — Source Company Plugin: Eleventh Hour Games

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 839                            |
| Slug           | source-company-eleventhhourgames               |
| Status         | accepted                               |
...
Eleventh Hour Games publishes its careers board through Greenhouse at
the bare slug `eleventhhourgames`. The run-398 batch sweep confirmed
...
- Ship a `source-company-eleventhhourgames` plugin returning live
...
- Backfilling historical Eleventh Hour Games postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ELEVENTH_HOUR_GAMES`** in the
> source registry, so that **a single `siteType: [Site.ELEVENTH_HOUR_GAMES]`
> request returns Eleventh Hour Games's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ELEVENTH_HOUR_GAMES = 'eleventhhourgames'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-eleventhhourgames`.                  | must     |
| FR-3  | `EleventhHourGamesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `eleventhhourgames-`, `site === Site.ELEVENTH_HOUR_GAMES`. | must |
...
```ts
@SourcePlugin({ site: Site.ELEVENTH_HOUR_GAMES, name: 'Eleventh Hour Games', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/eleventhhourgames/jobs/<id>`), wire
  `company_name` pass-through (`'Eleventh Hour Games'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/eleventhhourgames/jobs/<id>`).
