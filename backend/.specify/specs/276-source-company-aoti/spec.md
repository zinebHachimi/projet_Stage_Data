# Spec: 276 — Source Company Plugin: AOTI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 276                            |
| Slug           | source-company-aoti               |
| Status         | accepted                               |
...
AOTI publishes its careers board through Greenhouse at
the bare slug `aoti`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aoti` plugin returning live
...
- Backfilling historical AOTI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AOTI`** in the
> source registry, so that **a single `siteType: [Site.AOTI]`
> request returns AOTI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AOTI = 'aoti'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aoti`.                  | must     |
| FR-3  | `AotiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aoti-`, `site === Site.AOTI`. | must |
...
```ts
@SourcePlugin({ site: Site.AOTI, name: 'AOTI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aoti/jobs/<id>`), wire
  `company_name` pass-through (`'AOTI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aoti/jobs/<id>`).
