# Spec: 878 — Source Company Plugin: Legion Technologies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 878                            |
| Slug           | source-company-legion               |
| Status         | accepted                               |
...
Legion Technologies publishes its careers board through Greenhouse at
the bare slug `legion`. The run-398 batch sweep confirmed
...
- Ship a `source-company-legion` plugin returning live
...
- Backfilling historical Legion Technologies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LEGION_TECHNOLOGIES`** in the
> source registry, so that **a single `siteType: [Site.LEGION_TECHNOLOGIES]`
> request returns Legion Technologies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LEGION_TECHNOLOGIES = 'legion'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-legion`.                  | must     |
| FR-3  | `LegionTechnologiesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `legion-`, `site === Site.LEGION_TECHNOLOGIES`. | must |
...
```ts
@SourcePlugin({ site: Site.LEGION_TECHNOLOGIES, name: 'Legion Technologies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/legion/jobs/<id>`), wire
  `company_name` pass-through (`'Legion Technologies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/legion/jobs/<id>`).
