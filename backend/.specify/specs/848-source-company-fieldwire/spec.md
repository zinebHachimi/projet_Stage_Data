# Spec: 848 — Source Company Plugin: Fieldwire

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 848                            |
| Slug           | source-company-fieldwire               |
| Status         | accepted                               |
...
Fieldwire publishes its careers board through Greenhouse at
the bare slug `fieldwire`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fieldwire` plugin returning live
...
- Backfilling historical Fieldwire postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FIELDWIRE`** in the
> source registry, so that **a single `siteType: [Site.FIELDWIRE]`
> request returns Fieldwire's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FIELDWIRE = 'fieldwire'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fieldwire`.                  | must     |
| FR-3  | `FieldwireService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fieldwire-`, `site === Site.FIELDWIRE`. | must |
...
```ts
@SourcePlugin({ site: Site.FIELDWIRE, name: 'Fieldwire', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fieldwire/jobs/<id>`), wire
  `company_name` pass-through (`'Fieldwire'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fieldwire/jobs/<id>`).
