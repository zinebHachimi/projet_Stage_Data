# Spec: 472 — Source Company Plugin: Alloy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 472                            |
| Slug           | source-company-alloy               |
| Status         | accepted                               |
...
Alloy publishes its careers board through Greenhouse at
the bare slug `alloy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-alloy` plugin returning live
...
- Backfilling historical Alloy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALLOY`** in the
> source registry, so that **a single `siteType: [Site.ALLOY]`
> request returns Alloy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALLOY = 'alloy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alloy`.                  | must     |
| FR-3  | `AlloyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alloy-`, `site === Site.ALLOY`. | must |
...
```ts
@SourcePlugin({ site: Site.ALLOY, name: 'Alloy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alloy/jobs/<id>`), wire
  `company_name` pass-through (`'Alloy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alloy/jobs/<id>`).
