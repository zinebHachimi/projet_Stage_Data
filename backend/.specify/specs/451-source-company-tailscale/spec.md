# Spec: 451 — Source Company Plugin: Tailscale

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 451                            |
| Slug           | source-company-tailscale               |
| Status         | accepted                               |
...
Tailscale publishes its careers board through Greenhouse at
the bare slug `tailscale`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tailscale` plugin returning live
...
- Backfilling historical Tailscale postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TAILSCALE`** in the
> source registry, so that **a single `siteType: [Site.TAILSCALE]`
> request returns Tailscale's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TAILSCALE = 'tailscale'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tailscale`.                  | must     |
| FR-3  | `TailscaleService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tailscale-`, `site === Site.TAILSCALE`. | must |
...
```ts
@SourcePlugin({ site: Site.TAILSCALE, name: 'Tailscale', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tailscale/jobs/<id>`), wire
  `company_name` pass-through (`'Tailscale'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tailscale/jobs/<id>`).
