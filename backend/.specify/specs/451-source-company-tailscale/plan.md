# Plan: 451 — Source Company Plugin: Tailscale
...
Tailscale's careers board is hosted on Greenhouse at the slug
`tailscale`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-tailscale` plugin.
- Deliverables: package files, `Site.TAILSCALE` enum value, the
...
- `packages/plugins/source-company-tailscale` (new)
