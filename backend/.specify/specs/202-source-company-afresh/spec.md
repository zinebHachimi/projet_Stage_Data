# Spec: 202 — Source Company Plugin: Afresh

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 202                            |
| Slug           | source-company-afresh               |
| Status         | accepted                               |
...
Afresh publishes its careers board through Greenhouse at
the bare slug `afresh`. The run-398 batch sweep confirmed
...
- Ship a `source-company-afresh` plugin returning live
...
- Backfilling historical Afresh postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AFRESH`** in the
> source registry, so that **a single `siteType: [Site.AFRESH]`
> request returns Afresh's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AFRESH = 'afresh'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-afresh`.                  | must     |
| FR-3  | `AfreshService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `afresh-`, `site === Site.AFRESH`. | must |
...
```ts
@SourcePlugin({ site: Site.AFRESH, name: 'Afresh', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/afresh/jobs/<id>`), wire
  `company_name` pass-through (`'Afresh'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/afresh/jobs/<id>`).
