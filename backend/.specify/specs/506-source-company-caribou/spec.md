# Spec: 506 — Source Company Plugin: Caribou Financial

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 506                            |
| Slug           | source-company-caribou               |
| Status         | accepted                               |
...
Caribou Financial publishes its careers board through Greenhouse at
the bare slug `caribou`. The run-398 batch sweep confirmed
...
- Ship a `source-company-caribou` plugin returning live
...
- Backfilling historical Caribou Financial postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CARIBOU`** in the
> source registry, so that **a single `siteType: [Site.CARIBOU]`
> request returns Caribou Financial's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CARIBOU = 'caribou'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-caribou`.                  | must     |
| FR-3  | `CaribouService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `caribou-`, `site === Site.CARIBOU`. | must |
...
```ts
@SourcePlugin({ site: Site.CARIBOU, name: 'Caribou Financial', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/caribou/jobs/<id>`), wire
  `company_name` pass-through (`'Caribou Financial'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/caribou/jobs/<id>`).
