# Spec: 625 — Source Company Plugin: Ghost

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 625                            |
| Slug           | source-company-ghost               |
| Status         | accepted                               |
...
Ghost publishes its careers board through Greenhouse at
the bare slug `ghost`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ghost` plugin returning live
...
- Backfilling historical Ghost postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GHOST`** in the
> source registry, so that **a single `siteType: [Site.GHOST]`
> request returns Ghost's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GHOST = 'ghost'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ghost`.                  | must     |
| FR-3  | `GhostService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ghost-`, `site === Site.GHOST`. | must |
...
```ts
@SourcePlugin({ site: Site.GHOST, name: 'Ghost', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ghost/jobs/<id>`), wire
  `company_name` pass-through (`'Ghost'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ghost/jobs/<id>`).
