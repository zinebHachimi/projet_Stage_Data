# Plan: 625 — Source Company Plugin: Ghost
...
Ghost's careers board is hosted on Greenhouse at the slug
`ghost`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-ghost` plugin.
- Deliverables: package files, `Site.GHOST` enum value, the
...
- `packages/plugins/source-company-ghost` (new)
