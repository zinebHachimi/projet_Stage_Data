# Spec: 544 — Source Company Plugin: Figure

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 544                            |
| Slug           | source-company-figureai               |
| Status         | accepted                               |
...
Figure publishes its careers board through Greenhouse at
the bare slug `figureai`. The run-398 batch sweep confirmed
111 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/figureai/jobs?content=true`.
Sector: Humanoid Robotics / AI. HQ: San Jose, California, USA.

## 2. Goals

- Ship a `source-company-figureai` plugin returning live
...
- Backfilling historical Figure postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FIGURE`** in the
> source registry, so that **a single `siteType: [Site.FIGURE]`
> request returns Figure's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FIGURE = 'figureai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-figureai`.                  | must     |
| FR-3  | `FigureaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `figureai-`, `site === Site.FIGURE`. | must |
...
```ts
@SourcePlugin({ site: Site.FIGURE, name: 'Figure', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/figureai/jobs/<id>`), wire
  `company_name` pass-through (`'Figure'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/figureai/jobs/<id>`).
