# Spec: 503 — Source Company Plugin: Upside

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 503                            |
| Slug           | source-company-upside               |
| Status         | accepted                               |
...
Upside publishes its careers board through Greenhouse at
the bare slug `upside`. The run-398 batch sweep confirmed
...
- Ship a `source-company-upside` plugin returning live
...
- Backfilling historical Upside postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UPSIDE`** in the
> source registry, so that **a single `siteType: [Site.UPSIDE]`
> request returns Upside's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UPSIDE = 'upside'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-upside`.                  | must     |
| FR-3  | `UpsideService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `upside-`, `site === Site.UPSIDE`. | must |
...
```ts
@SourcePlugin({ site: Site.UPSIDE, name: 'Upside', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/upside/jobs/<id>`), wire
  `company_name` pass-through (`'Upside'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/upside/jobs/<id>`).
