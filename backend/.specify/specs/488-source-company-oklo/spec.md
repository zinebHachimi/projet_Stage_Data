# Spec: 488 — Source Company Plugin: Oklo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 488                            |
| Slug           | source-company-oklo               |
| Status         | accepted                               |
...
Oklo publishes its careers board through Greenhouse at
the bare slug `oklo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-oklo` plugin returning live
...
- Backfilling historical Oklo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OKLO`** in the
> source registry, so that **a single `siteType: [Site.OKLO]`
> request returns Oklo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OKLO = 'oklo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-oklo`.                  | must     |
| FR-3  | `OkloService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `oklo-`, `site === Site.OKLO`. | must |
...
```ts
@SourcePlugin({ site: Site.OKLO, name: 'Oklo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/oklo/jobs/<id>`), wire
  `company_name` pass-through (`'Oklo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/oklo/jobs/<id>`).
