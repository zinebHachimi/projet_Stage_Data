# Spec: 509 — Source Company Plugin: Upstart

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 509                            |
| Slug           | source-company-upstart               |
| Status         | accepted                               |
...
Upstart publishes its careers board through Greenhouse at
the bare slug `upstart`. The run-398 batch sweep confirmed
...
- Ship a `source-company-upstart` plugin returning live
...
- Backfilling historical Upstart postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UPSTART`** in the
> source registry, so that **a single `siteType: [Site.UPSTART]`
> request returns Upstart's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UPSTART = 'upstart'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-upstart`.                  | must     |
| FR-3  | `UpstartService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `upstart-`, `site === Site.UPSTART`. | must |
...
```ts
@SourcePlugin({ site: Site.UPSTART, name: 'Upstart', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/upstart/jobs/<id>`), wire
  `company_name` pass-through (`'Upstart'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/upstart/jobs/<id>`).
