# Plan: 509 — Source Company Plugin: Upstart
...
Upstart's careers board is hosted on Greenhouse at the slug
`upstart`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-upstart` plugin.
- Deliverables: package files, `Site.UPSTART` enum value, the
...
- `packages/plugins/source-company-upstart` (new)
