# Spec: 868 — Source Company Plugin: InCharge Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 868                            |
| Slug           | source-company-inchargeenergy               |
| Status         | accepted                               |
...
InCharge Energy publishes its careers board through Greenhouse at
the bare slug `inchargeenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-inchargeenergy` plugin returning live
...
- Backfilling historical InCharge Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INCHARGE_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.INCHARGE_ENERGY]`
> request returns InCharge Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INCHARGE_ENERGY = 'inchargeenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-inchargeenergy`.                  | must     |
| FR-3  | `InChargeEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `inchargeenergy-`, `site === Site.INCHARGE_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.INCHARGE_ENERGY, name: 'InCharge Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/inchargeenergy/jobs/<id>`), wire
  `company_name` pass-through (`'InCharge Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/inchargeenergy/jobs/<id>`).
