# Spec: 705 — Source Company Plugin: Riverlane

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 705                            |
| Slug           | source-company-riverlane               |
| Status         | accepted                               |
...
Riverlane publishes its careers board through Greenhouse at
the bare slug `riverlane`. The run-398 batch sweep confirmed
...
- Ship a `source-company-riverlane` plugin returning live
...
- Backfilling historical Riverlane postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RIVERLANE`** in the
> source registry, so that **a single `siteType: [Site.RIVERLANE]`
> request returns Riverlane's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RIVERLANE = 'riverlane'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-riverlane`.                  | must     |
| FR-3  | `RiverlaneService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `riverlane-`, `site === Site.RIVERLANE`. | must |
...
```ts
@SourcePlugin({ site: Site.RIVERLANE, name: 'Riverlane', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/riverlane/jobs/<id>`), wire
  `company_name` pass-through (`'Riverlane'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/riverlane/jobs/<id>`).
