# Spec: 459 — Source Company Plugin: Navan

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 459                            |
| Slug           | source-company-tripactions               |
| Status         | accepted                               |
...
Navan publishes its careers board through Greenhouse at
the bare slug `tripactions`. The run-398 batch sweep confirmed
215 live role(s) via a direct probe of
...
- Ship a `source-company-tripactions` plugin returning live
...
- Backfilling historical Navan postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TRIPACTIONS`** in the
> source registry, so that **a single `siteType: [Site.TRIPACTIONS]`
> request returns Navan's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TRIPACTIONS = 'tripactions'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tripactions`.                  | must     |
| FR-3  | `TripactionsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tripactions-`, `site === Site.TRIPACTIONS`. | must |
...
```ts
@SourcePlugin({ site: Site.TRIPACTIONS, name: 'Navan', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tripactions/jobs/<id>`), wire
  `company_name` pass-through (`'Navan'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tripactions/jobs/<id>`).
