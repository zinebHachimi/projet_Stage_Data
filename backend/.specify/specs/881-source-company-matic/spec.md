# Spec: 881 — Source Company Plugin: Matic Insurance

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 881                            |
| Slug           | source-company-matic               |
| Status         | accepted                               |
...
Matic Insurance publishes its careers board through Greenhouse at
the bare slug `matic`. The run-398 batch sweep confirmed
...
- Ship a `source-company-matic` plugin returning live
...
- Backfilling historical Matic Insurance postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MATIC_INSURANCE`** in the
> source registry, so that **a single `siteType: [Site.MATIC_INSURANCE]`
> request returns Matic Insurance's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MATIC_INSURANCE = 'matic'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-matic`.                  | must     |
| FR-3  | `MaticInsuranceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `matic-`, `site === Site.MATIC_INSURANCE`. | must |
...
```ts
@SourcePlugin({ site: Site.MATIC_INSURANCE, name: 'Matic Insurance', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/matic/jobs/<id>`), wire
  `company_name` pass-through (`'Matic Insurance'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/matic/jobs/<id>`).
