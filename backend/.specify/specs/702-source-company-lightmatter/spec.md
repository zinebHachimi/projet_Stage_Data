# Spec: 702 — Source Company Plugin: Lightmatter

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 702                            |
| Slug           | source-company-lightmatter               |
| Status         | accepted                               |
...
Lightmatter publishes its careers board through Greenhouse at
the bare slug `lightmatter`. The run-398 batch sweep confirmed
...
- Ship a `source-company-lightmatter` plugin returning live
...
- Backfilling historical Lightmatter postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LIGHTMATTER`** in the
> source registry, so that **a single `siteType: [Site.LIGHTMATTER]`
> request returns Lightmatter's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LIGHTMATTER = 'lightmatter'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-lightmatter`.                  | must     |
| FR-3  | `LightmatterService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `lightmatter-`, `site === Site.LIGHTMATTER`. | must |
...
```ts
@SourcePlugin({ site: Site.LIGHTMATTER, name: 'Lightmatter', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/lightmatter/jobs/<id>`), wire
  `company_name` pass-through (`'Lightmatter'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/lightmatter/jobs/<id>`).
