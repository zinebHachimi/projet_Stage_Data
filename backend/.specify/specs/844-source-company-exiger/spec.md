# Spec: 844 — Source Company Plugin: Exiger

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 844                            |
| Slug           | source-company-exiger               |
| Status         | accepted                               |
...
Exiger publishes its careers board through Greenhouse at
the bare slug `exiger`. The run-398 batch sweep confirmed
...
- Ship a `source-company-exiger` plugin returning live
...
- Backfilling historical Exiger postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EXIGER`** in the
> source registry, so that **a single `siteType: [Site.EXIGER]`
> request returns Exiger's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EXIGER = 'exiger'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-exiger`.                  | must     |
| FR-3  | `ExigerService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `exiger-`, `site === Site.EXIGER`. | must |
...
```ts
@SourcePlugin({ site: Site.EXIGER, name: 'Exiger', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/exiger/jobs/<id>`), wire
  `company_name` pass-through (`'Exiger'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/exiger/jobs/<id>`).
