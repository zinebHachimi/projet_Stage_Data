# Spec: 546 — Source Company Plugin: Chowbus

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 546                            |
| Slug           | source-company-chowbus               |
| Status         | accepted                               |
...
Chowbus publishes its careers board through Greenhouse at
the bare slug `chowbus`. The run-398 batch sweep confirmed
...
- Ship a `source-company-chowbus` plugin returning live
...
- Backfilling historical Chowbus postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CHOWBUS`** in the
> source registry, so that **a single `siteType: [Site.CHOWBUS]`
> request returns Chowbus's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CHOWBUS = 'chowbus'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-chowbus`.                  | must     |
| FR-3  | `ChowbusService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `chowbus-`, `site === Site.CHOWBUS`. | must |
...
```ts
@SourcePlugin({ site: Site.CHOWBUS, name: 'Chowbus', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/chowbus/jobs/<id>`), wire
  `company_name` pass-through (`'Chowbus'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/chowbus/jobs/<id>`).
