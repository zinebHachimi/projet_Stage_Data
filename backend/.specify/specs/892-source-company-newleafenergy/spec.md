# Spec: 892 — Source Company Plugin: New Leaf Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 892                            |
| Slug           | source-company-newleafenergy               |
| Status         | accepted                               |
...
New Leaf Energy publishes its careers board through Greenhouse at
the bare slug `newleafenergy`. The run-398 batch sweep confirmed
42 live role(s) via a direct probe of
...
- Ship a `source-company-newleafenergy` plugin returning live
...
- Backfilling historical New Leaf Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEW_LEAF_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.NEW_LEAF_ENERGY]`
> request returns New Leaf Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEW_LEAF_ENERGY = 'newleafenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-newleafenergy`.                  | must     |
| FR-3  | `NewLeafEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `newleafenergy-`, `site === Site.NEW_LEAF_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.NEW_LEAF_ENERGY, name: 'New Leaf Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/newleafenergy/jobs/<id>`), wire
  `company_name` pass-through (`'New Leaf Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/newleafenergy/jobs/<id>`).
