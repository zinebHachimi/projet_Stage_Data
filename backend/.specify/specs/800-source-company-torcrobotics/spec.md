# Spec: 800 — Source Company Plugin: Torc Robotics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 800                            |
| Slug           | source-company-torcrobotics               |
| Status         | accepted                               |
...
Torc Robotics publishes its careers board through Greenhouse at
the bare slug `torcrobotics`. The run-398 batch sweep confirmed
58 live role(s) via a direct probe of
...
- Ship a `source-company-torcrobotics` plugin returning live
...
- Backfilling historical Torc Robotics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TORC_ROBOTICS`** in the
> source registry, so that **a single `siteType: [Site.TORC_ROBOTICS]`
> request returns Torc Robotics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TORC_ROBOTICS = 'torcrobotics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-torcrobotics`.                  | must     |
| FR-3  | `TorcRoboticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `torcrobotics-`, `site === Site.TORC_ROBOTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.TORC_ROBOTICS, name: 'Torc Robotics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/torcrobotics/jobs/<id>`), wire
  `company_name` pass-through (`'Torc Robotics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/torcrobotics/jobs/<id>`).
