# Spec: 877 — Source Company Plugin: KnowBe4

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 877                            |
| Slug           | source-company-knowbe4               |
| Status         | accepted                               |
...
KnowBe4 publishes its careers board through Greenhouse at
the bare slug `knowbe4`. The run-398 batch sweep confirmed
98 live role(s) via a direct probe of
...
- Ship a `source-company-knowbe4` plugin returning live
...
- Backfilling historical KnowBe4 postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KNOWBE4`** in the
> source registry, so that **a single `siteType: [Site.KNOWBE4]`
> request returns KnowBe4's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KNOWBE4 = 'knowbe4'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-knowbe4`.                  | must     |
| FR-3  | `KnowBe4Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `knowbe4-`, `site === Site.KNOWBE4`. | must |
...
```ts
@SourcePlugin({ site: Site.KNOWBE4, name: 'KnowBe4', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/knowbe4/jobs/<id>`), wire
  `company_name` pass-through (`'KnowBe4'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/knowbe4/jobs/<id>`).
