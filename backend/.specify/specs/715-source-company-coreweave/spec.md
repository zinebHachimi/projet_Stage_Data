# Spec: 715 — Source Company Plugin: CoreWeave

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 715                            |
| Slug           | source-company-coreweave               |
| Status         | accepted                               |
...
CoreWeave publishes its careers board through Greenhouse at
the bare slug `coreweave`. The run-398 batch sweep confirmed
260 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/coreweave/jobs?content=true`.
Sector: AI Cloud Infrastructure. HQ: Livingston, New Jersey, USA.

## 2. Goals

- Ship a `source-company-coreweave` plugin returning live
...
- Backfilling historical CoreWeave postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COREWEAVE`** in the
> source registry, so that **a single `siteType: [Site.COREWEAVE]`
> request returns CoreWeave's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COREWEAVE = 'coreweave'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-coreweave`.                  | must     |
| FR-3  | `CoreWeaveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `coreweave-`, `site === Site.COREWEAVE`. | must |
...
```ts
@SourcePlugin({ site: Site.COREWEAVE, name: 'CoreWeave', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/coreweave/jobs/<id>`), wire
  `company_name` pass-through (`'CoreWeave'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/coreweave/jobs/<id>`).
