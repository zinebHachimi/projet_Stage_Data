# Spec: 275 — Source Company Plugin: Antora Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 275                            |
| Slug           | source-company-antora               |
| Status         | accepted                               |
...
Antora Energy publishes its careers board through Greenhouse at
the bare slug `antora`. The run-398 batch sweep confirmed
...
- Ship a `source-company-antora` plugin returning live
...
- Backfilling historical Antora Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ANTORA`** in the
> source registry, so that **a single `siteType: [Site.ANTORA]`
> request returns Antora Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ANTORA = 'antora'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-antora`.                  | must     |
| FR-3  | `AntoraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `antora-`, `site === Site.ANTORA`. | must |
...
```ts
@SourcePlugin({ site: Site.ANTORA, name: 'Antora Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/antora/jobs/<id>`), wire
  `company_name` pass-through (`'Antora Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/antora/jobs/<id>`).
