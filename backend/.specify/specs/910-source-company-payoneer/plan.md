# Plan: 910 — Source Company Plugin: Payoneer
...
Payoneer's careers board is hosted on Greenhouse at the slug
`payoneer`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-payoneer` plugin.
- Deliverables: package files, `Site.PAYONEER` enum value, the
...
- `packages/plugins/source-company-payoneer` (new)
