# Spec: 910 — Source Company Plugin: Payoneer

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 910                            |
| Slug           | source-company-payoneer               |
| Status         | accepted                               |
...
Payoneer publishes its careers board through Greenhouse at
the bare slug `payoneer`. The run-398 batch sweep confirmed
133 live role(s) via a direct probe of
...
- Ship a `source-company-payoneer` plugin returning live
...
- Backfilling historical Payoneer postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PAYONEER`** in the
> source registry, so that **a single `siteType: [Site.PAYONEER]`
> request returns Payoneer's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PAYONEER = 'payoneer'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-payoneer`.                  | must     |
| FR-3  | `PayoneerService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `payoneer-`, `site === Site.PAYONEER`. | must |
...
```ts
@SourcePlugin({ site: Site.PAYONEER, name: 'Payoneer', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/payoneer/jobs/<id>`), wire
  `company_name` pass-through (`'Payoneer'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/payoneer/jobs/<id>`).
