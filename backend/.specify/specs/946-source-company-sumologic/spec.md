# Spec: 946 — Source Company Plugin: Sumo Logic

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 946                            |
| Slug           | source-company-sumologic               |
| Status         | accepted                               |
...
Sumo Logic publishes its careers board through Greenhouse at
the bare slug `sumologic`. The run-398 batch sweep confirmed
25 live role(s) via a direct probe of
...
- Ship a `source-company-sumologic` plugin returning live
...
- Backfilling historical Sumo Logic postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SUMO_LOGIC`** in the
> source registry, so that **a single `siteType: [Site.SUMO_LOGIC]`
> request returns Sumo Logic's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SUMO_LOGIC = 'sumologic'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sumologic`.                  | must     |
| FR-3  | `SumoLogicService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sumologic-`, `site === Site.SUMO_LOGIC`. | must |
...
```ts
@SourcePlugin({ site: Site.SUMO_LOGIC, name: 'Sumo Logic', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sumologic/jobs/<id>`), wire
  `company_name` pass-through (`'Sumo Logic'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sumologic/jobs/<id>`).
