# Spec: 245 — Source Company Plugin: Alt

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 245                            |
| Slug           | source-company-alt               |
| Status         | accepted                               |
...
Alt publishes its careers board through Greenhouse at
the bare slug `alt`. The run-398 batch sweep confirmed
...
- Ship a `source-company-alt` plugin returning live
...
- Backfilling historical Alt postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALT`** in the
> source registry, so that **a single `siteType: [Site.ALT]`
> request returns Alt's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALT = 'alt'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alt`.                  | must     |
| FR-3  | `AltService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alt-`, `site === Site.ALT`. | must |
...
```ts
@SourcePlugin({ site: Site.ALT, name: 'Alt', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alt/jobs/<id>`), wire
  `company_name` pass-through (`'Alt'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alt/jobs/<id>`).
