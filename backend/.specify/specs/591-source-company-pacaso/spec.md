# Spec: 591 — Source Company Plugin: Pacaso

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 591                            |
| Slug           | source-company-pacaso               |
| Status         | accepted                               |
...
Pacaso publishes its careers board through Greenhouse at
the bare slug `pacaso`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pacaso` plugin returning live
...
- Backfilling historical Pacaso postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PACASO`** in the
> source registry, so that **a single `siteType: [Site.PACASO]`
> request returns Pacaso's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PACASO = 'pacaso'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pacaso`.                  | must     |
| FR-3  | `PacasoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pacaso-`, `site === Site.PACASO`. | must |
...
```ts
@SourcePlugin({ site: Site.PACASO, name: 'Pacaso', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pacaso/jobs/<id>`), wire
  `company_name` pass-through (`'Pacaso'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pacaso/jobs/<id>`).
