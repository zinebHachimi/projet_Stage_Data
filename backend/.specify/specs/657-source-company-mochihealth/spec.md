# Spec: 657 — Source Company Plugin: Mochi Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 657                            |
| Slug           | source-company-mochihealth               |
| Status         | accepted                               |
...
Mochi Health publishes its careers board through Greenhouse at
the bare slug `mochihealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mochihealth` plugin returning live
...
- Backfilling historical Mochi Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MOCHI_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.MOCHI_HEALTH]`
> request returns Mochi Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MOCHI_HEALTH = 'mochihealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mochihealth`.                  | must     |
| FR-3  | `MochiHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mochihealth-`, `site === Site.MOCHI_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.MOCHI_HEALTH, name: 'Mochi Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mochihealth/jobs/<id>`), wire
  `company_name` pass-through (`'Mochi Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mochihealth/jobs/<id>`).
