# Spec: 593 — Source Company Plugin: Roofstock

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 593                            |
| Slug           | source-company-roofstock               |
| Status         | accepted                               |
...
Roofstock publishes its careers board through Greenhouse at
the bare slug `roofstock`. The run-398 batch sweep confirmed
...
- Ship a `source-company-roofstock` plugin returning live
...
- Backfilling historical Roofstock postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ROOFSTOCK`** in the
> source registry, so that **a single `siteType: [Site.ROOFSTOCK]`
> request returns Roofstock's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ROOFSTOCK = 'roofstock'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-roofstock`.                  | must     |
| FR-3  | `RoofstockService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `roofstock-`, `site === Site.ROOFSTOCK`. | must |
...
```ts
@SourcePlugin({ site: Site.ROOFSTOCK, name: 'Roofstock', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/roofstock/jobs/<id>`), wire
  `company_name` pass-through (`'Roofstock'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/roofstock/jobs/<id>`).
