# Spec: 264 — Source Company Plugin: Amtech Software

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 264                            |
| Slug           | source-company-amtechsoftware               |
| Status         | accepted                               |
...
Amtech Software publishes its careers board through Greenhouse at
the bare slug `amtechsoftware`. The run-398 batch sweep confirmed
...
- Ship a `source-company-amtechsoftware` plugin returning live
...
- Backfilling historical Amtech Software postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMTECHSOFTWARE`** in the
> source registry, so that **a single `siteType: [Site.AMTECHSOFTWARE]`
> request returns Amtech Software's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMTECHSOFTWARE = 'amtechsoftware'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-amtechsoftware`.                  | must     |
| FR-3  | `AmtechsoftwareService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `amtechsoftware-`, `site === Site.AMTECHSOFTWARE`. | must |
...
```ts
@SourcePlugin({ site: Site.AMTECHSOFTWARE, name: 'Amtech Software', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/amtechsoftware/jobs/<id>`), wire
  `company_name` pass-through (`'Amtech Software'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/amtechsoftware/jobs/<id>`).
