# Spec: 232 — Source Company Plugin: Cortica - Neurodevelopmental

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 232                            |
| Slug           | source-company-allcareers               |
| Status         | accepted                               |
...
Cortica - Neurodevelopmental publishes its careers board through Greenhouse at
the bare slug `allcareers`. The run-398 batch sweep confirmed
...
- Ship a `source-company-allcareers` plugin returning live
...
- Backfilling historical Cortica - Neurodevelopmental postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALLCAREERS`** in the
> source registry, so that **a single `siteType: [Site.ALLCAREERS]`
> request returns Cortica - Neurodevelopmental's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALLCAREERS = 'allcareers'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-allcareers`.                  | must     |
| FR-3  | `AllcareersService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `allcareers-`, `site === Site.ALLCAREERS`. | must |
...
```ts
@SourcePlugin({ site: Site.ALLCAREERS, name: 'Cortica - Neurodevelopmental', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/allcareers/jobs/<id>`), wire
  `company_name` pass-through (`'Cortica - Neurodevelopmental'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/allcareers/jobs/<id>`).
