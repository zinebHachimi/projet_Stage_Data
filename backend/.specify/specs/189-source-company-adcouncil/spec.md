# Spec: 189 — Source Company Plugin: the Ad Council

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 189                            |
| Slug           | source-company-adcouncil               |
| Status         | accepted                               |
...
the Ad Council publishes its careers board through Greenhouse at
the bare slug `adcouncil`. The run-398 batch sweep confirmed
...
- Ship a `source-company-adcouncil` plugin returning live
...
- Backfilling historical the Ad Council postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ADCOUNCIL`** in the
> source registry, so that **a single `siteType: [Site.ADCOUNCIL]`
> request returns the Ad Council's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ADCOUNCIL = 'adcouncil'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-adcouncil`.                  | must     |
| FR-3  | `AdcouncilService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `adcouncil-`, `site === Site.ADCOUNCIL`. | must |
...
```ts
@SourcePlugin({ site: Site.ADCOUNCIL, name: 'the Ad Council', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/adcouncil/jobs/<id>`), wire
  `company_name` pass-through (`'the Ad Council'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/adcouncil/jobs/<id>`).
