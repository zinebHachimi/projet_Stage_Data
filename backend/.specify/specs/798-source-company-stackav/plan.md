# Plan: 798 — Source Company Plugin: Stack AV
...
Stack AV's careers board is hosted on Greenhouse at the slug
`stackav`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-stackav` plugin.
- Deliverables: package files, `Site.STACK_AV` enum value, the
...
- `packages/plugins/source-company-stackav` (new)
