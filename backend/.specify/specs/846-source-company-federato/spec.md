# Spec: 846 — Source Company Plugin: Federato

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 846                            |
| Slug           | source-company-federato               |
| Status         | accepted                               |
...
Federato publishes its careers board through Greenhouse at
the bare slug `federato`. The run-398 batch sweep confirmed
...
- Ship a `source-company-federato` plugin returning live
...
- Backfilling historical Federato postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FEDERATO`** in the
> source registry, so that **a single `siteType: [Site.FEDERATO]`
> request returns Federato's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FEDERATO = 'federato'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-federato`.                  | must     |
| FR-3  | `FederatoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `federato-`, `site === Site.FEDERATO`. | must |
...
```ts
@SourcePlugin({ site: Site.FEDERATO, name: 'Federato', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/federato/jobs/<id>`), wire
  `company_name` pass-through (`'Federato'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/federato/jobs/<id>`).
