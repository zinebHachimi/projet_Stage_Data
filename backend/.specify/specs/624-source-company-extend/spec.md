# Spec: 624 — Source Company Plugin: Extend

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 624                            |
| Slug           | source-company-extend               |
| Status         | accepted                               |
...
Extend publishes its careers board through Greenhouse at
the bare slug `extend`. The run-398 batch sweep confirmed
...
- Ship a `source-company-extend` plugin returning live
...
- Backfilling historical Extend postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EXTEND`** in the
> source registry, so that **a single `siteType: [Site.EXTEND]`
> request returns Extend's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EXTEND = 'extend'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-extend`.                  | must     |
| FR-3  | `ExtendService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `extend-`, `site === Site.EXTEND`. | must |
...
```ts
@SourcePlugin({ site: Site.EXTEND, name: 'Extend', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/extend/jobs/<id>`), wire
  `company_name` pass-through (`'Extend'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/extend/jobs/<id>`).
