# Plan: 624 — Source Company Plugin: Extend
...
Extend's careers board is hosted on Greenhouse at the slug
`extend`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-extend` plugin.
- Deliverables: package files, `Site.EXTEND` enum value, the
...
- `packages/plugins/source-company-extend` (new)
