# Spec: 832 — Source Company Plugin: Daybreak Game Company

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 832                            |
| Slug           | source-company-daybreakgames               |
| Status         | accepted                               |
...
Daybreak Game Company publishes its careers board through Greenhouse at
the bare slug `daybreakgames`. The run-398 batch sweep confirmed
...
- Ship a `source-company-daybreakgames` plugin returning live
...
- Backfilling historical Daybreak Game Company postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DAYBREAK_GAME_COMPANY`** in the
> source registry, so that **a single `siteType: [Site.DAYBREAK_GAME_COMPANY]`
> request returns Daybreak Game Company's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DAYBREAK_GAME_COMPANY = 'daybreakgames'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-daybreakgames`.                  | must     |
| FR-3  | `DaybreakGameCompanyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `daybreakgames-`, `site === Site.DAYBREAK_GAME_COMPANY`. | must |
...
```ts
@SourcePlugin({ site: Site.DAYBREAK_GAME_COMPANY, name: 'Daybreak Game Company', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/daybreakgames/jobs/<id>`), wire
  `company_name` pass-through (`'Daybreak Game Company'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/daybreakgames/jobs/<id>`).
