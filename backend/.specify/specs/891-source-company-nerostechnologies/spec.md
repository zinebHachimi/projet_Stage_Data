# Spec: 891 — Source Company Plugin: Neros Technologies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 891                            |
| Slug           | source-company-nerostechnologies               |
| Status         | accepted                               |
...
Neros Technologies publishes its careers board through Greenhouse at
the bare slug `nerostechnologies`. The run-398 batch sweep confirmed
63 live role(s) via a direct probe of
...
- Ship a `source-company-nerostechnologies` plugin returning live
...
- Backfilling historical Neros Technologies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NEROS_TECHNOLOGIES`** in the
> source registry, so that **a single `siteType: [Site.NEROS_TECHNOLOGIES]`
> request returns Neros Technologies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NEROS_TECHNOLOGIES = 'nerostechnologies'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nerostechnologies`.                  | must     |
| FR-3  | `NerosTechnologiesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nerostechnologies-`, `site === Site.NEROS_TECHNOLOGIES`. | must |
...
```ts
@SourcePlugin({ site: Site.NEROS_TECHNOLOGIES, name: 'Neros Technologies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nerostechnologies/jobs/<id>`), wire
  `company_name` pass-through (`'Neros Technologies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nerostechnologies/jobs/<id>`).
