# Spec: 564 — Source Company Plugin: Tide

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 564                            |
| Slug           | source-company-tide               |
| Status         | accepted                               |
...
Tide publishes its careers board through Greenhouse at
the bare slug `tide`. The run-398 batch sweep confirmed
630 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/tide/jobs?content=true`.
Sector: Fintech / SMB Business Banking. HQ: London, United Kingdom.

## 2. Goals

- Ship a `source-company-tide` plugin returning live
...
- Backfilling historical Tide postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TIDE`** in the
> source registry, so that **a single `siteType: [Site.TIDE]`
> request returns Tide's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TIDE = 'tide'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tide`.                  | must     |
| FR-3  | `TideService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tide-`, `site === Site.TIDE`. | must |
...
```ts
@SourcePlugin({ site: Site.TIDE, name: 'Tide', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tide/jobs/<id>`), wire
  `company_name` pass-through (`'Tide'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tide/jobs/<id>`).
