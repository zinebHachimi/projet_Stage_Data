# Spec: 516 — Source Company Plugin: Form3

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 516                            |
| Slug           | source-company-form3               |
| Status         | accepted                               |
...
Form3 publishes its careers board through Greenhouse at
the bare slug `form3`. The run-398 batch sweep confirmed
...
- Ship a `source-company-form3` plugin returning live
...
- Backfilling historical Form3 postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORM3`** in the
> source registry, so that **a single `siteType: [Site.FORM3]`
> request returns Form3's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORM3 = 'form3'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-form3`.                  | must     |
| FR-3  | `Form3Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `form3-`, `site === Site.FORM3`. | must |
...
```ts
@SourcePlugin({ site: Site.FORM3, name: 'Form3', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/form3/jobs/<id>`), wire
  `company_name` pass-through (`'Form3'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/form3/jobs/<id>`).
