# Spec: 811 — Source Company Plugin: Augury

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 811                            |
| Slug           | source-company-augury               |
| Status         | accepted                               |
...
Augury publishes its careers board through Greenhouse at
the bare slug `augury`. The run-398 batch sweep confirmed
...
- Ship a `source-company-augury` plugin returning live
...
- Backfilling historical Augury postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AUGURY`** in the
> source registry, so that **a single `siteType: [Site.AUGURY]`
> request returns Augury's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AUGURY = 'augury'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-augury`.                  | must     |
| FR-3  | `AuguryService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `augury-`, `site === Site.AUGURY`. | must |
...
```ts
@SourcePlugin({ site: Site.AUGURY, name: 'Augury', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/augury/jobs/<id>`), wire
  `company_name` pass-through (`'Augury'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/augury/jobs/<id>`).
