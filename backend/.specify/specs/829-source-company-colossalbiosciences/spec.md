# Spec: 829 — Source Company Plugin: Colossal Biosciences

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 829                            |
| Slug           | source-company-colossalbiosciences               |
| Status         | accepted                               |
...
Colossal Biosciences publishes its careers board through Greenhouse at
the bare slug `colossalbiosciences`. The run-398 batch sweep confirmed
...
- Ship a `source-company-colossalbiosciences` plugin returning live
...
- Backfilling historical Colossal Biosciences postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COLOSSAL_BIOSCIENCES`** in the
> source registry, so that **a single `siteType: [Site.COLOSSAL_BIOSCIENCES]`
> request returns Colossal Biosciences's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COLOSSAL_BIOSCIENCES = 'colossalbiosciences'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-colossalbiosciences`.                  | must     |
| FR-3  | `ColossalBiosciencesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `colossalbiosciences-`, `site === Site.COLOSSAL_BIOSCIENCES`. | must |
...
```ts
@SourcePlugin({ site: Site.COLOSSAL_BIOSCIENCES, name: 'Colossal Biosciences', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/colossalbiosciences/jobs/<id>`), wire
  `company_name` pass-through (`'Colossal Biosciences'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/colossalbiosciences/jobs/<id>`).
