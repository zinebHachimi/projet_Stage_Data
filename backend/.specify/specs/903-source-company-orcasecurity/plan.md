# Plan: 903 — Source Company Plugin: Orca Security
...
Orca Security's careers board is hosted on Greenhouse at the slug
`orcasecurity`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-orcasecurity` plugin.
- Deliverables: package files, `Site.ORCA_SECURITY` enum value, the
...
- `packages/plugins/source-company-orcasecurity` (new)
