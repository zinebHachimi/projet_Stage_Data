# Spec: 903 — Source Company Plugin: Orca Security

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 903                            |
| Slug           | source-company-orcasecurity               |
| Status         | accepted                               |
...
Orca Security publishes its careers board through Greenhouse at
the bare slug `orcasecurity`. The run-398 batch sweep confirmed
...
- Ship a `source-company-orcasecurity` plugin returning live
...
- Backfilling historical Orca Security postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ORCA_SECURITY`** in the
> source registry, so that **a single `siteType: [Site.ORCA_SECURITY]`
> request returns Orca Security's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ORCA_SECURITY = 'orcasecurity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-orcasecurity`.                  | must     |
| FR-3  | `OrcaSecurityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `orcasecurity-`, `site === Site.ORCA_SECURITY`. | must |
...
```ts
@SourcePlugin({ site: Site.ORCA_SECURITY, name: 'Orca Security', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/orcasecurity/jobs/<id>`), wire
  `company_name` pass-through (`'Orca Security'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/orcasecurity/jobs/<id>`).
