# Spec: 869 — Source Company Plugin: Innovid

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 869                            |
| Slug           | source-company-innovid               |
| Status         | accepted                               |
...
Innovid publishes its careers board through Greenhouse at
the bare slug `innovid`. The run-398 batch sweep confirmed
...
- Ship a `source-company-innovid` plugin returning live
...
- Backfilling historical Innovid postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INNOVID`** in the
> source registry, so that **a single `siteType: [Site.INNOVID]`
> request returns Innovid's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INNOVID = 'innovid'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-innovid`.                  | must     |
| FR-3  | `InnovidService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `innovid-`, `site === Site.INNOVID`. | must |
...
```ts
@SourcePlugin({ site: Site.INNOVID, name: 'Innovid', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/innovid/jobs/<id>`), wire
  `company_name` pass-through (`'Innovid'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/innovid/jobs/<id>`).
