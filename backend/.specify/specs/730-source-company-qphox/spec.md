# Spec: 730 — Source Company Plugin: QphoX

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 730                            |
| Slug           | source-company-qphox               |
| Status         | accepted                               |
...
QphoX publishes its careers board through Greenhouse at
the bare slug `qphox`. The run-398 batch sweep confirmed
...
- Ship a `source-company-qphox` plugin returning live
...
- Backfilling historical QphoX postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.QPHOX`** in the
> source registry, so that **a single `siteType: [Site.QPHOX]`
> request returns QphoX's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.QPHOX = 'qphox'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-qphox`.                  | must     |
| FR-3  | `QphoXService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `qphox-`, `site === Site.QPHOX`. | must |
...
```ts
@SourcePlugin({ site: Site.QPHOX, name: 'QphoX', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/qphox/jobs/<id>`), wire
  `company_name` pass-through (`'QphoX'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/qphox/jobs/<id>`).
