# Spec: 458 — Source Company Plugin: Relativity Space

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 458                            |
| Slug           | source-company-relativity               |
| Status         | accepted                               |
...
Relativity Space publishes its careers board through Greenhouse at
the bare slug `relativity`. The run-398 batch sweep confirmed
295 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/relativity/jobs?content=true`.
Sector: Aerospace & rocket manufacturing. HQ: Long Beach, CA, USA.

## 2. Goals

- Ship a `source-company-relativity` plugin returning live
...
- Backfilling historical Relativity Space postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RELATIVITY`** in the
> source registry, so that **a single `siteType: [Site.RELATIVITY]`
> request returns Relativity Space's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RELATIVITY = 'relativity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-relativity`.                  | must     |
| FR-3  | `RelativityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `relativity-`, `site === Site.RELATIVITY`. | must |
...
```ts
@SourcePlugin({ site: Site.RELATIVITY, name: 'Relativity Space', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/relativity/jobs/<id>`), wire
  `company_name` pass-through (`'Relativity Space'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/relativity/jobs/<id>`).
