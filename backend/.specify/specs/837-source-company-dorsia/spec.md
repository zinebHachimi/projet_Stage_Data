# Spec: 837 — Source Company Plugin: Dorsia

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 837                            |
| Slug           | source-company-dorsia               |
| Status         | accepted                               |
...
Dorsia publishes its careers board through Greenhouse at
the bare slug `dorsia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-dorsia` plugin returning live
...
- Backfilling historical Dorsia postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DORSIA`** in the
> source registry, so that **a single `siteType: [Site.DORSIA]`
> request returns Dorsia's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DORSIA = 'dorsia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-dorsia`.                  | must     |
| FR-3  | `DorsiaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `dorsia-`, `site === Site.DORSIA`. | must |
...
```ts
@SourcePlugin({ site: Site.DORSIA, name: 'Dorsia', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/dorsia/jobs/<id>`), wire
  `company_name` pass-through (`'Dorsia'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/dorsia/jobs/<id>`).
