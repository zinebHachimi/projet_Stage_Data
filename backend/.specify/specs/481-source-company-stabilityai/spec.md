# Spec: 481 — Source Company Plugin: Stability AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 481                            |
| Slug           | source-company-stabilityai               |
| Status         | accepted                               |
...
Stability AI publishes its careers board through Greenhouse at
the bare slug `stabilityai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-stabilityai` plugin returning live
...
- Backfilling historical Stability AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STABILITYAI`** in the
> source registry, so that **a single `siteType: [Site.STABILITYAI]`
> request returns Stability AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STABILITYAI = 'stabilityai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-stabilityai`.                  | must     |
| FR-3  | `StabilityaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `stabilityai-`, `site === Site.STABILITYAI`. | must |
...
```ts
@SourcePlugin({ site: Site.STABILITYAI, name: 'Stability AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/stabilityai/jobs/<id>`), wire
  `company_name` pass-through (`'Stability AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/stabilityai/jobs/<id>`).
