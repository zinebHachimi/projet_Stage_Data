# Plan: 218 — Source Company Plugin: AirTrunk
...
AirTrunk's careers board is hosted on Greenhouse at the slug
`airtrunk`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-airtrunk` plugin.
- Deliverables: package files, `Site.AIRTRUNK` enum value, the
...
- `packages/plugins/source-company-airtrunk` (new)
