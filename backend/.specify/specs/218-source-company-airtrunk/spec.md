# Spec: 218 — Source Company Plugin: AirTrunk

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 218                            |
| Slug           | source-company-airtrunk               |
| Status         | accepted                               |
...
AirTrunk publishes its careers board through Greenhouse at
the bare slug `airtrunk`. The run-398 batch sweep confirmed
...
- Ship a `source-company-airtrunk` plugin returning live
...
- Backfilling historical AirTrunk postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AIRTRUNK`** in the
> source registry, so that **a single `siteType: [Site.AIRTRUNK]`
> request returns AirTrunk's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AIRTRUNK = 'airtrunk'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-airtrunk`.                  | must     |
| FR-3  | `AirtrunkService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `airtrunk-`, `site === Site.AIRTRUNK`. | must |
...
```ts
@SourcePlugin({ site: Site.AIRTRUNK, name: 'AirTrunk', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/airtrunk/jobs/<id>`), wire
  `company_name` pass-through (`'AirTrunk'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/airtrunk/jobs/<id>`).
