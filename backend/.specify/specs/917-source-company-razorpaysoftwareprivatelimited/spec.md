# Spec: 917 — Source Company Plugin: Razorpay

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 917                            |
| Slug           | source-company-razorpaysoftwareprivatelimited               |
| Status         | accepted                               |
...
Razorpay publishes its careers board through Greenhouse at
the bare slug `razorpaysoftwareprivatelimited`. The run-398 batch sweep confirmed
...
- Ship a `source-company-razorpaysoftwareprivatelimited` plugin returning live
...
- Backfilling historical Razorpay postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RAZORPAY`** in the
> source registry, so that **a single `siteType: [Site.RAZORPAY]`
> request returns Razorpay's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RAZORPAY = 'razorpaysoftwareprivatelimited'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-razorpaysoftwareprivatelimited`.                  | must     |
| FR-3  | `RazorpayService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `razorpaysoftwareprivatelimited-`, `site === Site.RAZORPAY`. | must |
...
```ts
@SourcePlugin({ site: Site.RAZORPAY, name: 'Razorpay', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/razorpaysoftwareprivatelimited/jobs/<id>`), wire
  `company_name` pass-through (`'Razorpay'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/razorpaysoftwareprivatelimited/jobs/<id>`).
