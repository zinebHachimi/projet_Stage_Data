# Plan: 917 — Source Company Plugin: Razorpay
...
Razorpay's careers board is hosted on Greenhouse at the slug
`razorpaysoftwareprivatelimited`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-razorpaysoftwareprivatelimited` plugin.
- Deliverables: package files, `Site.RAZORPAY` enum value, the
...
- `packages/plugins/source-company-razorpaysoftwareprivatelimited` (new)
