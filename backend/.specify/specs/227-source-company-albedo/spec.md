# Spec: 227 — Source Company Plugin: Albedo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 227                            |
| Slug           | source-company-albedo               |
| Status         | accepted                               |
...
Albedo publishes its careers board through Greenhouse at
the bare slug `albedo`. The run-398 batch sweep confirmed
...
- Ship a `source-company-albedo` plugin returning live
...
- Backfilling historical Albedo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALBEDO`** in the
> source registry, so that **a single `siteType: [Site.ALBEDO]`
> request returns Albedo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALBEDO = 'albedo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-albedo`.                  | must     |
| FR-3  | `AlbedoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `albedo-`, `site === Site.ALBEDO`. | must |
...
```ts
@SourcePlugin({ site: Site.ALBEDO, name: 'Albedo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/albedo/jobs/<id>`), wire
  `company_name` pass-through (`'Albedo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/albedo/jobs/<id>`).
