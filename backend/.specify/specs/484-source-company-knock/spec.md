# Spec: 484 — Source Company Plugin: Knock

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 484                            |
| Slug           | source-company-knock               |
| Status         | accepted                               |
...
Knock publishes its careers board through Greenhouse at
the bare slug `knock`. The run-398 batch sweep confirmed
...
- Ship a `source-company-knock` plugin returning live
...
- Backfilling historical Knock postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KNOCK`** in the
> source registry, so that **a single `siteType: [Site.KNOCK]`
> request returns Knock's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KNOCK = 'knock'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-knock`.                  | must     |
| FR-3  | `KnockService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `knock-`, `site === Site.KNOCK`. | must |
...
```ts
@SourcePlugin({ site: Site.KNOCK, name: 'Knock', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/knock/jobs/<id>`), wire
  `company_name` pass-through (`'Knock'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/knock/jobs/<id>`).
