# Spec: 454 — Source Company Plugin: Builder.io

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 454                            |
| Slug           | source-company-builder               |
| Status         | accepted                               |
...
Builder.io publishes its careers board through Greenhouse at
the bare slug `builder`. The run-398 batch sweep confirmed
...
- Ship a `source-company-builder` plugin returning live
...
- Backfilling historical Builder.io postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BUILDER`** in the
> source registry, so that **a single `siteType: [Site.BUILDER]`
> request returns Builder.io's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BUILDER = 'builder'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-builder`.                  | must     |
| FR-3  | `BuilderService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `builder-`, `site === Site.BUILDER`. | must |
...
```ts
@SourcePlugin({ site: Site.BUILDER, name: 'Builder.io', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/builder/jobs/<id>`), wire
  `company_name` pass-through (`'Builder.io'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/builder/jobs/<id>`).
