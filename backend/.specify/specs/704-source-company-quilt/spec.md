# Spec: 704 — Source Company Plugin: Quilt

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 704                            |
| Slug           | source-company-quilt               |
| Status         | accepted                               |
...
Quilt publishes its careers board through Greenhouse at
the bare slug `quilt`. The run-398 batch sweep confirmed
...
- Ship a `source-company-quilt` plugin returning live
...
- Backfilling historical Quilt postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.QUILT`** in the
> source registry, so that **a single `siteType: [Site.QUILT]`
> request returns Quilt's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.QUILT = 'quilt'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-quilt`.                  | must     |
| FR-3  | `QuiltService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `quilt-`, `site === Site.QUILT`. | must |
...
```ts
@SourcePlugin({ site: Site.QUILT, name: 'Quilt', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/quilt/jobs/<id>`), wire
  `company_name` pass-through (`'Quilt'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/quilt/jobs/<id>`).
