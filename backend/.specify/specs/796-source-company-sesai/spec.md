# Spec: 796 — Source Company Plugin: SES AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 796                            |
| Slug           | source-company-sesai               |
| Status         | accepted                               |
...
SES AI publishes its careers board through Greenhouse at
the bare slug `sesai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-sesai` plugin returning live
...
- Backfilling historical SES AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SES_AI`** in the
> source registry, so that **a single `siteType: [Site.SES_AI]`
> request returns SES AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SES_AI = 'sesai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sesai`.                  | must     |
| FR-3  | `SESAIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sesai-`, `site === Site.SES_AI`. | must |
...
```ts
@SourcePlugin({ site: Site.SES_AI, name: 'SES AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sesai/jobs/<id>`), wire
  `company_name` pass-through (`'SES AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sesai/jobs/<id>`).
