# Spec: 462 — Source Company Plugin: fal

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 462                            |
| Slug           | source-company-fal               |
| Status         | accepted                               |
...
fal publishes its careers board through Greenhouse at
the bare slug `fal`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fal` plugin returning live
...
- Backfilling historical fal postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FAL`** in the
> source registry, so that **a single `siteType: [Site.FAL]`
> request returns fal's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FAL = 'fal'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fal`.                  | must     |
| FR-3  | `FalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fal-`, `site === Site.FAL`. | must |
...
```ts
@SourcePlugin({ site: Site.FAL, name: 'fal', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fal/jobs/<id>`), wire
  `company_name` pass-through (`'fal'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fal/jobs/<id>`).
