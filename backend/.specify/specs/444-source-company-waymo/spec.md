# Spec: 444 — Source Company Plugin: Waymo

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 444                            |
| Slug           | source-company-waymo               |
| Status         | accepted                               |
...
Waymo publishes its careers board through Greenhouse at
the bare slug `waymo`. The run-398 batch sweep confirmed
430 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/waymo/jobs?content=true`.
Sector: Autonomous driving technology. HQ: Mountain View, CA, USA.

## 2. Goals

- Ship a `source-company-waymo` plugin returning live
...
- Backfilling historical Waymo postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WAYMO`** in the
> source registry, so that **a single `siteType: [Site.WAYMO]`
> request returns Waymo's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WAYMO = 'waymo'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-waymo`.                  | must     |
| FR-3  | `WaymoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `waymo-`, `site === Site.WAYMO`. | must |
...
```ts
@SourcePlugin({ site: Site.WAYMO, name: 'Waymo', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/waymo/jobs/<id>`), wire
  `company_name` pass-through (`'Waymo'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/waymo/jobs/<id>`).
