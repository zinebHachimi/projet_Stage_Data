# Plan: 444 — Source Company Plugin: Waymo
...
Waymo's careers board is hosted on Greenhouse at the slug
`waymo`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-waymo` plugin.
- Deliverables: package files, `Site.WAYMO` enum value, the
...
- `packages/plugins/source-company-waymo` (new)
