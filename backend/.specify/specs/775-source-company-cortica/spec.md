# Spec: 775 — Source Company Plugin: Cortica

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 775                            |
| Slug           | source-company-cortica               |
| Status         | accepted                               |
...
Cortica publishes its careers board through Greenhouse at
the bare slug `cortica`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cortica` plugin returning live
...
- Backfilling historical Cortica postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CORTICA`** in the
> source registry, so that **a single `siteType: [Site.CORTICA]`
> request returns Cortica's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CORTICA = 'cortica'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cortica`.                  | must     |
| FR-3  | `CorticaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cortica-`, `site === Site.CORTICA`. | must |
...
```ts
@SourcePlugin({ site: Site.CORTICA, name: 'Cortica', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cortica/jobs/<id>`), wire
  `company_name` pass-through (`'Cortica'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cortica/jobs/<id>`).
