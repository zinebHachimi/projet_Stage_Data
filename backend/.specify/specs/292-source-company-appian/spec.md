# Spec: 292 — Source Company Plugin: Appian Corporation 

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 292                            |
| Slug           | source-company-appian               |
| Status         | accepted                               |
...
Appian Corporation  publishes its careers board through Greenhouse at
the bare slug `appian`. The run-398 batch sweep confirmed
192 live role(s) via a direct probe of
...
- Ship a `source-company-appian` plugin returning live
...
- Backfilling historical Appian Corporation  postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APPIAN`** in the
> source registry, so that **a single `siteType: [Site.APPIAN]`
> request returns Appian Corporation 's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APPIAN = 'appian'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-appian`.                  | must     |
| FR-3  | `AppianService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `appian-`, `site === Site.APPIAN`. | must |
...
```ts
@SourcePlugin({ site: Site.APPIAN, name: 'Appian Corporation ', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/appian/jobs/<id>`), wire
  `company_name` pass-through (`'Appian Corporation '`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/appian/jobs/<id>`).
