# Spec: 727 — Source Company Plugin: Zola

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 727                            |
| Slug           | source-company-zola               |
| Status         | accepted                               |
...
Zola publishes its careers board through Greenhouse at
the bare slug `zola`. The run-398 batch sweep confirmed
...
- Ship a `source-company-zola` plugin returning live
...
- Backfilling historical Zola postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ZOLA`** in the
> source registry, so that **a single `siteType: [Site.ZOLA]`
> request returns Zola's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ZOLA = 'zola'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-zola`.                  | must     |
| FR-3  | `ZolaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `zola-`, `site === Site.ZOLA`. | must |
...
```ts
@SourcePlugin({ site: Site.ZOLA, name: 'Zola', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/zola/jobs/<id>`), wire
  `company_name` pass-through (`'Zola'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/zola/jobs/<id>`).
