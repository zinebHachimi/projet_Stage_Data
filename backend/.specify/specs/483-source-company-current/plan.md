# Plan: 483 — Source Company Plugin: Current
...
Current's careers board is hosted on Greenhouse at the slug
`current`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-current` plugin.
- Deliverables: package files, `Site.CURRENT` enum value, the
...
- `packages/plugins/source-company-current` (new)
