# Spec: 483 — Source Company Plugin: Current

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 483                            |
| Slug           | source-company-current               |
| Status         | accepted                               |
...
Current publishes its careers board through Greenhouse at
the bare slug `current`. The run-398 batch sweep confirmed
...
- Ship a `source-company-current` plugin returning live
...
- Backfilling historical Current postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CURRENT`** in the
> source registry, so that **a single `siteType: [Site.CURRENT]`
> request returns Current's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CURRENT = 'current'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-current`.                  | must     |
| FR-3  | `CurrentService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `current-`, `site === Site.CURRENT`. | must |
...
```ts
@SourcePlugin({ site: Site.CURRENT, name: 'Current', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/current/jobs/<id>`), wire
  `company_name` pass-through (`'Current'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/current/jobs/<id>`).
