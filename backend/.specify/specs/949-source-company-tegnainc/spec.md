# Spec: 949 — Source Company Plugin: TEGNA

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 949                            |
| Slug           | source-company-tegnainc               |
| Status         | accepted                               |
...
TEGNA publishes its careers board through Greenhouse at
the bare slug `tegnainc`. The run-398 batch sweep confirmed
332 live role(s) via a direct probe of
...
- Ship a `source-company-tegnainc` plugin returning live
...
- Backfilling historical TEGNA postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TEGNA`** in the
> source registry, so that **a single `siteType: [Site.TEGNA]`
> request returns TEGNA's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TEGNA = 'tegnainc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tegnainc`.                  | must     |
| FR-3  | `TEGNAService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tegnainc-`, `site === Site.TEGNA`. | must |
...
```ts
@SourcePlugin({ site: Site.TEGNA, name: 'TEGNA', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tegnainc/jobs/<id>`), wire
  `company_name` pass-through (`'TEGNA'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tegnainc/jobs/<id>`).
