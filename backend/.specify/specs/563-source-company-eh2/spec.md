# Spec: 563 — Source Company Plugin: Electric Hydrogen

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 563                            |
| Slug           | source-company-eh2               |
| Status         | accepted                               |
...
Electric Hydrogen publishes its careers board through Greenhouse at
the bare slug `eh2`. The run-398 batch sweep confirmed
...
- Ship a `source-company-eh2` plugin returning live
...
- Backfilling historical Electric Hydrogen postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ELECTRIC_HYDROGEN`** in the
> source registry, so that **a single `siteType: [Site.ELECTRIC_HYDROGEN]`
> request returns Electric Hydrogen's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ELECTRIC_HYDROGEN = 'eh2'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-eh2`.                  | must     |
| FR-3  | `Eh2Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `eh2-`, `site === Site.ELECTRIC_HYDROGEN`. | must |
...
```ts
@SourcePlugin({ site: Site.ELECTRIC_HYDROGEN, name: 'Electric Hydrogen', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/eh2/jobs/<id>`), wire
  `company_name` pass-through (`'Electric Hydrogen'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/eh2/jobs/<id>`).
