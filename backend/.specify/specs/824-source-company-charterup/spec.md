# Spec: 824 — Source Company Plugin: CharterUP

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 824                            |
| Slug           | source-company-charterup               |
| Status         | accepted                               |
...
CharterUP publishes its careers board through Greenhouse at
the bare slug `charterup`. The run-398 batch sweep confirmed
...
- Ship a `source-company-charterup` plugin returning live
...
- Backfilling historical CharterUP postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CHARTERUP`** in the
> source registry, so that **a single `siteType: [Site.CHARTERUP]`
> request returns CharterUP's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CHARTERUP = 'charterup'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-charterup`.                  | must     |
| FR-3  | `CharterUPService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `charterup-`, `site === Site.CHARTERUP`. | must |
...
```ts
@SourcePlugin({ site: Site.CHARTERUP, name: 'CharterUP', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/charterup/jobs/<id>`), wire
  `company_name` pass-through (`'CharterUP'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/charterup/jobs/<id>`).
