# Plan: 437 — Source Company Plugin: Sendbird
...
Sendbird's careers board is hosted on Greenhouse at the slug
`sendbird`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-sendbird` plugin.
- Deliverables: package files, `Site.SENDBIRD` enum value, the
...
- `packages/plugins/source-company-sendbird` (new)
