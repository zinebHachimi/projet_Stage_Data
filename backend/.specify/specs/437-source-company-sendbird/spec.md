# Spec: 437 — Source Company Plugin: Sendbird

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 437                            |
| Slug           | source-company-sendbird               |
| Status         | accepted                               |
...
Sendbird publishes its careers board through Greenhouse at
the bare slug `sendbird`. The run-398 batch sweep confirmed
...
- Ship a `source-company-sendbird` plugin returning live
...
- Backfilling historical Sendbird postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SENDBIRD`** in the
> source registry, so that **a single `siteType: [Site.SENDBIRD]`
> request returns Sendbird's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SENDBIRD = 'sendbird'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-sendbird`.                  | must     |
| FR-3  | `SendbirdService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `sendbird-`, `site === Site.SENDBIRD`. | must |
...
```ts
@SourcePlugin({ site: Site.SENDBIRD, name: 'Sendbird', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/sendbird/jobs/<id>`), wire
  `company_name` pass-through (`'Sendbird'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/sendbird/jobs/<id>`).
