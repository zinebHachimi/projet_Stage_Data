# Spec: 548 — Source Company Plugin: PathAI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 548                            |
| Slug           | source-company-pathai               |
| Status         | accepted                               |
...
PathAI publishes its careers board through Greenhouse at
the bare slug `pathai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pathai` plugin returning live
...
- Backfilling historical PathAI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PATHAI`** in the
> source registry, so that **a single `siteType: [Site.PATHAI]`
> request returns PathAI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PATHAI = 'pathai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pathai`.                  | must     |
| FR-3  | `PathaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pathai-`, `site === Site.PATHAI`. | must |
...
```ts
@SourcePlugin({ site: Site.PATHAI, name: 'PathAI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pathai/jobs/<id>`), wire
  `company_name` pass-through (`'PathAI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pathai/jobs/<id>`).
