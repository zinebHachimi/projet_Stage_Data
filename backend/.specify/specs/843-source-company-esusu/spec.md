# Spec: 843 — Source Company Plugin: Esusu

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 843                            |
| Slug           | source-company-esusu               |
| Status         | accepted                               |
...
Esusu publishes its careers board through Greenhouse at
the bare slug `esusu`. The run-398 batch sweep confirmed
...
- Ship a `source-company-esusu` plugin returning live
...
- Backfilling historical Esusu postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ESUSU`** in the
> source registry, so that **a single `siteType: [Site.ESUSU]`
> request returns Esusu's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ESUSU = 'esusu'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-esusu`.                  | must     |
| FR-3  | `EsusuService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `esusu-`, `site === Site.ESUSU`. | must |
...
```ts
@SourcePlugin({ site: Site.ESUSU, name: 'Esusu', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/esusu/jobs/<id>`), wire
  `company_name` pass-through (`'Esusu'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/esusu/jobs/<id>`).
