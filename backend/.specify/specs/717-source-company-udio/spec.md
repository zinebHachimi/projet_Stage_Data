# Spec: 717 — Source Company Plugin: Udio

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 717                            |
| Slug           | source-company-udio               |
| Status         | accepted                               |
...
Udio publishes its careers board through Greenhouse at
the bare slug `udio`. The run-398 batch sweep confirmed
...
- Ship a `source-company-udio` plugin returning live
...
- Backfilling historical Udio postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UDIO`** in the
> source registry, so that **a single `siteType: [Site.UDIO]`
> request returns Udio's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UDIO = 'udio'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-udio`.                  | must     |
| FR-3  | `UdioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `udio-`, `site === Site.UDIO`. | must |
...
```ts
@SourcePlugin({ site: Site.UDIO, name: 'Udio', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/udio/jobs/<id>`), wire
  `company_name` pass-through (`'Udio'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/udio/jobs/<id>`).
