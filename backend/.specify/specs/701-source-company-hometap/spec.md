# Spec: 701 — Source Company Plugin: Hometap

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 701                            |
| Slug           | source-company-hometap               |
| Status         | accepted                               |
...
Hometap publishes its careers board through Greenhouse at
the bare slug `hometap`. The run-398 batch sweep confirmed
...
- Ship a `source-company-hometap` plugin returning live
...
- Backfilling historical Hometap postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HOMETAP`** in the
> source registry, so that **a single `siteType: [Site.HOMETAP]`
> request returns Hometap's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HOMETAP = 'hometap'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-hometap`.                  | must     |
| FR-3  | `HometapService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `hometap-`, `site === Site.HOMETAP`. | must |
...
```ts
@SourcePlugin({ site: Site.HOMETAP, name: 'Hometap', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/hometap/jobs/<id>`), wire
  `company_name` pass-through (`'Hometap'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/hometap/jobs/<id>`).
