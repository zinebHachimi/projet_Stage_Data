# Spec: 802 — Source Company Plugin: Via

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 802                            |
| Slug           | source-company-via               |
| Status         | accepted                               |
...
Via publishes its careers board through Greenhouse at
the bare slug `via`. The run-398 batch sweep confirmed
...
- Ship a `source-company-via` plugin returning live
...
- Backfilling historical Via postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VIA`** in the
> source registry, so that **a single `siteType: [Site.VIA]`
> request returns Via's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VIA = 'via'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-via`.                  | must     |
| FR-3  | `ViaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `via-`, `site === Site.VIA`. | must |
...
```ts
@SourcePlugin({ site: Site.VIA, name: 'Via', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/via/jobs/<id>`), wire
  `company_name` pass-through (`'Via'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/via/jobs/<id>`).
