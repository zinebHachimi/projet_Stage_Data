# Plan: 802 — Source Company Plugin: Via
...
Via's careers board is hosted on Greenhouse at the slug
`via`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-via` plugin.
- Deliverables: package files, `Site.VIA` enum value, the
...
- `packages/plugins/source-company-via` (new)
