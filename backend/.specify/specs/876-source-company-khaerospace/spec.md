# Spec: 876 — Source Company Plugin: KH Aerospace

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 876                            |
| Slug           | source-company-khaerospace               |
| Status         | accepted                               |
...
KH Aerospace publishes its careers board through Greenhouse at
the bare slug `khaerospace`. The run-398 batch sweep confirmed
...
- Ship a `source-company-khaerospace` plugin returning live
...
- Backfilling historical KH Aerospace postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KH_AEROSPACE`** in the
> source registry, so that **a single `siteType: [Site.KH_AEROSPACE]`
> request returns KH Aerospace's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KH_AEROSPACE = 'khaerospace'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-khaerospace`.                  | must     |
| FR-3  | `KHAerospaceService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `khaerospace-`, `site === Site.KH_AEROSPACE`. | must |
...
```ts
@SourcePlugin({ site: Site.KH_AEROSPACE, name: 'KH Aerospace', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/khaerospace/jobs/<id>`), wire
  `company_name` pass-through (`'KH Aerospace'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/khaerospace/jobs/<id>`).
