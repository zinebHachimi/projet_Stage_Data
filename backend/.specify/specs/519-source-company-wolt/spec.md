# Spec: 519 — Source Company Plugin: Wolt

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 519                            |
| Slug           | source-company-wolt               |
| Status         | accepted                               |
...
Wolt publishes its careers board through Greenhouse at
the bare slug `wolt`. The run-398 batch sweep confirmed
289 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/wolt/jobs?content=true`.
Sector: Food Delivery / Local Commerce. HQ: Helsinki, Finland.

## 2. Goals

- Ship a `source-company-wolt` plugin returning live
...
- Backfilling historical Wolt postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.WOLT`** in the
> source registry, so that **a single `siteType: [Site.WOLT]`
> request returns Wolt's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.WOLT = 'wolt'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-wolt`.                  | must     |
| FR-3  | `WoltService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `wolt-`, `site === Site.WOLT`. | must |
...
```ts
@SourcePlugin({ site: Site.WOLT, name: 'Wolt', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/wolt/jobs/<id>`), wire
  `company_name` pass-through (`'Wolt'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/wolt/jobs/<id>`).
