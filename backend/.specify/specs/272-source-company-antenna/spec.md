# Spec: 272 — Source Company Plugin: Antenna

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 272                            |
| Slug           | source-company-antenna               |
| Status         | accepted                               |
...
Antenna publishes its careers board through Greenhouse at
the bare slug `antenna`. The run-398 batch sweep confirmed
...
- Ship a `source-company-antenna` plugin returning live
...
- Backfilling historical Antenna postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ANTENNA`** in the
> source registry, so that **a single `siteType: [Site.ANTENNA]`
> request returns Antenna's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ANTENNA = 'antenna'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-antenna`.                  | must     |
| FR-3  | `AntennaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `antenna-`, `site === Site.ANTENNA`. | must |
...
```ts
@SourcePlugin({ site: Site.ANTENNA, name: 'Antenna', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/antenna/jobs/<id>`), wire
  `company_name` pass-through (`'Antenna'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/antenna/jobs/<id>`).
