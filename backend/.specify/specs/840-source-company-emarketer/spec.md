# Spec: 840 — Source Company Plugin: EMARKETER

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 840                            |
| Slug           | source-company-emarketer               |
| Status         | accepted                               |
...
EMARKETER publishes its careers board through Greenhouse at
the bare slug `emarketer`. The run-398 batch sweep confirmed
...
- Ship a `source-company-emarketer` plugin returning live
...
- Backfilling historical EMARKETER postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.EMARKETER`** in the
> source registry, so that **a single `siteType: [Site.EMARKETER]`
> request returns EMARKETER's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.EMARKETER = 'emarketer'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-emarketer`.                  | must     |
| FR-3  | `EMARKETERService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `emarketer-`, `site === Site.EMARKETER`. | must |
...
```ts
@SourcePlugin({ site: Site.EMARKETER, name: 'EMARKETER', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/emarketer/jobs/<id>`), wire
  `company_name` pass-through (`'EMARKETER'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/emarketer/jobs/<id>`).
