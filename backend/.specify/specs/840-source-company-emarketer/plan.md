# Plan: 840 — Source Company Plugin: EMARKETER
...
EMARKETER's careers board is hosted on Greenhouse at the slug
`emarketer`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-emarketer` plugin.
- Deliverables: package files, `Site.EMARKETER` enum value, the
...
- `packages/plugins/source-company-emarketer` (new)
