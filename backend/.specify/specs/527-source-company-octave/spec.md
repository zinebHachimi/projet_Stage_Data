# Spec: 527 — Source Company Plugin: Octave

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 527                            |
| Slug           | source-company-octave               |
| Status         | accepted                               |
...
Octave publishes its careers board through Greenhouse at
the bare slug `octave`. The run-398 batch sweep confirmed
...
- Ship a `source-company-octave` plugin returning live
...
- Backfilling historical Octave postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OCTAVE_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.OCTAVE_HEALTH]`
> request returns Octave's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OCTAVE_HEALTH = 'octave'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-octave`.                  | must     |
| FR-3  | `OctaveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `octave-`, `site === Site.OCTAVE_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.OCTAVE_HEALTH, name: 'Octave', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/octave/jobs/<id>`), wire
  `company_name` pass-through (`'Octave'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/octave/jobs/<id>`).
