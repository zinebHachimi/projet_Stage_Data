# Spec: 709 — Source Company Plugin: Bybit

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 709                            |
| Slug           | source-company-bybit               |
| Status         | accepted                               |
...
Bybit publishes its careers board through Greenhouse at
the bare slug `bybit`. The run-398 batch sweep confirmed
...
- Ship a `source-company-bybit` plugin returning live
...
- Backfilling historical Bybit postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BYBIT`** in the
> source registry, so that **a single `siteType: [Site.BYBIT]`
> request returns Bybit's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BYBIT = 'bybit'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-bybit`.                  | must     |
| FR-3  | `BybitService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `bybit-`, `site === Site.BYBIT`. | must |
...
```ts
@SourcePlugin({ site: Site.BYBIT, name: 'Bybit', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/bybit/jobs/<id>`), wire
  `company_name` pass-through (`'Bybit'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/bybit/jobs/<id>`).
