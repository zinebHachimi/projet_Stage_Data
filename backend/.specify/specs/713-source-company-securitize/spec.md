# Spec: 713 — Source Company Plugin: Securitize

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 713                            |
| Slug           | source-company-securitize               |
| Status         | accepted                               |
...
Securitize publishes its careers board through Greenhouse at
the bare slug `securitize`. The run-398 batch sweep confirmed
...
- Ship a `source-company-securitize` plugin returning live
...
- Backfilling historical Securitize postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SECURITIZE`** in the
> source registry, so that **a single `siteType: [Site.SECURITIZE]`
> request returns Securitize's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SECURITIZE = 'securitize'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-securitize`.                  | must     |
| FR-3  | `SecuritizeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `securitize-`, `site === Site.SECURITIZE`. | must |
...
```ts
@SourcePlugin({ site: Site.SECURITIZE, name: 'Securitize', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/securitize/jobs/<id>`), wire
  `company_name` pass-through (`'Securitize'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/securitize/jobs/<id>`).
