# Spec: 962 — Source Company Plugin: Uber Freight

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 962                            |
| Slug           | source-company-uberfreight               |
| Status         | accepted                               |
...
Uber Freight publishes its careers board through Greenhouse at
the bare slug `uberfreight`. The run-398 batch sweep confirmed
...
- Ship a `source-company-uberfreight` plugin returning live
...
- Backfilling historical Uber Freight postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UBER_FREIGHT`** in the
> source registry, so that **a single `siteType: [Site.UBER_FREIGHT]`
> request returns Uber Freight's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UBER_FREIGHT = 'uberfreight'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-uberfreight`.                  | must     |
| FR-3  | `UberFreightService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `uberfreight-`, `site === Site.UBER_FREIGHT`. | must |
...
```ts
@SourcePlugin({ site: Site.UBER_FREIGHT, name: 'Uber Freight', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/uberfreight/jobs/<id>`), wire
  `company_name` pass-through (`'Uber Freight'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/uberfreight/jobs/<id>`).
