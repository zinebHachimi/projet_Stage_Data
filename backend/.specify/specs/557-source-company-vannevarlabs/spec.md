# Spec: 557 — Source Company Plugin: Vannevar Labs

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 557                            |
| Slug           | source-company-vannevarlabs               |
| Status         | accepted                               |
...
Vannevar Labs publishes its careers board through Greenhouse at
the bare slug `vannevarlabs`. The run-398 batch sweep confirmed
...
- Ship a `source-company-vannevarlabs` plugin returning live
...
- Backfilling historical Vannevar Labs postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VANNEVAR_LABS`** in the
> source registry, so that **a single `siteType: [Site.VANNEVAR_LABS]`
> request returns Vannevar Labs's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VANNEVAR_LABS = 'vannevarlabs'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-vannevarlabs`.                  | must     |
| FR-3  | `VannevarlabsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `vannevarlabs-`, `site === Site.VANNEVAR_LABS`. | must |
...
```ts
@SourcePlugin({ site: Site.VANNEVAR_LABS, name: 'Vannevar Labs', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/vannevarlabs/jobs/<id>`), wire
  `company_name` pass-through (`'Vannevar Labs'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/vannevarlabs/jobs/<id>`).
