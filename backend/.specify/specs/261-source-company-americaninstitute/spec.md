# Spec: 261 — Source Company Plugin: American Institute

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 261                            |
| Slug           | source-company-americaninstitute               |
| Status         | accepted                               |
...
American Institute publishes its careers board through Greenhouse at
the bare slug `americaninstitute`. The run-398 batch sweep confirmed
17 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/americaninstitute/jobs?content=true`.
Sector: Education / Vocational Training. HQ: Unknown.

## 2. Goals

- Ship a `source-company-americaninstitute` plugin returning live
...
- Backfilling historical American Institute postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMERICANINSTITUTE`** in the
> source registry, so that **a single `siteType: [Site.AMERICANINSTITUTE]`
> request returns American Institute's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMERICANINSTITUTE = 'americaninstitute'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-americaninstitute`.                  | must     |
| FR-3  | `AmericaninstituteService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `americaninstitute-`, `site === Site.AMERICANINSTITUTE`. | must |
...
```ts
@SourcePlugin({ site: Site.AMERICANINSTITUTE, name: 'American Institute', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/americaninstitute/jobs/<id>`), wire
  `company_name` pass-through (`'American Institute'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/americaninstitute/jobs/<id>`).
