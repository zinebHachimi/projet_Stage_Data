# Spec: 492 — Source Company Plugin: Strive Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 492                            |
| Slug           | source-company-strivehealth               |
| Status         | accepted                               |
...
Strive Health publishes its careers board through Greenhouse at
the bare slug `strivehealth`. The run-398 batch sweep confirmed
47 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/strivehealth/jobs?content=true`.
Sector: Health-tech / Value-based care. HQ: Denver, CO, USA.

## 2. Goals

- Ship a `source-company-strivehealth` plugin returning live
...
- Backfilling historical Strive Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.STRIVEHEALTH`** in the
> source registry, so that **a single `siteType: [Site.STRIVEHEALTH]`
> request returns Strive Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.STRIVEHEALTH = 'strivehealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-strivehealth`.                  | must     |
| FR-3  | `StrivehealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `strivehealth-`, `site === Site.STRIVEHEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.STRIVEHEALTH, name: 'Strive Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/strivehealth/jobs/<id>`), wire
  `company_name` pass-through (`'Strive Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/strivehealth/jobs/<id>`).
