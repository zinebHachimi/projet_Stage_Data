# Spec: 638 — Source Company Plugin: Bitso

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 638                            |
| Slug           | source-company-bitso               |
| Status         | accepted                               |
...
Bitso publishes its careers board through Greenhouse at
the bare slug `bitso`. The run-398 batch sweep confirmed
...
- Ship a `source-company-bitso` plugin returning live
...
- Backfilling historical Bitso postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BITSO`** in the
> source registry, so that **a single `siteType: [Site.BITSO]`
> request returns Bitso's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BITSO = 'bitso'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-bitso`.                  | must     |
| FR-3  | `BitsoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `bitso-`, `site === Site.BITSO`. | must |
...
```ts
@SourcePlugin({ site: Site.BITSO, name: 'Bitso', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/bitso/jobs/<id>`), wire
  `company_name` pass-through (`'Bitso'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/bitso/jobs/<id>`).
