# Spec: 690 — Source Company Plugin: Ozow

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 690                            |
| Slug           | source-company-ozow               |
| Status         | accepted                               |
...
Ozow publishes its careers board through Greenhouse at
the bare slug `ozow`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ozow` plugin returning live
...
- Backfilling historical Ozow postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OZOW`** in the
> source registry, so that **a single `siteType: [Site.OZOW]`
> request returns Ozow's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OZOW = 'ozow'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ozow`.                  | must     |
| FR-3  | `OzowService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ozow-`, `site === Site.OZOW`. | must |
...
```ts
@SourcePlugin({ site: Site.OZOW, name: 'Ozow', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ozow/jobs/<id>`), wire
  `company_name` pass-through (`'Ozow'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ozow/jobs/<id>`).
