# Spec: 651 — Source Company Plugin: Ethos Life

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 651                            |
| Slug           | source-company-ethoslife               |
| Status         | accepted                               |
...
Ethos Life publishes its careers board through Greenhouse at
the bare slug `ethoslife`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ethoslife` plugin returning live
...
- Backfilling historical Ethos Life postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ETHOS_LIFE`** in the
> source registry, so that **a single `siteType: [Site.ETHOS_LIFE]`
> request returns Ethos Life's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ETHOS_LIFE = 'ethoslife'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ethoslife`.                  | must     |
| FR-3  | `EthosLifeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ethoslife-`, `site === Site.ETHOS_LIFE`. | must |
...
```ts
@SourcePlugin({ site: Site.ETHOS_LIFE, name: 'Ethos Life', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ethoslife/jobs/<id>`), wire
  `company_name` pass-through (`'Ethos Life'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ethoslife/jobs/<id>`).
