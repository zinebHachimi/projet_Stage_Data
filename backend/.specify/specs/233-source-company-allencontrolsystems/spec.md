# Spec: 233 — Source Company Plugin: Allen Control Systems

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 233                            |
| Slug           | source-company-allencontrolsystems               |
| Status         | accepted                               |
...
Allen Control Systems publishes its careers board through Greenhouse at
the bare slug `allencontrolsystems`. The run-398 batch sweep confirmed
68 live role(s) via a direct probe of
...
- Ship a `source-company-allencontrolsystems` plugin returning live
...
- Backfilling historical Allen Control Systems postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALLENCONTROLSYSTEMS`** in the
> source registry, so that **a single `siteType: [Site.ALLENCONTROLSYSTEMS]`
> request returns Allen Control Systems's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALLENCONTROLSYSTEMS = 'allencontrolsystems'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-allencontrolsystems`.                  | must     |
| FR-3  | `AllencontrolsystemsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `allencontrolsystems-`, `site === Site.ALLENCONTROLSYSTEMS`. | must |
...
```ts
@SourcePlugin({ site: Site.ALLENCONTROLSYSTEMS, name: 'Allen Control Systems', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/allencontrolsystems/jobs/<id>`), wire
  `company_name` pass-through (`'Allen Control Systems'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/allencontrolsystems/jobs/<id>`).
