# Spec: 512 — Source Company Plugin: Public

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 512                            |
| Slug           | source-company-public               |
| Status         | accepted                               |
...
Public publishes its careers board through Greenhouse at
the bare slug `public`. The run-398 batch sweep confirmed
...
- Ship a `source-company-public` plugin returning live
...
- Backfilling historical Public postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PUBLIC_INVEST`** in the
> source registry, so that **a single `siteType: [Site.PUBLIC_INVEST]`
> request returns Public's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PUBLIC_INVEST = 'public'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-public`.                  | must     |
| FR-3  | `PublicService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `public-`, `site === Site.PUBLIC_INVEST`. | must |
...
```ts
@SourcePlugin({ site: Site.PUBLIC_INVEST, name: 'Public', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/public/jobs/<id>`), wire
  `company_name` pass-through (`'Public'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/public/jobs/<id>`).
