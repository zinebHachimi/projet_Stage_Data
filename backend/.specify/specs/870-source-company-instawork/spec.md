# Spec: 870 — Source Company Plugin: Instawork

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 870                            |
| Slug           | source-company-instawork               |
| Status         | accepted                               |
...
Instawork publishes its careers board through Greenhouse at
the bare slug `instawork`. The run-398 batch sweep confirmed
64 live role(s) via a direct probe of
...
- Ship a `source-company-instawork` plugin returning live
...
- Backfilling historical Instawork postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INSTAWORK`** in the
> source registry, so that **a single `siteType: [Site.INSTAWORK]`
> request returns Instawork's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INSTAWORK = 'instawork'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-instawork`.                  | must     |
| FR-3  | `InstaworkService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `instawork-`, `site === Site.INSTAWORK`. | must |
...
```ts
@SourcePlugin({ site: Site.INSTAWORK, name: 'Instawork', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/instawork/jobs/<id>`), wire
  `company_name` pass-through (`'Instawork'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/instawork/jobs/<id>`).
