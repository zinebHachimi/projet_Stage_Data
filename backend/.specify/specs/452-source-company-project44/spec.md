# Spec: 452 — Source Company Plugin: project44

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 452                            |
| Slug           | source-company-project44               |
| Status         | accepted                               |
...
project44 publishes its careers board through Greenhouse at
the bare slug `project44`. The run-398 batch sweep confirmed
...
- Ship a `source-company-project44` plugin returning live
...
- Backfilling historical project44 postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PROJECT44`** in the
> source registry, so that **a single `siteType: [Site.PROJECT44]`
> request returns project44's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PROJECT44 = 'project44'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-project44`.                  | must     |
| FR-3  | `Project44Service.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `project44-`, `site === Site.PROJECT44`. | must |
...
```ts
@SourcePlugin({ site: Site.PROJECT44, name: 'project44', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/project44/jobs/<id>`), wire
  `company_name` pass-through (`'project44'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/project44/jobs/<id>`).
