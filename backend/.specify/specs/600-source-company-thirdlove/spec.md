# Spec: 600 — Source Company Plugin: ThirdLove

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 600                            |
| Slug           | source-company-thirdlove               |
| Status         | accepted                               |
...
ThirdLove publishes its careers board through Greenhouse at
the bare slug `thirdlove`. The run-398 batch sweep confirmed
...
- Ship a `source-company-thirdlove` plugin returning live
...
- Backfilling historical ThirdLove postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.THIRDLOVE`** in the
> source registry, so that **a single `siteType: [Site.THIRDLOVE]`
> request returns ThirdLove's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.THIRDLOVE = 'thirdlove'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-thirdlove`.                  | must     |
| FR-3  | `ThirdLoveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `thirdlove-`, `site === Site.THIRDLOVE`. | must |
...
```ts
@SourcePlugin({ site: Site.THIRDLOVE, name: 'ThirdLove', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/thirdlove/jobs/<id>`), wire
  `company_name` pass-through (`'ThirdLove'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/thirdlove/jobs/<id>`).
