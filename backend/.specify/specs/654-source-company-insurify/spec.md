# Spec: 654 — Source Company Plugin: Insurify

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 654                            |
| Slug           | source-company-insurify               |
| Status         | accepted                               |
...
Insurify publishes its careers board through Greenhouse at
the bare slug `insurify`. The run-398 batch sweep confirmed
...
- Ship a `source-company-insurify` plugin returning live
...
- Backfilling historical Insurify postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INSURIFY`** in the
> source registry, so that **a single `siteType: [Site.INSURIFY]`
> request returns Insurify's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INSURIFY = 'insurify'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-insurify`.                  | must     |
| FR-3  | `InsurifyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `insurify-`, `site === Site.INSURIFY`. | must |
...
```ts
@SourcePlugin({ site: Site.INSURIFY, name: 'Insurify', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/insurify/jobs/<id>`), wire
  `company_name` pass-through (`'Insurify'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/insurify/jobs/<id>`).
