# Spec: 524 — Source Company Plugin: Tia

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 524                            |
| Slug           | source-company-tia               |
| Status         | accepted                               |
...
Tia publishes its careers board through Greenhouse at
the bare slug `tia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-tia` plugin returning live
...
- Backfilling historical Tia postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TIA_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.TIA_HEALTH]`
> request returns Tia's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TIA_HEALTH = 'tia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-tia`.                  | must     |
| FR-3  | `TiaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `tia-`, `site === Site.TIA_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.TIA_HEALTH, name: 'Tia', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/tia/jobs/<id>`), wire
  `company_name` pass-through (`'Tia'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/tia/jobs/<id>`).
