# Plan: 032 — Source Company Plugin: Figma
...
Figma's careers board is hosted on Greenhouse at the slug `figma`, so
the implementation is a thin wrapper around the same public Greenhouse
endpoint that `source-company-asana` (Spec 031 / run #241),
`source-company-plaid` (Spec 030), `source-company-lyft` (Spec 029),
...
Asana plugin's shape (single-file `service.ts`, three-line
`module.ts`, two-line `index.ts`, four-line `package.json`,
three-line `tsconfig.json`) and rename references from Asana → Figma,
...
used twenty times before (Amazon, Apple, Cursor, Google, IBM,
...
- **Goal:** Land `source-company-figma` under `ALL_SOURCE_MODULES`
  with passing unit tests and a green doc-lint pass.
- **Deliverables:**
  - `packages/plugins/source-company-figma/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,figma.module.ts,figma.service.ts}`,
  ...
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 032 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-figma` → all green.
...
| `packages/models/src/enums/site.enum.ts`            | append `FIGMA = 'figma'`.                                    |
| `packages/plugins/index.ts`                         | import + append `FigmaModule` to `ALL_SOURCE_MODULES`.       |
| `tsconfig.base.json`                                | path-alias entry.                                            |
| `jest.config.js`                                    | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                   | add Figma shipped row.                                       |
| `docs/index.md`                                     | append Spec 032 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Figma moves off Greenhouse mid-maintenance.         | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-asana`'s shape.          | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.FIGMA` is unused outside this plugin, so the enum revert is
  safe.
...
(None — see Spec 032 § 10 Decisions for the five resolved points.)
