# Spec: 642 — Source Company Plugin: Incode Technologies

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 642                            |
| Slug           | source-company-incode               |
| Status         | accepted                               |
...
Incode Technologies publishes its careers board through Greenhouse at
the bare slug `incode`. The run-398 batch sweep confirmed
...
- Ship a `source-company-incode` plugin returning live
...
- Backfilling historical Incode Technologies postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.INCODE_TECHNOLOGIES`** in the
> source registry, so that **a single `siteType: [Site.INCODE_TECHNOLOGIES]`
> request returns Incode Technologies's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.INCODE_TECHNOLOGIES = 'incode'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-incode`.                  | must     |
| FR-3  | `IncodeTechnologiesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `incode-`, `site === Site.INCODE_TECHNOLOGIES`. | must |
...
```ts
@SourcePlugin({ site: Site.INCODE_TECHNOLOGIES, name: 'Incode Technologies', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/incode/jobs/<id>`), wire
  `company_name` pass-through (`'Incode Technologies'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/incode/jobs/<id>`).
