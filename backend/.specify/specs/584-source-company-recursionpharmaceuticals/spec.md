# Spec: 584 — Source Company Plugin: Recursion

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 584                            |
| Slug           | source-company-recursionpharmaceuticals               |
| Status         | accepted                               |
...
Recursion publishes its careers board through Greenhouse at
the bare slug `recursionpharmaceuticals`. The run-398 batch sweep confirmed
...
- Ship a `source-company-recursionpharmaceuticals` plugin returning live
...
- Backfilling historical Recursion postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RECURSION`** in the
> source registry, so that **a single `siteType: [Site.RECURSION]`
> request returns Recursion's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RECURSION = 'recursionpharmaceuticals'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-recursionpharmaceuticals`.                  | must     |
| FR-3  | `RecursionService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `recursionpharmaceuticals-`, `site === Site.RECURSION`. | must |
...
```ts
@SourcePlugin({ site: Site.RECURSION, name: 'Recursion', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/recursionpharmaceuticals/jobs/<id>`), wire
  `company_name` pass-through (`'Recursion'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/recursionpharmaceuticals/jobs/<id>`).
