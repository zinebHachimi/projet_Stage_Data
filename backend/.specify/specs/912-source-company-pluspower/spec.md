# Spec: 912 — Source Company Plugin: Plus Power

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 912                            |
| Slug           | source-company-pluspower               |
| Status         | accepted                               |
...
Plus Power publishes its careers board through Greenhouse at
the bare slug `pluspower`. The run-398 batch sweep confirmed
...
- Ship a `source-company-pluspower` plugin returning live
...
- Backfilling historical Plus Power postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PLUS_POWER`** in the
> source registry, so that **a single `siteType: [Site.PLUS_POWER]`
> request returns Plus Power's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PLUS_POWER = 'pluspower'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-pluspower`.                  | must     |
| FR-3  | `PlusPowerService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `pluspower-`, `site === Site.PLUS_POWER`. | must |
...
```ts
@SourcePlugin({ site: Site.PLUS_POWER, name: 'Plus Power', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/pluspower/jobs/<id>`), wire
  `company_name` pass-through (`'Plus Power'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/pluspower/jobs/<id>`).
