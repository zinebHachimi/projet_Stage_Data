# Spec: 757 — Source Company Plugin: Astranis

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 757                            |
| Slug           | source-company-astranis               |
| Status         | accepted                               |
...
Astranis publishes its careers board through Greenhouse at
the bare slug `astranis`. The run-398 batch sweep confirmed
...
- Ship a `source-company-astranis` plugin returning live
...
- Backfilling historical Astranis postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ASTRANIS`** in the
> source registry, so that **a single `siteType: [Site.ASTRANIS]`
> request returns Astranis's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ASTRANIS = 'astranis'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-astranis`.                  | must     |
| FR-3  | `AstranisService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `astranis-`, `site === Site.ASTRANIS`. | must |
...
```ts
@SourcePlugin({ site: Site.ASTRANIS, name: 'Astranis', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/astranis/jobs/<id>`), wire
  `company_name` pass-through (`'Astranis'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/astranis/jobs/<id>`).
