# Spec: 807 — Source Company Plugin: Anduril Industries

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 807                            |
| Slug           | source-company-andurilindustries               |
| Status         | accepted                               |
...
Anduril Industries publishes its careers board through Greenhouse at
the bare slug `andurilindustries`. The run-398 batch sweep confirmed
2128 live role(s) via a direct probe of
...
- Ship a `source-company-andurilindustries` plugin returning live
...
- Backfilling historical Anduril Industries postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ANDURIL_INDUSTRIES`** in the
> source registry, so that **a single `siteType: [Site.ANDURIL_INDUSTRIES]`
> request returns Anduril Industries's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ANDURIL_INDUSTRIES = 'andurilindustries'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-andurilindustries`.                  | must     |
| FR-3  | `AndurilIndustriesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `andurilindustries-`, `site === Site.ANDURIL_INDUSTRIES`. | must |
...
```ts
@SourcePlugin({ site: Site.ANDURIL_INDUSTRIES, name: 'Anduril Industries', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/andurilindustries/jobs/<id>`), wire
  `company_name` pass-through (`'Anduril Industries'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/andurilindustries/jobs/<id>`).
