# Spec: 707 — Source Company Plugin: Xendit

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 707                            |
| Slug           | source-company-xendit               |
| Status         | accepted                               |
...
Xendit publishes its careers board through Greenhouse at
the bare slug `xendit`. The run-398 batch sweep confirmed
...
- Ship a `source-company-xendit` plugin returning live
...
- Backfilling historical Xendit postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.XENDIT`** in the
> source registry, so that **a single `siteType: [Site.XENDIT]`
> request returns Xendit's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.XENDIT = 'xendit'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-xendit`.                  | must     |
| FR-3  | `XenditService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `xendit-`, `site === Site.XENDIT`. | must |
...
```ts
@SourcePlugin({ site: Site.XENDIT, name: 'Xendit', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/xendit/jobs/<id>`), wire
  `company_name` pass-through (`'Xendit'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/xendit/jobs/<id>`).
