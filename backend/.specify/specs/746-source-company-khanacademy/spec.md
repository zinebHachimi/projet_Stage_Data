# Spec: 746 — Source Company Plugin: Khan Academy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 746                            |
| Slug           | source-company-khanacademy               |
| Status         | accepted                               |
...
Khan Academy publishes its careers board through Greenhouse at
the bare slug `khanacademy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-khanacademy` plugin returning live
...
- Backfilling historical Khan Academy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KHAN_ACADEMY`** in the
> source registry, so that **a single `siteType: [Site.KHAN_ACADEMY]`
> request returns Khan Academy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KHAN_ACADEMY = 'khanacademy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-khanacademy`.                  | must     |
| FR-3  | `KhanAcademyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `khanacademy-`, `site === Site.KHAN_ACADEMY`. | must |
...
```ts
@SourcePlugin({ site: Site.KHAN_ACADEMY, name: 'Khan Academy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/khanacademy/jobs/<id>`), wire
  `company_name` pass-through (`'Khan Academy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/khanacademy/jobs/<id>`).
