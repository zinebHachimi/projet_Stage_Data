# Spec: 604 — Source Company Plugin: Karat

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 604                            |
| Slug           | source-company-karat               |
| Status         | accepted                               |
...
Karat publishes its careers board through Greenhouse at
the bare slug `karat`. The run-398 batch sweep confirmed
...
- Ship a `source-company-karat` plugin returning live
...
- Backfilling historical Karat postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.KARAT_INTERVIEWS`** in the
> source registry, so that **a single `siteType: [Site.KARAT_INTERVIEWS]`
> request returns Karat's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.KARAT_INTERVIEWS = 'karat'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-karat`.                  | must     |
| FR-3  | `KaratService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `karat-`, `site === Site.KARAT_INTERVIEWS`. | must |
...
```ts
@SourcePlugin({ site: Site.KARAT_INTERVIEWS, name: 'Karat', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/karat/jobs/<id>`), wire
  `company_name` pass-through (`'Karat'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/karat/jobs/<id>`).
