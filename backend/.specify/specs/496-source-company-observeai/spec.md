# Spec: 496 — Source Company Plugin: Observe.AI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 496                            |
| Slug           | source-company-observeai               |
| Status         | accepted                               |
...
Observe.AI publishes its careers board through Greenhouse at
the bare slug `observeai`. The run-398 batch sweep confirmed
...
- Ship a `source-company-observeai` plugin returning live
...
- Backfilling historical Observe.AI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OBSERVEAI`** in the
> source registry, so that **a single `siteType: [Site.OBSERVEAI]`
> request returns Observe.AI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OBSERVEAI = 'observeai'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-observeai`.                  | must     |
| FR-3  | `ObserveaiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `observeai-`, `site === Site.OBSERVEAI`. | must |
...
```ts
@SourcePlugin({ site: Site.OBSERVEAI, name: 'Observe.AI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/observeai/jobs/<id>`), wire
  `company_name` pass-through (`'Observe.AI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/observeai/jobs/<id>`).
