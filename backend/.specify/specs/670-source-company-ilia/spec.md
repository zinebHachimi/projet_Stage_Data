# Spec: 670 — Source Company Plugin: Ilia Digital

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 670                            |
| Slug           | source-company-ilia               |
| Status         | accepted                               |
...
Ilia Digital publishes its careers board through Greenhouse at
the bare slug `ilia`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ilia` plugin returning live
...
- Backfilling historical Ilia Digital postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ILIA_DIGITAL`** in the
> source registry, so that **a single `siteType: [Site.ILIA_DIGITAL]`
> request returns Ilia Digital's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ILIA_DIGITAL = 'ilia'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ilia`.                  | must     |
| FR-3  | `IliaDigitalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ilia-`, `site === Site.ILIA_DIGITAL`. | must |
...
```ts
@SourcePlugin({ site: Site.ILIA_DIGITAL, name: 'Ilia Digital', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ilia/jobs/<id>`), wire
  `company_name` pass-through (`'Ilia Digital'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ilia/jobs/<id>`).
