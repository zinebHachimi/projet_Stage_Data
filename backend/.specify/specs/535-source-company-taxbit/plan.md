# Plan: 535 — Source Company Plugin: TaxBit
...
TaxBit's careers board is hosted on Greenhouse at the slug
`taxbit`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-taxbit` plugin.
- Deliverables: package files, `Site.TAXBIT` enum value, the
...
- `packages/plugins/source-company-taxbit` (new)
