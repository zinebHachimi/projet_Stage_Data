# Spec: 535 — Source Company Plugin: TaxBit

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 535                            |
| Slug           | source-company-taxbit               |
| Status         | accepted                               |
...
TaxBit publishes its careers board through Greenhouse at
the bare slug `taxbit`. The run-398 batch sweep confirmed
...
- Ship a `source-company-taxbit` plugin returning live
...
- Backfilling historical TaxBit postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TAXBIT`** in the
> source registry, so that **a single `siteType: [Site.TAXBIT]`
> request returns TaxBit's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TAXBIT = 'taxbit'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-taxbit`.                  | must     |
| FR-3  | `TaxbitService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `taxbit-`, `site === Site.TAXBIT`. | must |
...
```ts
@SourcePlugin({ site: Site.TAXBIT, name: 'TaxBit', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/taxbit/jobs/<id>`), wire
  `company_name` pass-through (`'TaxBit'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/taxbit/jobs/<id>`).
