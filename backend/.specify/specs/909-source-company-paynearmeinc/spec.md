# Spec: 909 — Source Company Plugin: PayNearMe

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 909                            |
| Slug           | source-company-paynearmeinc               |
| Status         | accepted                               |
...
PayNearMe publishes its careers board through Greenhouse at
the bare slug `paynearmeinc`. The run-398 batch sweep confirmed
...
- Ship a `source-company-paynearmeinc` plugin returning live
...
- Backfilling historical PayNearMe postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PAYNEARME`** in the
> source registry, so that **a single `siteType: [Site.PAYNEARME]`
> request returns PayNearMe's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PAYNEARME = 'paynearmeinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-paynearmeinc`.                  | must     |
| FR-3  | `PayNearMeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `paynearmeinc-`, `site === Site.PAYNEARME`. | must |
...
```ts
@SourcePlugin({ site: Site.PAYNEARME, name: 'PayNearMe', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/paynearmeinc/jobs/<id>`), wire
  `company_name` pass-through (`'PayNearMe'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/paynearmeinc/jobs/<id>`).
