# Spec: 861 — Source Company Plugin: Ghost Story Games

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 861                            |
| Slug           | source-company-gsgcareers               |
| Status         | accepted                               |
...
Ghost Story Games publishes its careers board through Greenhouse at
the bare slug `gsgcareers`. The run-398 batch sweep confirmed
...
- Ship a `source-company-gsgcareers` plugin returning live
...
- Backfilling historical Ghost Story Games postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GHOST_STORY_GAMES`** in the
> source registry, so that **a single `siteType: [Site.GHOST_STORY_GAMES]`
> request returns Ghost Story Games's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GHOST_STORY_GAMES = 'gsgcareers'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-gsgcareers`.                  | must     |
| FR-3  | `GhostStoryGamesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `gsgcareers-`, `site === Site.GHOST_STORY_GAMES`. | must |
...
```ts
@SourcePlugin({ site: Site.GHOST_STORY_GAMES, name: 'Ghost Story Games', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/gsgcareers/jobs/<id>`), wire
  `company_name` pass-through (`'Ghost Story Games'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/gsgcareers/jobs/<id>`).
