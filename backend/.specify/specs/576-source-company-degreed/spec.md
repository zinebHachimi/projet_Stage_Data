# Spec: 576 — Source Company Plugin: Degreed

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 576                            |
| Slug           | source-company-degreed               |
| Status         | accepted                               |
...
Degreed publishes its careers board through Greenhouse at
the bare slug `degreed`. The run-398 batch sweep confirmed
...
- Ship a `source-company-degreed` plugin returning live
...
- Backfilling historical Degreed postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DEGREED`** in the
> source registry, so that **a single `siteType: [Site.DEGREED]`
> request returns Degreed's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DEGREED = 'degreed'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-degreed`.                  | must     |
| FR-3  | `DegreedService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `degreed-`, `site === Site.DEGREED`. | must |
...
```ts
@SourcePlugin({ site: Site.DEGREED, name: 'Degreed', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/degreed/jobs/<id>`), wire
  `company_name` pass-through (`'Degreed'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/degreed/jobs/<id>`).
