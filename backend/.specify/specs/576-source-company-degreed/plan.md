# Plan: 576 — Source Company Plugin: Degreed
...
Degreed's careers board is hosted on Greenhouse at the slug
`degreed`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-degreed` plugin.
- Deliverables: package files, `Site.DEGREED` enum value, the
...
- `packages/plugins/source-company-degreed` (new)
