# Spec: 574 — Source Company Plugin: Outschool

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 574                            |
| Slug           | source-company-outschool               |
| Status         | accepted                               |
...
Outschool publishes its careers board through Greenhouse at
the bare slug `outschool`. The run-398 batch sweep confirmed
...
- Ship a `source-company-outschool` plugin returning live
...
- Backfilling historical Outschool postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OUTSCHOOL`** in the
> source registry, so that **a single `siteType: [Site.OUTSCHOOL]`
> request returns Outschool's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OUTSCHOOL = 'outschool'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-outschool`.                  | must     |
| FR-3  | `OutschoolService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `outschool-`, `site === Site.OUTSCHOOL`. | must |
...
```ts
@SourcePlugin({ site: Site.OUTSCHOOL, name: 'Outschool', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/outschool/jobs/<id>`), wire
  `company_name` pass-through (`'Outschool'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/outschool/jobs/<id>`).
