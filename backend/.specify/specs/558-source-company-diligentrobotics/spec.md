# Spec: 558 — Source Company Plugin: Diligent Robotics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 558                            |
| Slug           | source-company-diligentrobotics               |
| Status         | accepted                               |
...
Diligent Robotics publishes its careers board through Greenhouse at
the bare slug `diligentrobotics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-diligentrobotics` plugin returning live
...
- Backfilling historical Diligent Robotics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DILIGENT_ROBOTICS`** in the
> source registry, so that **a single `siteType: [Site.DILIGENT_ROBOTICS]`
> request returns Diligent Robotics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DILIGENT_ROBOTICS = 'diligentrobotics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-diligentrobotics`.                  | must     |
| FR-3  | `DiligentroboticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `diligentrobotics-`, `site === Site.DILIGENT_ROBOTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.DILIGENT_ROBOTICS, name: 'Diligent Robotics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/diligentrobotics/jobs/<id>`), wire
  `company_name` pass-through (`'Diligent Robotics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/diligentrobotics/jobs/<id>`).
