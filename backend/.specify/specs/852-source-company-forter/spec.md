# Spec: 852 — Source Company Plugin: Forter

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 852                            |
| Slug           | source-company-forter               |
| Status         | accepted                               |
...
Forter publishes its careers board through Greenhouse at
the bare slug `forter`. The run-398 batch sweep confirmed
...
- Ship a `source-company-forter` plugin returning live
...
- Backfilling historical Forter postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FORTER`** in the
> source registry, so that **a single `siteType: [Site.FORTER]`
> request returns Forter's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FORTER = 'forter'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-forter`.                  | must     |
| FR-3  | `ForterService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `forter-`, `site === Site.FORTER`. | must |
...
```ts
@SourcePlugin({ site: Site.FORTER, name: 'Forter', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/forter/jobs/<id>`), wire
  `company_name` pass-through (`'Forter'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/forter/jobs/<id>`).
