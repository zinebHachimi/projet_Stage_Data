# Spec: 200 — Source Company Plugin: Affinidi

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 200                            |
| Slug           | source-company-affinidi               |
| Status         | accepted                               |
...
Affinidi publishes its careers board through Greenhouse at
the bare slug `affinidi`. The run-398 batch sweep confirmed
...
- Ship a `source-company-affinidi` plugin returning live
...
- Backfilling historical Affinidi postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AFFINIDI`** in the
> source registry, so that **a single `siteType: [Site.AFFINIDI]`
> request returns Affinidi's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AFFINIDI = 'affinidi'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-affinidi`.                  | must     |
| FR-3  | `AffinidiService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `affinidi-`, `site === Site.AFFINIDI`. | must |
...
```ts
@SourcePlugin({ site: Site.AFFINIDI, name: 'Affinidi', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/affinidi/jobs/<id>`), wire
  `company_name` pass-through (`'Affinidi'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/affinidi/jobs/<id>`).
