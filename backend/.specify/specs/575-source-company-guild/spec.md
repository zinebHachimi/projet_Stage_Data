# Spec: 575 — Source Company Plugin: Guild

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 575                            |
| Slug           | source-company-guild               |
| Status         | accepted                               |
...
Guild publishes its careers board through Greenhouse at
the bare slug `guild`. The run-398 batch sweep confirmed
...
- Ship a `source-company-guild` plugin returning live
...
- Backfilling historical Guild postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GUILD`** in the
> source registry, so that **a single `siteType: [Site.GUILD]`
> request returns Guild's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GUILD = 'guild'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-guild`.                  | must     |
| FR-3  | `GuildService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `guild-`, `site === Site.GUILD`. | must |
...
```ts
@SourcePlugin({ site: Site.GUILD, name: 'Guild', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/guild/jobs/<id>`), wire
  `company_name` pass-through (`'Guild'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/guild/jobs/<id>`).
