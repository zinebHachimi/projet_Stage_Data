# Spec: 595 — Source Company Plugin: Ruggable

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 595                            |
| Slug           | source-company-ruggable               |
| Status         | accepted                               |
...
Ruggable publishes its careers board through Greenhouse at
the bare slug `ruggable`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ruggable` plugin returning live
...
- Backfilling historical Ruggable postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RUGGABLE`** in the
> source registry, so that **a single `siteType: [Site.RUGGABLE]`
> request returns Ruggable's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RUGGABLE = 'ruggable'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ruggable`.                  | must     |
| FR-3  | `RuggableService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ruggable-`, `site === Site.RUGGABLE`. | must |
...
```ts
@SourcePlugin({ site: Site.RUGGABLE, name: 'Ruggable', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ruggable/jobs/<id>`), wire
  `company_name` pass-through (`'Ruggable'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ruggable/jobs/<id>`).
