# Spec: 628 — Source Company Plugin: Loop

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 628                            |
| Slug           | source-company-loop               |
| Status         | accepted                               |
...
Loop publishes its careers board through Greenhouse at
the bare slug `loop`. The run-398 batch sweep confirmed
...
- Ship a `source-company-loop` plugin returning live
...
- Backfilling historical Loop postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.LOOP`** in the
> source registry, so that **a single `siteType: [Site.LOOP]`
> request returns Loop's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.LOOP = 'loop'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-loop`.                  | must     |
| FR-3  | `LoopService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `loop-`, `site === Site.LOOP`. | must |
...
```ts
@SourcePlugin({ site: Site.LOOP, name: 'Loop', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/loop/jobs/<id>`), wire
  `company_name` pass-through (`'Loop'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/loop/jobs/<id>`).
