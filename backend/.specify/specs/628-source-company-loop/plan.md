# Plan: 628 — Source Company Plugin: Loop
...
Loop's careers board is hosted on Greenhouse at the slug
`loop`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-loop` plugin.
- Deliverables: package files, `Site.LOOP` enum value, the
...
- `packages/plugins/source-company-loop` (new)
