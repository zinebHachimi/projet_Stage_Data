# Spec: 873 — Source Company Plugin: ITS Logistics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 873                            |
| Slug           | source-company-itslogisticsllc               |
| Status         | accepted                               |
...
ITS Logistics publishes its careers board through Greenhouse at
the bare slug `itslogisticsllc`. The run-398 batch sweep confirmed
24 live role(s) via a direct probe of
...
- Ship a `source-company-itslogisticsllc` plugin returning live
...
- Backfilling historical ITS Logistics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ITS_LOGISTICS`** in the
> source registry, so that **a single `siteType: [Site.ITS_LOGISTICS]`
> request returns ITS Logistics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ITS_LOGISTICS = 'itslogisticsllc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-itslogisticsllc`.                  | must     |
| FR-3  | `ITSLogisticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `itslogisticsllc-`, `site === Site.ITS_LOGISTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.ITS_LOGISTICS, name: 'ITS Logistics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/itslogisticsllc/jobs/<id>`), wire
  `company_name` pass-through (`'ITS Logistics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/itslogisticsllc/jobs/<id>`).
