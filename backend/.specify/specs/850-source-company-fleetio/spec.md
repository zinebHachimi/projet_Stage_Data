# Spec: 850 — Source Company Plugin: Fleetio

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 850                            |
| Slug           | source-company-fleetio               |
| Status         | accepted                               |
...
Fleetio publishes its careers board through Greenhouse at
the bare slug `fleetio`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fleetio` plugin returning live
...
- Backfilling historical Fleetio postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FLEETIO`** in the
> source registry, so that **a single `siteType: [Site.FLEETIO]`
> request returns Fleetio's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FLEETIO = 'fleetio'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fleetio`.                  | must     |
| FR-3  | `FleetioService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fleetio-`, `site === Site.FLEETIO`. | must |
...
```ts
@SourcePlugin({ site: Site.FLEETIO, name: 'Fleetio', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fleetio/jobs/<id>`), wire
  `company_name` pass-through (`'Fleetio'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fleetio/jobs/<id>`).
