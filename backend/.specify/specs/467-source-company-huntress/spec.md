# Spec: 467 — Source Company Plugin: Huntress

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 467                            |
| Slug           | source-company-huntress               |
| Status         | accepted                               |
...
Huntress publishes its careers board through Greenhouse at
the bare slug `huntress`. The run-398 batch sweep confirmed
...
- Ship a `source-company-huntress` plugin returning live
...
- Backfilling historical Huntress postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HUNTRESS`** in the
> source registry, so that **a single `siteType: [Site.HUNTRESS]`
> request returns Huntress's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HUNTRESS = 'huntress'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-huntress`.                  | must     |
| FR-3  | `HuntressService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `huntress-`, `site === Site.HUNTRESS`. | must |
...
```ts
@SourcePlugin({ site: Site.HUNTRESS, name: 'Huntress', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/huntress/jobs/<id>`), wire
  `company_name` pass-through (`'Huntress'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/huntress/jobs/<id>`).
