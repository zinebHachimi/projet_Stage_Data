# Spec: 577 — Source Company Plugin: PhonePe

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 577                            |
| Slug           | source-company-phonepe               |
| Status         | accepted                               |
...
PhonePe publishes its careers board through Greenhouse at
the bare slug `phonepe`. The run-398 batch sweep confirmed
...
- Ship a `source-company-phonepe` plugin returning live
...
- Backfilling historical PhonePe postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PHONEPE`** in the
> source registry, so that **a single `siteType: [Site.PHONEPE]`
> request returns PhonePe's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PHONEPE = 'phonepe'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-phonepe`.                  | must     |
| FR-3  | `PhonepeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `phonepe-`, `site === Site.PHONEPE`. | must |
...
```ts
@SourcePlugin({ site: Site.PHONEPE, name: 'PhonePe', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/phonepe/jobs/<id>`), wire
  `company_name` pass-through (`'PhonePe'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/phonepe/jobs/<id>`).
