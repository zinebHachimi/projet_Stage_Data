# Spec: 247 — Source Company Plugin: ALTEN Technology USA

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 247                            |
| Slug           | source-company-altentechnologyusa               |
| Status         | accepted                               |
...
ALTEN Technology USA publishes its careers board through Greenhouse at
the bare slug `altentechnologyusa`. The run-398 batch sweep confirmed
139 live role(s) via a direct probe of
...
- Ship a `source-company-altentechnologyusa` plugin returning live
...
- Backfilling historical ALTEN Technology USA postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALTENTECHNOLOGYUSA`** in the
> source registry, so that **a single `siteType: [Site.ALTENTECHNOLOGYUSA]`
> request returns ALTEN Technology USA's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALTENTECHNOLOGYUSA = 'altentechnologyusa'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-altentechnologyusa`.                  | must     |
| FR-3  | `AltentechnologyusaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `altentechnologyusa-`, `site === Site.ALTENTECHNOLOGYUSA`. | must |
...
```ts
@SourcePlugin({ site: Site.ALTENTECHNOLOGYUSA, name: 'ALTEN Technology USA', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/altentechnologyusa/jobs/<id>`), wire
  `company_name` pass-through (`'ALTEN Technology USA'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/altentechnologyusa/jobs/<id>`).
