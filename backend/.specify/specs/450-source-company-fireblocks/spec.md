# Spec: 450 — Source Company Plugin: Fireblocks

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 450                            |
| Slug           | source-company-fireblocks               |
| Status         | accepted                               |
...
Fireblocks publishes its careers board through Greenhouse at
the bare slug `fireblocks`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fireblocks` plugin returning live
...
- Backfilling historical Fireblocks postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FIREBLOCKS`** in the
> source registry, so that **a single `siteType: [Site.FIREBLOCKS]`
> request returns Fireblocks's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FIREBLOCKS = 'fireblocks'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fireblocks`.                  | must     |
| FR-3  | `FireblocksService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fireblocks-`, `site === Site.FIREBLOCKS`. | must |
...
```ts
@SourcePlugin({ site: Site.FIREBLOCKS, name: 'Fireblocks', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fireblocks/jobs/<id>`), wire
  `company_name` pass-through (`'Fireblocks'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fireblocks/jobs/<id>`).
