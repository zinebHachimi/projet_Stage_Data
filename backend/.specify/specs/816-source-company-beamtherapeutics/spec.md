# Spec: 816 — Source Company Plugin: Beam Therapeutics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 816                            |
| Slug           | source-company-beamtherapeutics               |
| Status         | accepted                               |
...
Beam Therapeutics publishes its careers board through Greenhouse at
the bare slug `beamtherapeutics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-beamtherapeutics` plugin returning live
...
- Backfilling historical Beam Therapeutics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BEAM_THERAPEUTICS`** in the
> source registry, so that **a single `siteType: [Site.BEAM_THERAPEUTICS]`
> request returns Beam Therapeutics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BEAM_THERAPEUTICS = 'beamtherapeutics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-beamtherapeutics`.                  | must     |
| FR-3  | `BeamTherapeuticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `beamtherapeutics-`, `site === Site.BEAM_THERAPEUTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.BEAM_THERAPEUTICS, name: 'Beam Therapeutics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/beamtherapeutics/jobs/<id>`), wire
  `company_name` pass-through (`'Beam Therapeutics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/beamtherapeutics/jobs/<id>`).
