# Spec: 253 — Source Company Plugin: Alveole

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 253                            |
| Slug           | source-company-alveole               |
| Status         | accepted                               |
...
Alveole publishes its careers board through Greenhouse at
the bare slug `alveole`. The run-398 batch sweep confirmed
...
- Ship a `source-company-alveole` plugin returning live
...
- Backfilling historical Alveole postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALVEOLE`** in the
> source registry, so that **a single `siteType: [Site.ALVEOLE]`
> request returns Alveole's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALVEOLE = 'alveole'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alveole`.                  | must     |
| FR-3  | `AlveoleService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alveole-`, `site === Site.ALVEOLE`. | must |
...
```ts
@SourcePlugin({ site: Site.ALVEOLE, name: 'Alveole', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alveole/jobs/<id>`), wire
  `company_name` pass-through (`'Alveole'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alveole/jobs/<id>`).
