# Spec: 901 — Source Company Plugin: OpenTable

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 901                            |
| Slug           | source-company-opentable               |
| Status         | accepted                               |
...
OpenTable publishes its careers board through Greenhouse at
the bare slug `opentable`. The run-398 batch sweep confirmed
...
- Ship a `source-company-opentable` plugin returning live
...
- Backfilling historical OpenTable postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.OPENTABLE`** in the
> source registry, so that **a single `siteType: [Site.OPENTABLE]`
> request returns OpenTable's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.OPENTABLE = 'opentable'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-opentable`.                  | must     |
| FR-3  | `OpenTableService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `opentable-`, `site === Site.OPENTABLE`. | must |
...
```ts
@SourcePlugin({ site: Site.OPENTABLE, name: 'OpenTable', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/opentable/jobs/<id>`), wire
  `company_name` pass-through (`'OpenTable'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/opentable/jobs/<id>`).
