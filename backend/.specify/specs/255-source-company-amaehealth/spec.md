# Spec: 255 — Source Company Plugin: Amae Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 255                            |
| Slug           | source-company-amaehealth               |
| Status         | accepted                               |
...
Amae Health publishes its careers board through Greenhouse at
the bare slug `amaehealth`. The run-398 batch sweep confirmed
41 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/amaehealth/jobs?content=true`.
Sector: Healthcare / Behavioral Health. HQ: Los Angeles, CA, USA.

## 2. Goals

- Ship a `source-company-amaehealth` plugin returning live
...
- Backfilling historical Amae Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMAEHEALTH`** in the
> source registry, so that **a single `siteType: [Site.AMAEHEALTH]`
> request returns Amae Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMAEHEALTH = 'amaehealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-amaehealth`.                  | must     |
| FR-3  | `AmaehealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `amaehealth-`, `site === Site.AMAEHEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.AMAEHEALTH, name: 'Amae Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/amaehealth/jobs/<id>`), wire
  `company_name` pass-through (`'Amae Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/amaehealth/jobs/<id>`).
