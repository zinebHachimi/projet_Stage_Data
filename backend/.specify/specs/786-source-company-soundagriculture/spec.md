# Spec: 786 — Source Company Plugin: Sound Agriculture

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 786                            |
| Slug           | source-company-soundagriculture               |
| Status         | accepted                               |
...
Sound Agriculture publishes its careers board through Greenhouse at
the bare slug `soundagriculture`. The run-398 batch sweep confirmed
...
- Ship a `source-company-soundagriculture` plugin returning live
...
- Backfilling historical Sound Agriculture postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SOUND_AGRICULTURE`** in the
> source registry, so that **a single `siteType: [Site.SOUND_AGRICULTURE]`
> request returns Sound Agriculture's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SOUND_AGRICULTURE = 'soundagriculture'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-soundagriculture`.                  | must     |
| FR-3  | `SoundAgricultureService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `soundagriculture-`, `site === Site.SOUND_AGRICULTURE`. | must |
...
```ts
@SourcePlugin({ site: Site.SOUND_AGRICULTURE, name: 'Sound Agriculture', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/soundagriculture/jobs/<id>`), wire
  `company_name` pass-through (`'Sound Agriculture'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/soundagriculture/jobs/<id>`).
