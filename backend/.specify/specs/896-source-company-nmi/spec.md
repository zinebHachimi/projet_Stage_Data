# Spec: 896 — Source Company Plugin: NMI

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 896                            |
| Slug           | source-company-nmi               |
| Status         | accepted                               |
...
NMI publishes its careers board through Greenhouse at
the bare slug `nmi`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nmi` plugin returning live
...
- Backfilling historical NMI postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NMI`** in the
> source registry, so that **a single `siteType: [Site.NMI]`
> request returns NMI's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NMI = 'nmi'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nmi`.                  | must     |
| FR-3  | `NMIService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nmi-`, `site === Site.NMI`. | must |
...
```ts
@SourcePlugin({ site: Site.NMI, name: 'NMI', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nmi/jobs/<id>`), wire
  `company_name` pass-through (`'NMI'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nmi/jobs/<id>`).
