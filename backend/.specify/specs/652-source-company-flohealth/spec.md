# Spec: 652 — Source Company Plugin: Flo Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 652                            |
| Slug           | source-company-flohealth               |
| Status         | accepted                               |
...
Flo Health publishes its careers board through Greenhouse at
the bare slug `flohealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-flohealth` plugin returning live
...
- Backfilling historical Flo Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FLO_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.FLO_HEALTH]`
> request returns Flo Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FLO_HEALTH = 'flohealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-flohealth`.                  | must     |
| FR-3  | `FloHealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `flohealth-`, `site === Site.FLO_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.FLO_HEALTH, name: 'Flo Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/flohealth/jobs/<id>`), wire
  `company_name` pass-through (`'Flo Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/flohealth/jobs/<id>`).
