# Spec: 505 — Source Company Plugin: Nayya

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 505                            |
| Slug           | source-company-nayya               |
| Status         | accepted                               |
...
Nayya publishes its careers board through Greenhouse at
the bare slug `nayya`. The run-398 batch sweep confirmed
...
- Ship a `source-company-nayya` plugin returning live
...
- Backfilling historical Nayya postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.NAYYA`** in the
> source registry, so that **a single `siteType: [Site.NAYYA]`
> request returns Nayya's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.NAYYA = 'nayya'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-nayya`.                  | must     |
| FR-3  | `NayyaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `nayya-`, `site === Site.NAYYA`. | must |
...
```ts
@SourcePlugin({ site: Site.NAYYA, name: 'Nayya', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/nayya/jobs/<id>`), wire
  `company_name` pass-through (`'Nayya'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/nayya/jobs/<id>`).
