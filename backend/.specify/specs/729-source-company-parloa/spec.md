# Spec: 729 — Source Company Plugin: Parloa

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 729                            |
| Slug           | source-company-parloa               |
| Status         | accepted                               |
...
Parloa publishes its careers board through Greenhouse at
the bare slug `parloa`. The run-398 batch sweep confirmed
50 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/parloa/jobs?content=true`.
Sector: Conversational AI / Contact center. HQ: Berlin, Germany.

## 2. Goals

- Ship a `source-company-parloa` plugin returning live
...
- Backfilling historical Parloa postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PARLOA`** in the
> source registry, so that **a single `siteType: [Site.PARLOA]`
> request returns Parloa's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PARLOA = 'parloa'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-parloa`.                  | must     |
| FR-3  | `ParloaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `parloa-`, `site === Site.PARLOA`. | must |
...
```ts
@SourcePlugin({ site: Site.PARLOA, name: 'Parloa', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/parloa/jobs/<id>`), wire
  `company_name` pass-through (`'Parloa'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/parloa/jobs/<id>`).
