# Spec: 649 — Source Company Plugin: Clara

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 649                            |
| Slug           | source-company-clara               |
| Status         | accepted                               |
...
Clara publishes its careers board through Greenhouse at
the bare slug `clara`. The run-398 batch sweep confirmed
85 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/clara/jobs?content=true`.
Sector: Fintech (corporate spend management). HQ: Mexico City, Mexico.

## 2. Goals

- Ship a `source-company-clara` plugin returning live
...
- Backfilling historical Clara postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CLARA`** in the
> source registry, so that **a single `siteType: [Site.CLARA]`
> request returns Clara's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CLARA = 'clara'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-clara`.                  | must     |
| FR-3  | `ClaraService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `clara-`, `site === Site.CLARA`. | must |
...
```ts
@SourcePlugin({ site: Site.CLARA, name: 'Clara', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/clara/jobs/<id>`), wire
  `company_name` pass-through (`'Clara'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/clara/jobs/<id>`).
