# Spec: 974 — Source Company Plugin: Zynga

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 974                            |
| Slug           | source-company-zyngacareers               |
| Status         | accepted                               |
...
Zynga publishes its careers board through Greenhouse at
the bare slug `zyngacareers`. The run-398 batch sweep confirmed
...
- Ship a `source-company-zyngacareers` plugin returning live
...
- Backfilling historical Zynga postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ZYNGA`** in the
> source registry, so that **a single `siteType: [Site.ZYNGA]`
> request returns Zynga's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ZYNGA = 'zyngacareers'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-zyngacareers`.                  | must     |
| FR-3  | `ZyngaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `zyngacareers-`, `site === Site.ZYNGA`. | must |
...
```ts
@SourcePlugin({ site: Site.ZYNGA, name: 'Zynga', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/zyngacareers/jobs/<id>`), wire
  `company_name` pass-through (`'Zynga'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/zyngacareers/jobs/<id>`).
