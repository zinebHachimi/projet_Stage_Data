# Spec: 257 — Source Company Plugin: Ambient Enterprises

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 257                            |
| Slug           | source-company-ambiententerprises               |
| Status         | accepted                               |
...
Ambient Enterprises publishes its careers board through Greenhouse at
the bare slug `ambiententerprises`. The run-398 batch sweep confirmed
86 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/ambiententerprises/jobs?content=true`.
Sector: Commercial HVAC services. HQ: Unknown.

## 2. Goals

- Ship a `source-company-ambiententerprises` plugin returning live
...
- Backfilling historical Ambient Enterprises postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AMBIENTENTERPRISES`** in the
> source registry, so that **a single `siteType: [Site.AMBIENTENTERPRISES]`
> request returns Ambient Enterprises's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AMBIENTENTERPRISES = 'ambiententerprises'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ambiententerprises`.                  | must     |
| FR-3  | `AmbiententerprisesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ambiententerprises-`, `site === Site.AMBIENTENTERPRISES`. | must |
...
```ts
@SourcePlugin({ site: Site.AMBIENTENTERPRISES, name: 'Ambient Enterprises', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ambiententerprises/jobs/<id>`), wire
  `company_name` pass-through (`'Ambient Enterprises'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ambiententerprises/jobs/<id>`).
