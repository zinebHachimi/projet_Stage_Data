# Spec: 854 — Source Company Plugin: Galvanize Climate Solutions

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 854                            |
| Slug           | source-company-galvanizeclimatesolutions               |
| Status         | accepted                               |
...
Galvanize Climate Solutions publishes its careers board through Greenhouse at
the bare slug `galvanizeclimatesolutions`. The run-398 batch sweep confirmed
...
- Ship a `source-company-galvanizeclimatesolutions` plugin returning live
...
- Backfilling historical Galvanize Climate Solutions postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GALVANIZE_CLIMATE_SOLUTIONS`** in the
> source registry, so that **a single `siteType: [Site.GALVANIZE_CLIMATE_SOLUTIONS]`
> request returns Galvanize Climate Solutions's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GALVANIZE_CLIMATE_SOLUTIONS = 'galvanizeclimatesolutions'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-galvanizeclimatesolutions`.                  | must     |
| FR-3  | `GalvanizeClimateSolutionsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `galvanizeclimatesolutions-`, `site === Site.GALVANIZE_CLIMATE_SOLUTIONS`. | must |
...
```ts
@SourcePlugin({ site: Site.GALVANIZE_CLIMATE_SOLUTIONS, name: 'Galvanize Climate Solutions', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/galvanizeclimatesolutions/jobs/<id>`), wire
  `company_name` pass-through (`'Galvanize Climate Solutions'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/galvanizeclimatesolutions/jobs/<id>`).
