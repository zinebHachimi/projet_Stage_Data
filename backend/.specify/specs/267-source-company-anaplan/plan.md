# Plan: 267 — Source Company Plugin: Anaplan
...
Anaplan's careers board is hosted on Greenhouse at the slug
`anaplan`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-anaplan` plugin.
- Deliverables: package files, `Site.ANAPLAN` enum value, the
...
- `packages/plugins/source-company-anaplan` (new)
