# Spec: 267 — Source Company Plugin: Anaplan

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 267                            |
| Slug           | source-company-anaplan               |
| Status         | accepted                               |
...
Anaplan publishes its careers board through Greenhouse at
the bare slug `anaplan`. The run-398 batch sweep confirmed
...
- Ship a `source-company-anaplan` plugin returning live
...
- Backfilling historical Anaplan postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ANAPLAN`** in the
> source registry, so that **a single `siteType: [Site.ANAPLAN]`
> request returns Anaplan's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ANAPLAN = 'anaplan'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-anaplan`.                  | must     |
| FR-3  | `AnaplanService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `anaplan-`, `site === Site.ANAPLAN`. | must |
...
```ts
@SourcePlugin({ site: Site.ANAPLAN, name: 'Anaplan', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/anaplan/jobs/<id>`), wire
  `company_name` pass-through (`'Anaplan'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/anaplan/jobs/<id>`).
