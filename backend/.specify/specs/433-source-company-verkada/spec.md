# Spec: 433 — Source Company Plugin: Verkada

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 433                            |
| Slug           | source-company-verkada               |
| Status         | accepted                               |
...
Verkada publishes its careers board through Greenhouse at
the bare slug `verkada`. The run-398 batch sweep confirmed
311 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/verkada/jobs?content=true`.
Sector: Cloud physical security & IoT. HQ: San Mateo, CA, USA.

## 2. Goals

- Ship a `source-company-verkada` plugin returning live
...
- Backfilling historical Verkada postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.VERKADA`** in the
> source registry, so that **a single `siteType: [Site.VERKADA]`
> request returns Verkada's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.VERKADA = 'verkada'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-verkada`.                  | must     |
| FR-3  | `VerkadaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `verkada-`, `site === Site.VERKADA`. | must |
...
```ts
@SourcePlugin({ site: Site.VERKADA, name: 'Verkada', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/verkada/jobs/<id>`), wire
  `company_name` pass-through (`'Verkada'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/verkada/jobs/<id>`).
