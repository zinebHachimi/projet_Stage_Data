# Plan: 612 — Source Company Plugin: Range
...
Range's careers board is hosted on Greenhouse at the slug
`range`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-range` plugin.
- Deliverables: package files, `Site.RANGE_TRAVEL` enum value, the
...
- `packages/plugins/source-company-range` (new)
