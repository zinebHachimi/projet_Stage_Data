# Spec: 612 — Source Company Plugin: Range

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 612                            |
| Slug           | source-company-range               |
| Status         | accepted                               |
...
Range publishes its careers board through Greenhouse at
the bare slug `range`. The run-398 batch sweep confirmed
...
- Ship a `source-company-range` plugin returning live
...
- Backfilling historical Range postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RANGE_TRAVEL`** in the
> source registry, so that **a single `siteType: [Site.RANGE_TRAVEL]`
> request returns Range's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RANGE_TRAVEL = 'range'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-range`.                  | must     |
| FR-3  | `RangeService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `range-`, `site === Site.RANGE_TRAVEL`. | must |
...
```ts
@SourcePlugin({ site: Site.RANGE_TRAVEL, name: 'Range', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/range/jobs/<id>`), wire
  `company_name` pass-through (`'Range'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/range/jobs/<id>`).
