# Spec: 788 — Source Company Plugin: AIR COMPANY

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 788                            |
| Slug           | source-company-aircompany               |
| Status         | accepted                               |
...
AIR COMPANY publishes its careers board through Greenhouse at
the bare slug `aircompany`. The run-398 batch sweep confirmed
...
- Ship a `source-company-aircompany` plugin returning live
...
- Backfilling historical AIR COMPANY postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.AIR_COMPANY`** in the
> source registry, so that **a single `siteType: [Site.AIR_COMPANY]`
> request returns AIR COMPANY's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.AIR_COMPANY = 'aircompany'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-aircompany`.                  | must     |
| FR-3  | `AIRCOMPANYService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `aircompany-`, `site === Site.AIR_COMPANY`. | must |
...
```ts
@SourcePlugin({ site: Site.AIR_COMPANY, name: 'AIR COMPANY', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/aircompany/jobs/<id>`), wire
  `company_name` pass-through (`'AIR COMPANY'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/aircompany/jobs/<id>`).
