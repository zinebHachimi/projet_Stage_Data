# Spec: 487 — Source Company Plugin: CookUnity

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 487                            |
| Slug           | source-company-cookunity               |
| Status         | accepted                               |
...
CookUnity publishes its careers board through Greenhouse at
the bare slug `cookunity`. The run-398 batch sweep confirmed
138 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/cookunity/jobs?content=true`.
Sector: Food-tech / Meal delivery. HQ: New York, NY, USA.

## 2. Goals

- Ship a `source-company-cookunity` plugin returning live
...
- Backfilling historical CookUnity postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COOKUNITY`** in the
> source registry, so that **a single `siteType: [Site.COOKUNITY]`
> request returns CookUnity's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COOKUNITY = 'cookunity'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cookunity`.                  | must     |
| FR-3  | `CookunityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cookunity-`, `site === Site.COOKUNITY`. | must |
...
```ts
@SourcePlugin({ site: Site.COOKUNITY, name: 'CookUnity', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cookunity/jobs/<id>`), wire
  `company_name` pass-through (`'CookUnity'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cookunity/jobs/<id>`).
