# Spec: 461 — Source Company Plugin: DISCO

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 461                            |
| Slug           | source-company-disco               |
| Status         | accepted                               |
...
DISCO publishes its careers board through Greenhouse at
the bare slug `disco`. The run-398 batch sweep confirmed
...
- Ship a `source-company-disco` plugin returning live
...
- Backfilling historical DISCO postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DISCO`** in the
> source registry, so that **a single `siteType: [Site.DISCO]`
> request returns DISCO's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DISCO = 'disco'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-disco`.                  | must     |
| FR-3  | `DiscoService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `disco-`, `site === Site.DISCO`. | must |
...
```ts
@SourcePlugin({ site: Site.DISCO, name: 'DISCO', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/disco/jobs/<id>`), wire
  `company_name` pass-through (`'DISCO'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/disco/jobs/<id>`).
