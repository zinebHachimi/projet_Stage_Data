# Plan: 461 — Source Company Plugin: DISCO
...
DISCO's careers board is hosted on Greenhouse at the slug
`disco`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-disco` plugin.
- Deliverables: package files, `Site.DISCO` enum value, the
...
- `packages/plugins/source-company-disco` (new)
