# Spec: 921 — Source Company Plugin: Rithum

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 921                            |
| Slug           | source-company-rithum               |
| Status         | accepted                               |
...
Rithum publishes its careers board through Greenhouse at
the bare slug `rithum`. The run-398 batch sweep confirmed
...
- Ship a `source-company-rithum` plugin returning live
...
- Backfilling historical Rithum postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RITHUM`** in the
> source registry, so that **a single `siteType: [Site.RITHUM]`
> request returns Rithum's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RITHUM = 'rithum'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-rithum`.                  | must     |
| FR-3  | `RithumService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `rithum-`, `site === Site.RITHUM`. | must |
...
```ts
@SourcePlugin({ site: Site.RITHUM, name: 'Rithum', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/rithum/jobs/<id>`), wire
  `company_name` pass-through (`'Rithum'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/rithum/jobs/<id>`).
