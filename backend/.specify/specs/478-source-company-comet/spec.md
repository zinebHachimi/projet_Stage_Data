# Spec: 478 — Source Company Plugin: Comet

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 478                            |
| Slug           | source-company-comet               |
| Status         | accepted                               |
...
Comet publishes its careers board through Greenhouse at
the bare slug `comet`. The run-398 batch sweep confirmed
...
- Ship a `source-company-comet` plugin returning live
...
- Backfilling historical Comet postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.COMET`** in the
> source registry, so that **a single `siteType: [Site.COMET]`
> request returns Comet's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.COMET = 'comet'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-comet`.                  | must     |
| FR-3  | `CometService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `comet-`, `site === Site.COMET`. | must |
...
```ts
@SourcePlugin({ site: Site.COMET, name: 'Comet', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/comet/jobs/<id>`), wire
  `company_name` pass-through (`'Comet'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/comet/jobs/<id>`).
