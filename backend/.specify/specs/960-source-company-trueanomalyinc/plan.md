# Plan: 960 — Source Company Plugin: True Anomaly
...
True Anomaly's careers board is hosted on Greenhouse at the slug
`trueanomalyinc`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-trueanomalyinc` plugin.
- Deliverables: package files, `Site.TRUE_ANOMALY` enum value, the
...
- `packages/plugins/source-company-trueanomalyinc` (new)
