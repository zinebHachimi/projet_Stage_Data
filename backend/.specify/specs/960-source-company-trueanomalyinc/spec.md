# Spec: 960 — Source Company Plugin: True Anomaly

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 960                            |
| Slug           | source-company-trueanomalyinc               |
| Status         | accepted                               |
...
True Anomaly publishes its careers board through Greenhouse at
the bare slug `trueanomalyinc`. The run-398 batch sweep confirmed
184 live role(s) via a direct probe of
...
- Ship a `source-company-trueanomalyinc` plugin returning live
...
- Backfilling historical True Anomaly postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.TRUE_ANOMALY`** in the
> source registry, so that **a single `siteType: [Site.TRUE_ANOMALY]`
> request returns True Anomaly's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.TRUE_ANOMALY = 'trueanomalyinc'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-trueanomalyinc`.                  | must     |
| FR-3  | `TrueAnomalyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `trueanomalyinc-`, `site === Site.TRUE_ANOMALY`. | must |
...
```ts
@SourcePlugin({ site: Site.TRUE_ANOMALY, name: 'True Anomaly', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/trueanomalyinc/jobs/<id>`), wire
  `company_name` pass-through (`'True Anomaly'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/trueanomalyinc/jobs/<id>`).
