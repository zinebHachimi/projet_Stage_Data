# Plan: 039 — Source Company Plugin: Instacart
...
Instacart's careers board is hosted on Greenhouse at the slug
`instacart`, so the implementation is a thin wrapper around the same
public Greenhouse endpoint that `source-company-datadog` (Spec 038
/ run #248), `source-company-mongodb` (Spec 037 / run #247),
...
already call. The plan is to copy the Datadog plugin's shape
(single-file `service.ts`, three-line `module.ts`, two-line
`index.ts`, four-line `package.json`, three-line `tsconfig.json`)
and rename references from Datadog → Instacart, then wire the new
...
used twenty-seven times before (Amazon, Apple, Cursor, Google, IBM,
...
- **Goal:** Land `source-company-instacart` under
  `ALL_SOURCE_MODULES` with passing unit tests and a green doc-lint
  pass.
- **Deliverables:**
  - `packages/plugins/source-company-instacart/` (`package.json`,
    `tsconfig.json`,
    `src/{index.ts,instacart.module.ts,instacart.service.ts}`,
    `__tests__/instacart.service.spec.ts`,
    `__tests__/fixtures/instacart-jobs.json`).
  - One-line edits to: `packages/models/src/enums/site.enum.ts`,
    `packages/plugins/index.ts`, `tsconfig.base.json`,
    `jest.config.js`.
  - One-row update to `docs/SOURCE_ADOPTION_BACKLOG.md`
    ("shipped" status), index-table addition for Spec 039 in
    `docs/index.md`, log entry in `docs/log.md`.
- **Exit criteria:**
  - `npx jest packages/plugins/source-company-instacart` → all green.
...
| `packages/models/src/enums/site.enum.ts`             | append `INSTACART = 'instacart'`.                            |
| `packages/plugins/index.ts`                          | import + append `InstacartModule` to `ALL_SOURCE_MODULES`.   |
| `tsconfig.base.json`                                 | path-alias entry.                                            |
| `jest.config.js`                                     | `moduleNameMapper` entry.                                    |
| `docs/SOURCE_ADOPTION_BACKLOG.md`                    | add Instacart shipped row.                                   |
| `docs/index.md`                                      | append Spec 039 to the specs table.                          |
...
| --------------------------------------------------- | ---------- | ------ | ---------------------------------------------------------------- |
| Instacart moves off Greenhouse mid-maintenance.     | L          | M      | Plugin returns `{ jobs: [] }` on 404 (FR-9); Spec 005 circuit breaker isolates the failure; future spec re-targets the new ATS. |
...
| Drift from `source-company-datadog`'s shape.        | L          | L      | Pattern frozen by FR-1..FR-10; spec is reviewable side-by-side.  |
...
| Instacart acquisition splits to a separate Greenhouse tenant. | L | L | Plugin still resolves through `Site.INSTACART`; aggregator-side analytics is preserved. A follow-up spec can split this plugin in turn (see Spec 039 § 10 D-06). |
...
- The four registration touch-points each have one new line — strip
  them.
- `Site.INSTACART` is unused outside this plugin, so the enum revert
  is safe.
...
(None — see Spec 039 § 10 Decisions for the eight resolved points.)
