# Spec: 927 — Source Company Plugin: SecurityScorecard

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 927                            |
| Slug           | source-company-securityscorecard               |
| Status         | accepted                               |
...
SecurityScorecard publishes its careers board through Greenhouse at
the bare slug `securityscorecard`. The run-398 batch sweep confirmed
62 live role(s) via a direct probe of
...
- Ship a `source-company-securityscorecard` plugin returning live
...
- Backfilling historical SecurityScorecard postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SECURITYSCORECARD`** in the
> source registry, so that **a single `siteType: [Site.SECURITYSCORECARD]`
> request returns SecurityScorecard's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SECURITYSCORECARD = 'securityscorecard'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-securityscorecard`.                  | must     |
| FR-3  | `SecurityScorecardService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `securityscorecard-`, `site === Site.SECURITYSCORECARD`. | must |
...
```ts
@SourcePlugin({ site: Site.SECURITYSCORECARD, name: 'SecurityScorecard', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/securityscorecard/jobs/<id>`), wire
  `company_name` pass-through (`'SecurityScorecard'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/securityscorecard/jobs/<id>`).
