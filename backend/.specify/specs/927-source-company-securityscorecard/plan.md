# Plan: 927 — Source Company Plugin: SecurityScorecard
...
SecurityScorecard's careers board is hosted on Greenhouse at the slug
`securityscorecard`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-securityscorecard` plugin.
- Deliverables: package files, `Site.SECURITYSCORECARD` enum value, the
...
- `packages/plugins/source-company-securityscorecard` (new)
