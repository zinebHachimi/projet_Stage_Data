# Spec: 290 — Source Company Plugin: AppDirect

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 290                            |
| Slug           | source-company-appdirect               |
| Status         | accepted                               |
...
AppDirect publishes its careers board through Greenhouse at
the bare slug `appdirect`. The run-398 batch sweep confirmed
76 live role(s) via a direct probe of
...
- Ship a `source-company-appdirect` plugin returning live
...
- Backfilling historical AppDirect postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.APPDIRECT`** in the
> source registry, so that **a single `siteType: [Site.APPDIRECT]`
> request returns AppDirect's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.APPDIRECT = 'appdirect'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-appdirect`.                  | must     |
| FR-3  | `AppdirectService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `appdirect-`, `site === Site.APPDIRECT`. | must |
...
```ts
@SourcePlugin({ site: Site.APPDIRECT, name: 'AppDirect', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/appdirect/jobs/<id>`), wire
  `company_name` pass-through (`'AppDirect'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/appdirect/jobs/<id>`).
