# Plan: 290 — Source Company Plugin: AppDirect
...
AppDirect's careers board is hosted on Greenhouse at the slug
`appdirect`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-appdirect` plugin.
- Deliverables: package files, `Site.APPDIRECT` enum value, the
...
- `packages/plugins/source-company-appdirect` (new)
