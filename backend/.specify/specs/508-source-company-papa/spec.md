# Spec: 508 — Source Company Plugin: Papa

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 508                            |
| Slug           | source-company-papa               |
| Status         | accepted                               |
...
Papa publishes its careers board through Greenhouse at
the bare slug `papa`. The run-398 batch sweep confirmed
...
- Ship a `source-company-papa` plugin returning live
...
- Backfilling historical Papa postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PAPA`** in the
> source registry, so that **a single `siteType: [Site.PAPA]`
> request returns Papa's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PAPA = 'papa'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-papa`.                  | must     |
| FR-3  | `PapaService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `papa-`, `site === Site.PAPA`. | must |
...
```ts
@SourcePlugin({ site: Site.PAPA, name: 'Papa', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/papa/jobs/<id>`), wire
  `company_name` pass-through (`'Papa'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/papa/jobs/<id>`).
