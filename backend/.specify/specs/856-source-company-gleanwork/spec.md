# Spec: 856 — Source Company Plugin: Glean

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 856                            |
| Slug           | source-company-gleanwork               |
| Status         | accepted                               |
...
Glean publishes its careers board through Greenhouse at
the bare slug `gleanwork`. The run-398 batch sweep confirmed
...
- Ship a `source-company-gleanwork` plugin returning live
...
- Backfilling historical Glean postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.GLEAN`** in the
> source registry, so that **a single `siteType: [Site.GLEAN]`
> request returns Glean's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.GLEAN = 'gleanwork'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-gleanwork`.                  | must     |
| FR-3  | `GleanService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `gleanwork-`, `site === Site.GLEAN`. | must |
...
```ts
@SourcePlugin({ site: Site.GLEAN, name: 'Glean', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/gleanwork/jobs/<id>`), wire
  `company_name` pass-through (`'Glean'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/gleanwork/jobs/<id>`).
