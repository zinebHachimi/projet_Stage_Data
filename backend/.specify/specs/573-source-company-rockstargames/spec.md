# Spec: 573 — Source Company Plugin: Rockstar Games

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 573                            |
| Slug           | source-company-rockstargames               |
| Status         | accepted                               |
...
Rockstar Games publishes its careers board through Greenhouse at
the bare slug `rockstargames`. The run-398 batch sweep confirmed
78 live role(s) via a direct probe of
...
- Ship a `source-company-rockstargames` plugin returning live
...
- Backfilling historical Rockstar Games postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ROCKSTAR_GAMES`** in the
> source registry, so that **a single `siteType: [Site.ROCKSTAR_GAMES]`
> request returns Rockstar Games's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ROCKSTAR_GAMES = 'rockstargames'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-rockstargames`.                  | must     |
| FR-3  | `RockstargamesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `rockstargames-`, `site === Site.ROCKSTAR_GAMES`. | must |
...
```ts
@SourcePlugin({ site: Site.ROCKSTAR_GAMES, name: 'Rockstar Games', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/rockstargames/jobs/<id>`), wire
  `company_name` pass-through (`'Rockstar Games'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/rockstargames/jobs/<id>`).
