# Spec: 682 — Source Company Plugin: Camus Energy

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 682                            |
| Slug           | source-company-camusenergy               |
| Status         | accepted                               |
...
Camus Energy publishes its careers board through Greenhouse at
the bare slug `camusenergy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-camusenergy` plugin returning live
...
- Backfilling historical Camus Energy postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CAMUS_ENERGY`** in the
> source registry, so that **a single `siteType: [Site.CAMUS_ENERGY]`
> request returns Camus Energy's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CAMUS_ENERGY = 'camusenergy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-camusenergy`.                  | must     |
| FR-3  | `CamusEnergyService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `camusenergy-`, `site === Site.CAMUS_ENERGY`. | must |
...
```ts
@SourcePlugin({ site: Site.CAMUS_ENERGY, name: 'Camus Energy', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/camusenergy/jobs/<id>`), wire
  `company_name` pass-through (`'Camus Energy'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/camusenergy/jobs/<id>`).
