# Spec: 561 — Source Company Plugin: Bicycle Health

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 561                            |
| Slug           | source-company-bicyclehealth               |
| Status         | accepted                               |
...
Bicycle Health publishes its careers board through Greenhouse at
the bare slug `bicyclehealth`. The run-398 batch sweep confirmed
...
- Ship a `source-company-bicyclehealth` plugin returning live
...
- Backfilling historical Bicycle Health postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.BICYCLE_HEALTH`** in the
> source registry, so that **a single `siteType: [Site.BICYCLE_HEALTH]`
> request returns Bicycle Health's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.BICYCLE_HEALTH = 'bicyclehealth'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-bicyclehealth`.                  | must     |
| FR-3  | `BicyclehealthService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `bicyclehealth-`, `site === Site.BICYCLE_HEALTH`. | must |
...
```ts
@SourcePlugin({ site: Site.BICYCLE_HEALTH, name: 'Bicycle Health', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/bicyclehealth/jobs/<id>`), wire
  `company_name` pass-through (`'Bicycle Health'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/bicyclehealth/jobs/<id>`).
