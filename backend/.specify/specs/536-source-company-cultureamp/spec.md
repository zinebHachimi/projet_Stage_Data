# Spec: 536 — Source Company Plugin: Culture Amp

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 536                            |
| Slug           | source-company-cultureamp               |
| Status         | accepted                               |
...
Culture Amp publishes its careers board through Greenhouse at
the bare slug `cultureamp`. The run-398 batch sweep confirmed
...
- Ship a `source-company-cultureamp` plugin returning live
...
- Backfilling historical Culture Amp postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CULTURE_AMP`** in the
> source registry, so that **a single `siteType: [Site.CULTURE_AMP]`
> request returns Culture Amp's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CULTURE_AMP = 'cultureamp'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-cultureamp`.                  | must     |
| FR-3  | `CultureampService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `cultureamp-`, `site === Site.CULTURE_AMP`. | must |
...
```ts
@SourcePlugin({ site: Site.CULTURE_AMP, name: 'Culture Amp', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/cultureamp/jobs/<id>`), wire
  `company_name` pass-through (`'Culture Amp'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/cultureamp/jobs/<id>`).
