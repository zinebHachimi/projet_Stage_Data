# Spec: 714 — Source Company Plugin: Solid Power

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 714                            |
| Slug           | source-company-solidpower               |
| Status         | accepted                               |
...
Solid Power publishes its careers board through Greenhouse at
the bare slug `solidpower`. The run-398 batch sweep confirmed
...
- Ship a `source-company-solidpower` plugin returning live
...
- Backfilling historical Solid Power postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.SOLID_POWER`** in the
> source registry, so that **a single `siteType: [Site.SOLID_POWER]`
> request returns Solid Power's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.SOLID_POWER = 'solidpower'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-solidpower`.                  | must     |
| FR-3  | `SolidPowerService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `solidpower-`, `site === Site.SOLID_POWER`. | must |
...
```ts
@SourcePlugin({ site: Site.SOLID_POWER, name: 'Solid Power', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/solidpower/jobs/<id>`), wire
  `company_name` pass-through (`'Solid Power'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/solidpower/jobs/<id>`).
