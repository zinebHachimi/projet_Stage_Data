# Spec: 863 — Source Company Plugin: Herald

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 863                            |
| Slug           | source-company-heraldapi               |
| Status         | accepted                               |
...
Herald publishes its careers board through Greenhouse at
the bare slug `heraldapi`. The run-398 batch sweep confirmed
...
- Ship a `source-company-heraldapi` plugin returning live
...
- Backfilling historical Herald postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HERALD`** in the
> source registry, so that **a single `siteType: [Site.HERALD]`
> request returns Herald's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HERALD = 'heraldapi'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-heraldapi`.                  | must     |
| FR-3  | `HeraldService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `heraldapi-`, `site === Site.HERALD`. | must |
...
```ts
@SourcePlugin({ site: Site.HERALD, name: 'Herald', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/heraldapi/jobs/<id>`), wire
  `company_name` pass-through (`'Herald'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/heraldapi/jobs/<id>`).
