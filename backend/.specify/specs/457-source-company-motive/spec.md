# Spec: 457 — Source Company Plugin: Motive

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 457                            |
| Slug           | source-company-motive               |
| Status         | accepted                               |
...
Motive publishes its careers board through Greenhouse at
the bare slug `motive`. The run-398 batch sweep confirmed
...
- Ship a `source-company-motive` plugin returning live
...
- Backfilling historical Motive postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MOTIVE`** in the
> source registry, so that **a single `siteType: [Site.MOTIVE]`
> request returns Motive's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MOTIVE = 'motive'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-motive`.                  | must     |
| FR-3  | `MotiveService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `motive-`, `site === Site.MOTIVE`. | must |
...
```ts
@SourcePlugin({ site: Site.MOTIVE, name: 'Motive', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/motive/jobs/<id>`), wire
  `company_name` pass-through (`'Motive'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/motive/jobs/<id>`).
