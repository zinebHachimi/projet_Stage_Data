# Spec: 835 — Source Company Plugin: Defense Unicorns

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 835                            |
| Slug           | source-company-defenseunicorns               |
| Status         | accepted                               |
...
Defense Unicorns publishes its careers board through Greenhouse at
the bare slug `defenseunicorns`. The run-398 batch sweep confirmed
...
- Ship a `source-company-defenseunicorns` plugin returning live
...
- Backfilling historical Defense Unicorns postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.DEFENSE_UNICORNS`** in the
> source registry, so that **a single `siteType: [Site.DEFENSE_UNICORNS]`
> request returns Defense Unicorns's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.DEFENSE_UNICORNS = 'defenseunicorns'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-defenseunicorns`.                  | must     |
| FR-3  | `DefenseUnicornsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `defenseunicorns-`, `site === Site.DEFENSE_UNICORNS`. | must |
...
```ts
@SourcePlugin({ site: Site.DEFENSE_UNICORNS, name: 'Defense Unicorns', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/defenseunicorns/jobs/<id>`), wire
  `company_name` pass-through (`'Defense Unicorns'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/defenseunicorns/jobs/<id>`).
