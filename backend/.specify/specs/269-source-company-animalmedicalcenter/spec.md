# Spec: 269 — Source Company Plugin: Schwarzman Animal Medical Center

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 269                            |
| Slug           | source-company-animalmedicalcenter               |
| Status         | accepted                               |
...
Schwarzman Animal Medical Center publishes its careers board through Greenhouse at
the bare slug `animalmedicalcenter`. The run-398 batch sweep confirmed
40 live role(s) via a direct probe of
`https://api.greenhouse.io/v1/boards/animalmedicalcenter/jobs?content=true`.
Sector: Veterinary healthcare. HQ: New York, NY, USA.

## 2. Goals

- Ship a `source-company-animalmedicalcenter` plugin returning live
...
- Backfilling historical Schwarzman Animal Medical Center postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ANIMALMEDICALCENTER`** in the
> source registry, so that **a single `siteType: [Site.ANIMALMEDICALCENTER]`
> request returns Schwarzman Animal Medical Center's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ANIMALMEDICALCENTER = 'animalmedicalcenter'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-animalmedicalcenter`.                  | must     |
| FR-3  | `AnimalmedicalcenterService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `animalmedicalcenter-`, `site === Site.ANIMALMEDICALCENTER`. | must |
...
```ts
@SourcePlugin({ site: Site.ANIMALMEDICALCENTER, name: 'Schwarzman Animal Medical Center', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/animalmedicalcenter/jobs/<id>`), wire
  `company_name` pass-through (`'Schwarzman Animal Medical Center'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/animalmedicalcenter/jobs/<id>`).
