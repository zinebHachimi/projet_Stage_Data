# Spec: 698 — Source Company Plugin: mabl

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 698                            |
| Slug           | source-company-mabl               |
| Status         | accepted                               |
...
mabl publishes its careers board through Greenhouse at
the bare slug `mabl`. The run-398 batch sweep confirmed
...
- Ship a `source-company-mabl` plugin returning live
...
- Backfilling historical mabl postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MABL`** in the
> source registry, so that **a single `siteType: [Site.MABL]`
> request returns mabl's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MABL = 'mabl'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-mabl`.                  | must     |
| FR-3  | `MablService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `mabl-`, `site === Site.MABL`. | must |
...
```ts
@SourcePlugin({ site: Site.MABL, name: 'mabl', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/mabl/jobs/<id>`), wire
  `company_name` pass-through (`'mabl'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/mabl/jobs/<id>`).
