# Spec: 493 — Source Company Plugin: Home Chef

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 493                            |
| Slug           | source-company-homechef               |
| Status         | accepted                               |
...
Home Chef publishes its careers board through Greenhouse at
the bare slug `homechef`. The run-398 batch sweep confirmed
...
- Ship a `source-company-homechef` plugin returning live
...
- Backfilling historical Home Chef postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HOMECHEF`** in the
> source registry, so that **a single `siteType: [Site.HOMECHEF]`
> request returns Home Chef's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HOMECHEF = 'homechef'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-homechef`.                  | must     |
| FR-3  | `HomechefService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `homechef-`, `site === Site.HOMECHEF`. | must |
...
```ts
@SourcePlugin({ site: Site.HOMECHEF, name: 'Home Chef', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/homechef/jobs/<id>`), wire
  `company_name` pass-through (`'Home Chef'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/homechef/jobs/<id>`).
