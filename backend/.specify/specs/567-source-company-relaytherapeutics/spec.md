# Spec: 567 — Source Company Plugin: Relay Therapeutics

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 567                            |
| Slug           | source-company-relaytherapeutics               |
| Status         | accepted                               |
...
Relay Therapeutics publishes its careers board through Greenhouse at
the bare slug `relaytherapeutics`. The run-398 batch sweep confirmed
...
- Ship a `source-company-relaytherapeutics` plugin returning live
...
- Backfilling historical Relay Therapeutics postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RELAY_THERAPEUTICS`** in the
> source registry, so that **a single `siteType: [Site.RELAY_THERAPEUTICS]`
> request returns Relay Therapeutics's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RELAY_THERAPEUTICS = 'relaytherapeutics'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-relaytherapeutics`.                  | must     |
| FR-3  | `RelaytherapeuticsService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `relaytherapeutics-`, `site === Site.RELAY_THERAPEUTICS`. | must |
...
```ts
@SourcePlugin({ site: Site.RELAY_THERAPEUTICS, name: 'Relay Therapeutics', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/relaytherapeutics/jobs/<id>`), wire
  `company_name` pass-through (`'Relay Therapeutics'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/relaytherapeutics/jobs/<id>`).
