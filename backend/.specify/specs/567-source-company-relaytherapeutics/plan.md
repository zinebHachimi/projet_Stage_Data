# Plan: 567 — Source Company Plugin: Relay Therapeutics
...
Relay Therapeutics's careers board is hosted on Greenhouse at the slug
`relaytherapeutics`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-relaytherapeutics` plugin.
- Deliverables: package files, `Site.RELAY_THERAPEUTICS` enum value, the
...
- `packages/plugins/source-company-relaytherapeutics` (new)
