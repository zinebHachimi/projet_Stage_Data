# Spec: 648 — Source Company Plugin: Carrot Fertility

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 648                            |
| Slug           | source-company-carrotfertility               |
| Status         | accepted                               |
...
Carrot Fertility publishes its careers board through Greenhouse at
the bare slug `carrotfertility`. The run-398 batch sweep confirmed
...
- Ship a `source-company-carrotfertility` plugin returning live
...
- Backfilling historical Carrot Fertility postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CARROT_FERTILITY`** in the
> source registry, so that **a single `siteType: [Site.CARROT_FERTILITY]`
> request returns Carrot Fertility's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CARROT_FERTILITY = 'carrotfertility'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-carrotfertility`.                  | must     |
| FR-3  | `CarrotFertilityService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `carrotfertility-`, `site === Site.CARROT_FERTILITY`. | must |
...
```ts
@SourcePlugin({ site: Site.CARROT_FERTILITY, name: 'Carrot Fertility', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/carrotfertility/jobs/<id>`), wire
  `company_name` pass-through (`'Carrot Fertility'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/carrotfertility/jobs/<id>`).
