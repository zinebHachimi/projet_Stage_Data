# Plan: 550 — Source Company Plugin: Parsley Health
...
Parsley Health's careers board is hosted on Greenhouse at the slug
`parsleyhealth`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-parsleyhealth` plugin.
- Deliverables: package files, `Site.PARSLEY_HEALTH` enum value, the
...
- `packages/plugins/source-company-parsleyhealth` (new)
