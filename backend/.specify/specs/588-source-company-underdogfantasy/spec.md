# Spec: 588 — Source Company Plugin: Underdog

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 588                            |
| Slug           | source-company-underdogfantasy               |
| Status         | accepted                               |
...
Underdog publishes its careers board through Greenhouse at
the bare slug `underdogfantasy`. The run-398 batch sweep confirmed
...
- Ship a `source-company-underdogfantasy` plugin returning live
...
- Backfilling historical Underdog postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.UNDERDOG`** in the
> source registry, so that **a single `siteType: [Site.UNDERDOG]`
> request returns Underdog's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.UNDERDOG = 'underdogfantasy'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-underdogfantasy`.                  | must     |
| FR-3  | `UnderdogService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `underdogfantasy-`, `site === Site.UNDERDOG`. | must |
...
```ts
@SourcePlugin({ site: Site.UNDERDOG, name: 'Underdog', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/underdogfantasy/jobs/<id>`), wire
  `company_name` pass-through (`'Underdog'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/underdogfantasy/jobs/<id>`).
