# Spec: 225 — Source Company Plugin: Alamar Biosciences

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 225                            |
| Slug           | source-company-alamarbiosciences               |
| Status         | accepted                               |
...
Alamar Biosciences publishes its careers board through Greenhouse at
the bare slug `alamarbiosciences`. The run-398 batch sweep confirmed
...
- Ship a `source-company-alamarbiosciences` plugin returning live
...
- Backfilling historical Alamar Biosciences postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ALAMARBIOSCIENCES`** in the
> source registry, so that **a single `siteType: [Site.ALAMARBIOSCIENCES]`
> request returns Alamar Biosciences's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ALAMARBIOSCIENCES = 'alamarbiosciences'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-alamarbiosciences`.                  | must     |
| FR-3  | `AlamarbiosciencesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `alamarbiosciences-`, `site === Site.ALAMARBIOSCIENCES`. | must |
...
```ts
@SourcePlugin({ site: Site.ALAMARBIOSCIENCES, name: 'Alamar Biosciences', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/alamarbiosciences/jobs/<id>`), wire
  `company_name` pass-through (`'Alamar Biosciences'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/alamarbiosciences/jobs/<id>`).
