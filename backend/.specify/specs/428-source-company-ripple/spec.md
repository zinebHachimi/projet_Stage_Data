# Spec: 428 — Source Company Plugin: Ripple

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 428                            |
| Slug           | source-company-ripple               |
| Status         | accepted                               |
...
Ripple publishes its careers board through Greenhouse at
the bare slug `ripple`. The run-398 batch sweep confirmed
128 live role(s) via a direct probe of
...
- Ship a `source-company-ripple` plugin returning live
...
- Backfilling historical Ripple postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.RIPPLE`** in the
> source registry, so that **a single `siteType: [Site.RIPPLE]`
> request returns Ripple's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.RIPPLE = 'ripple'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ripple`.                  | must     |
| FR-3  | `RippleService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ripple-`, `site === Site.RIPPLE`. | must |
...
```ts
@SourcePlugin({ site: Site.RIPPLE, name: 'Ripple', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ripple/jobs/<id>`), wire
  `company_name` pass-through (`'Ripple'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ripple/jobs/<id>`).
