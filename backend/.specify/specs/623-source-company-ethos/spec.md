# Spec: 623 — Source Company Plugin: Ethos

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 623                            |
| Slug           | source-company-ethos               |
| Status         | accepted                               |
...
Ethos publishes its careers board through Greenhouse at
the bare slug `ethos`. The run-398 batch sweep confirmed
...
- Ship a `source-company-ethos` plugin returning live
...
- Backfilling historical Ethos postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.ETHOS`** in the
> source registry, so that **a single `siteType: [Site.ETHOS]`
> request returns Ethos's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.ETHOS = 'ethos'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-ethos`.                  | must     |
| FR-3  | `EthosService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `ethos-`, `site === Site.ETHOS`. | must |
...
```ts
@SourcePlugin({ site: Site.ETHOS, name: 'Ethos', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/ethos/jobs/<id>`), wire
  `company_name` pass-through (`'Ethos'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/ethos/jobs/<id>`).
