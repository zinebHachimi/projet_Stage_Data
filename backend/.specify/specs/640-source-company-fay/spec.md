# Spec: 640 — Source Company Plugin: Fay

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 640                            |
| Slug           | source-company-fay               |
| Status         | accepted                               |
...
Fay publishes its careers board through Greenhouse at
the bare slug `fay`. The run-398 batch sweep confirmed
...
- Ship a `source-company-fay` plugin returning live
...
- Backfilling historical Fay postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.FAY`** in the
> source registry, so that **a single `siteType: [Site.FAY]`
> request returns Fay's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.FAY = 'fay'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-fay`.                  | must     |
| FR-3  | `FayService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `fay-`, `site === Site.FAY`. | must |
...
```ts
@SourcePlugin({ site: Site.FAY, name: 'Fay', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/fay/jobs/<id>`), wire
  `company_name` pass-through (`'Fay'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/fay/jobs/<id>`).
