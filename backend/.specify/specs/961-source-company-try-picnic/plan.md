# Plan: 961 — Source Company Plugin: Picnic Delivery
...
Picnic Delivery's careers board is hosted on Greenhouse at the slug
`try-picnic`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-try-picnic` plugin.
- Deliverables: package files, `Site.PICNIC_DELIVERY` enum value, the
...
- `packages/plugins/source-company-try-picnic` (new)
