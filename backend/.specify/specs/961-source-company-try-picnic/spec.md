# Spec: 961 — Source Company Plugin: Picnic Delivery

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 961                            |
| Slug           | source-company-try-picnic               |
| Status         | accepted                               |
...
Picnic Delivery publishes its careers board through Greenhouse at
the bare slug `try-picnic`. The run-398 batch sweep confirmed
...
- Ship a `source-company-try-picnic` plugin returning live
...
- Backfilling historical Picnic Delivery postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.PICNIC_DELIVERY`** in the
> source registry, so that **a single `siteType: [Site.PICNIC_DELIVERY]`
> request returns Picnic Delivery's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.PICNIC_DELIVERY = 'try-picnic'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-try-picnic`.                  | must     |
| FR-3  | `PicnicDeliveryService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `try-picnic-`, `site === Site.PICNIC_DELIVERY`. | must |
...
```ts
@SourcePlugin({ site: Site.PICNIC_DELIVERY, name: 'Picnic Delivery', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/try-picnic/jobs/<id>`), wire
  `company_name` pass-through (`'Picnic Delivery'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/try-picnic/jobs/<id>`).
