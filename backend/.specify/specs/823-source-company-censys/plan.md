# Plan: 823 — Source Company Plugin: Censys
...
Censys's careers board is hosted on Greenhouse at the slug
`censys`. This plugin mirrors the canonical variant-2 + D-08
...
- Goal: a registered, tested `source-company-censys` plugin.
- Deliverables: package files, `Site.CENSYS` enum value, the
...
- `packages/plugins/source-company-censys` (new)
