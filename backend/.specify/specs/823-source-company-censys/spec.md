# Spec: 823 — Source Company Plugin: Censys

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 823                            |
| Slug           | source-company-censys               |
| Status         | accepted                               |
...
Censys publishes its careers board through Greenhouse at
the bare slug `censys`. The run-398 batch sweep confirmed
...
- Ship a `source-company-censys` plugin returning live
...
- Backfilling historical Censys postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CENSYS`** in the
> source registry, so that **a single `siteType: [Site.CENSYS]`
> request returns Censys's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CENSYS = 'censys'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-censys`.                  | must     |
| FR-3  | `CensysService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `censys-`, `site === Site.CENSYS`. | must |
...
```ts
@SourcePlugin({ site: Site.CENSYS, name: 'Censys', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/censys/jobs/<id>`), wire
  `company_name` pass-through (`'Censys'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/censys/jobs/<id>`).
