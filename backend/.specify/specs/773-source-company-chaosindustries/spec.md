# Spec: 773 — Source Company Plugin: CHAOS Industries

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 773                            |
| Slug           | source-company-chaosindustries               |
| Status         | accepted                               |
...
CHAOS Industries publishes its careers board through Greenhouse at
the bare slug `chaosindustries`. The run-398 batch sweep confirmed
155 live role(s) via a direct probe of
...
- Ship a `source-company-chaosindustries` plugin returning live
...
- Backfilling historical CHAOS Industries postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.CHAOS_INDUSTRIES`** in the
> source registry, so that **a single `siteType: [Site.CHAOS_INDUSTRIES]`
> request returns CHAOS Industries's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.CHAOS_INDUSTRIES = 'chaosindustries'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-chaosindustries`.                  | must     |
| FR-3  | `CHAOSIndustriesService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `chaosindustries-`, `site === Site.CHAOS_INDUSTRIES`. | must |
...
```ts
@SourcePlugin({ site: Site.CHAOS_INDUSTRIES, name: 'CHAOS Industries', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/chaosindustries/jobs/<id>`), wire
  `company_name` pass-through (`'CHAOS Industries'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/chaosindustries/jobs/<id>`).
