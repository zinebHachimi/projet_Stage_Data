# Spec: 504 — Source Company Plugin: Hungryroot

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 504                            |
| Slug           | source-company-hungryroot               |
| Status         | accepted                               |
...
Hungryroot publishes its careers board through Greenhouse at
the bare slug `hungryroot`. The run-398 batch sweep confirmed
...
- Ship a `source-company-hungryroot` plugin returning live
...
- Backfilling historical Hungryroot postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HUNGRYROOT`** in the
> source registry, so that **a single `siteType: [Site.HUNGRYROOT]`
> request returns Hungryroot's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HUNGRYROOT = 'hungryroot'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-hungryroot`.                  | must     |
| FR-3  | `HungryrootService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `hungryroot-`, `site === Site.HUNGRYROOT`. | must |
...
```ts
@SourcePlugin({ site: Site.HUNGRYROOT, name: 'Hungryroot', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/hungryroot/jobs/<id>`), wire
  `company_name` pass-through (`'Hungryroot'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/hungryroot/jobs/<id>`).
