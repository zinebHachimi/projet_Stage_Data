# Spec: 469 — Source Company Plugin: HeyGen

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 469                            |
| Slug           | source-company-heygen               |
| Status         | accepted                               |
...
HeyGen publishes its careers board through Greenhouse at
the bare slug `heygen`. The run-398 batch sweep confirmed
...
- Ship a `source-company-heygen` plugin returning live
...
- Backfilling historical HeyGen postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.HEYGEN`** in the
> source registry, so that **a single `siteType: [Site.HEYGEN]`
> request returns HeyGen's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.HEYGEN = 'heygen'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-heygen`.                  | must     |
| FR-3  | `HeygenService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `heygen-`, `site === Site.HEYGEN`. | must |
...
```ts
@SourcePlugin({ site: Site.HEYGEN, name: 'HeyGen', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/heygen/jobs/<id>`), wire
  `company_name` pass-through (`'HeyGen'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/heygen/jobs/<id>`).
