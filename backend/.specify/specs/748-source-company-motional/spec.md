# Spec: 748 — Source Company Plugin: Motional

| Field          | Value                                  |
| -------------- | -------------------------------------- |
| Spec ID        | 748                            |
| Slug           | source-company-motional               |
| Status         | accepted                               |
...
Motional publishes its careers board through Greenhouse at
the bare slug `motional`. The run-398 batch sweep confirmed
103 live role(s) via a direct probe of
...
- Ship a `source-company-motional` plugin returning live
...
- Backfilling historical Motional postings.
- Cross-board enrichment or salary inference.

## 4. User / Caller Stories

> As an **aggregator caller**, I want **`Site.MOTIONAL`** in the
> source registry, so that **a single `siteType: [Site.MOTIONAL]`
> request returns Motional's open roles**.

## 5. Functional Requirements

| ID    | Requirement                                                                 | Priority |
| ----- | --------------------------------------------------------------------------- | -------- |
| FR-1  | Add `Site.MOTIONAL = 'motional'` to the `Site` enum.               | must     |
| FR-2  | New plugin package `@ever-jobs/source-company-motional`.                  | must     |
| FR-3  | `MotionalService.scrape(input)` returns a `JobResponseDto`; never throws. | must   |
| FR-4  | The plugin is registered in `ALL_SOURCE_MODULES`.                          | must     |
| FR-5  | `tsconfig.base.json` path-alias + matching `jest.config.js` mapper.      | must     |
| FR-6  | Each `JobPostDto` `id` prefixed `motional-`, `site === Site.MOTIONAL`. | must |
...
```ts
@SourcePlugin({ site: Site.MOTIONAL, name: 'Motional', category: 'company' })
@Injectable()
...
- ≥ 9 cases. Happy-path asserts variant-2 URL pass-through
  (`job-boards.greenhouse.io/motional/jobs/<id>`), wire
  `company_name` pass-through (`'Motional'`),
...
- **D-04 (run #398):** Wire-shape variant 2 (canonical hosted-board
  host `job-boards.greenhouse.io/motional/jobs/<id>`).
